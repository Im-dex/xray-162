...
bool do_constrain(u32 base_edge_idx, u32 test_edg_idx, face_props& base_fprops,
                  face_props& test_fprops) {
    return (test_fprops.material != base_fprops.material) ||
           (test_fprops.sector != base_fprops.sector) ||
           !do_connect_faces_by_faces_edge_flags(base_fprops.flags, test_fprops.flags,
...
void SimplifyCFORM(CDB::CollectorPacked& CL) {
    ...
    for (u32 f_idx = 0; f_idx < base_faces_cnt; f_idx++) {
        ...
        FPs[f_idx].set(t.material, t.sector,
                       Fvector().mknormal(*(CL.getV() + t.verts[0]), *(CL.getV() + t.verts[1]),
                                          *(CL.getV() + t.verts[2])),
                       CL.getfFlags(f_idx));
    }
    CL.clear();
    ...
    slim->placement_policy =
        MX_PLACE_ENDPOINTS; // MX_PLACE_ENDPOINTS;//MX_PLACE_ENDORMID;//MX_PLACE_OPTIMAL;
    ...
    for (u32 f_idx = 0; f_idx < mdl->face_count(); f_idx++) {
        if (mdl->face_is_valid(f_idx)) {
            ...
            CL.add_face(*((Fvector*)&mdl->vertex(F[0])), *((Fvector*)&mdl->vertex(F[1])),
                        *((Fvector*)&mdl->vertex(F[2])), FP.material, FP.sector, FP.flags);
        }
    }
