...
void export_geometry(xrMU_Model& mu_model) {
    ...
    Fbox BB;
    BB.invalidate();
    for (xrMU_Model::v_vertices_it vit = mu_model.m_vertices.begin();
         vit != mu_model.m_vertices.end(); vit++)
        BB.modify((*vit)->P);
    ...
    // Begin building
    for (xrMU_Model::v_subdivs_it it = mu_model.m_subdivs.begin(); it != mu_model.m_subdivs.end();
         it++) {
        // Vertices
        {
            g_VB.Begin(D);
        ...
        // Indices
        g_IB.Register(LPWORD(&*it->ogf->data.faces.begin()), LPWORD(&*it->ogf->data.faces.end()),
                      &it->ib_id, &it->ib_start);
