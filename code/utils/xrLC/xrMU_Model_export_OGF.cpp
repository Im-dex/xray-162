...
void export_ogf(xrMU_Reference& mu_reference) {
    ...
    // Export nodes
    {
        for (xrMU_Model::v_subdivs_it it = model->m_subdivs.begin(); it != model->m_subdivs.end();
             it++) {
        ...
        return;
    {
        ...
        for (int lf = 0; lf < 8; lf++) {
            ...
            for (int lv = 0; lv < 4; lv++) {
                ...
                float s = 1 / (_N + EPS);
                _C.mul(s);
                F.v[lv].c_rgb_hemi = color_rgba(u8_clr(_C.rgb.x), u8_clr(_C.rgb.y),
                                                u8_clr(_C.rgb.z), u8_clr(_C.hemi));
