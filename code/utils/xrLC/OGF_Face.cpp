...
BOOL x_vertex::similar(OGF* ogf, const x_vertex& V) { return P.similar(V.P); }
u16 OGF::x_BuildVertex(const x_vertex& V1) {
...
void OGF::x_BuildFace(OGF_Vertex& V1, OGF_Vertex& V2, OGF_Vertex& V3, bool _tc_) {
    ...
    if (!F.Degenerate()) {
        ...
        if (fast_path_data.vertices.size() > VertCount)
            fast_path_data.vertices.erase(fast_path_data.vertices.begin() + VertCount,
                                          fast_path_data.vertices.end());
    }
}
        ...
        if (dest.empty()) {
            ...
            for (u32 vid = 0; vid < 3; vid++) {
                ...
                for (u32 sid = 0; sid < dest.size(); sid++) {
                    if (id == dest[sid]) {
                        bConnected = TRUE; // this face shares at least one vertex with already
                                           // selected faces
                        break;
                    }
                }
...
void OGF::MakeProgressive(float metric_limit) {
...

    {
        ...
        while (VR && VR->swr_records.size() > 0) {
            ...
            if (_metric < metric_limit) {
                progressive_clear();
                clMsg("* mesh simplified from [%4dv] to [%4dv], nf[%4d] ==> em[%0.2f]-discarded",
                      _full, _simple, VR->indices.size() / 3, metric_limit);
                break;
            } else {
                clMsg("* mesh simplified from [%4dv] to [%4dv], nf[%4d] ==> em[%0.2f]-accepted",
                      _full, _simple, VR->indices.size() / 3, metric_limit);
            }
    ...
    if (progressive_test() && fast_path_data.vertices.size() && fast_path_data.faces.size()) {
        ...
        for (u32 f_idx = 0; f_idx < fast_path_data.faces.size(); f_idx++)
            VIPM_AppendFace(fast_path_data.faces[f_idx].v[0], fast_path_data.faces[f_idx].v[1],
                            fast_path_data.faces[f_idx].v[2]);
        ...
        if (0 == VR) {
            ...
            */
            ...
            u32 _simple = _full - _remove;
            float _metric = float(_remove) / float(_full);
            clMsg("X mesh simplified from [%4dv] to [%4dv], nf[%4d]", _full, _simple,
                  VR ? VR->indices.size() / 3 : 0);
