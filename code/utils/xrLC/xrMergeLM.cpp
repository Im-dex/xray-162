...
// Initialization
void InitSurface() { FillMemory(surface, lmap_size * lmap_size, 0); }
...
void CBuild::MergeLM() {
    ...
    for (u32 light_layer = 0; light_layer < pBuild->lights.size(); light_layer++) {
        ...
        for (int it = 0; it < (int)g_deflectors.size(); it++) {
            if (g_deflectors[it].bMerged)
                continue;
            if (0 == g_deflectors[it].GetLayer(L_base))
                continue;
        ...
        while (Layer.size()) {
            ...
            if (SEL.empty()) {
                ...
                for (int K = 0; K < (int)SEL.size(); K++) {
                    _rect T;
                    T.a.set(0, 0);
                    T.b.set(SEL[K]->lm.dwWidth + 2 * BORDER - 1,
                            SEL[K]->lm.dwHeight + 2 * BORDER - 1);
                    T.iArea = SEL[K]->iArea;
                ...
                //  Analyze resuls
                clMsg("%3d / %3d - [%d,%d]", best.size(), selected.size(), brect.SizeX(),
                      brect.SizeY());
                ...
                for (K = 0; K < (int)best.size(); K++) {
                    ...
                    // Merge
                    pDEFL->Capture(SEL[iRealIndex], Offset.x, Offset.y, Place.SizeX(),
                                   Place.SizeY(), bRotated);

                    // Destroy old deflector
                    vecDeflIt OLD =
                        std::find(g_deflectors.begin(), g_deflectors.end(), SEL[iRealIndex]);
