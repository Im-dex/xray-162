...
void CDeflector::OA_Export() {
    ...
    // Surface
    u32 dwWidth = iCeil(size.x * g_params.m_lm_pixels_per_meter * density + .5f);
    clamp(dwWidth, 1u, 512u - 2 * BORDER);
    u32 dwHeight = iCeil(size.y * g_params.m_lm_pixels_per_meter * density + .5f);
...
BOOL CDeflector::OA_Place(Face* owner) {
    ...
    if (cosa < _cos(deg2rad(g_params.m_sm_angle + 1)))
        return FALSE;
