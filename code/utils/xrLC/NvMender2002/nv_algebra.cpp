...
mat3::mat3(const nv_scalar* array) { std::memcpy(mat_array, array, sizeof(nv_scalar) * 9); }

mat3::mat3(const mat3& M) { std::memcpy(mat_array, M.mat_array, sizeof(nv_scalar) * 9); }

mat4::mat4() {}

mat4::mat4(const nv_scalar* array) { std::memcpy(mat_array, array, sizeof(nv_scalar) * 16); }

mat4::mat4(const mat4& M) { std::memcpy(mat_array, M.mat_array, sizeof(nv_scalar) * 16); }
...
*/
nv_scalar det3x3(nv_scalar a1, nv_scalar a2, nv_scalar a3, nv_scalar b1, nv_scalar b2, nv_scalar b3,
...
mat4& frustum(mat4& M, const nv_scalar l, const nv_scalar r, const nv_scalar b, const nv_scalar t,
...
mat4& perspective(mat4& M, const nv_scalar fovy, const nv_scalar aspect, const nv_scalar n,
                  const nv_scalar f) {
...
mat3& tangent_basis(mat3& basis, const vec3& v0, const vec3& v1, const vec3& v2, const vec2& t0,
                    const vec2& t1, const vec2& t2, const vec3& n) {
    vec3 cp;
...
void nv_is_valid(const vec3& v) {
    assert(!_isnan(v.x) && !_isnan(v.y) && !_isnan(v.z) && _finite(v.x) && _finite(v.y) &&
           _finite(v.z));
}
