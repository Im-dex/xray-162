...
Comments:
  ...
  basis matrices.  It also can fix common texuring problems that come up when bump mapping,
including
...
stretching that can ruin per-pixel lighting.
  ...
  //  All unknown attribute types, including "tex1", "tex2", "random_attribute", "weights", "bones",
etc.
  // are simply passed through.  They will be duplicated as needed just like positions, normals,
etc.

  bool bSuccess = aMender.Munge( inputAtts,              // these are my positions & indices
                                 outputAtts,             // these are the outputs I requested, plus
extra stuff generated on my behalf 3.141592654f / 2.5f,    // tangent space smooth angle NULL,
// no Texture matrix applied to my tex0 coords FixTangents,            // fix degenerate bases &
texture mirroring FixCylindricalTexGen    // handle cylidrically mapped textures via vertex
duplication WeightNormalsByFaceSize // weight vertex normals by the triangle's size
                                 );
  ...
  //   If you ask for FixCylindricalTexGen, it will fix any absolute change in texture coordinates >
0.5
  // across a single edge.  Therefore, if you pass in a single quad or cube, it won't work.  Any
more
...
bool NVMeshMender::Munge(const NVMeshMender::VAVector& input, NVMeshMender::VAVector& output,
    ...
    if (bNeedTexCoords || bComputeTangentSpace) {
        ...
        if (_FixCylindricalTexGen == FixCylindricalTexGen) {
            ...
            for (unsigned int f = 0; f < theSize; f += 3) {
                for (int v = 0; v < 3; ++v) {
                    ...
                    nv_scalar dS =
                        texcoords[indices[end] * 3 + 0] - texcoords[indices[start] * 3 + 0];
                            ...
                            if (output[att].Name_ != "indices") {
                                if (output[att].Name_ == "tex0") {
                                    ...
                                    output[att].floatVector_.push_back(
                                        output[att]
                                            .floatVector_[indices[theOneToChange] * 3 + 1]); // x
                                    output[att].floatVector_.push_back(
                                        output[att]
                                            .floatVector_[indices[theOneToChange] * 3 + 2]); // z
                                } else {
                                    // *3 b/c we are looking up 3vectors in an array of floats
                                    output[att].floatVector_.push_back(
                                        output[att]
                                            .floatVector_[indices[theOneToChange] * 3 + 0]); // x
                                    output[att].floatVector_.push_back(
                                        output[att]
                                            .floatVector_[indices[theOneToChange] * 3 + 1]); // y
                                    output[att].floatVector_.push_back(
                                        output[att]
                                            .floatVector_[indices[theOneToChange] * 3 + 2]); // z
                                }
...

                {
                    ...
                    for (int v = 0; v < 3; ++v) {
                        ...
                        nv_scalar dT =
                            texcoords[indices[end] * 3 + 1] - texcoords[indices[start] * 3 + 1];
                        ...
                        if (bDoT == true) {
                            ...
                            for (unsigned int att = 0; att < output.size(); ++att) {
                                ...
                                if (output[att].Name_ != "indices") {
                                    if (output[att].Name_ == "tex0") {
                                        output[att].floatVector_.push_back(
                                            output[att].floatVector_[indices[theOneToChange] * 3 +
                                                                     0]);                // x
        ...
        if (pTextureMatrix) {
            const mat4 M(pTextureMatrix[0], pTextureMatrix[1], pTextureMatrix[2], pTextureMatrix[3],
                         ...
                         pTextureMatrix[8], pTextureMatrix[9], pTextureMatrix[10],
                         pTextureMatrix[11], pTextureMatrix[12], pTextureMatrix[13],
                         pTextureMatrix[14], pTextureMatrix[15]);
            Mapping::iterator texIter = outmap.find("tex0");
            VertexAttribute::FloatVector& texcoords = output[(*texIter).second].floatVector_;
    ...
    if (bComputeTangentSpace) {
        ...
        for (unsigned int f = 0; f < theSize; f += 3) {
            ...
            if (_FixTangents == FixTangents) {
                ...
                for (int e = 0; e < 3; ++e) {
                    ...
                    edge.v0 =
                        (unsigned int)nv_min((nv_scalar)indices[start], (nv_scalar)indices[end]);
                    edge.v1 =
                        (unsigned int)nv_max((nv_scalar)indices[start], (nv_scalar)indices[end]);
                    ...
                    if (iter == Edges.end()) {
                        ...
                        // Now that the vertices are duplicated, smoothing won't occur over this
                        // edge,
                        //  because the two faces will sum their tangent basis vectors into separate
                        //  indices
                    }
                }
            } // if fixtangents
        ...
        for (unsigned int b = 0; b < positions.size(); b += 3) {
            *reinterpret_cast<vec3*>(&output[(*tangent).second].floatVector_[b]) =
                normalize(avgS[b / 3]); // s
            *reinterpret_cast<vec3*>(&output[(*binormal).second].floatVector_[b]) =
                normalize(avgT[b / 3]); // T
        }
    }
