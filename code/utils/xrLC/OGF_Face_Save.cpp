...
struct r1v_lmap {
    ...
    r1v_lmap(Fvector3 _P, Fvector _N, base_basis _T, base_basis _B, base_color _CC,
             Fvector2 tc_base, Fvector2 tc_lmap) {
        base_color_c _C;
        _CC._get(_C);
        _N.normalize();
...
struct r1v_vert {
    ...
    r1v_vert(Fvector3 _P, Fvector _N, base_basis _T, base_basis _B, base_color _CC,
             Fvector2 tc_base) {
        base_color_c _C;
        _CC._get(_C);
        _N.normalize();
...
void OGF::Save(IWriter& fs) {
    ...
    string1024 sid;
    strconcat(sizeof(sid), sid, pBuild->shader_render[pBuild->materials()[material].shader].name,
              "/", Tname.c_str());
...
void OGF_Reference::Save(IWriter& fs) {
    ...
    // geom_batch_average	(vertices.size(),faces.size());	// don't use reference(s) as batch
    // estimate
...
void OGF::PreSave(u32 tree_id) {
    ...
    if (fast_path_data.vertices.size() && fast_path_data.faces.size()) {
        clMsg("%4d: v(%3d)/f(%3d)", tree_id, fast_path_data.vertices.size(),
    ...
    // Faces
    g_IB.Register(LPWORD(&*data.faces.begin()), LPWORD(&*data.faces.end()), &data.ib_id,
                  &data.ib_start);
}
