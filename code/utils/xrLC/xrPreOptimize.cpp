...
void CBuild::PreOptimize() {
    ...
    for (int it = 0; it < (int)lc_global_data()->g_vertices().size(); it++) {
        if (0 == (it % 100000)) {
            Progress(std::sqrt(float(it) / float(lc_global_data()->g_vertices().size())));
    ...
    for (u32 it = 0; it < lc_global_data()->g_vertices().size(); ++it) {
        if (lc_global_data()->g_vertices()[it] &&
            (lc_global_data()->g_vertices()[it]->m_adjacents.empty())) {
            lc_global_data()->destroy_vertex(lc_global_data()->g_vertices()[it]);
            ++Vremoved;
        }
    }
    ...
    mem_Compact();
    clMsg("%d vertices removed. (%d left)", Vcount - lc_global_data()->g_vertices().size(),
          lc_global_data()->g_vertices().size());
    clMsg("%d faces removed. (%d left)", Fcount - lc_global_data()->g_faces().size(),
          lc_global_data()->g_faces().size());
