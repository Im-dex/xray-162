...
int GetVertexIndex(Vertex* F) {
    vecVertexIt it = std::lower_bound(lc_global_data()->g_vertices().begin(),
                                      lc_global_data()->g_vertices().end(), F);
...
void CBuild::BuildCForm() {
    ...
    vecVertex* cfVertices = new vecVertex();
    {
        ...
        for (vecFaceIt I = lc_global_data()->g_faces().begin();
             I != lc_global_data()->g_faces().end(); ++I) {
            Face* F = *I;
    ...
    for (vecFaceIt F = cfFaces->begin(); F != cfFaces->end(); F++) {
        ...
        CL.add_face(T->v[0]->P, T->v[1]->P, T->v[2]->P, T->dwMaterialGame,
                    materials()[T->dwMaterial].sector, T->sm_group);
    ...
    for (vecFaceIt I = lc_global_data()->g_faces().begin(); I != lc_global_data()->g_faces().end();
         I++) {
        Face* F = *I;
        F->pDeflector = NULL;
    }
