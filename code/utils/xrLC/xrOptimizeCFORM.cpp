...
void CBuild::BuildCForm() {
    ...
    clMsg("%f%% geometry/artist quality",
          100.f * (1 - float(failedfaces.size()) / float(mesh.n_faces())));
    ...
    clMsg("vertices: was[%d], now[%d] => %f %% left", nv_before, nv_after,
          100.f * float(nv_after) / float(nv_before));
    clMsg("   faces: was[%d], now[%d] => %f %% left", nf_before, nf_after,
          100.f * float(nf_after) / float(nf_before));
    ...
    for (; fit != fend; ++fit) {
        ...
        CL.add_face_D(reinterpret_cast<Fvector&>(mesh.point(fhandles[0])),
