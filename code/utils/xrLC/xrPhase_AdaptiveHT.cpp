...
void CBuild::xrPhase_AdaptiveHT() {
    ...
    Status("Tesselating...");
    if (1) {
        for (u32 fit = 0; fit < lc_global_data()->g_faces().size();
             fit++) { // clear split flag from all faces + calculate normals
    ...
    Status("Precalculating...");
    {
        ...
        //	V->normalFromAdj	();
        //	LightPoint			(&DB, lc_global_data()->RCAST_Model(), vC, V->P, V->N,
        //pBuild->L_static(), LP_dont_rgb+LP_dont_sun,0); 	vC.mul
        //(0.5f); 	V->C._set			(vC);
        //}
    ...
    {
            for (u32 fit=0; fit<g_faces.size(); fit++)	{					// clear split flag
    from all faces + calculate normals g_faces[fit]->flags.bSplitted	= false;
...
void CBuild::u_Tesselate(tesscb_estimator* cb_E, tesscb_face* cb_F, tesscb_vertex* cb_V) {
    ...
    lc_global_data()->g_faces().erase(std::remove(lc_global_data()->g_faces().begin(),
                                                  lc_global_data()->g_faces().end(), (Face*)0),
                                      lc_global_data()->g_faces().end());
    lc_global_data()->g_vertices().erase(std::remove(lc_global_data()->g_vertices().begin(),
