...
//
NvFaceInfo* NvStripifier::FindOtherFace(NvEdgeInfoVec& edgeInfos, int v0, int v1,
                                        NvFaceInfo* faceInfo) {
...
void NvStripifier::BuildStripifyInfo(NvFaceInfoVec& faceInfos, NvEdgeInfoVec& edgeInfos) {
    ...
    for (u32 i = 0; i < numTriangles; i++) {
        ...
        // create the face info and add it to the list of faces, but only if this exact face doesn't
        // already
        //  exist in the list
        ...
        if (!AlreadyExists(faceInfo, faceInfos)) {
            ...
            if (edgeInfo01 == NULL) {
                ...
                if (edgeInfo01->m_face1 != NULL)
                    ;
                // Msg("! WARNING: BuildStripifyInfo: > 2 triangles on an edge... uncertain
                // consequences");
                else
...
void NvStripInfo::Build(NvEdgeInfoVec& edgeInfos, NvFaceInfoVec& faceInfos) {
    ...
    NvFaceInfo* nextFace =
        NvStripifier::FindOtherFace(edgeInfos, nv0, nv1, m_startInfo.m_startFace);
...
//
bool NvStripifier::FindTraversal(NvFaceInfoVec& faceInfos, NvEdgeInfoVec& edgeInfos,
                                 NvStripInfo* strip, NvStripStartInfo& startInfo) {
...
// RemoveSmallStrips()
//
// allStrips is the whole strip _vector_...all small strips will be deleted from this list, to avoid
// leaking mem allBigStrips is an out parameter which will contain all strips above minStripLength
// faceList is an out parameter which will contain all faces which were removed from the striplist
//
void NvStripifier::RemoveSmallStrips(NvStripInfoVec& allStrips, NvStripInfoVec& allBigStrips,
...
void NvStripifier::CreateStrips(const NvStripInfoVec& allStrips, IntVec& stripIndices,
    ...
    for (int i = 0; i < nStripCount; i++) {
        ...
        {
            NvFaceInfo tFirstFace(strip->m_faces[0]->m_v0, strip->m_faces[0]->m_v1,
...
void NvStripifier::Stripify(const WordVec& in_indices, const int in_cacheSize,
                            const int in_minStripLength, NvStripInfoVec& outStrips,
    ...
    minStripLength = in_minStripLength; // this is the strip size threshold below which we dump the
                                        // strip into a list
    ...
    for (i = 0; i < allStrips.size(); i++) {
        ...
        if (allStrips[i]->m_faces.size() > threshold) {
            ...
            for (j = 0; j < numTimes; j++) {
                ...
                for (int faceCtr = j * threshold; faceCtr < threshold + (j * threshold);
                     faceCtr++) {
...
//
void NvStripifier::FindAllStrips(NvStripInfoVec& allStrips, NvFaceInfoVec& allFaceInfos,
                                 NvEdgeInfoVec& allEdgeInfos, int numSamples) {
    // the experiments
    int experimentId = 0;
    int stripId = 0;
    bool done = false;
    ...
    while (!done) {
        ...
        for (i = 0; i < numSamples; i++) {
            ...
            NvStripInfo* strip01 = xr_new<NvStripInfo>(NvStripStartInfo(nextFace, edge01, true),
                                                       stripId++, experimentId++);
            ...
            NvStripInfo* strip10 = xr_new<NvStripInfo>(NvStripStartInfo(nextFace, edge10, false),
                                                       stripId++, experimentId++);
            ...
            NvStripInfo* strip12 = xr_new<NvStripInfo>(NvStripStartInfo(nextFace, edge12, true),
                                                       stripId++, experimentId++);
            ...
            NvStripInfo* strip21 = xr_new<NvStripInfo>(NvStripStartInfo(nextFace, edge21, false),
                                                       stripId++, experimentId++);
            ...
            NvStripInfo* strip20 = xr_new<NvStripInfo>(NvStripStartInfo(nextFace, edge20, true),
                                                       stripId++, experimentId++);
            ...
            NvStripInfo* strip02 = xr_new<NvStripInfo>(NvStripStartInfo(nextFace, edge02, false),
                                                       stripId++, experimentId++);
        ...
        for (i = 0; i < numExperiments; i++) {
            ...
            float value =
                avgStripSize * avgStripSizeWeight + (avgStripSize * numStrips * numTrisWeight);
