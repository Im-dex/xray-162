...
u32 add_vertex(const Vertex& V, const Fvector2& Ftc, xr_vector<float>& v_position,
...
void fill_mender_input(xr_vector<float>& v_position, xr_vector<float>& v_normal,
                       xr_vector<float>& v_tc, xr_vector<int>& v_indices) {
    ...
    for (u32 f = 0; f < g_faces.size(); f++) {
        ...
        for (u32 v = 0; v < 3; v++) {
            Vertex* V = F->v[v];
            u32 ID =
                u32(std::lower_bound(g_vertices.begin(), g_vertices.end(), V) - g_vertices.begin());
...
void retrive_data_from_mender_otput(const xr_vector<float>& o_tc, const xr_vector<float>& o_tangent,
...

{
    ...
    for (u32 f = 0; f < g_faces.size(); f++) {
        ...
        for (u32 v = 0; v < 3; v++) {
            ...
            F->basis_tangent[v].set(o_tangent[id * 3 + 0], o_tangent[id * 3 + 1],
                                    o_tangent[id * 3 + 2]);
            F->basis_binormal[v].set(o_binormal[id * 3 + 0], o_binormal[id * 3 + 1],
                                     o_binormal[id * 3 + 2]);
        }
    }
}
...
void CBuild::xrPhase_TangentBasis() {
    ...
    /*
    float	teps			= .5f / 4096.f;		// half pixel from 4096 texture
    (0.0001220703125) for (u32 f=0; f<g_faces.size(); f++)
    {
            Progress	(float(f)/float(g_faces.size()));
            Face*		F		= g_faces[f];
    ...
    */
    ...
    clMsg("duplication: was[%d] / become[%d] - %2.1f%%", v_was, v_become,
          100.f * float(v_become - v_was) / float(v_was));
