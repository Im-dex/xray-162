...
void CBuild::Load(const b_params& Params, const IReader& _in_FS) {
    ...
    Status("Faces...");
    {
        ...
        if (InvalideFaces()) {
            err_save();
            Debug.fatal(DEBUG_INFO, "* FATAL: %d invalid faces. Compilation aborted",
                        InvalideFaces());
        }
    }
    ...
    Status("Loading lights...");
    {
        ...
        // Static
        {
            ...
            for (i = 0; i < cnt; i++) {
                ...
                RL.falloff = 1.0f / (RL.range * (RL.attenuation0 + RL.attenuation1 * RL.range +
                                                 RL.attenuation2 * RL.range2));
                RL.energy = _e;
    ...
    Status("Processing textures...");
    {
        ...
        for (u32 t = 0; t < tex_count; t++) {
            ...
            if (0 == xr_strcmp(N, "level_lods")) {
                ...
                string_path th_name;
                FS.update_path(th_name, "$game_textures$",
                               strconcat(sizeof(th_name), th_name, N, ".thm"));
                ...
                // if( version!=THM_CURRENT_VERSION )	FATAL	("Unsupported version of THM
                // file.");
