...
int oe_encode(oe_enc_opt* opt) {
    ...
    if (opt->channels > 255) {
        fprintf(
            stderr,
            _("255 channels should be enough for anyone. (Sorry, vorbis doesn't support more)\n"));
        return 1;
    }
    ...
    opt->start_encode(opt->infilename, opt->filename, opt->bitrate, opt->quality, opt->quality_set,
                      opt->managed, opt->min_bitrate, opt->max_bitrate);
    ...
    if (opt->quality_set > 0) {
        ...
        if (vorbis_encode_setup_managed(
                &vi, opt->channels, opt->rate, opt->max_bitrate > 0 ? opt->max_bitrate * 1000 : -1,
                opt->bitrate * 1000, opt->min_bitrate > 0 ? opt->min_bitrate * 1000 : -1)) {
...

    {
        ...
        /* Build the packets */
        vorbis_analysis_headerout(&vd, opt->comments, &header_main, &header_comments,
                                  &header_codebooks);
    ...
    while (!eos) {
        ...
        if (samples_read == 0)
            ...
            if (packetsdone >= 40) {
                ...
                opt->progress_update(opt->filename, opt->total_samples_per_channel, samplesdone,
                                     time);
            }
...
void update_statistics_full(char* fn, long total, long done, double time) {
    ...
    fprintf(stderr, "\r");
    fprintf(stderr, _("\t[%5.1f%%] [%2dm%.2ds remaining] %c "), done * 100.0 / total, minutes,
            seconds, spinner[spinpoint++ % 4]);
}
...
void start_encode_full(char* fn, char* outfn, int bitrate, float quality, int qset, int managed,
                       int min, int max) {
    if (bitrate > 0) {
        if (managed > 0) {
            fprintf(stderr,
            ...
            fprintf(stderr,
                    _("Encoding %s%s%s to \n         %s%s%s \nat approximate bitrate %d kbps (VBR "
                      "encoding enabled)\n"),
                    fn ? "\"" : "", fn ? fn : _("standard input"), fn ? "\"" : "",
                    outfn ? "\"" : "", outfn ? outfn : _("standard output"), outfn ? "\"" : "",
                    bitrate);
...
void start_encode_null(char* fn, char* outfn, int bitrate, float quality, int qset, int managed,
                       int min, int max) {}
