...
float CacheAddTri(WORD wIndex1, WORD wIndex2, WORD wIndex3, BOOL bDontActuallyAdd = FALSE) {
    ...
    for (int iType = 0; iType < c_iCacheNumTypes; iType++) {
        ...
        // NOTE! If your target hardware doesn't, shorten the FIFO a bit to get roughly the same
        // behaviour.
        BOOL bFound[3];
        int iVert;
        ...
        if (bDontActuallyAdd) {
            ...
            fTotalScore +=
                c_ctiCacheTypeSize[iType].fMissScore * (ccs.iCacheCurrentMisses - iCurNumMisses);
        }
    }
...
float FindBestScoreLookahead(int iCountdown, float fCurrentScore, float fInputBestSoFar,
    ...
    for (i = pstTriLocal.size() - 1; i >= 0; i--) {
        ...
        // And the vertex cost.
        fTriScore +=
            CacheAddTri(pstCurTri->wIndex[0], pstCurTri->wIndex[1], pstCurTri->wIndex[2], TRUE);
...
// Do the valencies.
        ...
        if (iLookahead > 1) {
            ...
            // fScore = FindBestScoreLookahead ( iLookahead - 1, fScore, fBestSoFar, pwIndices + 3,
            // iNumTris - 1, pwResult + 3 );
            float fNewScore = FindBestScoreLookahead(iLookahead - 1, fScore, fBestSoFar,
                                                     iNumTris - 1, pwResult + 3);
...
// if ( !bNoMadJumps )
            ...
            // fNewBestScore = FindBestScoreLookahead ( iCountdown - 1, fScore, 1e10, pwIndices + 3,
            // iNumTris - 1, pwResult + 3 );
            fNewBestScore =
                FindBestScoreLookahead(iCountdown - 1, fScore, 1e10, iNumTris - 1, pwResult + 3);
...

{
    ...
    if (iNumTris == 1) {
        ...
        ScoreTri* pstCur = ppstCurTris[0];
        float fTriScore =
            CacheAddTri(pstCur->wIndex[0], pstCur->wIndex[1], pstCur->wIndex[2], TRUE);
        for (j = 0; j < 3; j++) {
            // Use the valence score after the tri has been removed.
            // int iValence = (*(iValenceCounts.item(pwIndices[j]))) - 1;
            int iValence = (svVertex.item(pstCur->wIndex[j])->iCurrentValence) - 1;
...
//.			TRACE ( "Countdown %i\n", iCountdown );
            ...
            // fNewBestScore = FindBestScore ( iCountdown - 1, fScore, 1e10, pwIndices + 3, iNumTris
            // - 1, pwResult + 3 );
            fNewBestScore = FindBestScore(iCountdown - 1, fScore, 1e10, ppstCurTris + 1,
                                          iNumTris - 1, pwResult + 3);
...
void _OptimiseVertexCoherencyTriList(WORD* pwList, int iHowManyTris, u32 optimize_mode) {
...
#else
    float fTemp =
        FindBestScore(iHowManyTris, 0.0f, 1e10, (pstTri.item(0)), iHowManyTris, pwNewIndex);
#endif
    ...
    // And if you definately use indices 0 to iBiggestIndex (which all this code does), then this is
    // the max possible:
