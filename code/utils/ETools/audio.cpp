...
input_format* open_audio_file(FILE* in, oe_enc_opt* opt) {
    ...
    while (formats[j].id_func) {
        ...
        if (size > buf_filled) {
            ...
            if (buf_filled < size) { /* File truncated */
                j++;
                continue;
            }
        }
...
double read_IEEE80(unsigned char* buf) {
    ...
    double f = ((unsigned long)(buf[2] & 0xff) << 24) | ((buf[3] & 0xff) << 16) |
    ...
    f = ldexp(f, 32);
    f += ((buf[6] & 0xff) << 24) | ((buf[7] & 0xff) << 16) | ((buf[8] & 0xff) << 8) |
         (buf[9] & 0xff);
...
int aiff_open(FILE* in, oe_enc_opt* opt, unsigned char* buf, int buflen) {
    ...
    if (format.blocksize == 0 && (format.samplesize == 16 || format.samplesize == 8)) {
        ...
        fprintf(stderr, _("Warning: OggEnc does not support this type of AIFF/AIFC file\n"
...
int wav_open(FILE* in, oe_enc_opt* opt, unsigned char* oldbuf, int buflen) {
    ...
    if (format.align == format.channels * samplesize && format.samplesize == samplesize * 8 &&
        ...
        wav->bigendian = 0;
        wav->channels =
            format.channels; /* This is in several places. The price
        ...
        fprintf(stderr, _("ERROR: Wav file is unsupported subformat (must be 8,16, or 24 bit PCM\n"
...
long wav_read(void* in, float** buffer, int samples) {
    ...
    if (f->totalsamples &&
        f->samplesread + bytes_read / (sampbyte * f->channels) > f->totalsamples) {
...
static long read_resampled(void* d, float** buffer, int samples) {
    ...
    if (out_samples <= 0) {
        fprintf(stderr, _("BUG: Got zero samples from resampler: your file will be truncated. "
                          "Please report this.\n"));
    }
