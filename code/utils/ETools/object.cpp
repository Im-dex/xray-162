...
bool Object::Valid(void) {
    for (MeshTri* pTriTgt = CurTriRoot.ListNext(); pTriTgt != NULL; pTriTgt = pTriTgt->ListNext())
        for (MeshTri* pTriLst = CurTriRoot.ListNext(); pTriLst != NULL;
             pTriLst = pTriLst->ListNext())
            if (pTriTgt != pTriLst)
    ...
    if (pNextCollapse->ListNext() == NULL) {
        ...
        if (pNextCollapse->ListNext() != NULL) {
            if (pNextCollapse->ListNext()->iSlidingWindowLevel != iCurSlidingWindowLevel) {
                ...
                VERIFY(pNextCollapse->ListNext()->iSlidingWindowLevel ==
                       iCurSlidingWindowLevel - 1);
                iCurSlidingWindowLevel--;
                SetNewLevel(iCurSlidingWindowLevel);
...
float Object::FindCollapseError(MeshPt* pptBinned, MeshEdge* pedgeCollapse,
                                long bTryToCacheResult /*= FALSE*/) {
    ...
    /*
            ...
            if ( bTryToCacheResult && ( pptLast == pptBinned ) ){
                    ...
                    for ( MeshTri *ptri = pptBinned->FirstTri(); ptri != NULL; ptri =
       pptBinned->NextTri() ) qSum += Quad3 ( ptri->pPt1->mypt.vPos, ptri->pPt2->mypt.vPos,
       ptri->pPt3->mypt.vPos );
