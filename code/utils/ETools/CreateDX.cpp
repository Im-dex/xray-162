...
ETOOLS_API HRESULT WINAPI D3DX_GetImageInfoFromFileInMemory(LPCVOID pSrcData, UINT SrcDataSize,
...
ETOOLS_API HRESULT WINAPI D3DX_CreateCubeTextureFromFileInMemoryEx(
    LPDIRECT3DDEVICE9 pDevice, LPCVOID pSrcData, UINT SrcDataSize, UINT Size, UINT MipLevels,
    ...
    return D3DXCreateCubeTextureFromFileInMemoryEx(pDevice, pSrcData, SrcDataSize, Size, MipLevels,
...
ETOOLS_API HRESULT WINAPI D3DX_CreateTextureFromFileInMemoryEx(
    ...
    return D3DXCreateTextureFromFileInMemoryEx(pDevice, pSrcData, SrcDataSize, Width, Height,
                                               MipLevels, Usage, Format, Pool, Filter, MipFilter,
                                               ColorKey, pSrcInfo, pPalette, ppTexture);
}

ETOOLS_API HRESULT WINAPI D3DX_CreateTexture(LPDIRECT3DDEVICE9 pDevice, UINT Width, UINT Height,
...
ETOOLS_API HRESULT WINAPI D3DX_ComputeNormalMap(LPDIRECT3DTEXTURE9 pTexture,
                                                LPDIRECT3DTEXTURE9 pSrcTexture,
                                                const PALETTEENTRY* pSrcPalette, DWORD Flags,
                                                DWORD Channel, FLOAT Amplitude) {
...
ETOOLS_API HRESULT WINAPI D3DX_LoadSurfaceFromSurface(
    ...
    return D3DXLoadSurfaceFromSurface(pDestSurface, pDestPalette, pDestRect, pSrcSurface,
                                      pSrcPalette, pSrcRect, Filter, ColorKey);
}

ETOOLS_API HRESULT WINAPI D3DX_CompileShader(LPCSTR pSrcData, UINT SrcDataLen,
    ...
    return D3DXCompileShader(pSrcData, SrcDataLen, pDefines, pInclude, pFunctionName, pTarget,
                             Flags, ppShader, ppErrorMsgs, ppConstantTable);
}

ETOOLS_API HRESULT WINAPI D3DX_CompileShaderFromFile(LPCSTR pSrcFile, CONST D3DXMACRO* pDefines,
                                                     LPD3DXINCLUDE pInclude, LPCSTR pFunctionName,
                                                     LPCSTR pTarget, DWORD Flags,
                                                     LPD3DXBUFFER* ppShader,
                                                     LPD3DXBUFFER* ppErrorMsgs,
                                                     LPD3DXCONSTANTTABLE* ppConstantTable) {
    return D3DXCompileShaderFromFile(pSrcFile, pDefines, pInclude, pFunctionName, pTarget, Flags,
                                     ppShader, ppErrorMsgs, ppConstantTable);
}

ETOOLS_API HRESULT WINAPI D3DX_FindShaderComment(CONST DWORD* pFunction, DWORD FourCC,
                                                 LPCVOID* ppData, UINT* pSizeInBytes) {
    return D3DXFindShaderComment(pFunction, FourCC, ppData, pSizeInBytes);
}

ETOOLS_API HRESULT WINAPI D3DX_DeclaratorFromFVF(DWORD FVF,
...
ETOOLS_API D3DXMATRIX* WINAPI D3DX_MatrixInverse(D3DXMATRIX* pOut, FLOAT* pDeterminant,
                                                 CONST D3DXMATRIX* pM) {
...
ETOOLS_API D3DXPLANE* WINAPI D3DX_PlaneTransform(D3DXPLANE* pOut, CONST D3DXPLANE* pP,
                                                 CONST D3DXMATRIX* pM) {
    return D3DXPlaneTransform(pOut, pP, pM);
}

ETOOLS_API HRESULT WINAPI D3DX_OptimizeFaces(LPCVOID pIndices, UINT NumFaces, UINT NumVertices,
                                             BOOL Indices32Bit, DWORD* pFaceRemap) {
    return D3DXOptimizeFaces(pIndices, NumFaces, NumVertices, Indices32Bit, pFaceRemap);
}

ETOOLS_API HRESULT WINAPI D3DX_OptimizeVertices(LPCVOID pIndices, UINT NumFaces, UINT NumVertices,
                                                BOOL Indices32Bit, DWORD* pVertexRemap) {
