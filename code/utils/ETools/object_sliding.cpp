...
BOOL CalculateSW(Object* object, VIPM_Result* result, u32 optimize_vertex_order) {
        ...
        while ((object->pNextCollapse->ListNext() != NULL) &&
            ...
            if (!bJustStartedANewLevel || (iCurSlidingWindowLevel == iMaxSlidingWindowLevel)) {
                ...
                if (pCollapse->TriCollapsed.size() > 0) {
                    ...
                    for (u32 i = 0; i < pCollapse->TriCollapsed.size(); i++) {
                        ...
                        MeshTri* tri =
                            pTriInfo->ppt[0]->FindTri(pTriInfo->ppt[1], pTriInfo->ppt[2]);
                        R_ASSERT(tri != NULL);
                        R_ASSERT(tri->mytri.dwNewIndex ==
                                 INVALID_INDEX); // Should not have been in a collapse this level.
                    ...
                    if (optimize_vertex_order)
                        OptimiseVertexCoherencyTriList(wTempIndices.ptr(),
                                                       pCollapse->TriCollapsed.size(),
                                                       optimize_vertex_order);

                    // And write them to the index list.
                    result->indices.insert(iCurTriBinned * 3, wTempIndices, 0,
                                           3 * pCollapse->TriCollapsed.size());
                    // memcpy ( result->indices.item ( iCurTriBinned * 3 ), wTempIndices.ptr(),
                    // sizeof(WORD) * 3 * pCollapse->TriCollapsed.size() );
            ...
            */
            ...
            if (pCollapse->TriOriginal.size() > 0) {
                ...
                result->indices.insert(iCurTriAdded * 3, wTempIndices, 0,
                                       3 * pCollapse->TriOriginal.size());
                // memcpy ( result->indices.item ( iCurTriAdded * 3 ), wTempIndices.ptr(),
                // sizeof(WORD) * 3 * pCollapse->TriOriginal.size() );
    ...
    /*
            ...
            for ( int i = 0; i <= iNumCollapses; i++ ){
                    ...
                    for ( int j = 0; j < swr->num_tris * 3; j++ ){
                            ...
                            //			R_ASSERT ( *(result->indices.item(j+swr->offset)) <
       swr->num_verts ); if (!(*(result->indices.item(j+swr->offset)) < swr->num_verts )){
                                    error_found = i;
                            }
                    }
            }
            if (error_found>-1){
                    ...
                    for ( int i = 0; i <= iNumCollapses; i++ ){
                            VIPM_SWR *swr = result->swr_records.item ( i );
                            xr_sprintf(tmp,"SWR %d [T:%d, V:%d,
       O:%d]\n",i,swr->num_tris,swr->num_verts,swr->offset); OutputDebugString(tmp); for ( int j =
       0; j < swr->num_tris; j++ ){ xr_sprintf(tmp,"%d -
    ...
    */
    ...
    for (int i = 0; i <= iNumCollapses; i++) {
        ...
        for (int j = 0; j < swr->num_tris * 3; j++) {
            R_ASSERT((j + swr->offset) < result->indices.size());
            swr->num_verts = std::max(
                swr->num_verts,
                *(result->indices.item(j + swr->offset))); // fignya index ne doljen bit bolshe!!!
