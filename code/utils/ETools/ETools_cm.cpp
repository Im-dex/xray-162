...
public:
    ...
    void xys_from_vector(u32& ux, u32& uy, u32& t_side, const Fvector& n, u32* pixels, u32 width,
                         u32 height) {
    ...
    void scale_map(u32* src_data, u32 src_width, u32 src_height, u32* dst_data, u32 dst_width,
                   u32 dst_height, float sample_factor, ETOOLS::pb_callback cb, void* pb_data) {
                ...
                for (u32 x_dst = 0; x_dst < dst_width; x_dst++) {
                    ...
                    for (float p = src_p - h_angle; p < src_p + h_angle; p += d_angle) {
                        for (float h = src_h - h_angle; h < src_h + h_angle; h += d_angle) {
                            dir.setHP(h, p);
                            sample_color.set(
                                pixel_from_vector(dir, src_data, src_width, src_height));
...
ETOOLS_API void __stdcall SimplifyCubeMap(u32* src_data, u32 src_width, u32 src_height,
                                          ...
                                          float sample_factor, pb_callback cb, void* pb_data) {
    cm.scale_map(src_data, src_width, src_height, dst_data, dst_width, dst_height, sample_factor,
                 cb, pb_data);
}
} // namespace ETOOLS
}
