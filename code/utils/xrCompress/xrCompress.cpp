...
xrCompressor::xrCompressor()
    : fs_pack_writer(NULL), bFast(false), files_list(NULL), folders_list(NULL), bStoreFiles(false),
      pPackHeader(NULL), config_ltx(NULL) {
    bytesSRC = 0;
    bytesDST = 0;
    filesTOTAL = 0;
    filesSKIP = 0;
...
bool xrCompressor::testSKIP(LPCSTR path) {
    ...
    if (stricmp(p_ext, ".thm") && strstr(path, "textures\\terrain\\terrain_") &&
        !is_tail(p_name, "_mask", 5))
        return true;

    if (strstr(path, "textures\\") && is_tail(p_name, "_nmap", 5) &&
        !strstr(p_name, "water_flowing_nmap"))
        return true;
...
void xrCompressor::write_file_header(LPCSTR file_name, const u32& crc, const u32& ptr,
                                     const u32& size_real, const u32& size_compressed) {
        ...
        if (testVFS(path)) {
            ...
            if (0 != c_size_real) {
                ...
                if (bFast) {
                    ...
                    R_ASSERT(LZO_E_OK == lzo1x_999_compress((u8*)src->pointer(), c_size_real,
                                                            c_data, &c_size_compressed, c_heap));
                }
                ...
                if ((c_size_compressed + 16) >= c_size_real) {
                    ...
                    if (!bFast) {
                        ...
                        R_ASSERT(LZO_E_OK ==
                                 lzo1x_optimize(c_data, c_size_compressed, c_out, &c_orig, NULL));
                    ...
                    Msg("%-80s   - OK (%3.1f%%)", path,
                        100.f * float(c_size_compressed) / float(src->length()));
                }
...
void xrCompressor::PerformWork() {
    if (!files_list->empty() && target_name.size()) {
        ...
        for (u32 it = 0; it < files_list->size(); it++) {
            xr_sprintf(caption, "Compress files: %d/%d - %d%%", it, files_list->size(),
                       (it * 100) / files_list->size());
...
void xrCompressor::ProcessLTX(CInifile& ltx) {
    ...
    // free
    auto it = files_list->begin();
    auto itE = files_list->end();
    for (; it != itE; ++it)
        xr_free(*it);
