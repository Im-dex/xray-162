...
void logThread() {
    ...
    logWindow =
        CreateDialog(HINSTANCE(GetModuleHandle(0)), MAKEINTRESOURCE(IDD_LOG), 0, logDlgProc);
    if (!logWindow) {
        R_CHK(GetLastError());
    };
    ...
    while (TRUE) {
        SetPriorityClass(
            GetCurrentProcess(),
            IDLE_PRIORITY_CLASS); // bHighPriority?NORMAL_PRIORITY_CLASS:IDLE_PRIORITY_CLASS
