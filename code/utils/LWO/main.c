...
int testload(char* filename, unsigned int* failID, int* failpos) {
    ...
    if (obj) {
        printf("Layers:  %d\n"
               ...
               obj->nlayers, obj->nsurfs, obj->nenvs, obj->nclips, obj->layer->point.count,
               obj->layer->polygon.count);
        nobjects++;
        nlayers += obj->nlayers;
        nsurfs += obj->nsurfs;
        nenvs += obj->nenvs;
...
int make_filename(char* spec, char* name, char* fullname) {
    char drive[_MAX_DRIVE], dir[_MAX_DIR], node[_MAX_FNAME], ext[_MAX_EXT];
...
int make_filespec(char* spec, char* subdir, char* fullname) {
    char name[_MAX_FNAME], drive[_MAX_DRIVE], dir[_MAX_DIR], node[_MAX_FNAME], ext[_MAX_EXT];
