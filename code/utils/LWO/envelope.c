    ...
    if (time < skey->time) {
        ...
        case BEH_LINEAR:
            ...
            case SHAPE_STEP:
                return skey->value;
            case SHAPE_LINE:
                return (skey->value - skey->next->value) / (skey->time - skey->next->time) *
                           (time - skey->next->time) +
                       skey->next->value;
            case SHAPE_TCB:
            case SHAPE_HERM:
            ...
            case SHAPE_BEZ2:
                return (skey->param[1] - skey->param[3]) / (skey->param[0] - skey->param[2]) *
                           (time - skey->time) +
                       skey->value;
            }
        }
    ...
    /* use post-behavior if time is after last key time */
        ...
        case BEH_LINEAR:
            ...
            case SHAPE_STEP:
                return ekey->value;
            case SHAPE_LINE:
                return (ekey->value - ekey->prev->value) / (ekey->time - ekey->prev->time) *
                           (time - ekey->prev->time) +
                       ekey->prev->value;
            case SHAPE_TCB:
            case SHAPE_HERM:
            ...
            case SHAPE_BEZ2:
                return (ekey->param[3] - ekey->param[1]) / (ekey->param[2] - ekey->param[0]) *
                           (time - ekey->time) +
                       ekey->value;
            }
        }
