...
void CalculateNormalMap(NVI_Image* pSrc, ConvolutionKernel* pKernels, int num_kernels, float scale,
                        bool wrap) {
...
void ConvertAlphaToNormalMap_5x5(NVI_Image* pSrc, float scale, bool wrap) {
    ...
    */
    ...
    ConvolutionKernelElement dv_elem[] = {
        { -2, 2, wt22 },   { -1, 2, wt12 },   { 0, 2, 1.0f / 4.0f },   { 1, 2, wt12 },
        { 2, 2, wt22 },    { -2, 1, wt12 },   { -1, 1, wt11 },         { 0, 1, 1.0f / 2.0f },
        { 1, 1, wt11 },    { 2, 1, wt12 },

        { -2, -1, -wt12 }, { -1, -1, -wt11 }, { 0, -1, -1.0f / 2.0f }, { 1, -1, -wt11 },
        { 2, -1, -wt12 },  { -2, -2, -wt22 }, { -1, -2, -wt12 },       { 0, -2, -1.0f / 4.0f },
        { 1, -2, -wt12 },  { 2, -2, -wt22 }
    };
...
void ConvertAlphaToNormalMap_7x7(NVI_Image* pSrc, float scale, bool wrap) {
    ...
    float du_f[] = { -1, -2, -3, 0,  3,  2,  1,  -2, -3, -4, 0,  4,  3,  2,  -3, -4, -5,
...
void ConvertAlphaToNormalMap_9x9(NVI_Image* pSrc, float scale, bool wrap) {
    ...
    float du_f[] = { -1, -2, -3, -4, 0,  4,  3,  2,  1, -2, -3, -4, -5, 0,  5,  4,  3,
...
template <class _It>
IC void TW_Iterate_2OP(u32 width, u32 height, u32 pitch, u8* dst, u8* src0, u8* src1,
                       const _It pred) {
    for (u32 y = 0; y < height; y++) {
        for (u32 x = 0; x < width; x++) {
...
int DXTCompressBump(LPCSTR out_name, u8* T_height_gloss, u8* T_normal_map, u32 w, u32 h, u32 pitch,
                    STextureParams* fmt, u32 depth) {
    VERIFY(4 == depth);
    ...
    gloss_power = 0.f;
    TW_Iterate_1OP(w, h, pitch, T_normal_1, T_height_gloss,
                   it_gloss_rev); // T_height_gloss.a (gloss) -> T_normal_1 + reverse of channels
...
(fnameB); t_temp->surface_set	(T_normal_2C	);
