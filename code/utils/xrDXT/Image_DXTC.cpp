...
bool Image_DXTC::LoadFromFile(LPCSTR filename) {
    ...
    if (m_CompFormat == PF_DXT1 || m_CompFormat == PF_DXT2 || m_CompFormat == PF_DXT3 ||
        m_CompFormat == PF_DXT4 || m_CompFormat == PF_DXT5) {
...
inline void DecodeColorBlock(DWORD* pImPos, DXTColBlock* pColorBlock, int width, DWORD* col_0,
...
VOID Image_DXTC::DecodePixelFormat(CHAR* strPixelFormat, DDPIXELFORMAT* pddpf) {
    ...
    case 0:
        ...
        xr_sprintf(strPixelFormat, sizeof(string256), "ARGB-%d%d%d%d%s",
                   GetNumberOfBits(pddpf->dwRGBAlphaBitMask), GetNumberOfBits(pddpf->dwRBitMask),
                   GetNumberOfBits(pddpf->dwGBitMask), GetNumberOfBits(pddpf->dwBBitMask),
...
//	total = (unsigned long) ( end_clk - start_clk );
        ...
        fprintf( pf, "start:  %u  %u       end:  %u  %u\n", start_clk.LowPart, start_clk.HighPart,
end_clk.LowPart, end_clk.HighPart );
...
start_clk.LowPart ), dur ); fprintf( pf, "\n\n");

        //TRACE( "Total timing session took:  %u cycles = %f seconds\n", ( end_clk.LowPart -
start_clk.LowPart ), dur );
        //TRACE( "\n\n");
        ...
        for(n=0; n < NMETHOD; n++ )
        {
                for(i=0; i < NBATCHES; i++ )
                {
                        ...
                        fprintf( pf, "  tot:   %u %u\n", method[n][i].m_interval_sum.HighPart,
