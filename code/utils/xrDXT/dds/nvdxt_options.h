...
public:
    ...
    nvNormalMapFilters filterKernel;                  // kernel used to create normal maps.  Done this way to be
...
public:
    nvCompressionOptions() { SetDefaultOptions(); }

    void SetDefaultOptions() {
        quality = kQualityProduction;
        rmsErrorSearchThreshold = 400; // in  kQualityHighest mode, rms error above which will cause
                                       // a long search for a better answer
        ...
        bDitherMip0 =
            false; // enable dithering during 16 bit conversion for each MIP level (after filtering)
    ...
    void DoNotGenerateMIPMaps() { mipMapGeneration = kNoMipMaps; }
    ...
    void NoBorderColor() { bBorder = false; }
    ...
    void SetFilterSharpness(float sharp) { filterBlur = sharp; }
    ...
    nvBoolean bDitherMip0;  // enable dithering during 16 bit conversion for each MIP level (after
                            // filtering)
