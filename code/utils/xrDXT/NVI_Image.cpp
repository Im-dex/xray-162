...
void NVI_ImageBordered::CopyDataFromSource() {
    ...
    for (j = 0; j < sy; j++) {
        ...
        memcpy(&pPadArray[(j - m_nBorderYLow) * m_nSizeX - m_nBorderXLow], &pSrcArray[j * sx],
               sizeof(DWORD) * sx);
    }
    ...
    if (m_bWrap) {
        ...
        for (j = 0; j < m_nSizeY; j++) {
            ...
            memcpy(&pPadArray[j * m_nSizeX], &pPadArray[(j * m_nSizeX) + sx],
                   sizeof(DWORD) * (-m_nBorderXLow));
        ...
        for (j = 0; j < -m_nBorderYLow; j++) {
            ...
            memcpy(&pPadArray[j * m_nSizeX], &pPadArray[(j + sy - 1) * m_nSizeX],
        ...
        for (j = 0; j < -m_nBorderYLow; j++) {
            ...
            memcpy(&pPadArray[j * m_nSizeX], &pPadArray[(-m_nBorderYLow) * m_nSizeX],
                   sizeof(DWORD) * m_nSizeX);
        }
        ...
        for (j = 0; j < m_nSizeY; j++) {
            ...
            for (i = sx - m_nBorderXLow; i < m_nSizeX; i++) {
                memcpy(&pPadArray[j * m_nSizeX + i], &pPadArray[j * m_nSizeX + i - 1],
                       sizeof(DWORD));
            }
