...
template <typename T>
typename CWrapHelper<T>::result_type* wrap_value(luabind::object object, luabind::object table,
                                                 LPCSTR name) {
    return (CWrapHelper<T>::wrap_value < std::is_class_v<T> &&
...
/*
void CScriptPropertiesListHelper::floatRDOnDraw			(PropValue* sender,  LPCSTR&
draw_val)
{
        shared_str		temp;
...
/*
void CScriptPropertiesListHelper::NameDraw				(PropValue* sender,
LPCSTR& draw_val)
{
        shared_str		temp;
...
CaptionValue* CScriptPropertiesListHelper::CreateCaption(PropItemVec* items, LPCSTR key,
                                                         LPCSTR val) {
    return (PHelper().CreateCaption(*items, key, val));
}

CanvasValue* CScriptPropertiesListHelper::CreateCanvas(PropItemVec* items, LPCSTR key, LPCSTR val,
                                                       int height) {
    return (PHelper().CreateCanvas(*items, key, val, height));
}

ButtonValue* CScriptPropertiesListHelper::CreateButton(PropItemVec* items, LPCSTR key, LPCSTR val,
                                                       u32 flags) {
    return (PHelper().CreateButton(*items, key, val, flags));
}

ChooseValue* CScriptPropertiesListHelper::CreateChoose(PropItemVec* items, LPCSTR key,
                                                       luabind::object object, LPCSTR name,
                                                       u32 mode, LPCSTR path, LPCSTR fill_param,
                                                       u32 sub_item_cnt) {
    return (PHelper().CreateChoose(*items, key, wrap_value<shared_str>(object, name), mode, path,
                                   (void*)fill_param, sub_item_cnt));
}
...
// S8Value* CScriptPropertiesListHelper::CreateS8		(PropItemVec* items, LPCSTR key,
// luabind::object object, LPCSTR name,  s8 mn,  s8 mx,  s8 inc)
//{
//	return		(PHelper().CreateS8(*items,key,wrap_value<s8>(object,name),mn,mx,inc));
//}
//
// S8Value* CScriptPropertiesListHelper::CreateS8		(PropItemVec* items, LPCSTR key,
// luabind::object object, LPCSTR name,  s8 mn,  s8 mx)
//{
//	return		(PHelper().CreateS8(*items,key,wrap_value<s8>(object,name),mn,mx));
//}
//
// S8Value* CScriptPropertiesListHelper::CreateS8		(PropItemVec* items, LPCSTR key,
// luabind::object object, LPCSTR name,  s8 mn)
//{
...
//}
//
S16Value* CScriptPropertiesListHelper::CreateS16(PropItemVec* items, LPCSTR key,
                                                 luabind::object object, LPCSTR name, s16 mn,
                                                 s16 mx, s16 inc) {
...
S32Value* CScriptPropertiesListHelper::CreateS32(PropItemVec* items, LPCSTR key,
                                                 luabind::object object, LPCSTR name, s32 mn,
                                                 s32 mx, s32 inc) {
...
U8Value* CScriptPropertiesListHelper::CreateU8(PropItemVec* items, LPCSTR key,
                                               luabind::object object, LPCSTR name, u8 mn, u8 mx,
                                               u8 inc) {
...
U8Value* CScriptPropertiesListHelper::CreateU8(PropItemVec* items, LPCSTR key,
                                               luabind::object object, LPCSTR name, u8 mn) {
...
U16Value* CScriptPropertiesListHelper::CreateU16(PropItemVec* items, LPCSTR key,
                                                 luabind::object object, LPCSTR name, u16 mn,
                                                 u16 mx, u16 inc) {
...
U32Value* CScriptPropertiesListHelper::CreateU32(PropItemVec* items, LPCSTR key,
                                                 luabind::object object, LPCSTR name, u32 mn,
                                                 u32 mx, u32 inc) {
...
FloatValue* CScriptPropertiesListHelper::CreateFloat(PropItemVec* items, LPCSTR key,
                                                     luabind::object object, LPCSTR name, float mn,
                                                     float mx, float inc, int decim) {
    return (
        PHelper().CreateFloat(*items, key, wrap_value<float>(object, name), mn, mx, inc, decim));
}
...
BOOLValue* CScriptPropertiesListHelper::CreateBOOL(PropItemVec* items, LPCSTR key,
                                                   luabind::object object, luabind::object table,
                                                   LPCSTR name) {
    return (PHelper().CreateBOOL(*items, key, wrap_value<bool>(object, table, name)));
}

VectorValue* CScriptPropertiesListHelper::CreateVector(PropItemVec* items, LPCSTR key,
                                                       luabind::object object, LPCSTR name,
                                                       float mn, float mx, float inc, int decim) {
    return (
        PHelper().CreateVector(*items, key, wrap_value<Fvector>(object, name), mn, mx, inc, decim));
}

VectorValue* CScriptPropertiesListHelper::CreateVector(PropItemVec* items, LPCSTR key,
                                                       luabind::object object, LPCSTR name,
                                                       float mn, float mx, float inc) {
...
Flag8Value* CScriptPropertiesListHelper::CreateFlag8(PropItemVec* items, LPCSTR key,
                                                     luabind::object object, LPCSTR name, u8 mask,
                                                     LPCSTR c0, LPCSTR c1, u32 flags) {
    return (
        PHelper().CreateFlag8(*items, key, wrap_value<Flags8>(object, name), mask, c0, c1, flags));
}
...
Flag16Value* CScriptPropertiesListHelper::CreateFlag16(PropItemVec* items, LPCSTR key,
                                                       luabind::object object, LPCSTR name,
                                                       u16 mask, LPCSTR c0, LPCSTR c1, u32 flags) {
...
Flag32Value* CScriptPropertiesListHelper::CreateFlag32(PropItemVec* items, LPCSTR key,
                                                       luabind::object object, LPCSTR name,
                                                       u32 mask, LPCSTR c0, LPCSTR c1, u32 flags) {
...
Token8Value* CScriptPropertiesListHelper::CreateToken8(PropItemVec* items, LPCSTR key,
                                                       luabind::object object, LPCSTR name,
                                                       CScriptTokenList* token) {
    return (PHelper().CreateToken8(*items, key, wrap_value<u8>(object, name),
                                   &*token->tokens().begin()));
}

Token16Value* CScriptPropertiesListHelper::CreateToken16(PropItemVec* items, LPCSTR key,
                                                         luabind::object object, LPCSTR name,
                                                         CScriptTokenList* token) {
    return (PHelper().CreateToken16(*items, key, wrap_value<u16>(object, name),
                                    &*token->tokens().begin()));
}

Token32Value* CScriptPropertiesListHelper::CreateToken32(PropItemVec* items, LPCSTR key,
                                                         luabind::object object, LPCSTR name,
                                                         CScriptTokenList* token) {
    return (PHelper().CreateToken32(*items, key, wrap_value<u32>(object, name),
                                    &*token->tokens().begin()));
}

/*
RToken8Value*	CScriptPropertiesListHelper::CreateRToken8	(PropItemVec* items, LPCSTR
key, luabind::object object, LPCSTR name, xr_rtoken* token, u32 t_cnt)
{
        return
(PHelper().CreateRToken8(*items,key,wrap_value<shared_str>(object,name),token));
}

RToken16Value*	CScriptPropertiesListHelper::CreateRToken16	(PropItemVec* items, LPCSTR
key, luabind::object object, LPCSTR name, RTokenVec* token)
{
        return
(PHelper().CreateRToken16(*items,key,wrap_value<shared_str>(object,name),token));
}

RToken32Value*	CScriptPropertiesListHelper::CreateRToken32	(PropItemVec* items, LPCSTR
key, luabind::object object, LPCSTR name, RTokenVec* token)
{
        return
(PHelper().CreateRToken32(*items,key,wrap_value<shared_str>(object,name),token));
}
*/

RListValue* CScriptPropertiesListHelper::CreateRList(PropItemVec* items, LPCSTR key,
                                                     luabind::object object, LPCSTR name,
                                                     CScriptRTokenList* tokens) {
    return (PHelper().CreateRList(*items, key, wrap_value<shared_str>(object, name),
                                  &*tokens->tokens().begin(), tokens->size()));
}

U32Value* CScriptPropertiesListHelper::CreateColor(PropItemVec* items, LPCSTR key,
                                                   luabind::object object, LPCSTR name) {
    return (PHelper().CreateColor(*items, key, wrap_value<u32>(object, name)));
}

ColorValue* CScriptPropertiesListHelper::CreateFColor(PropItemVec* items, LPCSTR key,
                                                      luabind::object object, LPCSTR name) {
    return (PHelper().CreateFColor(*items, key, wrap_value<Fcolor>(object, name)));
}

VectorValue* CScriptPropertiesListHelper::CreateVColor(PropItemVec* items, LPCSTR key,
                                                       luabind::object object, LPCSTR name) {
    return (PHelper().CreateVColor(*items, key, wrap_value<Fvector>(object, name)));
}

RTextValue* CScriptPropertiesListHelper::CreateRText(PropItemVec* items, LPCSTR key,
                                                     luabind::object object, LPCSTR name) {
    return (PHelper().CreateRText(*items, key, wrap_value<shared_str>(object, name)));
}

FloatValue* CScriptPropertiesListHelper::CreateTime(PropItemVec* items, LPCSTR key,
                                                    luabind::object object, LPCSTR name, float mn,
                                                    float mx) {
...
FloatValue* CScriptPropertiesListHelper::CreateAngle(PropItemVec* items, LPCSTR key,
                                                     luabind::object object, LPCSTR name, float mn,
                                                     float mx, float inc, int decim) {
    return (
        PHelper().CreateAngle(*items, key, wrap_value<float>(object, name), mn, mx, inc, decim));
}
...
VectorValue* CScriptPropertiesListHelper::CreateAngle3(PropItemVec* items, LPCSTR key,
                                                       luabind::object object, LPCSTR name,
                                                       float mn, float mx, float inc, int decim) {
    return (
        PHelper().CreateAngle3(*items, key, wrap_value<Fvector>(object, name), mn, mx, inc, decim));
}
