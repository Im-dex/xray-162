...
#ifdef _XBOX
    for (DWORD i = 0; i < count; i++) {
        *ptr = (((((*ptr)) << 24) & 0xFF000000) | ((((*ptr)) << 8) & 0x00FF0000) |
                ((((*ptr)) >> 8) & 0x0000FF00) | ((((*ptr)) >> 24) & 0x000000FF));
...
void __inline ChangeByteOrder(unsigned __int64* ptr, DWORD count) {
    ...
    for (DWORD i = 0; i < count; i++) {
        *ptr =
            ((((*ptr) << 56) & 0xFF00000000000000ULL) | (((*ptr) << 40) & 0x00FF000000000000ULL) |
             (((*ptr) << 24) & 0x0000FF0000000000ULL) | (((*ptr) << 8) & 0x000000FF00000000ULL) |
             (((*ptr) >> 8) & 0x00000000FF000000ULL) | (((*ptr) >> 24) & 0x0000000000FF0000ULL) |
             (((*ptr) >> 40) & 0x000000000000FF00ULL) | (((*ptr) >> 56) & 0x00000000000000FFULL));
