...
DECLARE_INTERFACE_(IAgent, IUnknown) {
    ...
    // dataDesc - symbolic identifier of data, f.e. 'geometry', zero-terminated string, case
    // sensetive.  Agent can cache data internally during the session to minimize network transfer.
    ...
    // IAgent->GetData(stream); IAgent->PurgeCache(); stream->Release() sequences are correctly
    // handled.
