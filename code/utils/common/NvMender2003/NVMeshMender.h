...
------------------------
        ...
        for(DWORD i = 0; i < numVerts; ++i)
        {
                ...
                v.s = myVerts[i].s;
                v.t = myVerts[i].t;
                //meshmender will computer normals, tangents, and binormals, no need to fill those
in.
                //however, if you do not have meshmender compute the normals, you _must_ pass in
valid
...
public:
    ...
    //				  number of indices passed in, but they may point to new vertices
    // now.
    //
    ...
    //				so it could be used to map any per vertex data you had in your
    // original 				mesh to the new mesh like so:
    //
    //					for each new vertex index
    //						newVert[index]->myData = oldVert[
    // mappingNewToOldVert[index]]->myData;
    //
    ...
    //							 so that they are allowed to be smoothed
    // together 							 ranges between -1.0 and
    // +1.0
    //							 this is ignored if computeNormals is set to
    ...
    //						unnormalized face normal together.  Thus weighting
    // the 						normal by the face area by a given ammount
    //						ranges between 0.0 and +1.0
    //						0.0 means use the normalized face normals (not weighted
    //by  area)
    //						1.0 means use the unnormalized face normal(weighted
    //by  area) 						this is ignored if computeNormals is set to
    // DONT_CALCULATE_NORMALS
    //
    // computeNormals - should mesh mender calculate normals? If this is set to
    // DONT_CALCULATE_NORMALS
    //					then the vertex normals after mesh mender is called will be
    // the
    //					same ones you pass in.  If you are automatically calculating
    //normals  yourself, 					you may find that meshmender
    // provides  greater control over how normals are smoothed
    //					together. I've been able to get better results using the Crease
    //angle  with 					meshmender's smoothing groups
    //
    // respectExistingSplits - DONT_RESPECT_SPLITS means that neighboring triangles for smoothing
    // will be determined
    //						  based on position and not on indices.
    //						  RESPECT_SPLITS means that neighboring triangles will be determined based on
    //the  indices of the
    //						  triangle and not the positions of the vertices.
    //						  you can usually get better smoothing by not respecting
    //existing  splits
    //						  only respect them if you know they should be
    // respected.
    //
    // fixCylindricalWrapping - DONT_FIX_CYLINDRICAL means take the texture coordinates as they come
    //						   FIX_CYLINDRICAL means we might need to split the
    // verts
    //						   at that point and generate the proper texture
    // coordinate.
    //						   for instance, if we have tex coords   0.9 -> 0.0-> 0.2 we would need
    //to  add
    //						   a new vert so that we have       0.9 -> 1.0 0.0->
    // 0.2
    //						   this is only supported for texture coordinates in the range [ 0.0f ,
    //1.0f ] 						   NOTE: don't leave this on for all meshes,
    // only use it when you know 							you need it.
