...
void MeshMender::UpdateIndices(const size_t oldIndex, const size_t newIndex,
...
void MeshMender::ProcessNormals(TriangleList& possibleNeighbors, std::vector<Vertex>& theVerts,
    ...
    for (unsigned i = 0; i < possibleNeighbors.size(); ++i) {
        Triangle* currTri = &(m_Triangles[possibleNeighbors[i]]);
        assert(currTri);
        if (!currTri->handled) {
            BuildGroups(currTri, possibleNeighbors, neighborGroups, theVerts,
    ...
    for (unsigned i = 0; i < neighborGroups.size(); ++i) {
        ...
        for (size_t t = 0; t < curGroup.size(); ++t) // for each tri
        {
            ...
            for (size_t indx = 0; indx < 3; ++indx) // for each vert in that tri
            {
                ...
                if (theVerts[m_Triangles[tID].indices[indx]].pos == workingPosition) {
                    ...
                    if (otherGroupsIndices.find(m_Triangles[tID].indices[indx]) !=
                        otherGroupsIndices.end()) {
        ...
        for (std::set<size_t>::iterator it = thisGroupIndices.begin(); it != thisGroupIndices.end();
             ++it) {
...
void MeshMender::ProcessTangents(TriangleList& possibleNeighbors, std::vector<Vertex>& theVerts,
    ...
    for (unsigned i = 0; i < neighborGroups.size(); ++i) {
        ...
        for (unsigned int t = 0; t < neighborGroups[i].size();
             ++t) // for each triangle in the group,
        {
...
bool MeshMender::Mend(std::vector<Vertex>& theVerts, std::vector<unsigned int>& theIndices,
                      std::vector<unsigned int>& mappingNewToOldVert,
                      const float minNormalsCreaseCosAngle, const float minTangentsCreaseCosAngle,
                      const float minBinormalsCreaseCosAngle, const float weightNormalsByArea,
    ...
    for (VertexChildrenMap::iterator vert = m_VertexChildrenMap.begin();
         vert != m_VertexChildrenMap.end(); ++vert) {
...
bool MeshMender::TriHasEdge(const size_t& p0, const size_t& p1, const size_t& triA,
                            const size_t& triB, const size_t& triC) {
...
bool MeshMender::SharesEdgeRespectSplits(Triangle* triA, Triangle* triB,
...
void MeshMender::SetUpFaceVectors(Triangle& t, const std::vector<Vertex>& verts,
                                  const NormalCalcOption computeNormals) {
    ...
    if (computeNormals == CALCULATE_NORMALS) {
        ...
        if (WeightNormalsByArea < 1.0f) {
            ...
            D3DXVECTOR3 finalNorm =
                (normalizedNorm * (1.0f - WeightNormalsByArea)) + (t.normal * WeightNormalsByArea);
            t.normal = finalNorm;
        }
    }
    // need to set up tangents, and binormals here
    GetGradients(verts[t.indices[0]], verts[t.indices[1]], verts[t.indices[2]], t.tangent,
                 t.binormal);
}
    ...
    for (size_t i = 0; i < len; ++i) {
        ...
        if ((lenTan <= 0.001f) || (lenBin <= 0.001f)) // should be approx 1.0f
        {
            ...
            if (lenTan > 0.5f) {
                ...
                D3DXVec3Cross(&(theVerts[i].binormal), &(theVerts[i].normal),
                              &(theVerts[i].tangent));

            } else if (lenBin > 0.5) {
                // the binormal is good and we can use it to calculate
                // the tangent
                D3DXVec3Cross(&(theVerts[i].tangent), &(theVerts[i].binormal),
                              &(theVerts[i].normal));
            } else {
                ...
                if (D3DXVec3Dot(&xAxis, &(theVerts[i].normal)) <
                    D3DXVec3Dot(&yAxis, &(theVerts[i].normal))) {
...
void MeshMender::GetGradients(const MeshMender::Vertex& v0, const MeshMender::Vertex& v1,
...
void MeshMender::UpdateTheIndicesWithFinalIndices(std::vector<unsigned int>& theIndices) {
    ...
    assert(((theIndices.size() / 3) == m_Triangles.size()) &&
           "invalid number of tris, or indices.");
...
void MeshMender::AppendToMapping(const size_t oldIndex, const size_t originalNumVerts,
                                 std::vector<unsigned int>& mappingNewToOldVert) {
