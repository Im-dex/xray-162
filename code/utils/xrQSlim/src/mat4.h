...
public:
    ...
    inline Vec4 col(int i) const { return Vec4(row[0][i], row[1][i], row[2][i], row[3][i]); }
...
inline Mat4 operator-(const Mat4& n) { return Mat4(-n[0], -n[1], -n[2], -n[3]); }

inline Mat4 operator*(double s, const Mat4& m) {
    return Mat4(m[0] * s, m[1] * s, m[2] * s, m[3] * s);
}
inline Mat4 operator*(const Mat4& m, double s) { return s * m; }
...
inline Mat4 transpose(const Mat4& m) { return Mat4(m.col(0), m.col(1), m.col(2), m.col(3)); }
