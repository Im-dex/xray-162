...
static void AddImpImplicitNetTask(u32 from, u32 to) {
    // tmanager.start		(xr_new<ImplicitThread>
    // (thID,&defl,thID*stride,thID*stride+stride));
    if (from == to)
        return;
    R_ASSERT(from < to);
    ImplicitExecute exec(from, to);

    tnet_execution_base<et_implicit_light>* el =
        lc_net::execution_factory.create<et_implicit_light>();
