...
public:
    itterate_adjacents_params_static(const type_vertex* _pTestVertex, vecFace& _new_adj_vec,
                                     float _sm_cos)
        : pTestVertex(_pTestVertex), new_adj_vec(_new_adj_vec), sm_cos(_sm_cos) {}

private:
    IC static bool has_same_edge(const type_face* F1, const type_face* F2, u16& F1_edge_index,
                                 u16& F2_edge_index) {
        F1_edge_index = u16(-1);
        F2_edge_index = u16(-1);
    ...
    IC static bool do_connect_faces(const type_face& start, const type_face& test,
...
public:
    ...
    IC bool is_processed(const type_face& f) const { return f.flags.bSplitted; }
    ...
    IC bool add_adjacents(u32 start_face_idx, u32 test_face_idx) {
        ...
        if (has_same_edge(start_face, test_face, StartFace_common_edge_index,
                          TestFace_common_edge_index)) {
            if ((start_face == test_face) ||
                do_connect_faces(*start_face, *test_face, StartFace_common_edge_index,
                                 TestFace_common_edge_index, sm_cos)) {
