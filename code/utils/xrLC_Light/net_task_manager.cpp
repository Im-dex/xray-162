...
void DumpDiff() {
    ...
    xr_vector<std::pair<CDeflector*, CDeflector*>>::iterator i = diff.begin(), b = diff.begin(),
                                                             e = diff.end();
    for (; i != e; ++i) {
        Msg("diff %d", u32(i - b));
...
void net_task_manager::create_global_data_write(LPCSTR save_path) {
    ...
    std::random_shuffle(inlc_global_data()->g_deflectors().begin(),
                        inlc_global_data()->g_deflectors().end());
#endif
...
// inlc_global_data()->create_read_faces();
    ...
    clMsg("num task complited : %d , num task left %d  (task num %d)", size - pool_size, pool_size,
          size);
    if (pool.empty()) {
