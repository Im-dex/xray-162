...
//[0...size_x) 	int s_z	= iFloor			(EYE.z/slot_size+.5f)+offs_z;		//
//[0...size_z)
...
public:
    DetailHeader()
        : _version(u32(-1)), object_count(u32(-1)), offs_x(u32(-1)), offs_z(u32(-1)),
    ...
    IC u32 slots_count() const { return size_x * size_z; }
...
IC bool is_empty(const DetailSlot& DS) {
    return (DS.id0 == DetailSlot::ID_Empty) && (DS.id1 == DetailSlot::ID_Empty) &&
