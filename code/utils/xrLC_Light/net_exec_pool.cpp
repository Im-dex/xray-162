...
void exec_pool::receive_result(IGenericStream* inStream) {
    ...
    clMsg("num task complited : %d , num task left %d  (task num %d)", l_completed,
          size - l_completed, size);
#endif
    ...
    if (l_completed == size) {
        ...
        // xr_sprintf( buf, "%s %s calculation time",make_time( buf,start_time.GetElapsed_sec() ),
        // _name );  Phase( buf );
        //		}
