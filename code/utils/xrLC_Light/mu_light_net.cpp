...
void RunRefModelsNet() {
    ...
    for (u32 thID = 0; thID < num_tasks; thID++) {
        ...
        tnet_execution_base<et_mu_ref_light>* el =
            lc_net::execution_factory.create<et_mu_ref_light>();
...
void RunBaseModelsNet() {
    ...
    if (num == 0)
        return;
    for (u32 i = 0; i < num; i++) {
        tnet_execution_base<et_mu_base_light>* el =
            lc_net::execution_factory.create<et_mu_base_light>();
