...
void CDeflector::L_Direct_Edge(CDB::COLLIDER* DB, base_lighting* LightsSelected, Fvector2& p1,
                               Fvector2& p2, Fvector& v1, Fvector& v2, Fvector& N, float texel_size,
                               Face* skip) {
    Fvector vdir;
    vdir.sub(v2, v1);
...
void CDeflector::L_Direct(CDB::COLLIDER* DB, base_lighting* LightsSelected, HASH& H) {
        ...
        for (u32 U = 0; U < lm.width; U++) {
            ...
            try {
                for (u32 J = 0; J < Jcount; J++) {
                    ...
                    std::vector<UVtri*>& space = H.query(P.x, P.y);
    ...
    for (u32 t = 0; t < UVpolys.size(); t++) {
        ...
        R_ASSERT(F);
        try {
            L_Direct_Edge(DB, LightsSelected, T.uv[0], T.uv[1], F->v[0]->P, F->v[1]->P, F->N,
                          texel_size, F);
            L_Direct_Edge(DB, LightsSelected, T.uv[1], T.uv[2], F->v[1]->P, F->v[2]->P, F->N,
                          texel_size, F);
            L_Direct_Edge(DB, LightsSelected, T.uv[2], T.uv[0], F->v[2]->P, F->v[0]->P, F->N,
                          texel_size, F);
        } catch (...) {
