...
static struct destruct_vertex_not_uregister {
    static void destruct(_vertex*& v) { mu_vertices_pool().destroy(v); }
} _destruct_vertex;

static struct destruct_face_not_uregister {
    static void destruct(_face*& f) { mu_faces_pool().destroy(f); }
} _destruct_face;
...
xrMU_Model* xrMU_Model::read_create() { return xr_new<xrMU_Model>(); }
...
void xrMU_Model::write_adjacents(IWriter& w, xrMU_Model::twrite_faces& write_faces,
                                 const _vertex& v) {
...
void xrMU_Model::write_face_verts(IWriter& w, xrMU_Model::twrite_vertices& write_verts,
                                  const _face& v) {
