...
static void thread_init(void) { printf("Using sproc() threads.\n"); }

static void thread_wrapper(void* ptr, size_t stack_len) {
...
/* Create a thread. */
static int thread_create(struct thread_st* st) {
    st->flags = 0;
    if (!st->sp)
...
static void wait_for_thread(struct thread_st st[], int n_thr, int (*end_thr)(struct thread_st*)) {
    int i;
    int id;
