...
void xrNetDOLight() {
    ...
    //			continue;
    //		tnet_execution_base< et_detail_light > *el =
    //lc_net::execution_factory.create<et_detail_light>(); 		el->implementation(
    //).construct( _x, _z ); 		get_task_manager().add_task( el );
    //	}
    //}
    ...
    do {
        ...
        if (task_slots_number == max_task_slots_number || end == slots_count) {
            tnet_execution_base<et_detail_light>* el =
                lc_net::execution_factory.create<et_detail_light>();
