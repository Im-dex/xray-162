...
void vfOptimizeParameters(xr_vector<xr_vector<REAL>>& A, xr_vector<xr_vector<REAL>>& B,
    ...
    // clMsg					("***MU-fitter***: %6d : %17.8f
    // (%17.8f)",i,dFunctional,dFunctional/dwTestCount);
    do {
        ...
        std::transform(daGradient.begin(), daGradient.end(), daDelta.begin(), daDelta.begin(),
                       std::plus<REAL>());
        ...
        i++;
    } while ((((dPreviousFunctional - dFunctional) / dwTestCount) > dEpsilon) &&
             (i <= dwMaxIterationCount));

    if (dPreviousFunctional < dFunctional) {
        std::transform(daDelta.begin(), daDelta.end(), daDelta.begin(),
                       std::bind(std::multiplies<REAL>(), std::placeholders::_1, -1));
        std::transform(C.begin(), C.end(), daDelta.begin(), C.begin(), std::plus<REAL>());
        std::transform(D.begin(), D.end(), daDelta.begin(), D.begin(), std::plus<REAL>());
    }
