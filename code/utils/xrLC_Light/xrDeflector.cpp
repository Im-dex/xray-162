...
void blit(lm_layer& dst, u32 ds_x, u32 ds_y, lm_layer& src, u32 ss_x, u32 ss_y, u32 px, u32 py,
          u32 aREF) {
    R_ASSERT(ds_x >= (ss_x + px));
    R_ASSERT(ds_y >= (ss_y + py));
    for (u32 y = 0; y < ss_y; y++)
...
void blit_r(lm_layer& dst, u32 ds_x, u32 ds_y, lm_layer& src, u32 ss_x, u32 ss_y, u32 px, u32 py,
            u32 aREF) {
    R_ASSERT(ds_x >= (ss_y + px));
    R_ASSERT(ds_y >= (ss_x + py));
    for (u32 y = 0; y < ss_y; y++)
...
void CDeflector::OA_Export() {
    ...
    if (tN.magnitude() > EPS_S && xr::valid(tN))
    ...
    u32 dwWidth =
        iCeil(size.x * inlc_global_data()->g_params().m_lm_pixels_per_meter * density + .5f);
    clamp(dwWidth, 1u, 512u - 2 * BORDER);
    u32 dwHeight =
        iCeil(size.y * inlc_global_data()->g_params().m_lm_pixels_per_meter * density + .5f);
