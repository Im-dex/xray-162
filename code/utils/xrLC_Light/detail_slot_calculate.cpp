...
public:
    ...
    void max(base_color& s) {
        rgb.max(s.rgb);
        hemi = std::max(hemi, s.hemi);
        sun = std::max(sun, s.sun);
    };
...
// IC	u8	u8_clr				(float a)	{ s32 _a = iFloor(a*255.f);
// clamp(_a,0,255); return u8(_a);		};
// IC	u8	u8_clr				(float a)	{ s32 _a = iFloor(a*255.f);
// clamp(_a,0,255); return u8(_a);		};
...
float getLastRP_Scale(CDB::COLLIDER* DB, R_Light& L) //, Face* skip)
{
    ...
    //	X_TRY
    {
        for (u32 I = 0; I < tris_count; I++) {
            ...
            const Shader_xrLC& SH =
                shader(F.dwMaterial, *(gl_data.g_shaders_xrlc), gl_data.g_materials);
            //			Shader_xrLCVec&	LIB = 		gl_data.g_shaders_xrlc->Library	();
            //			if (M.shader_xrlc>=LIB.size()) return		0;		//. hack - vy gonite rebyata -
            //eto ne hack - eto sledy zamesti - shader_xrlc - index ne togo masiva !!
            //			Shader_xrLC& SH	= LIB
            //[M.shader_xrlc];
...
bool detail_slot_calculate(u32 _x, u32 _z, DetailSlot& DS, DWORDVec& box_result, CDB::COLLIDER& DB,
