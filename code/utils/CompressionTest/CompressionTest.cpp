...
struct BlockInfo {
    ...
    BlockInfo(unsigned sz = 0)
        : size(sz), count(0), min_time(1000.0f), max_time(0), avg_time(0), min_ratio(1.0f),
          max_ratio(0), avg_ratio(0), total_time(0), total_ratio(0) {}
...
static bool _ProcessFile_LZO(const char* file_name) {
    ...
    if (file) {
        ...
        timer.start();
        if (_LZO_Dict) {
            lzo1x_999_compress_dict(src_data, src_size, comp_data, &comp_size, _LZOWrkMem,
                                    _LZO_Dict, _LZO_Dict_Sz);
        } else {
            lzo1x_999_compress(src_data, src_size, comp_data, &comp_size, _LZOWrkMem);
        }
        timer.stop();
        printf("LZO  :  %2.0f%% %1.5fms  %u->%u", 100.0f * float(comp_size) / float(src_size),
               (float)timer.time(), (unsigned)src_size, (unsigned)comp_size);
        ...
        if (_LZO_Dict) {
            lzo1x_decompress_dict_safe(comp_data, comp_size, uncomp_data, &uncomp_size, NULL,
                                       _LZO_Dict, _LZO_Dict_Sz);
        } else {
        ...
        if (ok)
            ...
            printf(" ERROR (#%u  %02X != %02X)\n", err_b, src_data[err_b] & 0xFF,
                   uncomp_data[err_b] & 0xFF);
...
int main(int argc, char* argv[]) {
    ...
    if (argc > 1) {
        ...
        printf("\n==========\nstats:\n");
        for (unsigned i = 0; i < _LZO_BlockInfo.size(); ++i) {
            printf("LZO :  %2.1f%%  %-2.3fms\n", _LZO_BlockInfo[i].avg_ratio,
                   _LZO_BlockInfo[i].avg_time);
        }
