...
public:
    ...
    CLevelGameGraph(LPCSTR graph_file_name, LPCSTR raw_cross_table_file_name,
                    CGameGraph::SLevel* tLevel, LPCSTR S, u32 dwOffset, u32 dwLevelID,
                    CInifile* Ini) {
        m_tLevel = *tLevel;
        m_dwOffset = dwOffset;
        ...
        VERIFY2(m_tpGraph->header().level(GameGraph::_LEVEL_ID(0)).guid() ==
                    l_tpAI_Map->header().guid(),
                "cross table doesn't correspond to the AI-map, rebuild graph!");

        VERIFY(l_tpAI_Map->header().vertex_count() ==
               l_tpCrossTable->header().level_vertex_count());
...

        {
            for (GameGraph::_GRAPH_ID i = 0, n = m_tpGraph->header().vertex_count(); i < n; ++i)
                if ((!l_tpAI_Map->valid_vertex_id(m_tpGraph->vertex(i)->level_vertex_id()) ||
                     (l_tpCrossTable->vertex(m_tpGraph->vertex(i)->level_vertex_id())
                          .game_vertex_id() != i) ||
                     !l_tpAI_Map->inside(m_tpGraph->vertex(i)->level_vertex_id(),
                                         m_tpGraph->vertex(i)->level_point()))) {
        ...
        for (; I != E; I++) {
            ...
            std::memcpy((*I).tVertexTypes, m_tpGraph->vertex(int(I - B))->vertex_type(),
            ...
            (*I).tpaEdges =
                (CGameGraph::CEdge*)xr_malloc((*I).tNeighbourCount * sizeof(CGameGraph::CEdge));
            b = i;
            for (; i != e; ++i) {
                GameGraph::CEdge& edge = (*I).tpaEdges[i - b];
                edge = *i;
                VERIFY((edge.vertex_id() + dwOffset) <
                       (u32(1) << (8 * sizeof(GameGraph::_GRAPH_ID))));
                edge.m_vertex_id = (GameGraph::_GRAPH_ID)(edge.m_vertex_id + dwOffset);
            }
            (*I).dwPointOffset = 0;
            std::mt19937 rng{ std::random_device()() };
            vfGenerateDeathPoints(rng, int(I - B), l_tpCrossTable, l_tpAI_Map,
                                  (*I).tDeathPointCount);
        }
        ...
        // updating cross-table
        {
            ...
            for (int i = 0; i < (int)tpCrossTable->header().level_vertex_count(); i++) {
                ...
                VERIFY(u32(tCrossTableUpdate[i].tGraphIndex) <
                       tpCrossTable->header().game_vertex_count());
                tCrossTableUpdate[i].tGraphIndex =
                    tCrossTableUpdate[i].tGraphIndex + (GameGraph::_GRAPH_ID)dwOffset;
            }
            ...
            tCrossTableHeader.dwGraphPointCount =
                tpCrossTable->m_tCrossTableHeader.dwGraphPointCount;
        ...
        // fill vertex map
        {
                    ...
                    if (fMinDistance < EPS_L) {
                        ...
                        for (; II != EE; ++II)
                            if (T.tOldGraphID == (*II).second.tOldGraphID) {
                                ok = false;
                                Msg("Graph point %s is removed,because it has the same position as "
                                    "some another graph point",
                                    E->name_replace());
                                break;
                            }
            ...
            //graph for the level ",m_tLevel.name());
            O->close();
            FS.r_close(F);
        }
    };
    ...
    void vfAddEdge(u32 dwVertexNumber, CGameGraph::CEdge& tGraphEdge) {
        ...
        m_tpVertices[dwVertexNumber].tpaEdges[m_tpVertices[dwVertexNumber].tNeighbourCount - 1] =
            tGraphEdge;
    }
    ...
    void vfSaveVertices(CMemoryWriter& tMemoryStream, u32& dwOffset, u32& dwPointOffset,
        ...
        for (; I != E; I++) {
            ...
            std::memcpy(tVertex.tVertexTypes, (*I).tVertexTypes,
                        GameGraph::LOCATION_TYPE_COUNT * sizeof(GameGraph::_LOCATION_ID));
    ...
    void vfGenerateDeathPoints(std::mt19937& rng, int iGraphIndex,
        ...
        R_ASSERT2(!l_dwaNodes.empty(),
                  "Can't create at least one death point for specified graph point");

        std::shuffle(l_dwaNodes.begin(), l_dwaNodes.end(), rng);

        u32 m = l_dwaNodes.size() > 10 ? std::min(iFloor(.1f * l_dwaNodes.size()), 255)
                                       : l_dwaNodes.size(),
...
void read_levels(CInifile* Ini, xr_set<CLevelInfo>& levels, bool rebuild_graph,
    ...
    for (u32 k = 0; Ini->r_line("levels", k, &_N, &V); k++) {
        ...
        if (!FS.exist(file_name)) {
            Msg("! There is no ai-map for the level %s! (level is not included into the game "
                "graph)",
                S);
            continue;
        }

        {
            ...
            if (header.version() != XRAI_CURRENT_VERSION) {
                Msg("! AI-map for the level %s is incompatible (version mismatch)! (level is not "
                    "included into the game graph)",
                    S);
                continue;
            }
        }
...
CGraphMerger::CGraphMerger(LPCSTR game_graph_id, LPCSTR name, bool rebuild) {
        ...
        for (; I != E; I++) {
            ...
            for (; i != e; i++)
                if ((*i).second.caConnectName[0]) {
                    ...
                    if (K == tpGraphs.end()) {
                        Msg("Cannot find level with level_id %d. Connection point will not be "
                            "generated!",
                    ...
                    //					tGraphEdge.dwVertexNumber	= (*i).second.tGraphID +
                    //(*I).second->m_dwOffset;
                    //					(*K).second->vfAddEdge
                    //((*M).second.tGraphID,tGraphEdge);
                }
        }
    }
    // counting edges
    ...
    }
    {
        ...
        auto E = tpGraphs.end();
        for (; I != E; I++)
            l_tpLevelPoints.insert(l_tpLevelPoints.end(), (*I).second->m_tpLevelPoints.begin(),
                                   (*I).second->m_tpLevelPoints.end());
    }
