...
void CGameSpawnConstructor::load_spawns(LPCSTR name, bool no_separator_check) {
    ...
    string256 temp;
    xr_sprintf(temp,
               "There are no valid levels (with AI-map and graph) in the section 'levels' in the "
               "'%s' to build spawn file from!",
               GAME_CONFIG);
...
void CGameSpawnConstructor::add_story_object(ALife::_STORY_ID id, CSE_ALifeDynamicObject* object,
    ...
    if (I != m_story_objects.end()) {
        ...
        VERIFY3(I == m_story_objects.end(),
                "There are several objects which has the same unique story ID, level ", level_name);
    }
