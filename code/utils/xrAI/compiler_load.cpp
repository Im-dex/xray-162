...
IC CNodePositionConverter::CNodePositionConverter(const SNodePositionOld& Psrc, hdrNODES& m_header,
                                                  NodePosition& np) {
    Fvector Pdest;
            ...
            Status("Processing textures...");
            {
                ...
                for (u32 t = 0; t < tex_count; t++) {
                    ...
                    if (0 == xr_strcmp(N, "level_lods")) {
                        ...
                        // if( version!=THM_CURRENT_VERSION )	FATAL	("Unsupported version of
                        // THM file.");
                        ...
                        if (!bLOD) {
                            if (BT.bHasAlpha ||
                                BT.THM.flags.test(STextureParams::flImplicitLighted)) {
                                clMsg("- loading: %s", N);
                                u32 w = 0, h = 0;
