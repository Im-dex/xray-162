...
void CSpaceRestrictorWrapper::fill_shape(const CShapeData::shape_def& shape) {
    ...
    case 0: {
        start.sub(Fvector().set(shape.data.sphere.P),
                  Fvector().set(shape.data.sphere.R, 0.f, shape.data.sphere.R));
...
bool CSpaceRestrictorWrapper::inside(u32 level_vertex_id, bool partially_inside,
                                     float radius) const {
    const Fvector& position = level_graph().vertex_position(level_vertex_id);
    float offset = level_graph().header().cell_size() * .5f - EPS_L;
    if (partially_inside)
        return (inside(construct_position(level_graph(), level_vertex_id, position.x + offset,
                                          position.z + offset),
                       radius) ||
                inside(construct_position(level_graph(), level_vertex_id, position.x + offset,
                                          position.z - offset),
                       radius) ||
                inside(construct_position(level_graph(), level_vertex_id, position.x - offset,
                                          position.z + offset),
...
struct sort_by_xz_predicate {
    ...
    IC bool operator()(u32 v0, u32 v1) const {
        return (m_level_graph->vertex(v0)->position().xz() <
                m_level_graph->vertex(v1)->position().xz());
    }
};
...
void CSpaceRestrictorWrapper::build_border() {
    ...
    {
        BORDER::iterator I = std::remove_if(m_border.begin(), m_border.end(),
                                            border_merge_predicate(this, m_level_graph));
...
void CSpaceRestrictorWrapper::verify_connectivity() {
    ...
    Msg("! %d nodes are disconnected!",
        level_graph().header().vertex_count() - (nodes.size() + m_internal.size()));
