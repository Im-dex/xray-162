...
void CLevelSpawnConstructor::init() {
    ...
    if (FS.exist(file_name)) {
        ...
        VERIFY(stream);
        m_game_spawn_constructor->patrol_path_storage().load_raw(&level_graph(), &cross_table(),
                                                                 &game_graph(), *stream);
        FS.r_close(stream);
    }
}
...
// void CLevelSpawnConstructor::add_spawn_group					(CSE_Abstract
// *abstract)
//{
...
void CLevelSpawnConstructor::add_space_restrictor(CSE_ALifeDynamicObject* dynamic_object) {
    CSE_ALifeSpaceRestrictor* space_restrictor =
        smart_cast<CSE_ALifeSpaceRestrictor*>(dynamic_object);
    if (!space_restrictor)
        return;
...
// void CLevelSpawnConstructor::add_group_object					(CSE_Abstract
// *abstract, shared_str group_section, bool)
//{
...
// void CLevelSpawnConstructor::add_group_object					(CSE_Abstract
// *abstract, shared_str group_section)
//{
//	string256					temp;
...
void CLevelSpawnConstructor::load_objects() {
    ...
    for (; chunk; chunk = level_spawn->open_chunk_iterator(id, chunk)) {
        ...
        //			add_group_object
        //(alife_object,alife_object->m_spawn_control);
...
// IC	void CLevelSpawnConstructor::normalize_probability			(CSE_ALifeAnomalousZone
// *zone)
//{
...
//	for ( ; I != E; I++) {
//		R_ASSERT
//(xr_strlen(*(*I).first)); 		R_ASSERT
//((*I).second); 		SPAWN_GROUPS::iterator					J =
//m_spawn_groups.find((*I).first); 		if (J == m_spawn_groups.end())
//			clMsg								("! ERROR (spawn group not
//found!) : %s",*(*I).first);
//		R_ASSERT3								(J !=
//m_spawn_groups.end(),"Specified group control not found!",(*(*I).second)[0]->name_replace());
//
//		GROUP_OBJECTS::iterator					i = (*I).second->begin();
//		GROUP_OBJECTS::iterator					e = (*I).second->end();
//		for ( ; i != e; i++) {
//			m_game_spawn_constructor->add_edge
//((*J).second->m_tSpawnID,(*i)->m_tSpawnID,(*i)->m_spawn_probability);
//			CSE_ALifeAnomalousZone				*zone =
//smart_cast<CSE_ALifeAnomalousZone*>(*i); 			if (zone)
...
void CLevelSpawnConstructor::correct_objects() {
    ...
    u32 dwStart = game_graph().header().vertex_count(),
        dwFinish = game_graph().header().vertex_count(), dwCount = 0;
    ...
    for (int i = 0; i < (int)m_spawns.size(); i++) {
        if (!m_spawns[i]->used_ai_locations()) {
            ...
            m_spawns[i]->m_tNodeID =
                game_graph().vertex(m_level_graph_vertex_id)->level_vertex_id();
            continue;
        }
        ...
        if (dwBest == u32(-1)) {
            ...
            S += xr_sprintf(S, sizeof(S1) - (S1 - &S[0]), "Spawn point : [%7.2f][%7.2f][%7.2f]\n",
                            m_spawns[i]->o_Position.x, m_spawns[i]->o_Position.y,
                            m_spawns[i]->o_Position.z);
...
public:
    IC remove_invalid_zones_predicate(const CLevelSpawnConstructor* level_spawn_constructor,
                                      const SPAWN_STORAGE* zones) {
...
void CLevelSpawnConstructor::generate_artefact_spawn_positions() {
    ...
    for (; I != E; I++) {
        ...
        //			zone->m_artefact_position_offset		=
        //m_level_points.size(); 			continue;
        //		}
        ...
        l_tpaStack.erase(
            std::remove_if(l_tpaStack.begin(), l_tpaStack.end(),
                           remove_too_far_predicate(&level_graph(), zone->o_Position,
        ...
        /*
                        if (zone->m_artefact_spawn_count >= l_tpaStack.size())
                        {
                                ...
                                if (!zone->m_artefact_spawn_count) {
                                        Msg						("!
        CANNOT GENERATE ARTEFACT SPAWN POSITIONS FOR ZONE [%s] ON LEVEL
        [%s]",zone->name_replace(),*level().name());
                                        Msg						("! ZONE
        [%s] ON LEVEL [%s] IS REMOVED BY AI COMPILER",zone->name_replace(),*level().name());
                                        R_ASSERT3				(zone->m_story_id
        == INVALID_STORY_ID,"Cannot remove story object",zone->name_replace()); R_ASSERT3
        (!zone->m_spawn_control,"Cannot remove spawn control object",zone->name_replace());
        ...
        //		Msg								("%s  %f [%f][%f][%f] :
        //artefact spawn positions",zone->name_replace(),zone->m_fRadius,VPUSH(zone->o_Position));
...
#ifndef IGNORE_ZERO_SPAWN_POSITIONS
    I = std::remove_if(m_spawns.begin(), m_spawns.end(),
                       remove_invalid_zones_predicate(this, &zones));
...
void CLevelSpawnConstructor::fill_level_changers() {
    for (u32 i = 0, n = (u32)level_changers().size(); i < n; ++i) {
        ...
        for (; I != E; ++I)
            if (!xr_strcmp(*level_changers()[i]->m_caLevelPointToChange, (*I)->name_replace())) {
                ...
                for (u32 ii = 0, nn = game_graph().header().vertex_count(); ii < nn; ++ii) {
                    ...
                    level_changers()[i]->m_dwNextNodeID =
                        game_graph().vertex(ii)->level_vertex_id();
                    ok = true;
                    break;
                }

                R_ASSERT3(ok,
                          "Cannot find a correspndance between graph and graph points from level "
                          "editor! Rebuild graph for the level ",
        ...
        if (!found) {
            clMsg("Graph point %s not found (level changer %s)",
                  *level_changers()[i]->m_caLevelPointToChange,
...
void CLevelSpawnConstructor::update_artefact_spawn_positions() {
    ...
    for (; I != E; ++I) {
        ...
        CSE_ALifeObject* alife_object = smart_cast<CSE_ALifeObject*>(abstract);
        //		R_ASSERT3
        //(level_graph().valid_vertex_id(alife_object->m_tNodeID),"Invalid node for object
        ...
        if (zone) {
            ...
            //			for (u32 i=zone->m_artefact_position_offset; i<level_point_count;
            //++i)
            //				Msg						("    [%f][%f][%f] :
