...
IC u32 const& GameGraph::CHeader::edge_count() const { return (m_edge_count); }

IC u32 const& GameGraph::CHeader::death_point_count() const { return (m_death_point_count); }
...
IC const GameGraph::SLevel& GameGraph::CHeader::level(const _LEVEL_ID& id) const {
    ...
    R_ASSERT2(I != levels().end(),
              make_string("there is no specified level in the game graph : %d", id));
    return ((*I).second);
}
...
IC u32 const& GameGraph::CVertex::edge_offset() const { return (dwEdgeOffset); }

IC u32 const& GameGraph::CVertex::death_point_offset() const { return (dwPointOffset); }
...
IC void CGameGraph::begin_spawn(u32 const vertex_id, const_spawn_iterator& start,
...
IC void CGameGraph::set_current_level(u32 const level_id) {
    ...
    for (; I != E; ++I) {
        ...
        m_current_level_cross_table =
            xr_new<CGameLevelCrossTable>(current_cross_table + 1, *current_cross_table);
        break;
    }
