...
IC void CLevelGraph::unpack_xz(const CLevelGraph::CPosition& vertex_position, u32& x,
                               u32& z) const {
...
IC void CLevelGraph::unpack_xz(const CLevelGraph::CPosition& vertex_position, int& x,
                               int& z) const {
    u32 _x, _z;
...
IC void CLevelGraph::unpack_xz(const CLevelGraph::CPosition& vertex_position, float& x,
                               float& z) const {
    u32 _x, _z;
    unpack_xz(vertex_position, _x, _z);
...
ICF const Fvector
CLevelGraph::vertex_position(const CLevelGraph::CPosition& source_position) const {
...
ICF const Fvector&
CLevelGraph::vertex_position(Fvector& dest_position,
...
CLevelGraph::vertex_position(CLevelGraph::CPosition& dest_position,
                             const Fvector& source_position) const {
    VERIFY(iFloor((source_position.z - header().box().min.z) / header().cell_size() + .5f) <
           (int)m_row_length);
    int pxz = iFloor(((source_position.x - header().box().min.x) / header().cell_size() + .5f)) *
                  m_row_length +
              iFloor((source_position.z - header().box().min.z) / header().cell_size() + .5f);
    int py =
        iFloor(65535.f * (source_position.y - header().box().min.y) / header().factor_y() + EPS_S);
...
IC bool CLevelGraph::inside(const CVertex* vertex, const Fvector& position,
                            const float epsilon) const {
...
IC bool CLevelGraph::inside(const u32 vertex_id, const Fvector& position,
                            const float epsilon) const {
...
IC bool CLevelGraph::inside(const u32 vertex_id, const Fvector2& position) const {
    int pxz =
        iFloor(((position.x - header().box().min.x) / header().cell_size() + .5f)) * m_row_length +
...
IC float CLevelGraph::vertex_plane_y(const CLevelGraph::CVertex& vertex, const float X,
                                     const float Z) const {
...
IC void CLevelGraph::begin(const CVertex& vertex, const_iterator& begin,
                           const_iterator& end) const {
    begin = 0;
    end = 4;
}

IC void CLevelGraph::begin(const CVertex* _vertex, const_iterator& _begin,
                           const_iterator& end) const {
...
IC bool CLevelGraph::create_straight_path(u32 start_vertex_id, const Fvector2& start_point,
                                          ...
                                          std::vector<T>& tpaOutputPoints, const T& example,
    ...
    if (vertex(start_vertex_id)->position().xz() == dest_xz) {
        ...
        if (bAssignY)
            tIntersectPoint.y =
                vertex_plane_y(vertex(cur_vertex_id), tIntersectPoint.x, tIntersectPoint.z);
            ...
            if (box.pick_exact(start, dir)) {
                ...
                u32 dwIntersect =
                    ...
                    intersect_no_check(start_point.x, start_point.y, finish_point.x, finish_point.y,
                                       next1.x, next1.y, next2.x, next2.y, &tIntersectPoint.x,
                                       &tIntersectPoint.z);
#ifdef DEBUG
                VERIFY(dwIntersect);
                ...
                if (dest_xz == v->position().xz() /**box.contains(dest)/**/) {
                    ...
                    if (bAssignY)
                        tIntersectPoint.y = vertex_plane_y(vertex(cur_vertex_id), tIntersectPoint.x,
                                                           tIntersectPoint.z);
                    path_node.set_position(tIntersectPoint);
                    path_node.set_vertex_id(next_vertex_id);
                    tpaOutputPoints.push_back(path_node);
                ...
                if (tpaOutputPoints.size() > 100000) {
                    Msg("CLevelGraph::create_straight_path : Loop became infinite "
                        "(%d,[%f][%f][%f],[%f][%f][%f])",
...
IC void CLevelGraph::assign_y_values(std::vector<T>& path) {
    ...
    std::vector<T>::iterator I = path.begin();
    std::vector<T>::iterator E = path.end();
    for (; I != E; ++I) {
...
IC bool CLevelGraph::valid_vertex_position(const Fvector& position) const {
    ...
    if (!(iFloor((position.z - header().box().min.z) / header().cell_size() + .5f) <
          (int)m_row_length))
        return (false);

    if (!(iFloor((position.x - header().box().min.x) / header().cell_size() + .5f) <
          (int)m_column_length))
        return (false);
...
IC void CLevelGraph::iterate_vertices(const Fvector& min_position, const Fvector& max_position,
    ...
    if (valid_vertex_position(min_position))
        I = std::lower_bound(m_nodes, m_nodes + header().vertex_count(),
                             vertex_position(min_position).xz(), &vertex::predicate2);
    else
        I = m_nodes;

    if (valid_vertex_position(max_position)) {
        E = std::upper_bound(m_nodes, m_nodes + header().vertex_count(),
                             vertex_position(max_position).xz(), &vertex::predicate);
