...
IC CSQuadTree::CQuadTree(const Fbox& box, float min_cell_size, u32 max_node_count,
...
TEMPLATE_SPECIALIZATION
IC void CSQuadTree::nearest(const Fvector& position, float radius,
                            std::vector<_object_type*>& objects, bool clear) const {
...
TEMPLATE_SPECIALIZATION
IC void CSQuadTree::nearest(const Fvector& position, float radius,
                            std::vector<_object_type*>& objects, CQuadNode* node, Fvector center,
                            float distance, int depth) const {
    if (!node)
        return;
    ...
    VERIFY(index < 4);
    if (xr::abs(position.z - center.z) < radius) {
        if (xr::abs(position.x - center.x) < radius) {
            if (xr::sqr(position.z - center.z) + xr::sqr(position.x - center.x) < xr::sqr(radius)) {
                nearest(position, radius, objects, node->m_neighbours[0],
                        next_center.set(center.x - distance, center.y, center.z - distance),
                        distance, depth + 1);
            ...
            nearest(position, radius, objects, node->m_neighbours[index], next_center, distance,
                    depth + 1);

            if (position.z > center.z) {
                if (index & 1)
                    nearest(position, radius, objects, node->m_neighbours[index == 1 ? 0 : 2],
                            next_center.set(center.x + (index == 1 ? -1 : 1) * distance, center.y,
                                            center.z - distance),
                            distance, depth + 1);
            } else if (!(index & 1))
                nearest(position, radius, objects, node->m_neighbours[!index ? 1 : 3],
                        next_center.set(center.x + (!index ? -1 : 1) * distance, center.y,
                                        center.z + distance),
                        distance, depth + 1);
...
TEMPLATE_SPECIALIZATION
IC _object_type* CSQuadTree::remove(const _object_type* object, CQuadNode*& node, Fvector center,
                                    float distance, int depth) {
    VERIFY(node);
    if (depth == m_max_depth) {
        CListItem* leaf = ((CListItem*)((void*)(node)));
    ...
    if (node->m_neighbours[index] || node->m_neighbours[0] || node->m_neighbours[1] ||
        node->m_neighbours[2] || node->m_neighbours[3])
        return (_object);
    m_nodes->remove(node);
    return (_object);
}
