...
xrGUID generate_guid() {
    ...
    std::memcpy(&result, &_result, sizeof(_result));
    switch (gen_result) {
    case RPC_S_OK:
        return (result);
...
#endif
    static_assert(sizeof(result) >= sizeof(u64),
                  "GUID must have size greater or equal to the long long");
...
LPCSTR generate_guid(const xrGUID& guid, LPSTR buffer, const u32& buffer_size) {
    ...
    static_assert(sizeof(xrGUID) == sizeof(GUID), "Different GUID types");
    GUID temp;
    std::memcpy(&temp, &guid, sizeof(guid));
    RPC_CSTR temp2;
