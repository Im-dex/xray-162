...
using vecTris = std::vector<cl_tri>;
...
//         rVector - std::vector describing direction of ray in world space
...
IC float intersectRayPlane(const Fvector& rayOrigin, const Fvector& rayDirection,
    ...
    if (denom == 0) // normal is orthogonal to std::vector, cant intersect
        return (-1.0f);
...
//         rV - std::vector describing direction of ray in world space
//         sO - Origin of sphere
//         sR - radius of sphere
// Notes : Normalized directional vectors expected
...
IC bool CheckPointInIdentitySphere(const Fvector& point) { return (point.square_magnitude() <= 1); }
// -----------------------------------------------------------------------
    ...
    for (u32 i_t = 0; i_t != clContactedT.size(); i_t++) {
        ...
        if (T.N.dotproduct(normalizedVelocity) < 0.0f) {
            ...
            if ((sIPoint.dotproduct(T.N) + T.d) < -EPS_S) {
                ...
                // shoot ray along the velocity std::vector
                bInsideTri =
                    CDB::TestRayTri2(sIPoint, normalizedVelocity, T.p, distToPlaneIntersection);
            ...
            if (!bInsideTri) {
                ...
                distToEllipsoidIntersection =
                    intersectRaySphere(polyIPoint, _normalizedVelocity, source, 1.0f);
            ...
            if ((distToEllipsoidIntersection >= 0) &&
                (distToEllipsoidIntersection <= distanceToTravel)) {
                if ((cl.bFoundCollision == FALSE) ||
                    (distToEllipsoidIntersection < cl.fNearestDistance)) {
...
//-----------------------------------------------------------------------------
Fvector msimulator_CollideWithWorld(SCollisionData& cl, Fvector position, Fvector velocity,
                                    WORD cnt) {
    //
    ...
    if (cl.bFoundCollision == FALSE && !cl.bStuck) {
        ...
        float l = intersectRayPlane(destinationPoint, slidePlaneNormal, slidePlaneOrigin,
                                    slidePlaneNormal);
        ...
        // Generate the slide std::vector, which will become our _new velocity std::vector
...
void msimulator_Simulate(Fvector& result, Fvector& start, Fvector& end, float _radius,
                         float _height) {
