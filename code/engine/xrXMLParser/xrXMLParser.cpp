...
void CXml::Load(LPCSTR path, LPCSTR xml_filename) {
    ...
    if (m_Doc.Error()) {
        string1024 str;
        xr_sprintf(str, "XML file:%s value:%s errDescr:%s", m_xml_file_name, m_Doc.Value(),
                   m_Doc.ErrorDesc());
        R_ASSERT2(false, str);
    }
...
XML_NODE* CXml::NavigateToNodeWithAttribute(LPCSTR tag_name, LPCSTR attrib_name,
                                            LPCSTR attrib_value) {
...
LPCSTR CXml::ReadAttrib(XML_NODE* start_node, LPCSTR path, int index, LPCSTR attrib,
...
int CXml::ReadAttribInt(XML_NODE* start_node, LPCSTR path, int index, LPCSTR attrib,
                        int default_int_val) {
...
float CXml::ReadAttribFlt(XML_NODE* start_node, LPCSTR path, int index, LPCSTR attrib,
...
XML_NODE* CXml::SearchForAttribute(LPCSTR path, int index, LPCSTR tag_name, LPCSTR attrib,
...
XML_NODE* CXml::SearchForAttribute(XML_NODE* start_node, LPCSTR tag_name, LPCSTR attrib,
                                   LPCSTR attrib_value_pattern) {
    while (start_node) {
