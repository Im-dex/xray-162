...
#define MSPACE_OFFSET \
    (((offsetof(struct malloc_arena, buf_) + CHUNK_ALIGN_MASK) & ~CHUNK_ALIGN_MASK))
...
#define FOOTER_OVERHEAD (2 * sizeof(struct malloc_arena*) - SIZE_T_SIZE)

#define arena_for_chunk(ptr)                                                               \
    (chunk_non_main_arena(ptr) ? *(struct malloc_arena**)((char*)(ptr) + chunksize(ptr) -  \
...
#define arena_for_mmap_chunk(ptr)                                                                 \
    (chunk_non_main_arena(ptr)                                                                    \
         ? *(struct malloc_arena**)((char*)(ptr) + chunksize(ptr) - sizeof(struct malloc_arena*)) \
         : &main_arena)
...
static struct malloc_arena* arena_get2(struct malloc_arena* a_tsd, size_t size) {
...
static struct malloc_arena* _int_new_arena(size_t size) {
    ...
    m = create_mspace_with_base((char*)a + MSPACE_OFFSET, mmap_sz - MSPACE_OFFSET, 0, 0, 0);
...
static void* memalign_starter(size_t align, size_t sz, const void* caller) {
    void* victim;
...
static void free_starter(void* mem, const void* caller) {
    if (mem) {
        mchunkptr p = mem2chunk(mem);
...
static void* (*save_memalign_hook)(size_t __align, size_t __size, const void*);
#endif
...
/*if (mp_.pagesize == 0)
  ptmalloc_init_minimal();*/
    ...
    mspace = create_mspace_with_base((char*)&main_arena + MSPACE_OFFSET,
                                     sizeof(main_arena) - MSPACE_OFFSET, 0, 0, 0);
...
libc_hidden_def(public_fREe)
    ...
    assert(!newp || is_mmapped(mem2chunk(newp)) || ar_ptr == arena_for_chunk(mem2chunk(newp)));
    return newp;
}
#ifdef libc_hidden_def
libc_hidden_def(public_rEALLOc)
    ...
    arena_get(ar_ptr, bytes + FOOTER_OVERHEAD + alignment + MIN_CHUNK_SIZE);
    if (!ar_ptr)
        return 0;
    ...
    assert(!p || is_mmapped(mem2chunk(p)) || ar_ptr == arena_for_chunk(mem2chunk(p)));
    return p;
}
#ifdef libc_hidden_def
...
int public_pMEMALIGn(void** memptr, size_t alignment, size_t size) {
    ...
    if (alignment % sizeof(void*) != 0 || !my_powerof2(alignment / sizeof(void*)) != 0 ||
        alignment == 0)
        return EINVAL;
...
#define HALF_INTERNAL_SIZE_T (((size_t)1) << (8 * sizeof(size_t) / 2))
    ...
    assert(!mem || is_mmapped(mem2chunk(mem)) || ar_ptr == arena_for_chunk(mem2chunk(mem)));
...
void** public_iCOMALLOc(size_t n, size_t sizes[], void* chunks[]) {
    ...
    if (ar_ptr != &main_arena) {
        ...
        if (!chunks) {
            chunks = mspace_malloc(arena_to_mspace(ar_ptr), n * sizeof(void*) + FOOTER_OVERHEAD);
            if (!chunks) {
...
void initialize_virtual_alloc_arena(struct virtual_alloc_arena* result, void* buffer,
                                    size_t buffer_size, size_t granularity, char const* arena_id,
