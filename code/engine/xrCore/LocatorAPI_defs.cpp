...
// FS_File
//////////////////////////////////////////////////////////////////////
FS_File::FS_File(std::string nm, long sz, time_t modif, unsigned attr) { set(nm, sz, modif, attr); }
FS_File::FS_File(std::string nm) { set(nm, 0, 0, 0); }
FS_File::FS_File(const _FINDDATA_T& f) {
    set(f.name, f.size, f.time_write, (f.attrib & _A_SUBDIR) ? flSubDir : 0);
}
FS_File::FS_File(std::string nm, const _FINDDATA_T& f) {
    set(nm, f.size, f.time_write, (f.attrib & _A_SUBDIR) ? flSubDir : 0);
}

void FS_File::set(std::string nm, long sz, time_t modif, unsigned attr) {
    name = nm;
    xr_strlwr(name);
    size = sz;
    time_write = modif;
