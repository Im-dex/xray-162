...
void CLocatorAPI::Register(LPCSTR name, u32 vfs, u32 crc, u32 ptr, u32 size_real,
...
bool ignore_name(const char* _name) {
    ...
    return (_name[0] == '.' && _name[1] == 's' && _name[2] == 'v' && _name[3] == 'n' &&
            _name[4] == 0);
}
    ...
    if (m_Flags.is(flTargetFolderOnly)) {
        ...
        /*
                        ...
                        if (!F)
                        {
                                ...
                                if (xr_strlen(tmpAppPath))
                                {
                                        ...
                                        rescan_path				(pFSRoot->m_Path,
           pFSRoot->m_Flags.is(FS_Path::flRecurse));
                                }
        ...
        */
        ...
        while (!pFSltx->eof()) {
            ...
            FS_Path* P = new FS_Path((p_it != pathes.end()) ? p_it->second->m_Path : root,
                                         lp_add, lp_def, fl);
            ...
            CHECK_OR_EXIT(I.second, "The file 'fsgame.ltx' is corrupted (it contains duplicated "
                                    "lines).\nPlease reinstall the game or fix the problem "
                                    "manually.");
        }
        r_close(pFSltx);
...
void CLocatorAPI::check_cached_files(LPSTR fname, const u32& fname_size, const file& desc,
                                     LPCSTR& source_name) {
...
template <typename T>
void CLocatorAPI::file_from_cache(T*& R, LPSTR fname, const u32& fname_size, const file& desc,
                                  LPCSTR& source_name) {
#ifdef DEBUG
...
bool CLocatorAPI::path_exist(LPCSTR path) { return pathes.find(path) != pathes.end(); }
...
FS_Path* CLocatorAPI::append_path(LPCSTR path_alias, LPCSTR root, LPCSTR add, BOOL recursive) {
    ...
    FS_Path* P = new FS_Path(root, add, LPCSTR(0), 0);
