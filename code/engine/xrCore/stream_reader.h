...
public:
    virtual void construct(const HANDLE& file_mapping_handle, const u32& start_offset,
                           const u32& file_size, const u32& archive_size, const u32& window_size);
...
public:
    ...
    //.			CStreamReader*open_chunk_iterator(const u32 &chunk_id, CStreamReader *previous = 0);
    //// 0 means first

public:
    //.			void			r_string			(char *dest, u32
    //tgt_sz);
    //.			void			r_string			(xr_string& dest);
    //.			void			skip_stringZ		();
    //.			void			r_stringZ			(char *dest, u32
    //tgt_sz);
