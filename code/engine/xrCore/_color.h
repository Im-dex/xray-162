...
inline u32 color_argb(u32 a, u32 r, u32 g, u32 b) {
...
inline u32 subst_alpha(u32 rgba, u32 a) {
    return rgba & ~color_rgba(0, 0, 0, 0xff) | color_rgba(0, 0, 0, a);
}
inline u32 bgr2rgb(u32 bgr) {
...
struct _color {
    ...
    SelfRef set(T _r, T _g, T _b, T _a) {
        r = _r;
        g = _g;
        b = _b;
        a = _a;
    ...
    SelfRef adjust_contrast(T f) // >1 - contrast will be increased
    {
        r = 0.5f + f * (r - 0.5f);
        g = 0.5f + f * (g - 0.5f);
        b = 0.5f + f * (b - 0.5f);
    ...
    SelfRef adjust_contrast(SelfCRef in, T f) // >1 - contrast will be increased
    {
        r = 0.5f + f * (in.r - 0.5f);
        g = 0.5f + f * (in.g - 0.5f);
        b = 0.5f + f * (in.b - 0.5f);
    ...
    SelfRef adjust_saturation(SelfCRef in, T s) {
        // Approximate values for each component's contribution to luminance.
        // Based upon the NTSC standard described in ITU-R Recommendation BT.709.
    ...
    SelfRef modulate(SelfCRef in1, SelfCRef in2) {
        r = in1.r * in2.r;
        g = in1.g * in2.g;
        b = in1.b * in2.b;
        a = in1.a * in2.a;
    ...
    SelfRef mul_rgba(SelfCRef c, T s) {
        r = c.r * s;
        g = c.g * s;
        b = c.b * s;
        a = c.a * s;
    ...
    SelfRef mul_rgb(SelfCRef c, T s) {
        r = c.r * s;
        g = c.g * s;
        b = c.b * s;
        return *this;
    ...
    // SQ magnitude
    T magnitude_sqr_rgb() const { return r * r + g * g + b * b; }
    // magnitude
    T magnitude_rgb() const { return std::sqrt(magnitude_sqr_rgb()); }
    T intensity() const { return (r + g + b) / 3.f; }

    // Normalize
    SelfRef normalize_rgb() {
        VERIFY(magnitude_sqr_rgb() > EPS_S);
        return mul_rgb(1.f / magnitude_rgb());
    }
    SelfRef normalize_rgb(SelfCRef c) {
        VERIFY(c.magnitude_sqr_rgb() > EPS_S);
        return mul_rgb(c, 1.f / c.magnitude_rgb());
    }
    SelfRef lerp(SelfCRef c1, SelfCRef c2, T t) {
        T invt = 1.f - t;
        r = c1.r * invt + c2.r * t;
        g = c1.g * invt + c2.g * t;
        b = c1.b * invt + c2.b * t;
    ...
    SelfRef lerp(SelfCRef c1, SelfCRef c2, SelfCRef c3, T t) {
        if (t > .5f) {
    ...
    BOOL similar_rgba(SelfCRef v, T E = EPS_L) const {
        return xr::abs(r - v.r) < E && xr::abs(g - v.g) < E && xr::abs(b - v.b) < E && xr::abs(a - v.a) < E;
    }
    BOOL similar_rgb(SelfCRef v, T E = EPS_L) const {
        return xr::abs(r - v.r) < E && xr::abs(g - v.g) < E && xr::abs(b - v.b) < E;
    }
};
