...
public:
    ...
    IC SelfRef build_camera_dir(const Tvector& vFrom, const Tvector& vView,
...
BOOL _valid(const _matrix<T>& m) {
    return _valid(m.i) && _valid(m._14_) && _valid(m.j) && _valid(m._24_) && _valid(m.k) &&
           _valid(m._34_) && _valid(m.c) && _valid(m._44_);
}
