...
struct _matrix {
    ...
    SelfRef set(const Tvector& R, const Tvector& N, const Tvector& D, const Tvector& C) {
        i.set(R);
        _14_ = 0;
        j.set(N);
        _24_ = 0;
    ...
    SelfRef mul(const Self& A, const Self& B) {
        ...
        m[0][2] = A.m[0][2] * B.m[0][0] + A.m[1][2] * B.m[0][1] + A.m[2][2] * B.m[0][2] +
                  A.m[3][2] * B.m[0][3];
        m[0][3] = A.m[0][3] * B.m[0][0] + A.m[1][3] * B.m[0][1] + A.m[2][3] * B.m[0][2] +
                  A.m[3][3] * B.m[0][3];
        ...
        m[1][2] = A.m[0][2] * B.m[1][0] + A.m[1][2] * B.m[1][1] + A.m[2][2] * B.m[1][2] +
                  A.m[3][2] * B.m[1][3];
        m[1][3] = A.m[0][3] * B.m[1][0] + A.m[1][3] * B.m[1][1] + A.m[2][3] * B.m[1][2] +
                  A.m[3][3] * B.m[1][3];
        ...
        m[2][2] = A.m[0][2] * B.m[2][0] + A.m[1][2] * B.m[2][1] + A.m[2][2] * B.m[2][2] +
                  A.m[3][2] * B.m[2][3];
        m[2][3] = A.m[0][3] * B.m[2][0] + A.m[1][3] * B.m[2][1] + A.m[2][3] * B.m[2][2] +
                  A.m[3][3] * B.m[2][3];

        m[3][0] = A.m[0][0] * B.m[3][0] + A.m[1][0] * B.m[3][1] + A.m[2][0] * B.m[3][2] +
                  A.m[3][0] * B.m[3][3];
        m[3][1] = A.m[0][1] * B.m[3][0] + A.m[1][1] * B.m[3][1] + A.m[2][1] * B.m[3][2] +
                  A.m[3][1] * B.m[3][3];
        m[3][2] = A.m[0][2] * B.m[3][0] + A.m[1][2] * B.m[3][1] + A.m[2][2] * B.m[3][2] +
                  A.m[3][2] * B.m[3][3];
        m[3][3] = A.m[0][3] * B.m[3][0] + A.m[1][3] * B.m[3][1] + A.m[2][3] * B.m[3][2] +
                  A.m[3][3] * B.m[3][3];
        return *this;
    }
    ...
    SelfRef mulA_44(const Self& A) // mul after
    {
        Self B;
        B.set(*this);
        mul(A, B);
    ...
    SelfRef mulB_44(const Self& B) // mul before
    {
        Self A;
        A.set(*this);
        mul(A, B);
    ...
    SelfRef mulA_43(const Self& A) // mul after (no projection)
    {
        Self B;
        B.set(*this);
        mul_43(A, B);
    ...
    SelfRef mulB_43(const Self& B) // mul before (no projection)
    {
        Self A;
        A.set(*this);
        mul_43(A, B);
    ...
    SelfRef invert(const Self& a) { // important: this is 4x3 invert, not the 4x4 one
        ...
        VERIFY(xr::abs(fDetInv) > flt_zero);
    ...
    SelfRef invert() // slower than invert other matrix
    {
        Self a;
        a.set(*this);
        invert(a);
    ...
    SelfRef transpose() // self transpose - slower
    {
        Self a;
        a.set(*this);
        transpose(a);
    ...
    SelfRef translate(const Tvector& Loc) // setup translation matrix
    {
        identity();
        c.set(Loc.x, Loc.y, Loc.z);
        return *this;
    }
    SelfRef translate(T _x, T _y, T _z) // setup translation matrix
    {
        identity();
        c.set(_x, _y, _z);
        return *this;
    }
    SelfRef translate_over(const Tvector& Loc) // modify only translation
    {
        c.set(Loc.x, Loc.y, Loc.z);
        return *this;
    }
    SelfRef translate_over(T _x, T _y, T _z) // modify only translation
    {
        c.set(_x, _y, _z);
        return *this;
    }
    SelfRef translate_add(const Tvector& Loc) // combine translation
    {
        c.add(Loc);
        return *this;
    }
    SelfRef scale(T x, T y, T z) // setup scale matrix
    {
        identity();
        m[0][0] = x;
        m[1][1] = y;
    ...
    SelfRef scale(const Tvector& v) // setup scale matrix
    {
    ...
    // fov
    SelfRef build_projection(T fFOV, T fAspect, T fNearPlane, T fFarPlane) {
        return build_projection_HAT(std::tan(fFOV / 2.f), fAspect, fNearPlane, fFarPlane);
    }
    // half_fov-angle-tangent
    SelfRef build_projection_HAT(T HAT, T fAspect, T fNearPlane, T fFarPlane) {
        VERIFY(xr::abs(fFarPlane - fNearPlane) > EPS_S);
        VERIFY(xr::abs(HAT) > EPS_S);
    ...
    SelfRef build_projection_ortho(T w, T h, T zn, T zf) {
        _11 = T(2) / w;
        _12 = 0;
        _13 = 0;
        _14 = 0;
    ...
    SelfRef build_camera(const Tvector& vFrom, const Tvector& vAt, const Tvector& vWorldUp) {
    ...
    SelfRef build_camera_dir(const Tvector& vFrom, const Tvector& vView,
    ...
    void transform_tiny(Tvector& dest, const Tvector& v) const // preferred to use
    {
        dest.x = v.x * _11 + v.y * _21 + v.z * _31 + _41;
        dest.y = v.x * _12 + v.y * _22 + v.z * _32 + _42;
        dest.z = v.x * _13 + v.y * _23 + v.z * _33 + _43;
    }
    void transform_tiny32(Fvector2& dest, const Tvector& v) const // preferred to use
    {
        dest.x = v.x * _11 + v.y * _21 + v.z * _31 + _41;
        dest.y = v.x * _12 + v.y * _22 + v.z * _32 + _42;
    }
    void transform_tiny23(Tvector& dest, const Fvector2& v) const // preferred to use
    {
        dest.x = v.x * _11 + v.y * _21 + _41;
        dest.y = v.x * _12 + v.y * _22 + _42;
        dest.z = v.x * _13 + v.y * _23 + _43;
    }
    void transform_dir(Tvector& dest, const Tvector& v) const // preferred to use
    {
    ...
    void transform_tiny(Tvector& v) const {
        Tvector res;
    ...
    void transform_dir(Tvector& v) const {
        Tvector res;
        transform_dir(res, v);
        v.set(res);
    }
    ...
    SelfRef setXYZ(T x, T y, T z) { return setHPB(y, x, z); }
    SelfRef setXYZ(Tvector const& xyz) { return setHPB(xyz.y, xyz.x, xyz.z); }
    SelfRef setXYZi(T x, T y, T z) { return setHPB(-y, -x, -z); }
    SelfRef setXYZi(Tvector const& xyz) { return setHPB(-xyz.y, -xyz.x, -xyz.z); }
    //
    ...
    void getHPB(Tvector& hpb) const { getHPB(hpb.x, hpb.y, hpb.z); }
    void getXYZ(T& x, T& y, T& z) const { getHPB(y, x, z); }
    void getXYZ(Tvector& xyz) const { getXYZ(xyz.x, xyz.y, xyz.z); }
    void getXYZi(T& x, T& y, T& z) const {
        getHPB(y, x, z);
        x *= -1.f;
        y *= -1.f;
        z *= -1.f;
    }
    void getXYZi(Tvector& xyz) const {
