    ...
    for (u32 it = 0; it < Registration.size(); it++) {
        ...
        if (R.OP) {
            ...
            if (!bFoundAndErased) {
                ...
                Msg("SCHEDULER: internal register [%s][%x][%s]", *R.Object->shedule_Name(),
                    R.Object, R.RT ? "true" : "false");
#endif // DEBUG_SCHEDULER
                internal_Register(R.Object, R.RT);
            }
#ifdef DEBUG_SCHEDULER
            else
                Msg("SCHEDULER: internal register skipped, because unregister found [%s][%x][%s]",
                    "unknown", R.Object, R.RT ? "true" : "false");
#endif // DEBUG_SCHEDULER
        } else {
            // unregister
...
bool CSheduler::internal_Unregister(ISheduled* O, BOOL RT, bool warn_on_not_found) {
    ...
    if (RT) {
        ...
        for (u32 i = 0; i < Items.size(); i++) {
            if (Items[i].Object == O) {
#ifdef DEBUG_SCHEDULER
                Msg("SCHEDULER: internal unregister [%s][%x][%s]", *Items[i].scheduled_name, O,
                    "false");
#endif // DEBUG_SCHEDULER
                Items[i].Object = NULL;
                return (true);
...
bool CSheduler::Registered(ISheduled* object) const {
    ...
    for (; I != E; ++I) {
        if ((*I).Object == object) {
            if ((*I).OP) {
                //				Msg				("0x%8x found in registration
                //on register",object);
                VERIFY(!count);
                ++count;
            } else {
                //				Msg				("0x%8x found in registration
                //on UNregister",object);
                VERIFY(count == 1);
                --count;
            }
        }
    ...
    if (!count && (m_current_step_obj == object)) {
        VERIFY2(m_processing_now,
                "trying to unregister self unregistering object while not processing now");
        count = 1;
    }
...
#endif      // DEBUG_SCHEDULER
            //			if (T.Object)
            //				Msg					("0x%08x UNREGISTERS because shedule_Needed() returned
            //false",T.Object); 			else 				Msg
            //("UNREGISTERS unknown object");
            Pop();
            continue;
        }
...
//		LPCSTR		_obj_name		= T.Object->shedule_Name().c_str();
        ...
        T.Object->shedule_Update(
            clampr(Elapsed, u32(1), u32(std::max(u32(T.Object->shedule.t_max), u32(1000)))));
        if (!m_current_step_obj) {
#ifdef DEBUG_SCHEDULER
            Msg("SCHEDULER: process unregister (self unregistering) [%s][%x][%s]",
                *T.scheduled_name, T.Object, "false");
#endif // DEBUG_SCHEDULER
            continue;
        }
//			} catch (...) {
#ifdef DEBUG
//				Msg		("! xrSheduler: object '%s' raised an exception",
//_obj_name); 				throw	;
#endif  // DEBUG
        //			}
        m_current_step_obj = NULL;
...
//		u32	execTime				= eTimer.GetElapsed_ms		();
        ...
        //		u32	execTime				= eTimer.GetElapsed_ms
        //();
        // VERIFY3					(T.Object->dbg_update_shedule ==
        // T.Object->dbg_startframe, "Broken sequence of calls to 'shedule_Update'", _obj_name );
        if (delta_ms > 3 * dwUpdate) {
            // Msg	("! xrSheduler: failed to shedule object [%s] (%dms)",	_obj_name, delta_ms
            // );
        }
//		if (execTime> 15)			{
//			Msg	("* xrSheduler: too much time consumed by object [%s] (%dms)",
//_obj_name, execTime	);
//		}
#endif // DEBUG
...
#ifdef DEBUG_SCHEDULER
            Msg("SCHEDULER: process unregister [%s][%x][%s]", *T.Object->shedule_Name(), T.Object,
                "false");
#endif // DEBUG_SCHEDULER
