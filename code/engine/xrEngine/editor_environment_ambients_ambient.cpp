...
void ambient::load(CInifile& ambients_config, CInifile& sound_channels_config,
...

    {
        ...
        for (u32 i = 0, n = _GetItemCount(effects_string); i < n; ++i) {
            string_path temp;
            effect_id* object =
                xr_new<effect_id>(m_manager.effects_manager(), _GetItem(effects_string, i, temp));
...

    {
        ...
        for (u32 i = 0, n = _GetItemCount(sounds_string); i < n; ++i) {
            string_path temp;
            sound_id* object =
                xr_new<sound_id>(m_manager.sounds_manager(), _GetItem(sounds_string, i, temp));
...
void ambient::fill(editor::property_holder_collection* collection) {
    ...
    m_property_holder->add_property(
        "minimum period", "effects",
        "this option is resposible for minimum effect period (in seconds)", m_effect_period.x,
        m_effect_period.x);
    m_property_holder->add_property(
        "maximum period", "effects",
        "this option is resposible for maximum effect period (in seconds)", m_effect_period.y,
        m_effect_period.y);
    m_property_holder->add_property("effects", "effects",
                                    "this option is resposible for maximum effects",
                                    m_effects_collection);
    m_property_holder->add_property("sound channels", "sounds",
