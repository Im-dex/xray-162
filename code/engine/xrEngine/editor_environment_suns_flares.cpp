...
template <>
void property_collection<flares::flares_type, flares>::display_name(u32 const& item_index,
                                                                    LPSTR const& buffer,
...
void flares::load(CInifile& config, shared_str const& section) {
    ...
    shared_str flare_opacity = READ_IF_EXISTS(&config, r_string, section, "flare_opacity",
                                              "0.340, 0.260, 0.500, 0.420, 0.260, 0.260");
    shared_str flare_position = READ_IF_EXISTS(&config, r_string, section, "flare_position",
                                               "1.300, 1.000, 0.500, -0.300, -0.600, -1.000");
    shared_str flare_radius = READ_IF_EXISTS(&config, r_string, section, "flare_radius",
                                             "0.080, 0.120, 0.040, 0.080, 0.120, 0.300");
    shared_str flare_textures =
    ...
    u32 min_flare_count =
        std::min(std::min(std::min(opacity_count, position_count), radius_count), texture_count);
    u32 max_flare_count =
        std::max(std::max(std::max(opacity_count, position_count), radius_count), texture_count);

    u32 max_string_count = std::max(std::max(std::max(flare_opacity.size(), flare_position.size()),
                                             flare_radius.size()),
                                    flare_textures.size()) +
                           1;

    if (min_flare_count != max_flare_count)
        Msg("! flare count for sun [%s] is setup incorrectly. only %d flares are correct",
            section.c_str(), min_flare_count);
...
void flares::fill(manager const& manager, editor::property_holder* holder,
                  editor::property_holder_collection* collection) {
    editor::property_holder* properties = holder;
    VERIFY(properties);

    properties->add_property("use", "flares", "this option is resposible for the flares usage",
                             m_use, m_use);
    properties->add_property(
        "shader", "flares", "this option is resposible for flares shader", m_shader.c_str(),
        m_shader, &*manager.m_environment.shader_ids().begin(),
        manager.m_environment.shader_ids().size(), editor::property_holder::value_editor_tree_view,
        editor::property_holder::cannot_enter_text);
    properties->add_property("flares", "flares", "this option is resposible for flares",
                             m_collection);
}
