...
void gradient::load(CInifile& config, shared_str const& section_id, LPCSTR prefix) {
    ...
    m_pFlare->CreateShader(*shader, texture.c_str());
}

void gradient::save(CInifile& config, shared_str const& section_id, LPCSTR prefix) {
    string_path temp;
    config.w_string(section_id.c_str(), strconcat(sizeof(temp), temp, prefix, "_shader"),
                    shader.c_str());
    config.w_string(section_id.c_str(), strconcat(sizeof(temp), temp, prefix, "_texture"),
                    texture.c_str());
    config.w_float(section_id.c_str(), strconcat(sizeof(temp), temp, prefix, "_opacity"), fOpacity);
    config.w_fvector2(section_id.c_str(), strconcat(sizeof(temp), temp, prefix, "_radius"),
                      fRadius);
}
...
void gradient::fill(::editor::environment::manager& environment, LPCSTR name, LPCSTR description,
    ...
    m_property_holder->add_property(
        "shader", "properties", "this option is resposible for thunderbolt gradient shader",
    ...
    m_property_holder->add_property(
        "texture", "", "this option is resposible for thunderbolt gradient texture",
