...
#ifdef DEBUG
    lua_sethook(tpLuaVM, LuaHookCall, LUA_HOOKCALL | LUA_HOOKRET | LUA_HOOKLINE | LUA_HOOKTAILRET,
                0);
#endif
...
bool bfCreateNamespaceTable(CLuaVirtualMachine* tpLuaVM, LPCSTR caNamespaceName) {
    ...
    for (;;) {
        if (!xr_strlen(S)) {
            lua_pop(tpLuaVM, 1);
            LuaOut(Lua::eLuaMessageTypeError, "the namespace name %s is incorrect!",
                   caNamespaceName);
            xr_free(S2);
            return (false);
        }
        ...
        if (lua_isnil(tpLuaVM, -1)) {
            ...
            LuaOut(Lua::eLuaMessageTypeError,
                   "the namespace name %s is already being used by the non-table object!",
                   caNamespaceName);
            return (false);
        }
...
bool Script::bfLoadBuffer(CLuaVirtualMachine* tpLuaVM, LPCSTR caBuffer, size_t tSize,
                          LPCSTR caScriptName, LPCSTR caNameSpaceName) {
    int l_iErrorCode;
    if (caNameSpaceName) {
        string256 insert;
        xr_sprintf(insert, sizeof(insert), "local this = %s\n", caNameSpaceName);
        size_t str_len = xr_strlen(insert);
        LPSTR script = new char[str_len + tSize];
...
bool bfDoFile(CLuaVirtualMachine* tpLuaVM, LPCSTR caScriptName, LPCSTR caNameSpaceName,
              bool bCall) {
    ...
    if (!bfLoadBuffer(tpLuaVM, static_cast<LPCSTR>(l_tpFileReader->pointer()),
                      (size_t)l_tpFileReader->length(), l_caLuaFileName, caNameSpaceName)) {
        lua_pop(tpLuaVM, 4);
...
bool Script::bfLoadFileIntoNamespace(CLuaVirtualMachine* tpLuaVM, LPCSTR caScriptName,
                                     LPCSTR caNamespaceName, bool bCall) {
...
bool Script::bfIsObjectPresent(CLuaVirtualMachine* tpLuaVM, LPCSTR namespace_name,
                               LPCSTR identifier, int type) {
