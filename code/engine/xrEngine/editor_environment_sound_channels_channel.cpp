...
void channel::fill(editor::property_holder_collection* collection) {
    ...
    m_property_holder->add_property("minimum distance", "properties",
                                    ...
                                    m_sound_dist.x, m_sound_dist.x);
    m_property_holder->add_property("maximum distance", "properties",
                                    "this option is resposible for maximum distance (in meters)",
                                    m_sound_dist.y, m_sound_dist.y);
    m_property_holder->add_property(
        "period 0", "properties",
        "this option is resposible for minimum start time interval (in seconds)", m_sound_period.x,
        m_sound_period.x);
    m_property_holder->add_property(
        "period 1", "properties",
        "this option is resposible for maximum start time interval (in seconds)", m_sound_period.y,
        m_sound_period.y);
    m_property_holder->add_property(
        "period 2", "properties",
        "this option is resposible for minimum pause interval (in seconds)", m_sound_period.z,
        m_sound_period.z);
    m_property_holder->add_property(
        "period 3", "properties",
        "this option is resposible for maximum pause interval (in seconds)", m_sound_period.w,
        m_sound_period.w);
    m_property_holder->add_property("sounds", "properties",
                                    "this option is resposible for sound sources", m_collection);
}
