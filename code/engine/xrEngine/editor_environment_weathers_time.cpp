...
void time::load(CInifile& config) {
    //	Ivector3 tm					={0,0,0};
    //	sscanf
    //(m_identifier.c_str(),"%d:%d:%d",&tm.x,&tm.y,&tm.z); 	R_ASSERT3
    //((tm.x>=0)&&(tm.x<24)&&(tm.y>=0)&&(tm.y<60)&&(tm.z>=0)&&(tm.z<60),"Incorrect weather
    ...
    //	fog_density					= config.r_float(m_identifier,
    //"fog_density");
    //	fog_color					= config.r_fvector3(m_identifier,
    //"fog_color");
    //	rain_color					= config.r_fvector3(m_identifier,
    //"rain_color"); 	rain_density				= config.r_float(m_identifier,
    //"rain_density");
    //	sky_color					= config.r_fvector3(m_identifier,
    //"sky_color"); 	sky_rotation				= config.r_float(m_identifier,
    //"sky_rotation");

    //	sky_texture_name			= config.r_string(m_identifier, "sky_texture");
    //	string_path					st_env;
    //	strconcat					(sizeof(st_env), st_env, sky_texture_name.c_str(),
    //"#small"); 	sky_texture_env_name		= st_env;
    ...
    //	hemi_color					= config.r_fvector4(m_identifier,
    //"hemi_color");

    //	Fvector2					coords = config.r_fvector2(m_identifier,
    //"sun_dir"); 	sun_dir.setHP				(deg2rad(coords.y),
    //deg2rad(coords.x));

    //	clouds_color				= config.r_fvector4(m_identifier, "clouds_color");

    //	LPCSTR						clouds = config.r_string(m_identifier,
    //"clouds_color"); 	VERIFY						(_GetItemCount(clouds) ==
    //5); 	string256					temp;
...
void time::save(CInifile& config) {
    ...
    config.w_string(m_identifier.c_str(), "thunderbolt_collection",
...
void time::thunderbolt_setter(LPCSTR value) {
    ...
    tb_id =
        m_manager.eff_Thunderbolt->AppendDef(m_manager, m_manager.m_thunderbolt_collections_config,
                                             m_manager.m_thunderbolts_config, value);
}
...
void time::fill(editor::property_holder_collection* collection) {
    ...
    m_property_holder->add_property("shafts intensity", "sun",
    ...
    m_property_holder->add_property(
        "altitude", "sun", "this option is resposible for sun altitude (in degrees)",
    ...
    m_property_holder->add_property(
        "longitude", "sun", "this option is resposible for sun longitude (in degrees)",
    ...
    m_property_holder->add_property("sun", "sun", "this option is resposible for ambient",
                                    m_sun.c_str(), m_sun, collection_getter, collection_size_getter,
    ...
    m_property_holder->add_property(
        "texture", "hemisphere", "this option is resposible for sky texture",
        sky_texture_name.c_str(), sky_texture_getter, sky_texture_setter, ".dds",
        "Texture files (*.dds)|*.dds", detail::real_path("$game_textures$", "").c_str(),
        "Select texture...", editor::property_holder::cannot_enter_text,
        editor::property_holder::remove_extension);

    m_property_holder->add_property("sky color", "hemisphere",
                                    "this option is resposible for sky color",
                                    (editor::color const&)sky_color, (editor::color&)sky_color);

    m_property_holder->add_property("hemi color", "hemisphere",
    ...
    m_property_holder->add_property("sky rotation", "hemisphere",
                                    "this option is resposible for sky rotation", sky_rotation,
    ...
    m_property_holder->add_property(
        "texture", "clouds", "this option is resposible for clouds texture",
        clouds_texture_name.c_str(), string_getter, string_setter, ".dds",
        "Texture files (*.dds)|*.dds", detail::real_path("$game_textures$", "").c_str(),
        "Select texture...", editor::property_holder::cannot_enter_text,
        editor::property_holder::remove_extension);
    ...
    m_property_holder->add_property("transparency", "clouds",
                                    "this option is resposible for clouds transparency",
                                    clouds_color.w, clouds_color.w, 0.f, 1.f);

    m_property_holder->add_property("color", "ambient",
    ...
    m_property_holder->add_property("ambient", "ambient", "this option is resposible for ambient",
                                    m_ambient.c_str(), string_getter, string_setter,
                                    collection_getter, collection_size_getter,
                                    editor::property_holder::value_editor_combo_box,
                                    editor::property_holder::cannot_enter_text);

    m_property_holder->add_property("color", "fog",
                                    "this option is resposible for fog density (0..1)",
                                    (editor::color const&)fog_color, (editor::color&)fog_color);
    m_property_holder->add_property("far plane", "fog", "this option is resposible for far plane",
                                    far_plane, far_plane);
    m_property_holder->add_property(
        "distance", "fog",
        "this option is resposible for fog distance (shoudl be less than far plane)", fog_distance,
        fog_distance);
    m_property_holder->add_property("density", "fog",
                                    "this option is resposible for fog density (0..1)", fog_density,
                                    fog_density, 0.f, 1.f);
    m_property_holder->add_property("water intensity", "fog",
                                    "this option is resposible for water intensity (0..1)",
                                    m_fWaterIntensity, m_fWaterIntensity, 0.f, 1.f);

    m_property_holder->add_property("rain color", "rain",
                                    "this option is resposible for rain color",
                                    (editor::color const&)rain_color, (editor::color&)rain_color);
    m_property_holder->add_property("rain density", "rain",
    ...
    m_property_holder->add_property(
        "collection", "thunderbolts", "this option is resposible for ambient",
        m_thunderbolt_collection.c_str(), m_thunderbolt_collection, collection_getter,
        collection_size_getter, editor::property_holder::value_editor_combo_box,
        editor::property_holder::cannot_enter_text);

    m_property_holder->add_property("duration", "thunderbolts",
                                    "this option is resposible for thunderbolt duration",
                                    bolt_duration, bolt_duration);
    m_property_holder->add_property("period", "thunderbolts",
                                    "this option is resposible for thunderbolt period", bolt_period,
                                    bolt_period);

    float_getter.bind(this, &time::wind_direction_getter);
    float_setter.bind(this, &time::wind_direction_setter);
    m_property_holder->add_property("direction", "wind",
                                    "this option is resposible for wind direction (in degrees)",
                                    wind_direction, float_getter, float_setter, -360.f, 360.f);
    m_property_holder->add_property(
        "velocity", "wind", "this option is resposible for wind velocity (meters per second)",
        wind_velocity, wind_velocity, 0.f, 1000.f);
}

void time::lerp(CEnvironment* parent, CEnvDescriptor& A, CEnvDescriptor& B, float f,
