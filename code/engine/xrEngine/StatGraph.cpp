...
void CStatGraph::OnDeviceCreate() {
    ...
    //	hGeomLine.create
    //(FVF::F_TL0uv,RCache.Vertex.Buffer(),RCache.Index.Buffer()); 	hGeomTri.create
    //(FVF::F_TL0uv,RCache.Vertex.Buffer(),RCache.QuadIB);
}
...
void CStatGraph::OnRender() {
    ...
    if (TriElem)
    {
            ...
            for (SubGraphVecIt it=subgraphs.begin(); it!=subgraphs.end(); it++)
            {
                    switch(it->style)
                    {
                    case stBar:		RenderBars		(&pv_Tri, &(it->elements));
    break;
                    };
            };
    ...
    {
            pv_Line_start =
    (FVF::TL0uv*)RCache.Vertex.Lock(LineElem,hGeomLine->vb_stride,dwOffsetLine); pv_Line =
    pv_Line_start;
            ...
            for (SubGraphVecIt it=subgraphs.begin(); it!=subgraphs.end(); it++)
            {
                    switch(it->style)
                    {
                    case stCurve:	RenderLines		(&pv_Line, &(it->elements));
    break; case stBarLine:	RenderBarLines	(&pv_Line, &(it->elements));	break;
                    };
            };
...
= pv_start;
        ...
        //draw rect
        pv_start				=
(FVF::TL0uv*)RCache.Vertex.Lock(5,hGeomLine->vb_stride,dwOffset); pv
= pv_start;
        // base rect
...
pv++; pv->set				(int(lt.x + g_x*grid_step.x*elem_factor),rb.y,grid_color);
pv++;
        }
        for (int g_y=1; g_y<=Num_H_LinesDwn; g_y++)
        {
                pv->set
(lt.x,int(base_y+g_y*grid_step.y*elem_factor),grid_color); pv++; pv->set
(rb.x,int(base_y+g_y*grid_step.y*elem_factor),grid_color); pv++;
        };
...
void CStatGraph::RenderLines( FVF::TL0uv** ppv, ElementsDeq* pelements )
{
        ...
        for (ElementsDeqIt it=pelements->begin()+1;  it!=pelements->end() && it!=pelements->end()+1;
it++)
        {
