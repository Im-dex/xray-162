...
BOOL CAviPlayerCustom::Load(char* fname) {
    ...
    if (mmckinfoParent.cksize !=
        (DWORD)mmioRead(hmmioFile, (HPSTR)&m_biInFormat, mmckinfoParent.cksize)) {
    ...
    if (MMSYSERR_NOERROR !=
            (res = mmioDescend(hmmioFile, &mmckinfoSubchunk, NULL, MMIO_FINDLIST)) ||
        ...
        if ((int)m_dwFrameCurrent == i) {
            ...
            for (i++; i < (int)dwFrameNum; i++) {
                ...
                res = ICDecompress(
                    ...
                    &m_biOutFormat, m_pDecompressedBuf);
                if (ICERR_OK != res &&
                    ICERR_DONTDRAW != res) { // проверка на ICERR_DONTDRAW введена из-за indeo 5.11
...
DWORD CAviPlayerCustom::CalcFrame() {
    ...
    return DWORD(
               floor((RDEVICE.dwTimeContinual - m_dwFirstFrameOffset) * m_fCurrentRate / 1000.0f)) %
           m_dwFrameTotal;
}
