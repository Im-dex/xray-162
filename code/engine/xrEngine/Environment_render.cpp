...
D3DFVF_TEXCOORDSIZE3(0) | D3DFVF_TEXCOORDSIZE3(1); struct v_clouds				{
        Fvector3	p;
        u32			color;
        u32			intensity;
...
void CEnvironment::RenderSky() {
    ...
    RCache.Index.Buffer()); clouds_sh.create		("clouds","null"); clouds_geom.create
    (v_clouds_fvf,RCache.Vertex.Buffer(), RCache.Index.Buffer()); bNeed_re_create_env		=
    FALSE;
    }
    ::Render->rmFar				();
    ...
    iFloor(CurrentEnv->sky_color.y*255.f), iFloor(CurrentEnv->sky_color.z*255.f),
    ...
    (hbox_verts[v*2],C,hbox_verts[v*2+1]); RCache.Vertex.Unlock		(12,sh_2geom.stride());
...
void CEnvironment::RenderClouds() {
    ...
    wind_dir.set
    (wd0.x,wd0.z,wd1.x,wd1.z).mul(0.5f).add(0.5f).mul(255.f); u32		i_offset,v_offset;
    u32		C0					=
    color_rgba(iFloor(wind_dir.x),iFloor(wind_dir.y),iFloor(wind_dir.w),iFloor(wind_dir.z)); u32
    C1					=
    ...
    // Fill index buffer
    u16*	pib					= RCache.Index.Lock
    (CloudsIndices.size(),i_offset); CopyMemory
    (pib,&CloudsIndices.front(),CloudsIndices.size()*sizeof(u16)); RCache.Index.Unlock
    (CloudsIndices.size());

    // Fill vertex buffer
    v_clouds* pv				= (v_clouds*)	RCache.Vertex.Lock
    (CloudsVerts.size(),clouds_geom.stride(),v_offset); for (FvectorIt it=CloudsVerts.begin();
    it!=CloudsVerts.end(); it++,pv++) pv->set					(*it,C0,C1);
...
void CEnvironment::OnDeviceCreate() {
    ...
    sh_2geom.create			(v_skybox_fvf,RCache.Vertex.Buffer(),
    RCache.Index.Buffer()); clouds_sh.create		("clouds","null"); clouds_geom.create
    (v_clouds_fvf,RCache.Vertex.Buffer(), RCache.Index.Buffer());
    */
