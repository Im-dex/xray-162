...
CEffect_Rain::CEffect_Rain() {
    ...
    VERIFY3							(F,"Can't open
    file.","dm\\rain.dm");
    DM_Drop							= ::Render->model_CreateDM
    (F);
...
void CEffect_Rain::RenewItem(Item& dest, float height, BOOL bHit) {
    ...
    if (bHit) {
        dest.dwTime_Life =
            Device.dwTimeGlobal + iFloor(1000.f * height / dest.fSpeed) - Device.dwTimeDelta;
        dest.dwTime_Hit =
            Device.dwTimeGlobal + iFloor(1000.f * height / dest.fSpeed) - Device.dwTimeDelta;
        ...
        dest.dwTime_Life =
            Device.dwTimeGlobal + iFloor(1000.f * height / dest.fSpeed) - Device.dwTimeDelta;
        dest.dwTime_Hit =
            Device.dwTimeGlobal + iFloor(2 * 1000.f * height / dest.fSpeed) - Device.dwTimeDelta;
...
void CEffect_Rain::OnFrame() {
    ...
    if (snd_Ambient._feedback()) {
        //		Fvector					sndP;
        //		sndP.mad
        //(Device.vCameraPosition,Fvector().set(0,1,0),source_offset);
...
void CEffect_Rain::Render() {
    ...
    /*
    float	factor				=
g_pGamePersistent->Environment().CurrentEnv->rain_density; if (factor<EPS_L)
return;

    u32 desired_items			= iFloor
(0.5f*(1.f+factor)*float(max_desired_items));
    // visual
    float		factor_visual	= factor/2.f+.5f;
    Fvector3	f_rain_color	= g_pGamePersistent->Environment().CurrentEnv->rain_color;
    u32			u_rain_color	=
color_rgba_f(f_rain_color.x,f_rain_color.y,f_rain_color.z,factor_visual);
...
RCache.Vertex.Lock(desired_items*4,hGeom_Rain->vb_stride,vOffset); FVF::LIT	*start		=
verts; const Fvector&	vEye	= Device.vCameraPosition; for (u32 I=0; I<items.size(); I++){
...
//.		float dt		=
Device.fTimeDelta;//xdt<Device.fTimeDelta?xdt:Device.fTimeDelta; float dt		=
Device.fTimeDelta; one.P.mad		(one.D,one.fSpeed*dt);
...
//							Log("2",height-dist);
                                            ...
                                            RenewItem
(one,max_distance-_sqrt(dist_sqr),FALSE);		// fly ...
...
(pos_head,one.D,-drop_length*factor_visual);
            ...
            float w = drop_width;
            u32 s	= one.uv_set;
            P.mad(pos_trail,lineTop,-w);	verts->set(P,u_rain_color,UV[s][0].x,UV[s][0].y);
verts++; P.mad(pos_trail,lineTop,w);		verts->set(P,u_rain_color,UV[s][1].x,UV[s][1].y);
verts++; P.mad(pos_head, lineTop,-w);	verts->set(P,u_rain_color,UV[s][2].x,UV[s][2].y);
verts++; P.mad(pos_head, lineTop,w);		verts->set(P,u_rain_color,UV[s][3].x,UV[s][3].y);
verts++;
    }
...
(D3DRS_CULLMODE,D3DCULL_CCW);
...

    {
            ...
            u32						vCount_Lock		=
particles_cache*DM_Drop->number_vertices; u32						iCount_Lock
= particles_cache*DM_Drop->number_indices; IRender_DetailModel::fvfVertexOut* v_ptr=
...
DM_Drop->number_vertices; i_ptr			+=	DM_Drop->number_indices; pcount
++;
...
(vCount_Lock,hGeom_Drops->vb_stride); _IS.Unlock				(iCount_Lock);
...
(iCount_Lock, i_offset);
            ...
            if (pcount)	{
                    RCache.set_Geometry		(hGeom_Drops);
                    RCache.Render
(D3DPT_TRIANGLELIST,v_offset,0,vCount_Lock,i_offset,dwNumPrimitives);
            }
    }
    */
}
