...
void CObjectAnimator::DrawPath() {
    ...
    if (m_Current) {
        ...
        if (!path_points.empty())
            DU_impl.DrawPrimitiveL(D3DPT_LINESTRIP, path_points.size() - 1, path_points.begin(),
                                   path_points.size(), clr, true, false);
        ...
        for (KeyIt k_it = E->keys.begin(); k_it != E->keys.end(); k_it++) {
            ...
            if (EDevice.m_Camera.GetPosition().distance_to_sqr(T) < 50.f * 50.f) {
                ...
                DU_impl.OutText(T, AnsiString().sprintf("K: %3.3f", (*k_it)->time).c_str(),
