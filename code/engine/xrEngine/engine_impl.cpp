    ...
    for (; i != e; ++i)
        if ((*i)->m_identifier == new_frame_id) {
            ...
            if (set_time) {
                ...
                g_pGameLevel->SetEnvironmentGameTimeFactor(iFloor((*i)->exec_time * 1000.f),
                                                           environment.fTimeFactor);
            }
            break;
        }
}
...
void engine_impl::track_frame(float const& time) {
    ...
    g_pGameLevel->SetEnvironmentGameTimeFactor(iFloor(current_time * 1000.f),
                                               environment.fTimeFactor);
}
...
void engine_impl::track_weather(float const& time) {
    ...
    g_pGamePersistent->Environment().SetGameTime(time * 24 * 60 * 60,
...
void engine_impl::save_weathers() {
    ...
    editor::environment::manager& manager =
        dynamic_cast<editor::environment::manager&>(environment);
    manager.save();
}
...
void engine_impl::weather_current_time(char const* time) {
    ...
    g_pGamePersistent->Environment().m_paused = false;
    g_pGamePersistent->Environment().SetGameTime(float(hours * 60 * 60 + minutes * 60 + seconds),
                                                 g_pGamePersistent->Environment().fTimeFactor);
    g_pGamePersistent->Environment().m_paused = paused;
