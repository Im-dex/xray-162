...
CObject* CObjectList::FindObjectByCLS_ID(CLASS_ID cls) {
    {
        Objects::iterator O =
            std::find_if(objects_active.begin(), objects_active.end(), fClassEQ(cls));
...
#ifdef DEBUG
//		if (O->getDestroy())
//			Msg					("- !!!processing_enabled
//->destroy_queue.push_back %s[%d] frame [%d]",O->cName().c_str(), O->ID(), Device.dwFrame);
#endif // #ifdef DEBUG
...
//->destroy_queue.push_back %s[%d] frame [%d]",O->cName().c_str(), O->ID(), Device.dwFrame);
    ...
    //Device.dwFrame, dynamic_cast<void*>(O));

    O->UpdateCL();

    VERIFY3(O->dbg_update_cl == Device.dwFrame, "Broken sequence of calls to 'UpdateCL'",
            *O->cName());
#if 0 // ndef DEBUG
	__try
	{
    ...
    if (O->H_Parent() && (O->H_Parent()->getDestroy() || O->H_Root()->getDestroy())) {
        ...
        Msg("! ERROR: incorrect destroy sequence for object[%d:%s], section[%s], parent[%d:%s]",
            O->ID(), *O->cName(), *O->cNameSect(), O->H_Parent()->ID(), *O->H_Parent()->cName());
    }
#if 0 // ndef DEBUG
	}
...
void CObjectList::clear_crow_vec(Objects& o) {
    for (u32 _it = 0; _it < o.size(); _it++) {
        ...
        //Device.dwFrame, dynamic_cast<void*>(o[_it]));
...
#ifdef DEBUG
            if (debug_destroy)
                Msg("Destroying object[%x][%x] [%d][%s] frame[%d]", dynamic_cast<void*>(O), O,
                    O->ID(), *O->cName(), Device.dwFrame);
#endif // DEBUG
            O->net_Destroy();
            Destroy(O);
        }
...
void CObjectList::Load() {
    R_ASSERT(/*map_NETID.empty() &&*/ objects_active.empty() && destroy_queue.empty() &&
...
void CObjectList::Destroy(CObject* O) {
    ...
    if (_i != objects_active.end()) {
        ...
        VERIFY(std::find(objects_sleeping.begin(), objects_sleeping.end(), O) ==
...
void CObjectList::dump_list(Objects& v, LPCSTR reason) {
    ...
    for (; it != it_e; ++it)
        Msg("%x - name [%s] ID[%d] parent[%s] getDestroy()=[%s]", (*it), (*it)->cName().c_str(),
            (*it)->ID(), ((*it)->H_Parent()) ? (*it)->H_Parent()->cName().c_str() : "",
...
void CObjectList::register_object_to_destroy(CObject* object_to_destroy) {
    ...
    for (; it != it_e; ++it) {
        ...
        if (!O->getDestroy() && O->H_Parent() == object_to_destroy) {
            Msg("setDestroy called, but not-destroyed child found parent[%d] child[%d]",
...
bool CObjectList::registered_object_to_destroy(const CObject* object_to_destroy) const {
    return (std::find(destroy_queue.begin(), destroy_queue.end(), object_to_destroy) !=
            destroy_queue.end());
}
#endif // DEBUG