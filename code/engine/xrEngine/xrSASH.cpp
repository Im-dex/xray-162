...
xrSASH::xrSASH()
    : m_bInited(false), m_bOpenAutomate(false), m_bBenchmarkRunning(false), m_bRunning(false),
      m_bReinitEngine(false), m_bExecutingConsoleCommand(false) {
    ;
}
...
void xrSASH::ReportNative(LPCSTR pszTestName) {
    ...
    // float(::Render->get_generation())/10.f,	"dx-level required"		);  res.w_float
    // ("general","average",	rfps_average,							"average for this run"
