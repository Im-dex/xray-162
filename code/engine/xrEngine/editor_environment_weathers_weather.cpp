...
template <>
void property_collection<weather::container_type, weather>::display_name(u32 const& item_index,
                                                                         LPSTR const& buffer,
...
void weather::fill(editor::property_holder_collection* collection) {
    ...
    m_property_holder->add_property("times", "properties", "this option is resposible for times",
                                    m_collection);
}
...
bool weather::add_time_frame(char const* buffer, u32 const& buffer_size) {
    ...
    container_type::iterator found =
        std::lower_bound(m_times.begin(), m_times.end(), section, &id::predicate);
