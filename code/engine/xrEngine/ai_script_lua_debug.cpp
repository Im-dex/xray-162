    ...
    for (int i = -1;; --i)
        if (lua_isstring(tpLuaVirtualMachine, i)) {
            ...
            if (!xr_strcmp(S, "cannot resume dead coroutine")) {
                ...
                LuaOut(iErorCode ? Lua::eLuaMessageTypeError : Lua::eLuaMessageTypeMessage, "%s",
                       S);
            }
        } else {
            for (i = 0;; ++i)
                if (lua_isstring(tpLuaVirtualMachine, i)) {
                    ...
                    if (!xr_strcmp(S, "cannot resume dead coroutine")) {
                        ...
                        if (!i && !iErorCode)
                            LuaOut(Lua::eLuaMessageTypeInfo, "Output from %s", caScriptFileName);
                        LuaOut(iErorCode ? Lua::eLuaMessageTypeError : Lua::eLuaMessageTypeMessage,
                               "%s", S);
                    }
                } else
                    return (false);
...
bool Script::bfListLevelVars(CLuaVirtualMachine* tpLuaVirtualMachine, int iStackLevel) {
    ...
    //	Msg			("  Event			:
    //%d",cafEventToString(l_tDebugInfo.event)); 	Msg			("  Name
    //: %s",l_tDebugInfo.name); 	Msg			("  Name what		:
    //%s",l_tDebugInfo.namewhat); 	Msg			("  What			:
    //%s",l_tDebugInfo.what); 	Msg			("  Source			:
    //%s",l_tDebugInfo.source); 	Msg			("  Source (short)	:
    //%s",l_tDebugInfo.short_src); 	Msg			("  Current line	:
    //%d",l_tDebugInfo.currentline); 	Msg			("  Nups			:
    //%d",l_tDebugInfo.nups); 	Msg			("  Line defined	:
    //%d",l_tDebugInfo.linedefined); 	i			= 1; 	while (NULL != (name =
    //lua_getlocal(tpLuaVirtualMachine, &l_tDebugInfo, i++))) { 		Msg
    //("    local   %d %s", i-1, name); 		lua_pop	(tpLuaVirtualMachine, 1);  /*
