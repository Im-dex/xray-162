...
void blend::fill(manager const& manager, editor::property_holder* holder,
                 editor::property_holder_collection* collection) {
    editor::property_holder* properties = holder;
    VERIFY(properties);

    properties->add_property("down time", "blend",
                             "this option is resposible for the blend down time", m_down_time,
                             m_down_time);
    properties->add_property("rise time", "blend",
                             "this option is resposible for the blend rise time", m_rise_time,
                             m_rise_time);
    properties->add_property("time", "blend", "this option is resposible for the blend time",
                             m_time, m_time);
}
