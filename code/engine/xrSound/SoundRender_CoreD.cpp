...
(KSPROPERTY_SUPPORT_GET|KSPROPERTY_SUPPORT_SET)) return TRUE;
...
support = FALSE; if (!EAXQuerySupport(&DSPROPSETID_EAX_ListenerProperties, ext |
DSPROPERTY_EAXLISTENER_ALLPARAMETERS)) 			support = FALSE; if
(!EAXQuerySupport(&DSPROPSETID_EAX_ListenerProperties, ext | DSPROPERTY_EAXLISTENER_ROOM))
support = FALSE; if (!EAXQuerySupport(&DSPROPSETID_EAX_ListenerProperties, ext |
DSPROPERTY_EAXLISTENER_ROOMHF)) 			  	support = FALSE; if
(!EAXQuerySupport(&DSPROPSETID_EAX_ListenerProperties, ext |
DSPROPERTY_EAXLISTENER_ROOMROLLOFFFACTOR)) 		support = FALSE; if
(!EAXQuerySupport(&DSPROPSETID_EAX_ListenerProperties, ext | DSPROPERTY_EAXLISTENER_DECAYTIME))
support = FALSE; if (!EAXQuerySupport(&DSPROPSETID_EAX_ListenerProperties, ext |
DSPROPERTY_EAXLISTENER_DECAYHFRATIO)) 			support = FALSE; if
(!EAXQuerySupport(&DSPROPSETID_EAX_ListenerProperties, ext | DSPROPERTY_EAXLISTENER_REFLECTIONS))
support = FALSE; if (!EAXQuerySupport(&DSPROPSETID_EAX_ListenerProperties, ext |
DSPROPERTY_EAXLISTENER_REFLECTIONSDELAY)) 		support = FALSE; if
(!EAXQuerySupport(&DSPROPSETID_EAX_ListenerProperties, ext | DSPROPERTY_EAXLISTENER_REVERB))
support = FALSE; if (!EAXQuerySupport(&DSPROPSETID_EAX_ListenerProperties, ext |
DSPROPERTY_EAXLISTENER_REVERBDELAY)) 		    support = FALSE; if
(!EAXQuerySupport(&DSPROPSETID_EAX_ListenerProperties, ext | DSPROPERTY_EAXLISTENER_ENVIRONMENT))
support = FALSE; if (!EAXQuerySupport(&DSPROPSETID_EAX_ListenerProperties, ext |
DSPROPERTY_EAXLISTENER_ENVIRONMENTSIZE)) 	    support = FALSE; if
(!EAXQuerySupport(&DSPROPSETID_EAX_ListenerProperties, ext |
DSPROPERTY_EAXLISTENER_ENVIRONMENTDIFFUSION)) 	support = FALSE; if
(!EAXQuerySupport(&DSPROPSETID_EAX_ListenerProperties, ext |
DSPROPERTY_EAXLISTENER_AIRABSORPTIONHF)) 		support = FALSE; if
(!EAXQuerySupport(&DSPROPSETID_EAX_ListenerProperties, ext | DSPROPERTY_EAXLISTENER_FLAGS))
...
return; if( FAILED			( pDevice->SetCooperativeLevel(  (HWND)window,
DSSCL_PRIORITY ) ) )
        {
                _destroy();
                return;
    ...
    // Create property set
        ...
        desc.lpwfxFormat 		= &wave;
                ...
                bEAX				= TRUE;
                        if (FAILED(pTempBuf->QueryInterface(IID_IKsPropertySet, (LPVOID
*)&pExtensions))){ bEAX			= FALSE; }else{ bEAX 			=
EAXTestSupport(FALSE); bDeferredEAX	= EAXTestSupport(TRUE);
            }
        }
        _RELEASE				(pTempBuf);
    }
...
clamp	(hw_volume,DSBVOLUME_MIN,DSBVOLUME_MAX); pBuffer->SetVolume	(hw_volume);
        }
}
...
void	CSoundRender_CoreD::i_eax_set			(const GUID* guid, u32 prop, void*
val, u32 sz)
{
        VERIFY	(pExtensions);
        R_CHK	(pExtensions->Set		(*guid, prop, NULL, 0, val, sz));
}
void	CSoundRender_CoreD::i_eax_get			(const GUID* guid, u32 prop, void*
val, u32 sz)
{
...
void CSoundRender_CoreD::update_listener( const Fvector& P, const Fvector& D, const Fvector& N,
float dt )
{
