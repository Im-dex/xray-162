...
void CSoundStream::Update() {
    ...
    if (bNeedUpdate) {
        ...
        pBuffer->SetVolume(
            LONG((1 - fRealVolume * psSoundVMusic * fBaseVolume) * float(DSBVOLUME_MIN)));
        bNeedUpdate = false;
    }
}
...
void CSoundStream::Play(BOOL loop, int cnt) {
    ...
    CHK_DX(acmStreamSize(hAcmStream, dwDestBufSize, LPDWORD(&dwSrcBufSize),
                                        ...
                                        u32 dwSoundBytes)    // size of block to copy
{
    ...
    if (DS_OK ==
        pBuffer->Lock(dwOffset, dwSoundBytes, &lpvPtr1, &dwBytes1, &lpvPtr2, &dwBytes2, 0)) {
        // Write to pointers.
