...
void message_filter::filter(u16 const& msg_type, u32 const& msg_subtype,
...
void message_filter::dbg_print_msg(NET_Packet& packet, msg_type_subtype_t const& msg_type) {
    ...
    case M_EVENT: {
        ...
        default: {
            xr_sprintf(tmp_string, "--- CL_EVENT [%7u][%5u]: EVENT_ID=[%d]",
                       msg_type.msg_receive_time, msg_type.dest_obj_id, msg_type.msg_subtype);
        };
    ...
    case M_GAMEMESSAGE: {
        ...
        case GAME_EVENT_PLAYER_KILLED: {
            xr_sprintf(tmp_string, "--- GM_EVENT [%7u]: GAME_EVENT_PLAYER_KILLED",
                       msg_type.msg_receive_time);
        } break;
        case GAME_EVENT_ROUND_STARTED: {
            xr_sprintf(tmp_string, "--- GM_EVENT [%7u]: GAME_EVENT_ROUND_STARTED",
                       msg_type.msg_receive_time);
        } break;
        case GAME_EVENT_ARTEFACT_TAKEN: {
            xr_sprintf(tmp_string, "--- GM_EVENT [%7u]: GAME_EVENT_ARTEFACT_TAKEN",
                       msg_type.msg_receive_time);
        } break;
        default: {
            xr_sprintf(tmp_string, "--- GM_EVENT [%7u]: GAME_EVENT_ID=[%d]",
    ...
    default: {
        xr_sprintf(tmp_string, "--- MESSAGE_ID[%u]         [%7u]", msg_type.msg_type,
                   msg_type.msg_receive_time);
    };
    }; // switch (m_type)
