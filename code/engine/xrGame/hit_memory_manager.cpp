...
const CHitObject* CHitMemoryManager::hit(const CEntityAlive* object) const {
    ...
    HITS::const_iterator I =
        std::find_if(m_hits->begin(), m_hits->end(), CHitObjectPredicate(object));
    if (m_hits->end() != I)
        return (&*I);
...
void CHitMemoryManager::add(float amount, const Fvector& vLocalDir, const CObject* who,
                            s16 element) {
#ifndef MASTER_GOLD
    ...
    if (m_hits->end() == J) {
        ...
        if (m_max_hit_count <= m_hits->size()) {
            HITS::iterator I = std::min_element(m_hits->begin(), m_hits->end(),
...
#ifndef MASTER_GOLD
    if (_hit_object.m_object && smart_cast<CActor const*>(_hit_object.m_object) &&
...
void CHitMemoryManager::update() {
    ...
    VERIFY(m_hits);
    m_hits->erase(std::remove_if(m_hits->begin(), m_hits->end(), CRemoveOfflinePredicate()),
                  m_hits->end());
...
void CHitMemoryManager::load(IReader& packet) {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        object.m_object =
            smart_cast<CEntityAlive*>(Level().Objects.net_Find(delayed_object.m_object_id));
        // object params
