...
void RELATION_REGISTRY::Action(CEntityAlive* from, CEntityAlive* to, ERelationAction action) {
    ...
    static u32 min_attack_delta_time =
        u32(1000.f * pSettings->r_float(ACTIONS_POINTS_SECT, "min_attack_delta_time"));
    ...
    case ATTACK: {
            ...
            if (fight_data) {
                ...
                if (defending_stalker) {
                    ...
                    Action(actor, defending_stalker,
                           attacking_stalker ? FIGHT_HELP_HUMAN : FIGHT_HELP_MONSTER);
                }
            }
        }
        ...
        if (stalker) {
            ...
            if (delta_goodwill && !stalker_attack_team_mate) {
                ...
                CHARACTER_GOODWILL community_goodwill = (CHARACTER_GOODWILL)(
                    stalker->Sympathy() * (float)(st->community_member_attack_goodwill));
    ...
    case KILL: {
        if (stalker) {
            ...
            bool stalker_kills_team_mate =
                stalker_from && (stalker_from->Community() == stalker->Community());
            ...
            if (delta_goodwill && !stalker_kills_team_mate) {
                ...
                CHARACTER_GOODWILL community_goodwill = (CHARACTER_GOODWILL)(
                    stalker->Sympathy() * (float)(community_member_kill_goodwill));
                if (community_goodwill) {
                    ChangeCommunityGoodwill(stalker->Community(), from->ID(), community_goodwill);
                }
            }
            ...
            delta_rank =
                CHARACTER_RANK::rank_kill_points(CHARACTER_RANK::ValueToIndex(stalker->Rank()));
            if (delta_rank)
    ...
    case FIGHT_HELP_MONSTER: {
        if (stalker && stalker->g_Alive()) {
            ...
            if (delta_goodwill) {
                ...
                //from->ID(), (CHARACTER_GOODWILL)( stalker->Sympathy() * (float)delta_goodwill ));
                CHARACTER_GOODWILL community_goodwill = (CHARACTER_GOODWILL)(
                    stalker->Sympathy() * (float)(community_member_fight_help_goodwill));
                if (community_goodwill) {
                    ChangeCommunityGoodwill(stalker->Community(), from->ID(), community_goodwill);
                }
            }
