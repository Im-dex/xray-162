...
private:
    void loophole_path(cover const& cover, shared_str const& source, shared_str const& target,
                       LoopholePath& path) const;
    void build_enter_path();
    void actualize_path();
    void try_actualize_path();
    transition_action const& transition(cover const& cover, shared_str const& loophole_id0,
