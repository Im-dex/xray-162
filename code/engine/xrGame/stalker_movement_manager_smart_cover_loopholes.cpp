...
float stalker_movement_manager_smart_cover::enter_path(LoopholePath* result,
...
void stalker_movement_manager_smart_cover::build_enter_path() {
    ...
    enter_path(&m_path, position, object().ai_location().level_vertex_id(), target_cover,
               target_loophole_id);
...
float stalker_movement_manager_smart_cover::exit_path_weight(u32 const& source_node,
...
bool stalker_movement_manager_smart_cover::test_pick(Fvector source, Fvector destination) const {
    ...
    Level().ObjectSpace.RayQuery(m_ray_query_storage, ray_defs, &test_pick::callback, &params, NULL,
                                 NULL);
...
stalker_movement_manager_smart_cover::nearest_action(
    ...
    Fvector const& position, Fvector& result_position, u32& result_vertex_id,
    ...
    for (; i != e; ++i) {
        ...
        for (; I != E; ++I) {
            ...
            if ((*I)->has_animation()) {
                ...
                float const y = ai().level_graph().vertex_plane_y(vertex_id, action_position.x,
                                                                  action_position.z);
    ...
    VERIFY2(result, make_string("cover[%s][%s], loophole[%s -> %s], body_state[%s] [%f][%f][%f]",
                                ...
                                !target_body_state
                                    ...
                                    : (*target_body_state == eBodyStateStand
                                           ? "stand"
                                           : (*target_body_state == eBodyStateCrouch ? "crouch"
                                                                                     : "invalid!")),
                                VPUSH(position)));
    return (*result);
}
...
stalker_movement_manager_smart_cover::transition_action const&
stalker_movement_manager_smart_cover::action(smart_cover::cover const& cover,
...
void stalker_movement_manager_smart_cover::build_exit_path() {
    ...
    for (; I != E; ++I) {
        ...
        nearest_action(cur_cover, exitable_loophole_id, smart_cover::transform_vertex("", false),
                       target_position, exit_position, exit_vertex_id, &m_target.m_body_state);

        new_value +=
            exit_path_weight(exit_vertex_id, exit_position, targe_vertex_id, target_position);
...
void stalker_movement_manager_smart_cover::build_exit_path_to_cover() {
    ...
    for (; I != E; ++I) {
        ...
        shared_str const& exitable_loophole_id = (*I)->id();
        loophole_path(current_cover, current_loophole.id(), exitable_loophole_id,
        ...
        smart_cover::transitions::action const& current_action = nearest_action(
            current_cover, exitable_loophole_id, smart_cover::transform_vertex("", false),
            target_position, exit_position, exit_vertex_id, &exit_body_state);
...
void stalker_movement_manager_smart_cover::go_next_loophole() {
    ...
    if (m_path[0]._get() == smart_cover::transform_vertex("", true)._get()) {
        ...
        VERIFY(!m_current.cover());
#ifdef DEBUG
        Msg("setting up cover (direct from target): %s (%s)", m_target.cover_id().c_str(),
...
void stalker_movement_manager_smart_cover::non_animated_change_loophole() {
    ...
    if (!exit_loophole(loophole_id)) {
        if (!target_approached(m_apply_loophole_direction_distance))
            ...
            object().sight().setup(
                CSightAction(SightManager::eSightTypeDirection, direction, true));
        }
    }
...
loophole const&
stalker_movement_manager_smart_cover::loophole(smart_cover::cover const& cover,
                                               shared_str const& loophole_id) const {
    typedef smart_cover::cover::Loopholes Loopholes;
    Loopholes const& loopholes = cover.description()->loopholes();
    Loopholes::const_iterator i =
        std::find_if(loopholes.begin(), loopholes.end(), loophole_id_predicate(loophole_id));
