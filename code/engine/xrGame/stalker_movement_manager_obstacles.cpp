...
stalker_movement_manager_obstacles::stalker_movement_manager_obstacles(CAI_Stalker* object)
    : inherited(object), m_last_dest_vertex_id(u32(-1)), m_last_fail_time(0),
...
CRestrictedObject* stalker_movement_manager_obstacles::create_restricted_object() {
    m_restricted_object = new CRestrictedObjectObstacle(
...
bool stalker_movement_manager_obstacles::can_build_restricted_path(const obstacles_query& query) {
    ...
    m_failed_to_build_path = !ai().graph_engine().search(
        ai().level_graph(), object().ai_location().level_vertex_id(), level_path().dest_vertex_id(),
        &m_temp_path, evaluator_type(type_max<_dist_type>, _iteration_type(-1), 4096));
...
void stalker_movement_manager_obstacles::move_along_path(CPHMovementControl* movement_control,
    ...
    if (!ai().doors().actualize_doors_state(*m_doors_actor, old_desirable_speed())) {
        //		Msg							( "%6d stalker %s waits for the
        //some door to be open/closed", Device.dwTimeGlobal, object().cName().c_str() );
        float desirable_speed = old_desirable_speed();
        set_desirable_speed(0.f);
...
static float get_distance(Fvector const& a_first, Fvector const& a_second, Fvector const& b_first,
    ...
    switch (ai().level_graph().intersect(a_first.x, a_first.z, a_second.x, a_second.z, b_first.x,
                                         b_first.z, b_second.x, b_second.z, &intersection.x,
                                         &intersection.y)) {
...
float stalker_movement_manager_obstacles::is_going_through(Fmatrix const& matrix,
    ...
    for (; i != e; ++i) {
        float const distance =
            get_distance((*(i - 1)).position, (*i).position, start_position, stop_position, .35f);
