...
void CInfoPortion::load_shared	(LPCSTR)
{
        ...
        //loading from XML
        XML_NODE* pNode			= pXML->NavigateToNode(id_to_index::tag_name,
item_data->pos_in_file); THROW3					(pNode, "info_portion id=",
*item_data->id);
...
void _destroy_item_data_vector_cont(T_VECTOR* vec) {
    ...
    for (; it != it_e; ++it) {
        std::vector<CUIXml*>::iterator it_f = std::find(_tmp.begin(), _tmp.end(), (*it)._xml);
        if (it_f == _tmp.end())
