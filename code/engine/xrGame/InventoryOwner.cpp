...
LPCSTR CInventoryOwner::trade_section() const {
    ...
    VERIFY(game_object);
    return (
        READ_IF_EXISTS(pSettings, r_string, game_object->cNameSect(), "trade_section", "trade"));
}
...
bool CInventoryOwner::AllowItemToTrade(CInventoryItem const* item,
    ...
    return (
        trade_parameters().enabled(CTradeParameters::action_sell(0), item->object().cNameSect()));
}
