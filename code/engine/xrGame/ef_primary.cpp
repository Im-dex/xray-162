...
float CPersonalHealthFunction::ffGetValue() {
    if (ef_storage().non_alife().member()) {
        ...
        VERIFY3(l_tpALifeMonsterAbstract, "Invalid object passed to the evaluation function ",
                m_caName);
        ...
        if (tpInventoryOwner) {
            ...
            for (; I <= E; ++I) {
                ...
                if (iitem) {
                    ...
                    if (tpCustomWeapon && (tpCustomWeapon->GetSuitableAmmoTotal(true) >
...
float CGraphPointType0::ffGetValue() {
    return (
        ai().game_graph().vertex(ef_storage().alife().member_item()->m_tGraphID)->vertex_type()[0]);
}
...
#pragma todo("Dima to Dima : Append ItemDeterioration with non-ALife non-weapon branch")
        ...
        R_ASSERT2(l_tpALifeInventoryItem,
                  "Non-item object specified for the ItemDeterioration evaluation function");
...
float CEnemyDistanceToGraphPoint::ffGetValue() {
    ...
    R_ASSERT3(l_tpALifeDynamicObject, "Invalid object passed to the evaluation function ",
              m_caName);
