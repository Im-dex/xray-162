...
void CSightManager::reinit() {
    ...
    VERIFY(xr::valid(s_free_factors));
...
void CSightManager::reload(LPCSTR section) {
    m_max_left_angle =
        deg2rad(READ_IF_EXISTS(pSettings, r_float, section, "max_left_torso_angle", 90.f));
    m_max_right_angle =
        deg2rad(READ_IF_EXISTS(pSettings, r_float, section, "max_right_torso_angle", 60.f));
}
...
// s_stats_graph->AppendItem			( angle_normalize_signed(head.current.yaw),   0xff00ff00,
// 0 );
// s_stats_graph->AppendItem			( angle_normalize_signed(head.current.pitch), 0xffff0000,
// 1 );

#ifdef DEBUG
    if (g_ai_dbg_sight)
        Msg("%6d [%s] before body[%f]->[%f], head[%f]->[%f]", Device.dwTimeGlobal,
            object().cName().c_str(), body.current.yaw, body.target.yaw, head.current.yaw,
            head.target.yaw);
#endif // #ifdef DEBUG
    vfValidateAngleDependency(body.current.yaw, body.target.yaw, head.current.yaw);
#ifdef DEBUG
    if (g_ai_dbg_sight)
        Msg("%6d [%s] after  body[%f]->[%f], head[%f]->[%f]", Device.dwTimeGlobal,
            object().cName().c_str(), body.current.yaw, body.target.yaw, head.current.yaw,
            head.target.yaw);
#endif // #ifdef DEBUG
...
#ifdef DEBUG
    if (g_ai_dbg_sight)
        Msg("%6d [%s] after2 body[%f]->[%f], head[%f]->[%f]", Device.dwTimeGlobal,
            object().cName().c_str(), body.current.yaw, body.target.yaw, head.current.yaw,
            head.target.yaw);
#endif // #ifdef DEBUG
...
#ifdef DEBUG
    if (g_ai_dbg_sight)
        Msg("%6d [%s] after3 body[%f]->[%f], head[%f]->[%f]", Device.dwTimeGlobal,
            object().cName().c_str(), body.current.yaw, body.target.yaw, head.current.yaw,
            head.target.yaw);
#endif // #ifdef DEBUG
...
void CSightManager::update() {
    ...
    if (!m_turning_in_place) {
        if (angle_difference(object().movement().m_body.current.yaw,
            ...
            //			Msg				("%6d started turning in
            //place",Device.dwTimeGlobal);
...
Fvector CSightManager::aiming_position() const {
    ...
    case eSightTypeCurrentDirection: {
        VERIFY2(xr::valid(object().Position()),
                make_string("[%f][%f][%f]", VPUSH(object().Position())));
        VERIFY2(xr::valid(-object().movement().m_head.current.yaw),
                make_string("%f", -object().movement().m_head.current.yaw));
        VERIFY2(xr::valid(-object().movement().m_head.current.yaw),
                make_string("%f", -object().movement().m_head.current.pitch));
        VERIFY(xr::valid(Fvector().setHP(-object().movement().m_head.current.yaw,
    ...
    case eSightTypeDirection: {
        VERIFY(xr::valid(current_action().vector3d()));
    ...
    case eSightTypeFirePosition: {
        ...
        VERIFY2(result.magnitude() < 100000.f,
                make_string("[%f][%f][%f]", VPUSH(current_action().vector3d())));
        VERIFY(xr::valid(current_action().vector3d()));
        break;
    }
...
void CSightManager::process_action(float const time_delta) {
    VERIFY(xr::valid(time_delta));
    VERIFY(xr::valid(s_factor_lerp_speed));
    ...
    VERIFY(xr::valid(factors));
    //	if ( object().cName() == "level_prefix_stalker" ) {
    //		Msg							("[%6d][%6d] [%f] + [%f] = [%f] ([%f])",  Device.dwFrame,
    //Device.dwTimeGlobal, m_current.m_head.m_factor,		s_factor_lerp_speed*time_delta,		lerp (
    //m_current.m_head.m_factor,		factors.x, s_factor_lerp_speed*time_delta ), factors.x
    //);
    //		Msg							("[%6d][%6d] [%f] + [%f] = [%f] ([%f])",  Device.dwFrame,
    //Device.dwTimeGlobal, m_current.m_shoulder.m_factor,	s_factor_lerp_speed*time_delta,		lerp (
    //m_current.m_shoulder.m_factor,	factors.y, s_factor_lerp_speed*time_delta ), factors.y );
    //		Msg							("[%6d][%6d] [%f] + [%f] = [%f] ([%f])",  Device.dwFrame,
    //Device.dwTimeGlobal, m_current.m_spine.m_factor,	s_factor_lerp_speed*time_delta,		lerp (
    //m_current.m_spine.m_factor,		factors.z, s_factor_lerp_speed*time_delta ), factors.z
    //);
    //	}

    VERIFY(xr::valid(m_current.m_head.m_factor));
    m_current.m_head.m_factor =
        lerp(m_current.m_head.m_factor, factors.x, s_factor_lerp_speed * time_delta);
    VERIFY(xr::valid(m_current.m_head.m_factor));

    VERIFY(xr::valid(m_current.m_shoulder.m_factor));
    m_current.m_shoulder.m_factor =
        lerp(m_current.m_shoulder.m_factor, factors.y, s_factor_lerp_speed * time_delta);
    VERIFY(xr::valid(m_current.m_shoulder.m_factor));

    VERIFY(xr::valid(m_current.m_spine.m_factor));
    m_current.m_spine.m_factor =
        lerp(m_current.m_spine.m_factor, factors.z, s_factor_lerp_speed * time_delta);
    VERIFY(xr::valid(m_current.m_spine.m_factor));

    Fvector const angles =
        Fvector().set(angle_normalize_signed(-(head.current.pitch - body.current.pitch)),
...
void CSightManager::compute_aiming(float const time_delta, float const angular_speed) {
    ...
    case aiming_weapon: {
        ...
        VERIFY(xr::valid(aiming_position()));
        aimers::weapon aimer(&object(), m_animation_id.c_str(),
    ...
    case aiming_head: {
        ...
        aimers::bone<3> aimer(&object(), m_animation_id.c_str(),
                              m_animation_frame == animation_frame_start, aiming_position(), bones);
        if (forward_blend_callbacks)
            object().animation().assign_bone_blend_callbacks(true);
        else {
            if (backward_blend_callbacks)
        ...
        if (!forward_blend_callbacks && !backward_blend_callbacks) {
            if (!fis_zero(time_delta)) {
                ...
                if (m_object->animation_movement())
                    slerp_rotations(time_delta, m_object->animation_movement()->IsBlending()
                                                    ? .1f
                                                    : angular_speed);
            }
...
static void slerp_rotations(float const time_delta, float const angular_speed, Fmatrix& current,
    ...
    float const test_angle = clampr(xr::abs(angle), EPS_L, PI);
...
void CSightManager::slerp_rotations(float const time_delta, float const angular_speed) {
    ...
    ::slerp_rotations(time_delta, angular_speed, m_current.m_shoulder.m_rotation,
...
void CSightManager::enable(bool const value) {
    ...
    m_enabled = value;
    //	Msg					("[%d][%s] sight_enabled[%c]", Device.dwTimeGlobal, object().cName().c_str(),
    //value ? '+' : '-');
