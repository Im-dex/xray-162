...
void CWeaponMounted::net_Export(NET_Packet& P) { inherited::net_Export(P); }

void CWeaponMounted::net_Import(NET_Packet& P) { inherited::net_Import(P); }
...
void CWeaponMounted::OnShot() {
    ...
    FireBullet(get_CurrentFirePoint(), fire_dir, fireDispersionBase, m_CurrentAmmo, Owner()->ID(),
               ID(), SendHitAllowed(Owner()));
...
void CWeaponMounted::AddShotEffector() {
    if (OwnerActor()) {
        CCameraShotEffector* S =
            smart_cast<CCameraShotEffector*>(OwnerActor()->Cameras().GetCamEffector(eCEShot));
        if (!S)
