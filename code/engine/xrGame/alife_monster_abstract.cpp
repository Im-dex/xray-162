...
void CSE_ALifeMonsterAbstract::add_offline(const std::vector<ALife::_OBJECT_ID>& saved_children,
...
void CSE_ALifeMonsterAbstract::update() {
    ...
    ai().alife().time_manager().game_time(); m_fDistanceFromPoint		+= float(tCurTime -
    m_tTimeID)/1000.f/ai().alife().time_manager().normal_time_factor()*m_fCurSpeed; if
    (m_fDistanceToPoint - m_fDistanceFromPoint < EPS_L) { bContinue = true; if
    ((m_fDistanceFromPoint - m_fDistanceToPoint > EPS_L) && (m_fCurSpeed > EPS_L)) m_tTimeID
    = tCurTime - ALife::_TIME_ID(iFloor((m_fDistanceFromPoint -
    ...
    smart_cast<CSE_ALifeGroupAbstract*>(this); if (tpALifeGroupAbstract)
                    ...
                    if (move_offline() && (m_tNextGraphID == m_tGraphID)) {
                            ...
                            int							iPointCount =
    (int)tpaTerrain.size(); int							iBranches = 0;
                            ...
                            if (!iBranches) {
                                    for ( ; i != e; ++i) {
                                            for (int j=0; j<iPointCount; ++j)
                                                    if
    (ai().game_graph().mask(tpaTerrain[j].tMask,ai().game_graph().vertex((*i).vertex_id())->vertex_type()))
    { m_tNextGraphID = (*i).vertex_id(); m_fDistanceToPoint = (*i).distance(); bOk = true; break;
                                                    }
                                            if (bOk)
                                                    break;
                                    }
                                    ...
                                    for ( ; i != e; ++i)
                                            if ((*i).vertex_id() != m_tPrevGraphID) {
                                                    for (int j=0; j<iPointCount; ++j)
                                                            if
    (ai().game_graph().mask(tpaTerrain[j].tMask,ai().game_graph().vertex((*i).vertex_id())->vertex_type())
    && ((*i).vertex_id() != m_tPrevGraphID)) { if (iBranches == iChosenBranch) { m_tNextGraphID
...
CSE_ALifeItemWeapon* CSE_ALifeMonsterAbstract::tpfGetBestWeapon(ALife::EHitType& tHitType,
                                                                float& fHitPower) {
...
CSE_ALifeMonsterAbstract::tfGetActionType(CSE_ALifeSchedulable* tpALifeSchedulable, int iGroupIndex,
    ...
    (smart_zone);
                    return					(smart_zone->tfGetActionType(this,iGroupIndex ? 0 :
    1,bMutualDetection));
            }
            else
...
void CSE_ALifeMonsterAbstract::vfCheckForPopulationChanges() {
    ...
    if (l_tTimeID >= l_tpALifeGroupAbstract->m_tNextBirthTime) {
        ...
        l_tpALifeGroupAbstract->m_tNextBirthTime =
            l_tTimeID +
            ALife::_TIME_ID(ai().ef_storage().m_pfBirthSpeed->ffGetValue() * 24 * 60 * 60 * 1000);
        if (randF(100) < ai().ef_storage().m_pfBirthProbability->ffGetValue()) {
            u32 l_dwBornCount =
                iFloor(float(l_tpALifeGroupAbstract->m_wCount) * randF(.5f, 1.5f) *
            ...
            if (l_dwBornCount) {
                l_tpALifeGroupAbstract->m_tpMembers.resize(l_tpALifeGroupAbstract->m_wCount +
                                                           l_dwBornCount);
                ALife::OBJECT_IT I =
                    l_tpALifeGroupAbstract->m_tpMembers.begin() + l_tpALifeGroupAbstract->m_wCount;
...
bool CSE_ALifeMonsterAbstract::redundant() const {
    ...
    VERIFY2(
        ...
        make_string("incorrect death time for monster %s[death time = %I64d][current time = %I64d]",
