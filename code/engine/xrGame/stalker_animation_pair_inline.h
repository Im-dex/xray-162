...
IC CStalkerAnimationPair::CStalkerAnimationPair(CAI_Stalker* object)
    : m_object(object), m_step_dependence(false), m_global_animation(false),
      m_callback_on_collision(false), m_target_matrix(0)
#ifdef DEBUG
      ,
...
IC const CStalkerAnimationPair::CALLBACK_ID*
CStalkerAnimationPair::callback(const CALLBACK_ID& callback) const {
