...
bool CAgentCorpseManager::process_corpse(CMemberOrder& member) {
    ...
    for (; I != E; ++I) {
        ...
        if (dist_sqr < min_dist_sqr) {
            if ((*I).reactor() && ((*I).reactor()->Position().distance_to_sqr(
...
void CAgentCorpseManager::react_on_member_death() {
...

    {
        ...
        m_corpses.erase(
            std::remove_if(m_corpses.begin(), m_corpses.end(), CRemoveMemberCorpsesPredicate()),
            m_corpses.end());
    }
}
...
void CAgentCorpseManager::remove_links(CObject* object) {
    m_corpses.erase(
        std::remove_if(m_corpses.begin(), m_corpses.end(), CRemoveOfflineCorpsesPredicate(object)),
        m_corpses.end());
}
