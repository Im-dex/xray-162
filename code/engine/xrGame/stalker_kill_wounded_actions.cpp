...
void CStalkerActionReachWounded::initialize() {
    ...
    object().set_goal(eObjectActionIdle, weapon_to_kill(&object()), MIN_QUEUE, MAX_QUEUE,
...
void CStalkerActionReachWounded::execute() {
    ...
    //	CObject									*processor =
    //Level().Objects.net_Find(processor_id); 	if (processor &&
...
void CStalkerActionAimWounded::initialize() {
    ...
    object().movement().set_body_state(eBodyStateStand);
    object().movement().set_movement_type(eMovementTypeStand);
    object().CObjectHandler::set_goal(eObjectActionAimReady1, weapon_to_kill(&object()), MIN_QUEUE,
    ...
    //	m_speed									=
    //object().movement().m_head.speed; 	object().movement().danger_head_speed	(PI_DIV_4);
}
...
void CStalkerActionKillWounded::execute() {
    ...
    const CEntityAlive* enemy = object().memory().enemy().selected();
    object().sight().setup(CSightAction(enemy, true));
    object().set_goal(eObjectActionFire1, weapon_to_kill(&object()), MIN_QUEUE, MAX_QUEUE,
                      MIN_INTERVAL, MAX_INTERVAL);

    if (!object().inventory().ActiveItem())
        return;

    if (object().memory().visual().visible_now(enemy) && object().can_kill_enemy() &&
        !object().can_kill_member())
        return;
    ...
    HS.power = 1.f;
    HS.boneID =
        smart_cast<IKinematics*>((const_cast<CEntityAlive*>(enemy))->Visual())->LL_GetBoneRoot();
