...
static float trajectory_pick_error(float low, float high, const Fvector& position,
                                   const Fvector& velocity, const Fvector& gravity) {
...
static bool trajectory_check_collision(float low, float high, Fvector const& position,
    ...
    if (box_size.magnitude() <= epsilon)
        Level().ObjectSpace.RayQuery(temp_rq_results, ray_defs, trajectory_query_callback, &range,
                                     NULL, self_object);
    else {
        Fvector box_center;
        ...
        box_result = !Level().ObjectSpace.BoxQuery(box_center, box_z_axis, box_y_axis, box_size,
                                                   out_collide_tris);
    }
...
bool trajectory_intersects_geometry(float trajectory_time, Fvector const& trajectory_start,
    ...
    for (;;) {
        float time = trajectory_select_pick_time(low, high, trajectory_start, trajectory_start,
                                                 gravity, epsilon);

        if (!trajectory_check_collision(low, time, trajectory_start, trajectory_velocity, gravity,
                                        self_object, ignored_object, collide_position,
                                        temp_rq_results, box_size, out_trajectory_picks,
                                        out_collide_tris)) {
