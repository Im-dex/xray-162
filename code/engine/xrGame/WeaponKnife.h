...
private:
    ...
    bool SelectBestHitVictim(Fvector const& f_pos, Fmatrix& parent_xform_dest,
    ...
    public:
        best_victim_selector(u16 except_id, Fvector const& pos, float query_distance,
    ...
    void fill_shapes_list(CEntityAlive const* entity, Fvector const& camera_endpos,
                          victims_shapes_list_t& dest_shapes);
    void make_hit_sort_vectors(Fvector& basis_hit_specific, float& max_dist);
    void fill_shots_list(victims_shapes_list_t& victims_shapres, Fsphere const& query,
