...
TEMPLATE_SPECIALIZATION
IC void CGenericPathManager::setup(const _Graph* _graph, _DataStorage* _data_storage,
...
IC bool CGenericPathManager::is_limit_reached(const _iteration_type iteration_count) const {
    VERIFY(data_storage);
    return ((data_storage->get_best().f() >= max_range) ||
...
TEMPLATE_SPECIALIZATION
IC void CGenericPathManager::begin(const _index_type& vertex_id, const_iterator& begin,
                                   const_iterator& end) {
