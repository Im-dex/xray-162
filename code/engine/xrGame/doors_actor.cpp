...
public:
    inline passed_doors_predicate(stalker_movement_manager_smart_cover const& movement,
                                  actor& actor, door_state const state, float const danger_distance)
        : m_movement(&movement), m_actor(&actor), m_state(state),
...
void actor::process_doors(float const average_speed, doors_type& processed_doors,
                          temp_doors_type const& new_doors, door_state const start_state,
    ...
    std::inplace_merge(processed_doors.begin(), processed_doors.begin() + processed_count,
                       processed_doors.end());
}
...
bool actor::add_new_door(float const average_speed, door* const door,
                         doors_type const& processed_doors, doors_type& locked_doors,
                         temp_doors_type& new_doors, door_state const state) {
...
#ifdef DEBUG
            if (g_debug_doors)
                Msg("actor[%s] is waiting for the door[%s] blocked by %s", get_name(),
                    door->get_name(), door->get_initiators_ids().c_str());
#endif // #ifdef DEBUG
            return false;
        }

        VERIFY(std::find(processed_doors.begin(), processed_doors.end(), door) ==
               processed_doors.end());
...
//	}
        ...
        if (distance_to_open >= 0.f) {
            if (distance_to_closed >= 0.f)
                if (distance_to_open < distance_to_closed) {
                    ...
                    if (!add_new_door(average_speed, *i, m_closed_doors, m_open_doors,
                                      new_doors_to_close, door_state_closed))
                        return false;
                }
            else {
                if (!add_new_door(average_speed, *i, m_closed_doors, m_open_doors,
    ...
    process_doors(average_speed, m_open_doors, new_doors_to_open, door_state_open,
                  door_state_closed);
    process_doors(average_speed, m_closed_doors, new_doors_to_close, door_state_closed,
                  door_state_open);
