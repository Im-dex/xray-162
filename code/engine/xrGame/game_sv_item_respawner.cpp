...
bool item_respawn_manager::search_by_id_predicate::operator()(spawn_item const& left,
                                                              u16 right) const {
...
CSE_Abstract* item_respawn_manager::make_respawn_entity(shared_str const& section_name, u8 addons,
                                                        u16 count_of_ammo) {
    R_ASSERT(m_server);
    R_ASSERT(m_server->game);
    CSE_Abstract* temp_entity = F_entity_Create(section_name.c_str());
    R_ASSERT2(temp_entity,
              make_string("failed to create entity [%s]", section_name.c_str()).c_str());
...
u32 item_respawn_manager::load_section_items(CInifile& ini, const char* section_name,
                                             section_items* items) {
...
void item_respawn_manager::clear_respawn_sections() {
    ...
    for (respawn_section_iter temp_iter = m_respawn_sections_cache.begin(); temp_iter != temp_ie;
         ++temp_iter) {
...
void item_respawn_manager::add_new_rpoint(shared_str profile_sect, RPoint const& point) {
    ...
    R_ASSERT2(tmp_resp_sect->second,
              make_string("collection of respawn items section (%s) is NULL", profile_sect.c_str())
                  .c_str());

    section_items_iter iter_ie = tmp_resp_sect->second->end();
    for (section_items_iter iter_rsect = tmp_resp_sect->second->begin(); iter_rsect != iter_ie;
         ++iter_rsect) {
        spawn_item new_item(iter_rsect->respawn_time);
        new_item.item_object = make_respawn_entity(iter_rsect->section_name, iter_rsect->addons,
...
#ifndef MASTER_GOLD
            Msg("! ERROR: failed to create entity [%s] with addons [%d]", iter_rsect->section_name,
                iter_rsect->addons);
#endif // #ifndef MASTER_GOLD
        }
    }
...
u16 item_respawn_manager::respawn_item(CSE_Abstract* item_object) {
    ...
    CSE_Abstract* spawned_item =
        m_server->Process_spawn(spawn_packet_store, m_server->GetServerClient()->ID);
    if (!spawned_item)
        return 0;
