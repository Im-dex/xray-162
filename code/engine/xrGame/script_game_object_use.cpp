...
void CScriptGameObject::Kill(CScriptGameObject* who) {
    ...
    if (!l_tpEntity) {
        ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,
                                        "%s cannot access class member Kill!", *object().cName());
        return;
    }
    if (!l_tpEntity->AlreadyDie())
        ...
        ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,
                                        "attempt to kill dead object %s", *object().cName());
}
...
void CScriptGameObject::set_fastcall(const luabind::functor<bool>& functor,
    ...
    CPHScriptGameObjectCondition* c =
        xr_new<CPHScriptGameObjectCondition>(object, functor, m_game_object);
...
void CScriptGameObject::set_const_force(const Fvector& dir, float value, u32 time_interval) {
    ...
    //#ifdef DEBUG
    //	Msg( "const force added: force: %f,  time: %d ,dir(%f,%f,%f)", value, time_interval, dir.x,
    //dir.y, dir.z ); #endif
    Fvector force;
    force.set(dir);
    force.mul(value);
