...
//					обязательно имеет конструктор с параметрами (T_INDEX index, T_ID id,
//LPCSTR r1, ..., LPCSTR rN)
...
#define TEMPLATE_SPECIALIZATION                                                      \
    template <u32 ITEM_REC_NUM, typename ITEM_DATA, typename T_ID, typename T_INDEX, \
              typename T_INIT>
...
protected:
    ...
    static void LoadItemData(u32, LPCSTR) {
        static_assert(std::is_same_v<decltype(NUM), bool>,
                      "Specialization for LoadItemData in CIni IdToIndex not found");
        NODEFAULT;
    }
...
public:
    ...
    static const T_INDEX IdToIndex(const T_ID& str_id, T_INDEX default_index = T_INDEX(-1),
...
const typename ITEM_DATA* CSINI_IdToIndex::GetByIndex(T_INDEX index, bool no_assert) {
    if ((size_t)index >= m_pItemDataVector->size()) {
        if (!no_assert)
            Debug.fatal(DEBUG_INFO, "item by index not found in section %s, line %s", section_name,
                        line_name);
        return NULL;
    }
