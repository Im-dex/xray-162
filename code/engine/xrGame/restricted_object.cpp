...
IC void construct_string(LPSTR result, u32 const result_size,
                         const std::vector<ALife::_OBJECT_ID>& restrictions) {
...
#if 0
IC	void construct_id_string					(LPSTR result, const std::vector<ALife::_OBJECT_ID> &restrictions)
{
	xr_strcpy			(result,"");
	string16		temp;
	u32				count = 0;
...
u32 CRestrictedObject::accessible_nearest(const Fvector& position, Fvector& result) const {
    ...
    VERIFY2(!accessible(position),
            make_string("[%s] [%f][%f][%f]", object().cName().c_str(), VPUSH(position)));

    return (
        Level().space_restriction_manager().accessible_nearest(object().ID(), position, result));
    STOP_PROFILE;
}
...
void CRestrictedObject::add_border(const Fvector& start_position,
    ...
    if (accessible(start_position)) {
        m_applied = true;
        Level().space_restriction_manager().add_border(object().ID(), start_position,
                                                       dest_position);
    }
...
void CRestrictedObject::add_border(u32 start_vertex_id, u32 dest_vertex_id) const {
    ...
    if (accessible(start_vertex_id)) {
        m_applied = true;
        Level().space_restriction_manager().add_border(object().ID(), start_vertex_id,
                                                       dest_vertex_id);
    }
    STOP_PROFILE;
}
...
IC void CRestrictedObject::construct_restriction_string(
    LPSTR temp_restrictions, u32 const temp_restrictions_size,
...
void CRestrictedObject::add_restrictions(const std::vector<ALife::_OBJECT_ID>& out_restrictions,
                                         const std::vector<ALife::_OBJECT_ID>& in_restrictions) {
    ...
    construct_restriction_string<CRestrictionPredicate<true>, true>(
        temp_out_restrictions, sizeof(temp_out_restrictions), out_restrictions,
    ...
    construct_restriction_string<CRestrictionPredicate<true>, true>(
        ...
        this->in_restrictions(), CRestrictionPredicate<true>(RestrictionSpace::eRestrictorTypeIn));

    Level().space_restriction_manager().add_restrictions(object().ID(), temp_out_restrictions,
...
void CRestrictedObject::remove_restrictions(const std::vector<ALife::_OBJECT_ID>& out_restrictions,
                                            const std::vector<ALife::_OBJECT_ID>& in_restrictions) {
    if (out_restrictions.empty() && in_restrictions.empty())
        return;
    ...
    construct_restriction_string<CRestrictionPredicate<false>, false>(
        temp_in_restrictions, sizeof(temp_in_restrictions), in_restrictions,
        this->in_restrictions(), CRestrictionPredicate<false>(RestrictionSpace::eRestrictorTypeIn));
...
void CRestrictedObject::add_restrictions(const shared_str& out_restrictions,
    ...
    Level().space_restriction_manager().add_restrictions(object().ID(), *out_restrictions,
                                                         *in_restrictions);
...
void CRestrictedObject::remove_restrictions(const shared_str& out_restrictions,
                                            const shared_str& in_restrictions) {
    if (!out_restrictions.size() && !in_restrictions.size())
        return;
    ...
    Level().space_restriction_manager().remove_restrictions(object().ID(), *out_restrictions,
                                                            *in_restrictions);
