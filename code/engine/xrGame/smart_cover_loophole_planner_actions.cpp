...
void loophole_action::initialize() {
    ...
    ActionAnimations const& animations =
        object().movement().current_params().cover_loophole()->action_animations(m_action_id,
                                                                                 animation_id);
...
transition::transition(CAI_Stalker* object, LPCSTR action_name, LPCSTR action_from,
    ...
    : inherited(object, action_name), m_action_from(action_from), m_action_to(action_to),
      m_state_from(state_from), m_state_to(state_to), m_planner(planner) {}
...
void transition::initialize() {
    ...
    TransitionData const& animations =
        object().movement().current_params().cover_loophole()->transition_animations(m_action_from,
                                                                                     m_action_to);
    m_animation = animations[m_random.randI(animations.size())];
}
...
void loophole_lookout::initialize() {
    ...
    object().sight().bone_aiming(m_animation, CSightManager::animation_frame_start,
...
void loophole_fire::on_mark() {
    ...
    //);
    object().set_goal(eObjectActionFireNoReload, object().best_weapon(), magazine_size,
                      magazine_size);
}
...
idle_2_fire_transition::idle_2_fire_transition(CAI_Stalker* object, LPCSTR action_name,
                                               LPCSTR action_from, LPCSTR action_to,
...
void idle_2_fire_transition::initialize() {
    ...
    object().sight().bone_aiming(m_animation, CSightManager::animation_frame_end,
                                 CSightManager::aiming_weapon);
    setup_sight(true);
...
fire_2_idle_transition::fire_2_idle_transition(CAI_Stalker* object, LPCSTR action_name,
                                               LPCSTR action_from, LPCSTR action_to,
                                               StalkerDecisionSpace::EWorldProperties state_from,
                                               StalkerDecisionSpace::EWorldProperties state_to,
...
idle_2_lookout_transition::idle_2_lookout_transition(
    CAI_Stalker* object, LPCSTR action_name, LPCSTR action_from, LPCSTR action_to,
    StalkerDecisionSpace::EWorldProperties state_from,
    StalkerDecisionSpace::EWorldProperties state_to, animation_planner* planner)
    : inherited(object, action_name, action_from, action_to, state_from, state_to, planner) {}
