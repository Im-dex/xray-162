...
static bool SimmulateNetworkLag() {
    ...
    if (!max_lag_time || (max_lag_time <= Device.dwTimeGlobal)) {
        ...
        max_lag_time = Device.dwTimeGlobal +
                       tmp_random.randI(lag_simmulator_min_ping, lag_simmulator_max_ping);
        return false;
    }
    return true;
}
...
#ifdef DEBUG
//					Msg("! TimeServer[%d] < TimeReceive[%d]", Level().timeServer(),
//P->timeReceive);
#endif
                dTime = Ping;
            } else
