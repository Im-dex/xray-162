...
cover::cover(smart_cover::object const& object, DescriptionPtr description,
    ...
    : inherited(object.Position(), object.ai_location().level_vertex_id()), m_object(object),
      m_description(description), m_id(m_object.cName()), m_is_combat_cover(is_combat_cover),
      m_can_fire(can_fire) {
    m_is_smart_cover = 1;
...
u32 const& cover::action_level_vertex_id(smart_cover::loophole const& loophole,
    ...
    Vertices::const_iterator found =
        std::find_if(m_vertices.begin(), m_vertices.end(), id_predicate(loophole.id()));
    VERIFY(found != m_vertices.end());

    loophole_data::ActionVertices::const_iterator found2 =
        std::find_if(found->second.m_action_vertices.begin(), found->second.m_action_vertices.end(),
...
smart_cover::loophole* cover::best_loophole(Fvector const& position, float& value,
...
void cover::evaluate_loophole(Fvector const& position, smart_cover::loophole*& source,
                              smart_cover::loophole*& result, float& value,
                              bool const is_smart_cover_entered) const {
    VERIFY(source);
    VERIFY2(xr::valid(position), make_string("[%f][%f][%f]", VPUSH(position)));

    if (!source->usable())
        return;

    Fvector fov_position = this->fov_position(*source);
    VERIFY2(xr::valid(fov_position), make_string("[%f][%f][%f]", VPUSH(fov_position)));
    float const distance_to_target = fov_position.distance_to(position);
    if (distance_to_target > source->range())
        return;

    float const min_enemy_distance = is_smart_cover_entered ? object().enter_min_enemy_distance()
    ...
    VERIFY2(xr::valid(direction), make_string("[%f][%f][%f]", VPUSH(direction)));
...
u32 const& cover::level_vertex_id(smart_cover::loophole const& loophole) const {
    Vertices::const_iterator I =
        std::find_if(m_vertices.begin(), m_vertices.end(), loophole_predicate(&loophole));
...
bool cover::loophole_path(shared_str const& source_raw, shared_str const& target_raw) const {
    ...
    bool result =
        ai().graph_engine().search(m_description->transitions(), source, target, 0, parameters);

    VERIFY2(result, make_string("failde to find loophole path [%s]->[%s] in cover [%s]",
    ...
    for (; I != E; ++I) {
        ...
        for (; J != E; ++J) {
            ...
            VERIFY2(loophole_path(lhs, rhs),
                    make_string("failed to find path [%s -> %s] in smart_cover [%s]", lhs.c_str(),
                                rhs.c_str(), m_description->table_id().c_str()));
            VERIFY2(loophole_path(rhs, lhs),
                    make_string("failed to find path [%s -> %s] in smart_cover [%s]", rhs.c_str(),
                                lhs.c_str(), m_description->table_id().c_str()));
        }
        ...
        VERIFY2(loophole_path(enter, lhs),
                make_string("failed to find path [%s -> %s] in smart_cover [%s]", enter.c_str(),
                            lhs.c_str(), m_description->table_id().c_str()));
    }
}
#endif // DEBUG

static bool in_fov(Fvector const& position, Fvector const& fov_position,
                   Fvector const& fov_direction, float const fov_angle) {
    Fvector direction = Fvector().sub(position, fov_position);
    if (direction.magnitude() < 1.f)
        return (false);
...
bool cover::is_position_in_danger_fov(smart_cover::loophole const& source,
    ...
    return in_fov(position, fov_position(source), danger_fov_direction(source),
                  source.danger_fov());
}
