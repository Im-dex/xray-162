...
void CTrade::TransferItem(CInventoryItem* pItem, bool bBuying) {
    ...
    if ((pPartner.type == TT_ACTOR) || (pThis.type == TT_ACTOR)) {
        ...
        Actor()->callback(GameObject::eTradeSellBuyItem)(pItem->object().lua_game_object(), bDir,
                                                         dwTransferMoney);
    }
}
...
u32 CTrade::GetItemPrice(PIItem pItem, bool b_buying) {
    ...
    if (buying) {
        if (!pThis.inv_owner->trade_parameters().enabled(CTradeParameters::action_buy(0),
        ...
        if (!pThis.inv_owner->trade_parameters().enabled(CTradeParameters::action_sell(0),
                                                         pItem->object().cNameSect()))
            return 0;
        p_trade_factors = &pThis.inv_owner->trade_parameters().factors(
