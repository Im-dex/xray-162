...
void CSoundMemoryManager::reload(LPCSTR section) {
    ...
    LPCSTR sound_perceive_section =
        READ_IF_EXISTS(pSettings, r_string, section, "sound_perceive_section", section);
...
IC void CSoundMemoryManager::update_sound_threshold() {
    ...
    m_sound_threshold = std::max(m_self_sound_factor * m_sound_threshold *
...
IC u32 CSoundMemoryManager::priority(const MemorySpace::CSoundObject& sound) const {
    u32 priority = u32(-1);
    std::map<ESoundTypes, u32>::const_iterator I = m_priorities.begin();
    std::map<ESoundTypes, u32>::const_iterator E = m_priorities.end();
    for (; I != E; ++I)
...
void CSoundMemoryManager::enable(const CObject* object, bool enable) {
    std::vector<CSoundObject>::iterator J =
...
void CSoundMemoryManager::feel_sound_new(CObject* object, int sound_type,
                                         CSound_UserDataPtr user_data, const Fvector& position,
                                         float sound_power) {
#ifndef MASTER_GOLD
    ...
    Msg("%s (%d) - sound type %x from %s at %d in (%.2f,%.2f,%.2f) with power %.2f", *self->cName(),
        Device.dwTimeGlobal, sound_type, object ? *object->cName() : "world", Device.dwTimeGlobal,
...
void CSoundMemoryManager::add(const CSoundObject& sound_object, bool check_for_existance) {
    if (check_for_existance) {
        if (m_sounds->end() !=
            std::find(m_sounds->begin(), m_sounds->end(), object_id(sound_object.m_object)))
            return;
    }

    VERIFY(m_max_sound_count);
    if (m_max_sound_count <= m_sounds->size()) {
        std::vector<CSoundObject>::iterator I = std::min_element(m_sounds->begin(), m_sounds->end(),
...
void CSoundMemoryManager::add(const CObject* object, int sound_type, const Fvector& position,
                              float sound_power) {
    ...
    if (object && object->H_Parent() &&
        (me->tfGetRelationType(smart_cast<const CEntityAlive*>(object->H_Parent())) ==
...
void CSoundMemoryManager::update() {
    ...
    VERIFY(m_sounds);
    m_sounds->erase(std::remove_if(m_sounds->begin(), m_sounds->end(), CRemoveOfflinePredicate()),
                    m_sounds->end());
...
void CSoundMemoryManager::remove_links(CObject* object) {
    VERIFY(m_sounds);
    SOUNDS::iterator I =
        std::find_if(m_sounds->begin(), m_sounds->end(), CSoundObjectPredicate(object));
    if (I != m_sounds->end())
        m_sounds->erase(I);
