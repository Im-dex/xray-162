...
bool CPhraseDialog::SayPhrase(DIALOG_SHARED_PTR& phrase_dialog, const shared_str& phrase_id) {
    ...
    CPhraseGraph::CVertex* phrase_vertex =
        phrase_dialog->data()->m_PhraseGraph.vertex(phrase_dialog->m_SaidPhraseID);
    THROW(phrase_vertex);
    ...
    last_phrase->GetScriptHelper()->Action(pSpeakerGO1, pSpeakerGO2, *phrase_dialog->m_DialogId,
                                           phrase_id.c_str());
    ...
    if (phrase_vertex->edges().empty()) {
        ...
        for (xr_vector<CPhraseGraph::CEdge>::const_iterator it = phrase_vertex->edges().begin();
            ...
            if (next_phrase_vertex->data()->GetScriptHelper()->Precondition(
                ...
                if (psAI_Flags.test(aiDialogs)) {
                    ...
                    Msg("----added phrase text [%s] phrase_id=[%s] id=[%s] to dialog [%s]",
        ...
        std::sort(phrase_dialog->m_PhraseVector.begin(), phrase_dialog->m_PhraseVector.end(),
                  PhraseGoodwillPred);
    }
...
LPCSTR CPhraseDialog::GetPhraseText(const shared_str& phrase_id, bool current_speaking) {
    ...
    CGameObject* pSpeakerGO2 =
        (current_speaking) ? smart_cast<CGameObject*>(SecondSpeaker()) : NULL;
    ...
    if (ph->m_script_text_id.length() > 0) {
        ...
        bool functor_exists =
            ai().script_engine().functor(ph->m_script_text_id.c_str(), lua_function);
        THROW3(functor_exists, "Cannot find function", ph->m_script_text_id.c_str());

        ph->m_script_text_val = lua_function((pSpeakerGO) ? pSpeakerGO->lua_game_object() : NULL,
                                             m_DialogId.c_str(), phrase_id.c_str());
        return ph->m_script_text_val.c_str();
    } else
        return ph->GetScriptHelper()->GetScriptText(ph->GetText(), pSpeakerGO1, pSpeakerGO2,
                                                    m_DialogId.c_str(), phrase_id.c_str());
}
...
CPhrase* CPhraseDialog::AddPhrase(LPCSTR text, const shared_str& phrase_id,
                                  const shared_str& prev_phrase_id, int goodwil_level) {
    CPhrase* phrase = NULL;
