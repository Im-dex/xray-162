...
CSE_ALifeItemWeapon* CSE_ALifeSmartZone::tpfGetBestWeapon(ALife::EHitType& tHitType,
                                                          float& fHitPower) {
    m_tpCurrentBestWeapon = 0;
    return (m_tpCurrentBestWeapon);
}

ALife::EMeetActionType CSE_ALifeSmartZone::tfGetActionType(CSE_ALifeSchedulable* tpALifeSchedulable,
                                                           int iGroupIndex, bool bMutualDetection) {
    CSE_ALifeObject* object = smart_cast<CSE_ALifeObject*>(tpALifeSchedulable->base());
    VERIFY(object);
    return ((object->m_tGraphID == m_tGraphID) ? ALife::eMeetActionSmartTerrain
