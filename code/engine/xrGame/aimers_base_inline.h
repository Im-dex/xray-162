...
inline void aimers::base::fill_bones(u32 const (&bones)[bone_count0],
    ...
    if (m_animation_start) {
        for (u16 i = 0; i < MAX_PARTS; ++i) {
            ...
            for (u32 j = 0; j < blend_count; ++j) {
                ...
                CBlend* const new_blend =
                    m_animated.LL_PlayCycle(i, blend->motionID, TRUE, 0, 0, channel_id);
                VERIFY(new_blend);
                *new_blend = *blend;
        ...
        for (u16 i = 0; i < MAX_PARTS; ++i) {
            ...
            if (blend)
                blend->timeCurrent =
                    m_animation_start ? 0.f : (blend->timeTotal - (SAMPLE_SPF + EPS));
        }
    }
