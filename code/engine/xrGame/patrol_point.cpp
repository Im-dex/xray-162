...
void CPatrolPoint::verify_vertex_id(const CLevelGraph* level_graph,
    ...
    VERIFY(m_path);
    string1024 temp;
    xr_sprintf(temp, "\n! Patrol point %s in path %s is not on the level graph vertex!", *m_name,
               *m_path->m_name);
...
IC void CPatrolPoint::correct_position(const CLevelGraph* level_graph,
    ...
    m_game_vertex_id =
        cross->vertex(level_vertex_id(level_graph, cross, game_graph)).game_vertex_id();
}

CPatrolPoint::CPatrolPoint(const CLevelGraph* level_graph, const CGameLevelCrossTable* cross,
                           const CGameGraph* game_graph, const CPatrolPath* path,
                           const Fvector& position, u32 level_vertex_id, u32 flags,
                           shared_str name) {
#ifdef DEBUG
    VERIFY(path);
    m_path = path;
...
const GameGraph::_GRAPH_ID& CPatrolPoint::game_vertex_id() const {
    ...
    VERIFY2(vertex, make_string("invalid game vertex id[%d] (level_vertex_id[%d]) for patrol "
                                "point[%s] in path[%s] in position[%f][%f][%f]",
