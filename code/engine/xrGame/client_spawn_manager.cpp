...
void CClientSpawnManager::merge_spawn_callbacks(CSpawnCallback& new_callback,
...
CClientSpawnManager::callback(ALife::_OBJECT_ID requesting_id,
...
void CClientSpawnManager::dump() const {
    ...
    for (; I != E; ++I) {
        Msg("[%d], i.e. object with id %d left with hanging callbacks on it", (*I).first,
            (*I).first);
        REQUESTED_REGISTRY::const_iterator i = (*I).second.begin();
        REQUESTED_REGISTRY::const_iterator e = (*I).second.end();
        for (; i != e; ++i)
            Msg("[%d][%d], i.e. object with id %d waits for object with id %d", (*I).first,
                (*i).first, (*i).first, (*I).first);
    }
}
