...
demoplay_control::demoplay_control() {
    ...
    m_on_artefactdelivering =
        fastdelegate::MakeDelegate(this, &demoplay_control::on_artefactdelivering_impl);
    m_on_artefactcapturing =
        fastdelegate::MakeDelegate(this, &demoplay_control::on_artefactcapturing_impl);
    m_on_artefactloosing =
        fastdelegate::MakeDelegate(this, &demoplay_control::on_artefactloosing_impl);
...
bool demoplay_control::rewind_until(EAction const action, shared_str const& param,
                                    user_callback_t ucb) {
    if (m_current_mode != not_active) {
        Msg("! ERROR: already active.");
        return false;
...
void __stdcall demoplay_control::on_artefactdelivering_impl(u32 message, u32 subtype,
                                                            NET_Packet& packet) {
    u16 msg_type;
    packet.r_begin(msg_type);
    R_ASSERT(msg_type == M_GAMEMESSAGE);
    ...
    FATAL("incorect message for current game type, multiplayer doesn't supported with this engine "
          "build");
...
void __stdcall demoplay_control::on_artefactcapturing_impl(u32 message, u32 subtype,
                                                           NET_Packet& packet) {
    u16 msg_type;
    packet.r_begin(msg_type);
    R_ASSERT(msg_type == M_GAMEMESSAGE);
...
void __stdcall demoplay_control::on_artefactloosing_impl(u32 message, u32 subtype,
                                                         NET_Packet& packet) {
    u16 msg_type;
    packet.r_begin(msg_type);
    R_ASSERT(msg_type == M_GAMEMESSAGE);
