...
void CStalkerCombatPlanner::on_best_cover_changed(const CCoverPoint* new_cover,
...
void CStalkerCombatPlanner::setup(CAI_Stalker* object, CPropertyStorage* storage) {
    ...
    this->object().brain().CStalkerPlanner::m_storage.set_property(eWorldPropertyCriticallyWounded,
                                                                   false);
...
void CStalkerCombatPlanner::update() {
    ...
    //(enemy); 	const CAI_Stalker				*stalker = smart_cast<const
    //CAI_Stalker*>(enemy); 	m_last_wounded					= stalker &&
    //stalker->wounded();
}
...
//	inherited_planner::m_use_log = true;
    ...
    if (!m_loaded) {
        ...
        object().brain().CStalkerPlanner::m_storage.set_property(eWorldPropertyCriticallyWounded,
                                                                 false);
    }
...
void CStalkerCombatPlanner::finalize() {
    ...
    if (object().inventory().ItemFromSlot(INV_SLOT_2)) {
        CWeaponMagazined* temp =
            smart_cast<CWeaponMagazined*>(object().inventory().ItemFromSlot(INV_SLOT_2));
...
void CStalkerCombatPlanner::add_evaluators() {
    add_evaluator(eWorldPropertyPureEnemy,
                  new CStalkerPropertyEvaluatorEnemies(m_object, "is_there_enemies", 0));
    add_evaluator(eWorldPropertyEnemy, new CStalkerPropertyEvaluatorEnemies(
                                           m_object, "is_there_enemies_delayed",
                                           POST_COMBAT_WAIT_INTERVAL, &m_last_wounded));
    add_evaluator(eWorldPropertySeeEnemy,
                  new CStalkerPropertyEvaluatorSeeEnemy(m_object, "see enemy"));
    add_evaluator(eWorldPropertyEnemySeeMe,
                  new CStalkerPropertyEvaluatorEnemySeeMe(m_object, "enemy see me"));
    add_evaluator(eWorldPropertyItemToKill,
                  new CStalkerPropertyEvaluatorItemToKill(m_object, "item to kill"));
    add_evaluator(eWorldPropertyItemCanKill,
                  new CStalkerPropertyEvaluatorItemCanKill(m_object, "item can kill"));
    add_evaluator(eWorldPropertyFoundItemToKill,
                  new CStalkerPropertyEvaluatorFoundItemToKill(m_object, "found item to kill"));
    add_evaluator(eWorldPropertyFoundAmmo,
                  new CStalkerPropertyEvaluatorFoundAmmo(m_object, "found ammo"));
    add_evaluator(eWorldPropertyReadyToKill,
                  new CStalkerPropertyEvaluatorReadyToKill(m_object, "ready to kill"));
    add_evaluator(eWorldPropertyReadyToDetour,
                  new CStalkerPropertyEvaluatorReadyToDetour(m_object, "ready to detour"));
    add_evaluator(eWorldPropertyPanic, new CStalkerPropertyEvaluatorPanic(m_object, "panic"));
    add_evaluator(eWorldPropertyDangerGrenade, new CStalkerPropertyEvaluatorGrenadeToExplode(
                                                   m_object, "is there grenade to explode"));
    add_evaluator(eWorldPropertyEnemyWounded,
                  new CStalkerPropertyEvaluatorEnemyWounded(m_object, "is enemy wounded"));
    add_evaluator(eWorldPropertyPlayerOnThePath,
                  new CStalkerPropertyEvaluatorPlayerOnThePath(m_object, "player on the path"));
    add_evaluator(eWorldPropertyEnemyCriticallyWounded,
                  new CStalkerPropertyEvaluatorEnemyCriticallyWounded(
    ...
    add_evaluator(eWorldPropertyPositionHolded,
                  new CStalkerPropertyEvaluatorMember((CPropertyStorage*)0,
                                                          eWorldPropertyPositionHolded, true, true,
                                                          "position holded"));
    add_evaluator(eWorldPropertyEnemyDetoured,
                  new CStalkerPropertyEvaluatorMember((CPropertyStorage*)0,
                                                          eWorldPropertyEnemyDetoured, true, true,
                                                          "enemy detoured"));
    add_evaluator(eWorldPropertyUseSuddenness,
                  new CStalkerPropertyEvaluatorMember((CPropertyStorage*)0,
                                                          eWorldPropertyUseSuddenness, true, true,
                                                          "use suddenness"));
    add_evaluator(eWorldPropertyCriticallyWounded,
                  new CStalkerPropertyEvaluatorMember(
                      &object().brain().CStalkerPlanner::m_storage, eWorldPropertyCriticallyWounded,
                      true, true, "critically wounded"));
    add_evaluator(eWorldPropertyKilledWounded,
                  new CStalkerPropertyEvaluatorMember(
                      &object().brain().CStalkerPlanner::m_storage, eWorldPropertyKilledWounded,
                      true, true, "killed critically wounded"));
    ...
    add_evaluator(eWorldPropertyLowCover,
                  new CStalkerPropertyEvaluatorLowCover(m_object, "using low cover"));

    add_evaluator(eWorldPropertyInSmartCover,
                  new smart_cover::evaluators::in_cover_evaluator(m_object, "in smart cover"));
}
...
void CStalkerCombatPlanner::add_actions() {
...

    {
        CStalkerSearchPlanner* action =
            new CStalkerSearchPlanner(m_object, "search enemy planner");
        add_condition(action, eWorldPropertyCriticallyWounded, false);
        add_condition(action, eWorldPropertyDangerGrenade, false);
        add_condition(action, eWorldPropertyUseSuddenness, false);
        add_condition(action, eWorldPropertyReadyToKill, true);
    ...
    action = new CStalkerActionKillEnemyIfPlayerOnThePath(
...

    {
        CStalkerKillWoundedPlanner* planner =
            new CStalkerKillWoundedPlanner(m_object, "kill wounded enemy");
...

    {
        CStalkerGetDistancePlanner* planner =
            new CStalkerGetDistancePlanner(m_object, "get distance");
        add_condition(planner, eWorldPropertyCriticallyWounded, false);
        add_condition(planner, eWorldPropertyDangerGrenade, false);
...

    {
        stalker_low_cover_planner* planner =
            new stalker_low_cover_planner(m_object, "use low cover");
        add_condition(planner, eWorldPropertyCriticallyWounded, false);
        add_condition(planner, eWorldPropertyDangerGrenade, false);
        add_condition(planner, eWorldPropertyUseSuddenness, false);
