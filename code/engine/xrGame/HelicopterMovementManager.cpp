...
void SHeliMovementState::UpdatePatrolPath() {
    if (AlreadyOnPoint()) {
        ...
        if (b != e) {
            ...
            if (need_to_del_path &&
                currPatrolVertex->data()
                    .flags()) // fake flags that signals entrypoint for round path
...
void SHeliMovementState::goByRoundPath(Fvector center_, float radius_, bool clockwise_) {
    ...
    for (; it != it_e; ++it, ++pt_idx) {
        ...
        CPatrolPoint pt = CPatrolPoint((CLevelGraph*)0, (CGameLevelCrossTable*)0, (CGameGraph*)0,
                                       pp, (*it).point, u32(-1), 0, pt_name);
...
void SHeliMovementState::SetPointFlags(u32 idx, u32 new_flags) {
    ...
    CPatrolPoint* pt_new = xr_new<CPatrolPoint>((CLevelGraph*)0, (CGameLevelCrossTable*)0,
                                                (CGameGraph*)0, currPatrolPath, pt_curr->position(),
                                                u32(-1), new_flags, pt_curr->name());
