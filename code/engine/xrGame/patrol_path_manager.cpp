...
void CPatrolPathManager::select_point(const Fvector& position, u32& dest_vertex_id) {
    ...
    if (!actual() || !m_path->vertex(m_curr_point_index)) {
        ...
        case ePatrolStartTypeFirst: {
            ...
            VERIFY3(accessible(vertex) || show_restrictions(m_object), *m_path_name,
                    *m_game_object->cName());
            break;
        }
        ...
        if (!(vertex || show_restrictions(m_object))) {
            ...
            VERIFY2(vertex || show_restrictions(m_object),
                    make_string("any vertex in patrol path [%s] is inaccessible for object [%s]",
        ...
        R_ASSERT2(ai().level_graph().valid_vertex_id(vertex->data().level_vertex_id()),
                  make_string("patrol path[%s], point on path [%s],object [%s]", *m_path_name,
                              *vertex->data().name(), *m_game_object->cName()));
    ...
    VERIFY3(m_path->vertex(m_curr_point_index) || show_restrictions(m_object), *m_path_name,
            *m_game_object->cName());

    m_game_object->callback(GameObject::ePatrolPathInPoint)(m_game_object->lua_game_object(),
                                                            u32(ScriptEntity::eActionTypeMovement),
                                                            m_curr_point_index);
    ...
    VERIFY3(m_path->vertex(target) || show_restrictions(m_object), *m_path_name,
            *m_game_object->cName());
    ...
    VERIFY3(accessible(m_dest_position) || show_restrictions(m_object), *m_path_name,
            *m_game_object->cName());
    m_actuality = true;
    m_completed = false;
}
...
void CPatrolPathManager::set_previous_point(int point_index) {
    ...
    if (!m_path->vertex(point_index)) {
        ai().script_engine().script_log(eLuaMessageTypeError,
                                        "Start point violates path bounds %s (object %s)!",
                                        *m_path_name, *m_game_object->cName());
        return;
    }
    VERIFY(m_path);
