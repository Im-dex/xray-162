...
void print_class(lua_State* L, luabind::detail::class_rep* crep) {
    ...
    {
#ifndef USE_NATIVE_LUA_STRINGS
        typedef luabind::internal_map<const char*, luabind::detail::class_rep::callback,
            ...
            if (lua_getupvalue(L, -1, 2) != 0) {
                ...
                if (lua_touserdata(L, -1) == (void*)0x1337) {
                    if (lua_getupvalue(L, -2, 1) != 0) {
                        ...
                        ++count;
                        rep = static_cast<luabind::detail::free_functions::function_rep*>(
                            lua_touserdata(L, -1));
                        std::vector<luabind::detail::free_functions::overload_rep>::const_iterator
                            i = rep->overloads().begin();
                        std::vector<luabind::detail::free_functions::overload_rep>::const_iterator
                            e = rep->overloads().end();
                        for (; i != e; ++i) {
                            luabind::internal_string luaS;
