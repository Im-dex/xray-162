...
CStalkerActionSolveZonePuzzle::CStalkerActionSolveZonePuzzle(CAI_Stalker* object,
                                                             LPCSTR action_name)
...
//(CSightAction(SightManager::eSightTypeCurrentDirection));
    ...
    object().movement().set_detail_path_type(DetailPathManager::eDetailPathTypeSmooth);
    object().movement().patrol().set_path("test_sight", PatrolPathManager::ePatrolStartTypeNearest,
...
void CStalkerActionSolveZonePuzzle::execute() {
    ...
    //g_actor); 		if (object().throw_enabled()) {
    //			object().CObjectHandler::set_goal
