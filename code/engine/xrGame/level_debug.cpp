...
void CLevelDebug::draw_debug_text() {
    ...
    if (!smart_cast<CBaseMonster*>(Level().CurrentEntity())) {
        ...
        if (debug_actor_view && actor_view) {
            debug::draw_text_tree(*actor_view, 2, x_start, y_start, m_texttree_offs, column_size,
    ...
    if (m_p_texttree->find_node("General")) {
        debug::draw_text_tree(*m_p_texttree->find_node("General"), 2, x_start, y_start,
                              m_texttree_offs, column_size, 80, D3DCOLOR_XRGB(0, 255, 0),
                              D3DCOLOR_XRGB(255, 255, 0));
    }

    if (m_p_texttree->find_node("Brain")) {
        debug::draw_text_tree(*m_p_texttree->find_node("Brain"), 2, x_start * 2 + column_size,
                              y_start, m_texttree_offs, column_size, 80, D3DCOLOR_XRGB(0, 255, 0),
                              D3DCOLOR_XRGB(255, 255, 0));
    }

    if (m_p_texttree->find_node("Controllers")) {
        debug::draw_text_tree(*m_p_texttree->find_node("Controllers"), 2,
                              x_start * 3 + column_size * 2, y_start, m_texttree_offs, column_size,
                              80, D3DCOLOR_XRGB(0, 255, 0), D3DCOLOR_XRGB(255, 255, 0));
    }
}
...
void CLevelDebug::CLevelInfo::add_item(const Fvector& pos1, const Fvector& pos2, u32 color,
                                       u32 id) {
