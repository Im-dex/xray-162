...
door::door(CPhysicObject* object)
    : m_object(*object), m_state(door_state_open), m_previous_state(door_state_open),
      m_target_state(door_state_open), m_registered_position(object->Position()), m_locked(false) {
...
void door::change_state(actor* const initiator, door_state const start_state,
...
#ifdef DEBUG
        if (g_debug_doors)
            Msg("door[%s] added initiator[%s] to keep door %s", m_object.cName().c_str(),
                initiator->get_name(), m_target_state == door_state_open ? "open" : "closed");
#endif  // #ifdef DEBUG
    ...
    if (m_target_state == start_state) {
        ...
        //			int i=0; (void)i;
        //		}
        VERIFY(std::find(m_initiators.begin(), m_initiators.end(), initiator) ==
               m_initiators.end());
        m_initiators.push_back(initiator);
        return;
    }
...
#ifdef DEBUG
        if (g_debug_doors)
            Msg("door[%s] removed initiator[%s] to keep door %s", m_object.cName().c_str(),
                initiator->get_name(), m_target_state == door_state_open ? "open" : "closed");
#endif  // #ifdef DEBUG
        //		if ( !xr_strcmp( "sim_default_duty_28212", initiator->get_name()) ) {
        //			int i=0; (void)i;
