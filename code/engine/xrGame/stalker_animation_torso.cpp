...
MotionID CStalkerAnimationManager::aim_animation(const u32& slot,
    ...
    if (animation[6].A.size() < 7) {
        Msg("! cannot find special danger animations for object with visual %s",
...
MotionID CStalkerAnimationManager::no_object_animation(const EBodyState& body_state) const {
    ...
    if (eMentalStateFree == movement.mental_state()) {
        VERIFY3(eBodyStateStand == movement.body_state(),
                "Cannot run FREE animations, when body state is not stand!", *stalker.cName());
...
MotionID CStalkerAnimationManager::unknown_object_animation(u32 slot,
    ...
    const std::vector<CAniVector>& animation_stand =
    ...
    if (eMentalStateFree == movement.mental_state()) {
        VERIFY3(eBodyStateStand == movement.body_state(),
                "Cannot run FREE animation when body state is not stand!", *object().cName());
