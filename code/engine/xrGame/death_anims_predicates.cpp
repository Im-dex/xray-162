...
void death_anims::setup(IKinematicsAnimated* k, LPCSTR section, CInifile const* ini) {
    ...
    anims[1] =
        xr_new<type_motion1>()->setup(k, ini, section, "kill_burst"); // 2.	Изрешетить пулями
    anims[2] = xr_new<type_motion2>()->setup(k, ini, section, "kill_shortgun"); // 3.	Шотган

    anims[6] = xr_new<type_motion3>()->setup(
        k, ini, section, "kill_headshot"); // 4.	Хедшот (по вероятности), кроме 5 (4)
    anims[4] = xr_new<type_motion4>()->setup(k, ini, section,
                                             "kill_sniper_headshot"); // 5.	Снайперка в голову.
    anims[5] = xr_new<type_motion5>()->setup(k, ini, section,
