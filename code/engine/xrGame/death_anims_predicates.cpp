...
type_motion::edirection type_motion::dir(CEntityAlive& ea, const SHit& H, float& angle) {
    ...
    if (xr::abs(front_factor) > M_SQRT1_2) {
...
void type_motion_diagnostic(LPCSTR message, type_motion::edirection dr, const CEntityAlive& ea,
                            const SHit& H, const MotionID& m) {
#ifdef DEBUG
    ...
    Msg("death anims: %s, dir: %s, motion: %s,  obj: %s, model: %s, bone: %s ", message,
        motion_dirs[dr].name, motion_name, ea.cName().c_str(), ea.cNameVisual().c_str(), bone_name);
...
class type_motion0 : public type_motion {
    bool predicate(CEntityAlive& ea, const SHit& H, MotionID& m, float& angle) const {
        ...
        const Fvector dir_to_actor =
            Fvector().sub(H.initiator()->Position(), ea.Position()).normalize_safe();

        const float front_angle_cos = std::cos(deg2rad(20.f));
        ...
        if (type_motion::front != type_motion::dir(ea, H, angle))
            return false;
        Fvector p;
        if (Fvector().sub(H.initiator()->Position(), global_hit_position(p, ea, H)).magnitude() >
            30.f)
            return false;

        m = motion(front);
        type_motion_diagnostic(
            " type_motion0: 1. = Инерционное движение вперед от попадания в голову ", front, ea, H,
            m);
        return true;
    }
};
...
class type_motion3 : public type_motion {
    bool predicate(CEntityAlive& ea, const SHit& H, MotionID& m, float& angle) const {
        ...
        if (is_bone_head(*K, H.bone())) {
            edirection dr = dir(ea, H, angle);
            m = motion(dr);
            type_motion_diagnostic(" type_motion3: 4.	Хедшот (по вероятности), кроме 5 (4)", dr,
                                   ea, H, m);
            return true;
        }
        return false;
...
void death_anims::setup(IKinematicsAnimated* k, LPCSTR section, CInifile const* ini) {
    ...
    anims[0] = (new type_motion0())->setup(
        k, ini, section,
        "kill_enertion"); // 1.	Инерционное движение вперед от попадания в голову
    anims[1] =
        (new type_motion1())->setup(k, ini, section, "kill_burst"); // 2.	Изрешетить пулями
    anims[2] = (new type_motion2())->setup(k, ini, section, "kill_shortgun"); // 3.	Шотган

    anims[6] = (new type_motion3())->setup(
        k, ini, section, "kill_headshot"); // 4.	Хедшот (по вероятности), кроме 5 (4)
    anims[4] = (new type_motion4())->setup(k, ini, section,
                                             "kill_sniper_headshot"); // 5.	Снайперка в голову.
    anims[5] = (new type_motion5())->setup(k, ini, section,
                                             "kill_sniper_body"); // 6.	Снайперка в тело.
