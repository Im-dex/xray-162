...
void obstacles_query::set_intersection(const obstacles_query& query) {
    ...
    m_obstacles.erase(std::set_intersection(temp, temp + n, query.obstacles().begin(),
...
void obstacles_query::merge(const AREA& object_area) {
    ...
    m_area.erase(std::set_union(temp, temp + area_size, object_area.begin(), object_area.end(),
                                m_area.begin()),
                 m_area.end());
}
...
bool obstacles_query::merge(const Fvector& position, const float& radius,
...
bool obstacles_query::remove_objects(const Fvector& position, const float& radius) {
    ...
    OBSTACLES::iterator I =
        std::remove_if(m_obstacles.begin(), m_obstacles.end(), too_far_predicate(position, radius));
