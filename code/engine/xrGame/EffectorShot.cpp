...
void CWeaponShotEffector::Shot(CWeapon* weapon) {
    ...
    angle +=
        m_cam_recoil.DispersionInc * weapon->cur_silencer_koef.cam_disper_inc * (float)m_shot_numer;
    Shot2(angle);
}

void CWeaponShotEffector::Shot2(float angle) {
    m_angle_vert += angle * (m_cam_recoil.DispersionFrac +
                             m_Random.randF(-1.0f, 1.0f) * (1.0f - m_cam_recoil.DispersionFrac));
...
void CWeaponShotEffector::Update() {
    ...
    //	Msg( " <<[%d]  v=%.4f  dv=%.4f   a=%d s=%d  fr=%d", m_shot_numer, m_angle_vert,
    //m_delta_vert, m_actived, m_first_shot, Device.dwFrame );
}
...
void CWeaponShotEffector::ChangeHP(float* pitch, float* yaw) {
    ...
    *yaw -= m_delta_horz;   // x = yaw   = h = horz
    ...
    //	if ( m_cam_recoil.ReturnMode && m_cam_recoil.StopReturn && (*pitch > m_first_shot_pos +
    //0.1f) )
    //	{
    //		m_actived = false;
    //	}
    //	Msg( "[%d]  pitch = %.4f   yaw = %.4f    fs=%d    a=%d  fr=%d", m_shot_numer, *pitch, *yaw,
    //m_first_shot, m_actived, Device.dwFrame );
}
