...
IC _action_type& CSSetupManager::action(const _action_id_type& action_id) const {
    setup_actions::const_iterator I =
        std::find_if(actions().begin(), actions().end(), setup_pred(action_id));
...
IC void CSSetupManager::add_action(const _action_id_type& action_id, _action_type* action) {
    m_actuality = false;
    VERIFY(action);
    VERIFY(std::find_if(actions().begin(), actions().end(), setup_pred(action_id)) ==
           actions().end());
        ...
        for (; I != E; ++I) {
            ...
            if (m_total_weight > m_random) {
                if (std::find_if(actions().begin(), actions().end(),
                                 setup_pred(m_current_action_id)) != actions().end())
