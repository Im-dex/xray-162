...
void CPHCommander::remove_call(PHCALL_I i, PHCALL_STORAGE& cs) {
    ...
    if (esc && cfa) {
        ...
        if (m > EPS_S)
            f.mul(1.f / m);
        // Msg(" const force removed: force: %f,  remove step: %d  world step: %d ,dir(%f,%f,%f) ",
        // m, esc->step(), (u32)physics_world()->StepsNum(), f.x, f.y , f.z );
    }
#endif
    delete_call(*i);
    cs.erase(i);
...
void CPHCommander::remove_call(CPHReqComparerV* cmp_condition, CPHReqComparerV* cmp_action,
                               PHCALL_STORAGE& cs) {
    cs.erase(std::remove_if(cs.begin(), cs.end(), SFRemovePred2(cmp_condition, cmp_action)),
             cs.end());
}
...
bool CPHCommander::add_call_unique(CPHCondition* condition, CPHReqComparerV* cmp_condition,
...
void CPHCommander::add_call_unique_as(CPHCondition* condition, CPHReqComparerV* cmp_condition,
                                      CPHAction* action, CPHReqComparerV* cmp_action) {
