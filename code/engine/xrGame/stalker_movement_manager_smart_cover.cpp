...
stalker_movement_manager_smart_cover::stalker_movement_manager_smart_cover(CAI_Stalker* object)
    : inherited(object), m_property_storage(0), m_current_transition(0),
      m_current_transition_animation(0), m_non_animated_loophole_change(false),
      m_apply_loophole_direction_distance(4.f), m_animation_selector(0),
      m_entering_smart_cover_with_animation(false), m_default_behaviour(false),
      m_enter_cover_id(""), m_enter_loophole_id(""), m_check_can_kill_enemy(false),
...
MotionID
stalker_movement_manager_smart_cover::select_animation(bool& animation_movement_controller) {
    if (!object().g_Alive())
        return (MotionID());
...
void stalker_movement_manager_smart_cover::on_frame(CPHMovementControl* movement_control,
...
void stalker_movement_manager_smart_cover::modify_animation(CBlend* blend) {
    ...
    CMotionDef* motion_def =
        smart_cast<IKinematicsAnimated*>(object().Visual())->LL_GetMotionDef(blend->motionID);
    VERIFY(motion_def);
...
void stalker_movement_manager_smart_cover::reach_enter_location(u32 const& time_delta) {
    ...
    Fvector position;
    m_target.cover()->object().XFORM().transform_tiny(position,
    ...
    if (!accessible(level_vertex_id) || !accessible(position)) {
        ...
        if (!restrictions().accessible(position)) {
            ...
            if (!restrictions().accessible(level_vertex_id)) {
                level_vertex_id = restrictions().accessible_nearest(
                    ai().level_graph().vertex_position(level_vertex_id), position);
                VERIFY(restrictions().accessible(level_vertex_id));
                VERIFY(restrictions().accessible(position));
            }
        }
        ...
        VERIFY2(restrictions().accessible(position) || show_restrictions(&restrictions()),
                *object().cName());
    ...
    m_enter_animation = smart_cast<IKinematicsAnimated*>(object().Visual())
...
void stalker_movement_manager_smart_cover::loophole_path(smart_cover::cover const& cover,
    ...
    path.clear_not_free();
    R_ASSERT2(ai().graph_engine().search(cover.description()->transitions(), source, target, &path,
                                         parameters),
...
void stalker_movement_manager_smart_cover::target_idle() {
    //	if (!m_current.cover()) {
    //		Msg								("! Cannot set target idle. Bad or absent
    //smart_cover."); 		return;
    //	}

    //	if (!m_current.cover_loophole()->is_action_available("idle")) {
    //		Msg								("! Cannot set target idle. Loophole has no such
    //action."); 		return;
    //	}
...
void stalker_movement_manager_smart_cover::target_lookout() {
    //	if (!m_current.cover()) {
    //		Msg								("! Cannot set target lookout. Bad or absent
    //smart_cover."); 		return;
    //	}

    //	if (!m_current.cover_loophole()->is_action_available("lookout")) {
    //		Msg								("! Cannot set target lookout. Loophole has no such
    //action."); 		return;
    //	}
...
void stalker_movement_manager_smart_cover::target_fire() {
    //	if (!m_current.cover()) {
    //		Msg								("! Cannot set target fire. Bad or absent
    //smart_cover."); 		return;
    //	}

    //	if (!m_current.cover_loophole()->is_action_available("fire")) {
    //		Msg								("! Cannot set target fire. Loophole has no such
    //action."); 		return;
    //	}

    //	if (!enemy_in_fov()) {
    //		Msg								("! Cannot set target fire. Enemy is not in current loophole's
    //fov."); 		return;
    //	}
...
void stalker_movement_manager_smart_cover::target_fire_no_lookout() {
    ...
    //		Msg								("! Cannot set target fire_no_lookout. Bad or absent
    //smart_cover."); 		return;
    //	}

    //	if (!current_params().cover_loophole()->is_action_available("fire_no_lookout")) {
    //		Msg								("! Cannot set target fire_no_lookout. Loophole has no such
    //action."); 		return;
    //	}
