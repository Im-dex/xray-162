...
CAbstractGraph::vertex(const _vertex_id_type& vertex_id) const {
...
TEMPLATE_SPECIALIZATION
IC void CAbstractGraph::begin(_vertex_id_type const& vertex_index, const_iterator& b,
...
IC void CAbstractGraph::save(IWriter& stream) {
    ...
    stream.open_chunk(1);
    auto I = this->vertices().cbegin();
    auto E = this->vertices().cend();
    for (int i = 0; I != E; ++I, ++i) {
        stream.open_chunk(i);
        {
            stream.open_chunk(0);
