...
void CActor::OnEvent(NET_Packet& P, u16 type) {
    ...
    case GE_OWNERSHIP_TAKE: {
        ...
        //			R_ASSERT2( Obj, make_string("GE_OWNERSHIP_TAKE: Object not found.
        //object_id = [%d]", id).c_str() );
        VERIFY2(Obj,
                make_string("GE_OWNERSHIP_TAKE: Object not found. object_id = [%d]", id).c_str());
        if (!Obj) {
        ...
        if (!IsGameTypeSingle() && !g_Alive()) {
            Msg("! WARNING: dead player [%d][%s] can't take items [%d][%s]", ID(), Name(),
        ...
        if (inventory().CanTakeItem(smart_cast<CInventoryItem*>(_GO))) {
            ...
            Msg("--- Actor [%d][%s]  %s  [%d][%s]", ID(), Name(), act, _GO->ID(),
                _GO->cNameSect().c_str());
#endif // MP_LOGGING
            ...
            if (IsGameTypeSingle()) {
                ...
                Msg("! ERROR: Actor [%d][%s]  tries to drop on take [%d][%s]", ID(), Name(),
                    _GO->ID(), _GO->cNameSect().c_str());
            }
        }
    } break;
    ...
    case GE_OWNERSHIP_REJECT: {
        ...
        //			R_ASSERT2( Obj, make_string("GE_OWNERSHIP_REJECT: Object not found, id =
        //%d", id).c_str() );
        ...
        Msg("--- Actor [%d][%s]  %s  [%d][%s]", ID(), Name(), act, GO->ID(),
            GO->cNameSect().c_str());
#endif // MP_LOGGING

        VERIFY(GO->H_Parent());
        if (!GO->H_Parent()) {
            Msg("! ERROR: Actor [%d][%s] tries to reject item [%d][%s] that has no parent", ID(),
                Name(), GO->ID(), GO->cNameSect().c_str());
            break;
        }

        VERIFY2(GO->H_Parent()->ID() == ID(),
                make_string("actor [%d][%s] tries to drop not own object [%d][%s]", ID(), Name(),
                            GO->ID(), GO->cNameSect().c_str())
                    .c_str());

        if (GO->H_Parent()->ID() != ID()) {
            CActor* real_parent = smart_cast<CActor*>(GO->H_Parent());
            Msg("! ERROR: Actor [%d][%s] tries to drop not own item [%d][%s], his parent is "
                "[%d][%s]",
                ID(), Name(), GO->ID(), GO->cNameSect().c_str(), real_parent->ID(),
                real_parent->Name());
            break;
        }

        if (!Obj->getDestroy() &&
            inventory().DropItem(GO, just_before_destroy, dont_create_shell)) {
            // O->H_SetParent(0,just_before_destroy);//moved to DropItem
            // feel_touch_deny(O,2000);
            Level().m_feel_deny.feel_touch_deny(Obj, 1000);

            // [12.11.07] Alexander Maniluk: extended GE_OWNERSHIP_REJECT packet for drop item to
            // selected position
    ...
    case GE_INV_ACTION: {
        ...
        if (!IsGameTypeSingle() && !g_Alive()) {
            //				Msg("! WARNING: dead player tries to rize inventory
            //action");
            break;
        }
    ...
    case GEG_PLAYER_ACTIVATEARTEFACT: {
        ...
        //			R_ASSERT2( Obj, make_string("GEG_PLAYER_ITEM_EAT(use): Object not found.
        //object_id = [%d]", id).c_str() );
        VERIFY2(Obj, make_string("GEG_PLAYER_ITEM_EAT(use): Object not found. object_id = [%d]", id)
                         .c_str());
        if (!Obj) {
            //				Msg                 ( "! GEG_PLAYER_ITEM_EAT(use): Object not found. object_id
            //= [%d]", id );
            break;
        }

        //			R_ASSERT2( !Obj->getDestroy(), make_string("GEG_PLAYER_ITEM_EAT(use):
        //Object is destroying. object_id = [%d]", id).c_str() );
        VERIFY2(!Obj->getDestroy(),
                make_string("GEG_PLAYER_ITEM_EAT(use): Object is destroying. object_id = [%d]", id)
                    .c_str());
        if (Obj->getDestroy()) {
            //				Msg                                ( "! GEG_PLAYER_ITEM_EAT(use): Object is
            //destroying. object_id = [%d]", id );
            break;
        }
        ...
        if (type == GEG_PLAYER_ACTIVATEARTEFACT) {
            ...
            //			R_ASSERT2( pArtefact, make_string("GEG_PLAYER_ACTIVATEARTEFACT: Artefact not
            //found. artefact_id = [%d]", id).c_str() );
            VERIFY2(pArtefact,
                    make_string(
                        "GEG_PLAYER_ACTIVATEARTEFACT: Artefact not found. artefact_id = [%d]", id)
                        .c_str());
            if (!pArtefact) {
