...
//	Description : moving objects with dynamic objects collision, i.e. objects with predictable
//behaviour
////////////////////////////////////////////////////////////////////////////
...
IC MagicBox3& moving_objects::continuous_box(moving_object* object, const Fvector& position,
                                             MagicBox3& result,
...
void moving_objects::resolve_collision_first(boxes& current, moving_object* object0,
...
void moving_objects::resolve_collision_previous(boxes& current, moving_object* object0,
                                                moving_object* object1,
                                                possible_actions& action) const {
    boxes start;
...
void moving_objects::resolve_collision(boxes& current, moving_object* object0,
                                       const Fvector& position0, moving_object* object1,
...
#endif // 0
    VERIFY2(object0->action_frame() != Device.dwFrame,
            make_string("%d %s", Device.dwFrame, *object0->object().cName()));
    VERIFY2(object0->action_frame() < Device.dwFrame,
            make_string("%d %s", Device.dwFrame, *object0->object().cName()));

    VERIFY2(object1->action_frame() != Device.dwFrame,
            make_string("%d %s", Device.dwFrame, *object0->object().cName()));
    VERIFY2(object1->action_frame() < Device.dwFrame,
            make_string("%d %s", Device.dwFrame, *object0->object().cName()));
    ...
    if (object1->action() == moving_object::action_wait) {
        ...
        action = possible_actions(action ^
                                  (possible_action_1_can_wait_2 | possible_action_2_can_wait_1));
        return;
    }

    VERIFY2(false, make_string("NODEFAULT: [%s][%s]", *object0->object().cName(),
                               *object1->object().cName()));
}

bool moving_objects::collided_dynamic(moving_object* object0, const Fvector& position0,
                                      moving_object* object1, const Fvector& position1,
                                      boxes& result) const {
...
bool moving_objects::collided_dynamic(moving_object* object0, const Fvector& position0,
                                      moving_object* object1, const Fvector& position1) const {
    boxes temp;
