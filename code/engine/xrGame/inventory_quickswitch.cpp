...
public:
    ...
    next_weapon_searcher(next_weapon_searcher const& copy)
        : m_prior_group(copy.m_prior_group), m_best_fit(copy.m_best_fit),
...
PIItem CInventory::GetNextItemInActiveSlot(u8 const priority_value, bool ignore_ammo) {
    ...
    next_weapon_searcher tmp_predicate(tmp_prior_group, best_fit, m_next_items_exceptions,
                                       ignore_ammo);
...
bool CInventory::ActivateNextItemInActiveSlot() {
    ...
    if (current_item) {
        ...
        R_ASSERT(res);
        current_item->object().u_EventGen(P, GEG_PLAYER_ITEM2RUCK,
                                          current_item->object().H_Parent()->ID());
...
char const* groups_names[CInventory::qs_priorities_count] = { "pistols", "shotgun", "assault",
                                                              "sniper_rifles", "heavy_weapons" };
