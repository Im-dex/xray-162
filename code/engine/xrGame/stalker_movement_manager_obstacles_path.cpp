...
bool stalker_movement_manager_obstacles::simulate_path_navigation() {
    ...
    while (!detail().completed(current_position, !detail().state_patrol_path(),
        ...
        current_position =
            predict_position(check_time_delta, current_position, current_travel_point, 1.f);
    }
...
IC void stalker_movement_manager_obstacles::remove_query_objects(const Fvector& position,
                                                                 const float& radius) {
