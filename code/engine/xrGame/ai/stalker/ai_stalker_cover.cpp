...
void CAI_Stalker::subscribe_on_best_cover_changed(const on_best_cover_changed_delegate& delegate) {
    VERIFY(m_cover_delegates.end() ==
           std::find(m_cover_delegates.begin(), m_cover_delegates.end(), delegate));
...
//performed",Device.dwTimeGlobal,*cName());
    ...
    m_ce_best->setup(position_to_cover_from, minimum_enemy_distance, maximum_enemy_distance,
                     minimum_enemy_distance);
    const CCoverPoint* point = ai().cover_manager().best_cover(
        Position(), 10.f, *m_ce_best, CStalkerMovementRestrictor(this, true));
    if (point)
        return (point);
    ...
    m_ce_best->setup(position_to_cover_from, minimum_enemy_distance, maximum_enemy_distance,
                     minimum_enemy_distance);
    point = ai().cover_manager().best_cover(Position(), 30.f, *m_ce_best,
                                            CStalkerMovementRestrictor(this, true));
    return (point);
}

float CAI_Stalker::best_cover_value(const Fvector& position_to_cover_from) {
    m_ce_best->setup(position_to_cover_from, MIN_SUITABLE_ENEMY_DISTANCE, 170.f,
                     MIN_SUITABLE_ENEMY_DISTANCE);
...
void CAI_Stalker::update_best_cover_actuality(const Fvector& position_to_cover_from) {
    ...
    if (m_best_cover->m_is_smart_cover) {
        ...
        smart_cover::loophole* loophole =
            cover->best_loophole(position_to_cover_from, value, false,
    ...
    if (m_best_cover->position().distance_to_sqr(position_to_cover_from) <
        xr::sqr(MIN_SUITABLE_ENEMY_DISTANCE)) {
        m_best_cover_actual = false;
#if 0 // def _DEBUG
    ...
    //		m_best_cover_actual				= false;
    //		Msg								("* [%6d][%s] cover became too
    //bad2",Device.dwTimeGlobal,*cName()); 		return;
    //	}
...
//performed",Device.dwTimeGlobal,*cName());
    ...
    m_ce_best->setup(position_to_cover_from, MIN_SUITABLE_ENEMY_DISTANCE, 170.f,
                     MIN_SUITABLE_ENEMY_DISTANCE);
    m_best_cover = ai().cover_manager().best_cover(Position(), 10.f, *m_ce_best,
                                                   CStalkerMovementRestrictor(this, true));
}
...
#ifdef _DEBUG
//	Msg									("* [%6d][%s]
//on_enemy_change",Device.dwTimeGlobal,*cName());
#endif
}
...
void CAI_Stalker::on_danger_location_add(const CDangerLocation& location) {
    ...
    if (m_best_cover->position().distance_to_sqr(location.position()) <= xr::sqr(location.m_radius)) {
#ifdef _DEBUG
//		Msg								("* [%6d][%s]
//on_danger_add",Device.dwTimeGlobal,*cName());
#endif
        m_best_cover_actual = false;
    }
}
...
void CAI_Stalker::on_danger_location_remove(const CDangerLocation& location) {
    if (!m_best_cover) {
        if (Position().distance_to_sqr(location.position()) <= xr::sqr(location.m_radius)) {
#ifdef _DEBUG
//			Msg							("* [%6d][%s]
//on_danger_remove",Device.dwTimeGlobal,*cName());
#endif
            m_best_cover_actual = false;
        }
...
#ifdef _DEBUG
//	Msg									("* [%6d][%s] cover is
//blocked",Device.dwTimeGlobal,*cName());
#endif
    m_best_cover_actual = false;
}
