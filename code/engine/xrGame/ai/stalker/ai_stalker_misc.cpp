...
bool CAI_Stalker::useful(const CItemManager* manager, const CGameObject* object) const {
    ...
    if (explosive && (explosive->CurrentParentID() != 0xffff)) {
        ...
        CEntityAlive* entity_alive =
            smart_cast<CEntityAlive*>(Level().Objects.net_Find(explosive->CurrentParentID()));
        if (entity_alive)
            memory().danger().add(CDangerObject(entity_alive, object->Position(),
                                                Device.dwTimeGlobal,
...
ALife::ERelationType CAI_Stalker::tfGetRelationType(const CEntityAlive* tpEntityAlive) const {
    ...
    if (pOtherIO && !(const_cast<CEntityAlive*>(tpEntityAlive)->cast_base_monster()))
        relation = RELATION_REGISTRY().GetRelationType(static_cast<const CInventoryOwner*>(this),
                                                       pOtherIO);
        ...
        if (initiator) {
            if (is_relation_enemy(initiator))
                ...
                u32 const time = missile->destroy_time() >= Device.dwTimeGlobal
                                     ...
                                     : 0;
                sound().play(StalkerSpace::eStalkerSoundFriendlyGrenadeAlarm, time + 1500,
                             time + 1000);
            }
        }
    }
