...
void draw_planner(const planner_type& brain, LPCSTR start_indent, LPCSTR indent,
                  LPCSTR planner_id) {
...
void draw_restrictions(const shared_str& restrictions, LPCSTR start_indent, LPCSTR indent,
                       LPCSTR header) {
...
void CAI_Stalker::debug_text() {
    ...
    if (g_Alive()) {
        ...
        if (memory().visual().visible_now(actor))
            ...
            if (object && !fis_zero(object->m_value))
                DBG_OutText("%s%sactor       : not yet visible : %f", indent, indent,
                            object->m_value);
            else
        ...
        CObject* object =
            (object_id == ALife::_OBJECT_ID(-1)) ? 0 : Level().Objects.net_Find(object_id);
    ...
    if (memory().enemy().selected()) {
        ...
        if (g_Alive()) {
            if (true || !g_mt_config.test(mtAiVision))
                VERIFY(!memory().visual().visible_now(memory().enemy().selected()) ||
                       (fuzzy > 0.f));
        ...
        if (g_Alive()) {
            ...
            u32 result = dwfChooseAction(2000, 1.f - interval, 1.f - 2 * interval,
                                         1.f - 3 * interval, panic_threshold(), g_Team(), g_Squad(),
            ...
            DBG_OutText("%s%s%svictory   : [%5.2f%%,%5.2f%%] -> %s", indent, indent, indent,
    ...
    DBG_OutText("%s%sitem to spawn       : %s", indent, indent,
    ...
    if (!movement().detail().path().empty()) {
        ...
        DBG_OutText("%s%s%s%sangular   : %f deg", indent, indent, indent, indent,
    ...
    if (movement().current_params().cover_fire_position()) {
        ...
        DBG_OutText("%s%s%sfire position current : [%f][%f][%f]", indent, indent, indent,
                    VPUSH(position));
    }

    if (movement().target_params().cover_fire_position()) {
        Fvector position = *movement().target_params().cover_fire_position();
        DBG_OutText("%s%s%sfire position target  : [%f][%f][%f]", indent, indent, indent,
                    VPUSH(position));
    }
    ...
    if (movement().restrictions().out_restrictions().size() ||
        ...
        draw_restrictions(movement().restrictions().base_out_restrictions(), temp, indent,
                          "base out");
        draw_restrictions(movement().restrictions().base_in_restrictions(), temp, indent,
                          "base in");
    }
    ...
    }
    {
        ...
        for (; I != E; ++I)
            DBG_OutText("%s%s%s[%s]%s", indent, indent, indent,
                        ...
                        (*I).m_sound->_handle() ? (*I).m_sound->_handle()->file_name()
                                                : "no source");
    }
...
public:
    IC ray_query_param(CCustomMonster* holder, float power_threshold, float distance,
                       const Fvector& start_position, const Fvector& direction,
...
BOOL _ray_query_callback(collide::rq_result& result, LPVOID params) {
    ...
    param->m_points->push_back(
        Fvector().mad(param->m_start_position, param->m_direction, result.range));
...
void fill_points(CCustomMonster* self, const Fvector& position, const Fvector& direction,
                 float distance, collide::rq_results& rq_storage, COLLIDE_POINTS& points,
                 float& pick_distance) {
    ...
    ray_query_param params(self, self->memory().visual().transparency_threshold(), distance,
...
static Fmatrix aim_on_actor(Fvector const& bone_position, Fvector const& weapon_position,
                            Fvector const& weapon_direction, Fvector const& target,
...
static void fill_bones(CAI_Stalker& self, Fmatrix const& transform,
                       IKinematicsAnimated* kinematics_animated, LPCSTR animation_id,
                       bool const local) {
    ...
    for (u16 i = 0; i < MAX_PARTS; ++i) {
        ...
        for (u32 j = 0; j < blend_count; ++j) {
            ...
            CBlend* const new_blend =
                kinematics_animated->LL_PlayCycle(i, blend->motionID, TRUE, 0, 0, 1);
            VERIFY(new_blend);
            *new_blend = *blend;
            new_blend->channel = 1;
        }
...
#ifdef DEBUG_RENDER
static void draw_bones(IKinematics& kinematics, Fvector const& box_size, u32 const& box_color,
                       u32 const& line_color, Fmatrix const* const transform = 0) {
    CDebugRenderer& renderer = Level().debug_renderer();
...
static void draw_animation_bones(CAI_Stalker& self, Fmatrix const& transform,
                                 IKinematicsAnimated* kinematics_animated, LPCSTR animation_id) {
    IKinematics* kinematics = smart_cast<IKinematics*>(kinematics_animated);

    u16 spine_bone_id =
        (u16)kinematics->LL_BoneID(pSettings->r_string(self.cNameSect().c_str(), "bone_spin"));
    u16 shoulder_bone_id =
        (u16)kinematics->LL_BoneID(pSettings->r_string(self.cNameSect().c_str(), "bone_shoulder"));
    u16 weapon_bone_id0 =
        (u16)kinematics->LL_BoneID(pSettings->r_string(self.cNameSect().c_str(), "weapon_bone0"));
    u16 weapon_bone_id1 =
        (u16)kinematics->LL_BoneID(pSettings->r_string(self.cNameSect().c_str(), "weapon_bone2"));
    ...
    draw_bones(*kinematics, Fvector().set(.011f, .011f, .011f), D3DCOLOR_XRGB(0, 255, 0),
               D3DCOLOR_XRGB(0, 255, 255), &transform);
#endif // #ifdef DEBUG_RENDER
    ...
    spine_offset = aim_on_actor(spine_bone.c, weapon_bone_position,
    ...
    shoulder_offset = aim_on_actor(shoulder_bone.c, weapon_bone_position,
                                   weapon_bone_direction.normalize(), target, true);

    callback_param param2;
    param2.transform =
        Fmatrix(transform).invert().mulB_43(shoulder_offset).mulB_43(Fmatrix(transform));
    ;
    ...
    Fvector const weapon_position_target = Fvector().mad(
        weapon_bone_position, weapon_bone_direction, weapon_bone_position.distance_to(target));
    ...
    draw_bones(*kinematics, Fvector().set(.01f, .01f, .01f), D3DCOLOR_XRGB(0, 0, 255),
               D3DCOLOR_XRGB(255, 255, 0));

    renderer.draw_obb(g_stalker_skeleton[weapon_bone_id0], Fvector().set(.01f, .01f, .01f),
                      D3DCOLOR_XRGB(255, 0, 0));
    renderer.draw_obb(g_stalker_skeleton[weapon_bone_id1], Fvector().set(.01f, .01f, .01f),
                      D3DCOLOR_XRGB(255, 0, 0));
    renderer.draw_line(Fidentity, g_stalker_skeleton[weapon_bone_id0].c,
                       g_stalker_skeleton[weapon_bone_id1].c, D3DCOLOR_XRGB(255, 0, 0));
#endif // #ifdef DEBUG_RENDER
}
...
//	draw_animation_bones		(*this, XFORM(), kinematics, "loophole_3_attack_in_0");
    ...
    if (inventory().ActiveItem()) {
        ...
        temp.mul(1.f);
        temp.add(position);
        Level().debug_renderer().draw_line(Fidentity, position, temp,
    ...
    if (IsMyCamera()) {
        ...
        if (std::find(objects.begin(), objects.end(), memory().enemy().selected()) !=
            objects.end()) {
            Fvector position =
                feel_vision_get_vispoint(const_cast<CEntityAlive*>(memory().enemy().selected()));
            Level().debug_renderer().draw_aabb(position, .05f, .05f, .05f,
                                               D3DCOLOR_XRGB(0 * 255, 255, 0 * 255));
            return;
        }
...

    {
        ...
        Level().debug_renderer().draw_line(Fidentity, eye_matrix.c, current_direction,
                                           D3DCOLOR_XRGB(255, 0, 255));
        Level().debug_renderer().draw_line(Fidentity, eye_matrix.c, target_direction,
                                           D3DCOLOR_XRGB(255, 255, 0));
    }
    ...
    if (memory().danger().selected() &&
        ...
        if (ai().level_graph().valid_vertex_id(level_vertex_id)) {
            ...
            for (u32 i = 0; i < 36; ++i) {
                float value =
                    ai().level_graph().high_cover_in_direction(float(10 * i) / 180.f * PI, v);
                ...
                Level().debug_renderer().draw_line(Fidentity, position, direction,
                                                   D3DCOLOR_XRGB(0, 0, 255));
                value =
                    ai().level_graph().compute_high_square(float(10 * i) / 180.f * PI, PI / 2.f, v);
                if (value > best_value) {
                    best_value = value;
                    j = i;
                }
            ...
            direction.set(position.x - half_size * float(v->high_cover(0)) / 15.f, position.y,
                          position.z);
            Level().debug_renderer().draw_line(Fidentity, position, direction,
                                               D3DCOLOR_XRGB(255, 0, 0));
            ...
            direction.set(position.x + half_size * float(v->high_cover(2)) / 15.f, position.y,
                          position.z);
            Level().debug_renderer().draw_line(Fidentity, position, direction,
                                               D3DCOLOR_XRGB(255, 0, 0));
            ...
            // low
            {
                ...
                for (u32 i = 0; i < 36; ++i) {
                    float value =
                        ai().level_graph().low_cover_in_direction(float(10 * i) / 180.f * PI, v);
                    direction.setHP(float(10 * i) / 180.f * PI, 0);
                    direction.normalize();
                    direction.mul(value * half_size);
                    direction.add(position);
                    ...
                    Level().debug_renderer().draw_line(Fidentity, position, direction,
                                                       D3DCOLOR_XRGB(0, 0, 255));
                    value = ai().level_graph().compute_low_square(float(10 * i) / 180.f * PI,
                                                                  PI / 2.f, v);
                    if (value > best_value) {
                        best_value = value;
                        j = i;
                ...
                direction.set(position.x - half_size * float(v->low_cover(0)) / 15.f, position.y,
                              position.z);
                Level().debug_renderer().draw_line(Fidentity, position, direction,
                                                   D3DCOLOR_XRGB(255, 0, 0));
                ...
                direction.set(position.x + half_size * float(v->low_cover(2)) / 15.f, position.y,
                              position.z);
                Level().debug_renderer().draw_line(Fidentity, position, direction,
                                                   D3DCOLOR_XRGB(255, 0, 0));
                ...
                float value =
                    ai().level_graph().low_cover_in_direction(float(10 * j) / 180.f * PI, v);
                direction.setHP(float(10 * j) / 180.f * PI, 0);
                direction.normalize();
                direction.mul(value * half_size);
                direction.add(position);
