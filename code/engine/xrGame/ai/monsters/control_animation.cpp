...
// *(m_object->cName()));
    ...
    part.blend =
        m_skeleton_animated->LL_PlayCycle(bone_or_part, part.get_motion(), TRUE, callback, this);
...
void CControlAnimation::check_events(SAnimationPart& part) {
    if (part.get_motion().valid() && part.actual && part.blend) {
        ...
        if (it != m_anim_events.end()) {
            ...
            float cur_perc = float(Device.dwTimeGlobal - part.time_started) /
            ...
            for (auto event_it = it->second.begin(); event_it != it->second.end(); ++event_it) {
                ...
                if (!event.handled && (event.time_perc < cur_perc)) {
                    ...
                    // gen event
                    SAnimationSignalEventData anim_event(part.get_motion(), event.time_perc,
                                                         event.event_id);
