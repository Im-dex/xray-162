...
void CControl_Manager::update_frame() {
    ...
    m_active_elems.erase(
        std::remove_if(m_active_elems.begin(), m_active_elems.end(), predicate_remove()),
        m_active_elems.end());
}
...
bool CControl_Manager::build_path_line(CControl_Com* com, const Fvector& target, u32 node,
                                       u32 vel_mask) {
