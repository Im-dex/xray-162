    ...
    if (object->b_end_state_eat) {
        ...
        if (m_target_node == u32(-1)) {
            const CCoverPoint* point = object->CoverMan->find_cover(
                object->Home->get_home_point(), 1, object->Home->get_min_radius());
            if (!point)
                return;
        ...
        if (m_target_node == u32(-1)) {
            const CCoverPoint* point = object->CoverMan->find_cover(
                object->Home->get_home_point(), 1, object->Home->get_mid_radius());
            if (!point)
                return;
            m_target_node = point->level_vertex_id();
        }
    ...
    if (current_substate == eStateRest_WalkGraphPoint) {
        ...
        if (object->Position().distance_to(data.point) > 8.f) {
            ...
            if (object->m_start_smelling == u32(-1) ||
                object->m_start_smelling > u32(4) + object->m_smelling_count) {
