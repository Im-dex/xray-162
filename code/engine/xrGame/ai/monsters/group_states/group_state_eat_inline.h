...
void CStateGroupEatAbstract::finalize() {
    ...
    if ((corpse == this->object->EatedCorpse) && this->object->EatedCorpse) {
        const_cast<CEntityAlive*>(this->object->EatedCorpse)->m_use_timeout =
            this->object->m_corpse_use_timeout;
...
void CStateGroupEatAbstract::reselect_state() {
    ...
    /*
            if (prev_substate == eStateEat_CorpseApproachRun) { select_state(eStateEat_CheckCorpse);
       return; }
...
bool CStateGroupEatAbstract::check_start_conditions() {
    ...
    return (this->object->CorpseMan.get_corpse() &&
            this->object->Home->at_home(this->object->CorpseMan.get_corpse()->Position()) && hungry() &&
