...
CStateGroupHearDangerousSoundAbstract::CStateGroupHearDangerousSound(_Object* obj)
    : inherited(obj) {
...
void CStateGroupHearDangerousSoundAbstract::setup_substates() {
    ...
    if (this->current_substate == eStateSquad) {
        ...
        if (this->object->control().path_builder().get_node_in_radius(
                squad->GetLeader()->ai_location().level_vertex_id(), 8.f, LEADER_RADIUS,
                FIND_POINT_ATTEMPTS, data.vertex)) {
            ...
            if (!this->object->control().path_builder().restrictions().accessible(dest_pos)) {
                data.vertex = this->object->control().path_builder().restrictions().accessible_nearest(
                    dest_pos, data.point);
            } else {
                data.point = dest_pos;
                data.vertex = u32(-1);
