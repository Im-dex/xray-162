...
void CStateGroupAttackRunAbstract::execute() {
    ...
    if (Device.dwTimeGlobal > m_memorized_tick + memory_update_ms) {
        m_predicted_vel =
            (enemy_pos - m_memorized_pos) * (1000.f / (Device.dwTimeGlobal - m_memorized_tick));
    ...
    const SVelocityParam velocity_run =
        this->object->move().get_velocity(MonsterMovement::eVelocityParameterRunNormal);
    ...
    const Fvector radial_prediction =
        self_pos + Fvector().setHP(h_angle, p_angle).normalize_safe() * self2enemy_dist;
...
// object->EnemyMan.get_enemy_vertex()); 	object->path().set_rebuild_time
