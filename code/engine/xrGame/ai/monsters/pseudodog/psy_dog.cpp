...
void CPsyDog::Load(LPCSTR section) {
    ...
    m_phantoms_die_time.resize(m_max_phantoms_count);
...
bool CPsyDog::spawn_phantom() {
    u32 node;
    if (!control().path_builder().get_node_in_radius(ai_location().level_vertex_id(), 4, 8, 5,
                                                     node))
        return false;

    // set id to created server object
    CSE_Abstract* phantom = Level().spawn_item(
        "psy_dog_phantom", ai().level_graph().vertex_position(node), node, 0xffff, true);
...
void CPsyDogPhantom::Think() {
    ...
    // (control().movement().real_velocity() < 2*velocity_run.velocity.linear/3) return;
    ...
    u32 node = ai().level_graph().check_position_in_direction(ai_location().level_vertex_id(),
                                                              Position(), target);
    ...
    Actor()->Cameras().AddPPEffector(
        new CMonsterEffector(m_appear_effector.ppi, m_appear_effector.time,
