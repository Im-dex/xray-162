...
bool CControlAnimationBase::accel_chain_get(float cur_speed, EMotionAnim target_anim,
                                            EMotionAnim& new_anim, float& a_speed) {
    VERIFY2(xr::abs(cur_speed) < 1000, "CControlAnimationBase cur_speed too big");
    ...
    for (auto I = B; I != E; I++) {
        ...
        VERIFY2(xr::abs(tmp) < 1000, "CControlAnimationBase GetAnimSpeed returns too big speed");
        a_speed = tmp * cur_speed / best_param->velocity.linear;
        VERIFY2(xr::abs(a_speed) < 1000, "CControlAnimationBase a_speed too big");
        return true;
    }
    return false;
}
...
bool CControlAnimationBase::accel_chain_test() {
    ...
    for (auto I = m_accel.chain.begin(); I != m_accel.chain.end(); I++) {
        ...
        for (auto IT = I->begin() + 1; IT != I->end(); IT++) {
            ...
            xr_sprintf(
                error_msg,
                "Incompatible speed ranges. Monster[%s] From animation  [%s] To animation [%s]",
...
bool CControlAnimationBase::accel_check_braking(float before_interval, float nominal_speed) {
    ...
    float braking_dist =
        (nominal_speed * ((braking_mode) ? nominal_speed : m_man->movement().velocity_current())) /
        (2 * acceleration);
