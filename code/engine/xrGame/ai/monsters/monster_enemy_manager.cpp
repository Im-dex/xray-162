...
void CMonsterEnemyManager::update() {
    ...
    switch (dwfChooseAction(0, monster->panic_threshold(), 0.f, 0.f, 0.f, monster->g_Team(),
                            monster->g_Squad(), monster->g_Group(), 0, 1, 2, 3, 4, monster, 30.f)) {
    case 4:
    case 3:
    case 2:
    case 1:
...
bool CMonsterEnemyManager::is_enemy(const CEntityAlive* obj) {
    return ((monster->g_Team() != obj->g_Team()) && monster->is_relation_enemy(obj) &&
            obj->g_Alive());
}
...
void CMonsterEnemyManager::transfer_enemy(CBaseMonster* friend_monster) {
    ...
    monster->EnemyMemory.add_enemy(friend_monster->EnemyMan.get_enemy(),
