...
void CAI_Rat::FreeHuntingActive() {
    ...
    CHECK_IF_SWITCH_TO_NEW_STATE_THIS_UPDATE(
        (memory().enemy().selected() &&
         ((memory().enemy().selected()->Position().distance_to(m_tSafeSpawnPosition) <
           m_fMaxPursuitRadius) ||
    ...
    if ((m_tLastSound.dwTime >= m_dwLastUpdateTime) &&
        (!m_tLastSound.tpEntity ||
         ((!memory().item().selected() ||
           (memory().item().selected()->ID() != m_tLastSound.tpEntity->ID())) &&
    ...
    if (m_bStateChanged ||
        (fis_zero(m_fSpeed) && (angle_difference(movement().m_body.target.yaw,
...
void CAI_Rat::UnderFire() {
    ...
    if (m_bStateChanged) //(Device.dwTimeGlobal - m_previous_query_time > TIME_TO_GO) ||
...
void CAI_Rat::AttackRun() {
    ...
    ERatStates eState = ERatStates(dwfChooseAction(
        ...
        m_eCurrentState, m_eCurrentState, m_eCurrentState, aiRatRetreat, aiRatRetreat, this, 30.f));
    if (eState != m_eCurrentState) {
    ...
    CHECK_IF_GO_TO_NEW_STATE_THIS_UPDATE(
        memory().enemy().selected() &&
            (m_tSafeSpawnPosition.distance_to(memory().enemy().selected()->Position()) >
             m_fMaxPursuitRadius),
        aiRatReturnHome);
...
void CAI_Rat::Retreat() {
    ...
    if (memory().enemy().selected() && memory().enemy().selected()->g_Alive()) {
        ERatStates eState = ERatStates(dwfChooseAction(
            m_dwActionRefreshRate, m_fAttackSuccessProbability, m_fAttackSuccessProbability,
            m_fAttackSuccessProbability, m_fAttackSuccessProbability, g_Team(), g_Squad(),
            g_Group(), aiRatAttackRun, aiRatAttackRun, aiRatAttackRun, aiRatRetreat, aiRatRetreat,
            this, 30.f));
        //		ERatStates eState =
...
void CAI_Rat::Pursuit() {
    ...
    //	if ((m_tLastSound.dwTime >= m_dwLastUpdateTime) && ((m_tLastSound.eSoundType &
    //SOUND_TYPE_WEAPON_BULLET_HIT) == SOUND_TYPE_WEAPON_BULLET_HIT)) {
    if ((m_tLastSound.dwTime >= m_dwLastUpdateTime) &&
        (!m_tLastSound.tpEntity ||
         ((!memory().item().selected() ||
           (memory().item().selected()->ID() != m_tLastSound.tpEntity->ID())) &&
...
void CAI_Rat::EatCorpse() {
    ...
    CHECK_IF_GO_TO_PREV_STATE_THIS_UPDATE(!memory().item().selected() ||
    ...
    //	R_ASSERT							(V);
    //	u16									head_bone =
    //V->LL_BoneID("bip01_head"); 	Fmatrix
