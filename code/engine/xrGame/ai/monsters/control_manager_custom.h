...
public:
    ...
    void ta_pointbreak();
    void ta_fill_data(SAnimationTripleData& data, LPCSTR s1, LPCSTR s2, LPCSTR s3,
                      bool execute_once, bool skip_prep,
    ...
    void load_jump_data(LPCSTR s1, LPCSTR s2, LPCSTR s3, LPCSTR s4, u32 vel_mask_prepare,
                        u32 vel_mask_ground, u32 flags);
    bool is_jumping();

    bool check_if_jump_possible(Fvector const& target, bool full_check);
    bool jump_if_possible(Fvector const& target, CEntityAlive* target_object,
                          bool use_target_direction, bool use_velocity_bounce = true,
    ...
    //-------------------------------------------------------------------------------
    // Rotation Jump
    void add_rotation_jump_data(LPCSTR left1, LPCSTR left2, LPCSTR right1, LPCSTR right2,
                                float angle, u32 flags = 0);
...
private:
    ...
    void fill_rotation_data(SControlRotationJumpData& data, LPCSTR left1, LPCSTR left2,
