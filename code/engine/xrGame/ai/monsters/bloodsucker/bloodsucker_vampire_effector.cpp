...
BOOL CVampirePPEffector::Process(SPPInfo& pp) {
    ...
    if (time_past_perc < TIME_ATTACK) {
        ...
        float time_past_sine_perc =
            (time_past_perc - TIME_ATTACK) * (1 / (1 - TIME_ATTACK + TIME_ATTACK));
...
CVampireCameraEffector::CVampireCameraEffector(float time, const Fvector& src, const Fvector& tgt)
    ...
    dangle_target.set(Random.randFs(DELTA_ANGLE_X), Random.randFs(DELTA_ANGLE_Y),
...
BOOL CVampireCameraEffector::ProcessCam(SCamEffectorInfo& info) {
    ...
    if (time_left_perc < 0.2f) {
        ...
        angle_lerp(dangle_current.x, dangle_target.x, xr::abs(dangle_current.x / fLifeTime + 0.001f),
                   Device.fTimeDelta);
        angle_lerp(dangle_current.y, dangle_target.y, xr::abs(dangle_current.y / fLifeTime + 0.001f),
                   Device.fTimeDelta);
        angle_lerp(dangle_current.z, dangle_target.z, xr::abs(dangle_current.z / fLifeTime + 0.001f),
                   Device.fTimeDelta);
