...
CStateManagerFlesh::CStateManagerFlesh(CAI_Flesh* monster) : inherited(monster) {
    ...
    add_state(eStateHearInterestingSound,
              new CStateMonsterHearInterestingSound<CAI_Flesh>(monster));
    add_state(eStateHearDangerousSound,
              new CStateMonsterHearDangerousSound<CAI_Flesh>(monster));
...
void CStateManagerFlesh::execute() {
    ...
    if (!object->is_under_control()) {
        ...
        if (enemy) {
            state_id = object->EnemyMan.get_danger_type() == eStrong && !object->HitMemory.is_hit()
                           ? eStatePanic
                           : eStateAttack;
