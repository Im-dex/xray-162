...
void CAnomalyDetector::load(LPCSTR section) {
    ...
    m_time_to_rememeber =
        READ_IF_EXISTS(pSettings, r_u32, section, "Anomaly_Detect_Time_Remember", 30000);
}
...
void CAnomalyDetector::update_schedule() {
    ...
    m_object->control().path_builder().restrictions().add_restrictions(temp_out_restrictors,
                                                                       temp_in_restrictors);
    ...
    m_object->control().path_builder().restrictions().remove_restrictions(temp_out_restrictors,
                                                                          temp_in_restrictors);

    // remove from storage
    m_storage.erase(
        std::remove_if(m_storage.begin(), m_storage.end(), remove_predicate(m_time_to_rememeber)),
        m_storage.end());
}
...
void CAnomalyDetector::on_contact(CObject* obj) {
    ...
    if (Level().space_restriction_manager().restriction_presented(
            m_object->control().path_builder().restrictions().in_restrictions(),
            custom_zone->cName()))
        return;
