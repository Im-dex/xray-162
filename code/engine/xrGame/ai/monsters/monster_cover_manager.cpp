...
public:
    ...
    // manual setup
    void setup(CBaseMonster* object, const Fvector& position, float min_pos_distance,
...
void CCoverEvaluator::evaluate_smart_cover(smart_cover::cover const* smart_cover,
                                           float const& weight) {}
...
const CCoverPoint* CMonsterCoverManager::find_cover(const Fvector& position, float min_pos_distance,
    ...
    const CCoverPoint* point =
        ai().cover_manager().best_cover(m_object->Position(), 30.f, *m_ce_best);
...
const CCoverPoint* CMonsterCoverManager::find_cover(const Fvector& src_pos, const Fvector& dest_pos,
                                                    float min_pos_distance, float max_pos_distance,
                                                    float deviation) {
...
void CMonsterCoverManager::less_cover_direction(Fvector& dir) {
    ...
    for (float ang = angle; angle_difference(ang, angle) < ANGLE_DISP;
        ...
        if (Level().ObjectSpace.RayPick(trace_from, direction, TRACE_STATIC_DIST,
                                        collide::rqtStatic, l_rq, m_object)) {
