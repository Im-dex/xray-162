...
CMonsterSquad::CMonsterSquad()
    : leader(0), m_home_danger_end_tick(0), m_home_danger_mode_time(8000) {
...
bool CMonsterSquad::is_locked_cover(u32 node) {
    return (std::find(m_locked_covers.begin(), m_locked_covers.end(), node) !=
            m_locked_covers.end());
}
...
bool CMonsterSquad::is_locked_corpse(const CEntityAlive* corpse) {
    return (std::find(m_locked_corpses.begin(), m_locked_corpses.end(), corpse) !=
...
squad_grouping_behaviour::squad_grouping_behaviour(CEntity* self, Fvector cohesion_factor,
                                                   Fvector separate_factor,
                                                   float max_separate_range)
    : self(self), squad(NULL), steering_behaviour::grouping::params(
