...
void CMonsterSoundMemory::HearSound(const CObject* who, int eType, const Fvector& Position,
                                    float power, TTime time) {
    SoundElem s;
...
void CMonsterSoundMemory::UpdateHearing() {
    ...
    Sounds.erase(std::remove_if(Sounds.begin(), Sounds.end(),
                                pred_remove_nonactual_sounds(Device.dwTimeGlobal - time_memory)),
                 Sounds.end());
...
void CMonsterSoundMemory::remove_links(CObject* O) {
    // удаление устаревших звуков
    Sounds.erase(std::remove_if(Sounds.begin(), Sounds.end(), pred_remove_relcase(O)),
                 Sounds.end());
}
