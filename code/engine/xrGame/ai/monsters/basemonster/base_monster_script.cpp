...
void CBaseMonster::GenerateNewOffsetFromLeader() {
    ...
    float const offset_magnitude =
        min_squad_offset + (max_squad_offset - min_squad_offset) * Random.randF(1.f);
...
bool CBaseMonster::AssignGamePathIfNeeded(Fvector const target_pos, u32 const level_vertex) {
    ...
    if (!level_vertex_is_valid) {
        if (m_action_target_pos == target_pos && m_action_target_node != u32(-1)) {
            ...
            if (path().is_target_actual() && path().get_target_set() == target_pos &&
                path_node != u32(-1)) {
...
bool CBaseMonster::bfAssignMovement(CScriptEntityAction* tpEntityAction) {
    ...
    case CScriptMovementAction::eGoalTypeObject: {
        ...
        if (AssignGamePathIfNeeded(Fvector().set(0.f, 0.f, 0.f),
                                   l_tpGameObject->ai_location().level_vertex_id()))
            break;

        path().set_target_point(l_tpGameObject->Position(),
                                l_tpGameObject->ai_location().level_vertex_id());
        break;
    }
    case CScriptMovementAction::eGoalTypePatrolPath:
        ...
        control().path_builder().patrol().set_path(l_tMovementAction.m_path,
    ...
    case CScriptMovementAction::eGoalTypeFollowLeader: {
        ...
        CCustomMonster* const leader =
            should_follow_leader ? smart_cast<CCustomMonster*>(Level().Objects.net_Find(leader_id))
                                 : NULL;
    ...
    case CScriptMovementAction::eGoalTypePathNodePosition:
        ...
        if (AssignGamePathIfNeeded(l_tMovementAction.m_tDestinationPosition,
...
bool CBaseMonster::bfAssignObject(CScriptEntityAction* tpEntityAction) {
    ...
    //smart_cast<CEntityAlive*>(l_tObjectAction.m_tpObject); 	if (!l_tpEntity) return	(false ==
