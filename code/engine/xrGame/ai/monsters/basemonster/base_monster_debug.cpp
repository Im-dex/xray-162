...
CBaseMonster::SDebugInfo CBaseMonster::show_debug_info() {
    ...
    float y = 200;
    float x =
        (m_show_debug_info == 1) ? 40.f : float(::Render->getTarget()->get_width() / 2) + 40.f;
    ...
    DBG().text(this).add_item("-----------   MEMORY   ----------------", x, y += delta_y,
                              delimiter_color);
    ...
    if (CorpseMan.get_corpse()) {
        xr_sprintf(text, "Current Corpse = [%s] Satiety = [%.2f]", *CorpseMan.get_corpse()->cName(),
                   GetSatiety());
    } else
    ...
    if (SoundMemory.IsRememberSound()) {
        ...
        if (sound_elem.who)
            xr_sprintf(text, "Sound: type[%s] time[%u] power[%.3f] val[%i] src[+]", s_type,
                       sound_elem.time, sound_elem.power, sound_elem.value);
        else
            xr_sprintf(text, "Sound: type[%s] time[%u] power[%.3f] val[%i] src[?]", s_type,
                       sound_elem.time, sound_elem.power, sound_elem.value);
    ...
    if (HitMemory.is_hit()) {
        if (HitMemory.get_last_hit_object()) {
            xr_sprintf(text, "Hit Info: object=[%s] time=[%u]",
                       *(HitMemory.get_last_hit_object()->cName()), HitMemory.get_last_hit_time());
        } else {
    ...
    DBG().text(this).add_item("-----------   MOVEMENT   ------------", x, y += delta_y,
                              delimiter_color);

    xr_sprintf(text, "Actual = [%u] Enabled = [%u]", control().path_builder().actual(),
    ...
    DBG().text(this).add_item("------- Attack Distances -------------", x, y += delta_y,
                              delimiter_color);
...
void add_enemy_debug_info(debug::text_tree& root_s, const CCustomMonster* pThis,
...
void add_debug_info(debug::text_tree& root_s, CScriptEntityAction* p_action) {
    ...
    TextTree& movement_action_s =
        root_s.add_line("Movement_Completed", p_action->CheckIfMovementCompleted());
...
void CBaseMonster::add_debug_info(debug::text_tree& root_s) {
    ...
    corpse_s.add_line("Current_Corpse",
                      CorpseMan.get_corpse() ? *CorpseMan.get_corpse()->cName() : "none");
    ...
    if (EnemyMan.get_enemy()) {
        ...
        map_home_s.add_line("Dist_To_Enemy",
    ...
    level_s.add_line("Start_Vertex", movement().level_path().path().empty()
                                         ? -1
                                         : movement().level_path().path().front());
    level_s.add_line("End_Vertex", movement().level_path().path().empty()
                                       ? -1
    ...
    if (!movement().detail().path().empty()) {
        ...
        detail_s.add_line("speed(physics)",
    ...
    if (movement().restrictions().out_restrictions().size() ||
        ...
        detail::add_debug_info_restrictions(restrictions_s.add_line("out"),
                                            *movement().restrictions().out_restrictions());
        detail::add_debug_info_restrictions(restrictions_s.add_line("in"),
                                            *movement().restrictions().in_restrictions());
        detail::add_debug_info_restrictions(restrictions_s.add_line("base_out"),
                                            *movement().restrictions().base_out_restrictions());
        detail::add_debug_info_restrictions(restrictions_s.add_line("base_in"),
                                            *movement().restrictions().base_in_restrictions());

        restrictions_s.add_line("Actor_Accessible?",
