...
void CControlPathBuilder::update_schedule() {
    ...
    if (m_data.enable) {
        ...
        if (m_data.path_type == MovementManager::ePathTypeGamePath) {
            ...
            // set target
            // TODO: make it VERIFY
            //			VERIFY3(m_data.target_node != u32(-1), "Error: Object set wrong path params!
            //Object name:",*inherited_com::m_object->cName());
...
bool CControlPathBuilder::valid_destination(const Fvector& pos, u32 node) {
    return (ai().level_graph().valid_vertex_id(node) &&
            ai().level_graph().valid_vertex_position(pos) && ai().level_graph().inside(node, pos));
}
...
void CControlPathBuilder::fix_position(const Fvector& pos, u32 node, Fvector& res_pos) {
    ...
    if (!accessible(res_pos)) {
        ...
        if (level_vertex_id != node) {
            Msg("! src_node[%d] res_node[%d] src_pos[%f,%f,%f] res_pos[%f,%f,%f]", node,
                level_vertex_id, VPUSH(pos), VPUSH(res_pos));
        }
...
bool CControlPathBuilder::get_node_in_radius(u32 src_node, float min_radius, float max_radius,
                                             u32 attempts, u32& dest_node) {
    ...
    for (u32 i = 0; i < attempts; i++) {
        ...
        dest_node =
            ai().level_graph().check_position_in_direction(src_node, vertex_position, new_pos);
        restrictions().remove_border();
