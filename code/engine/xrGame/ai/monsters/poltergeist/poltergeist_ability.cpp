...
void CPolterSpecialAbility::load(LPCSTR section) {
    ...
    m_sound_base.create(pSettings->r_string(section, "Sound_Idle"), st_Effect,
...
void CPolterSpecialAbility::on_die() {
    ...
    m_object->PlayParticles(m_particles_death, particles_position, Fvector().set(0.0f, 1.0f, 0.0f),
                            TRUE, FALSE);
...
void CPolterSpecialAbility::on_hit(SHit* pHDS) {
    if (m_object->g_Alive() && (pHDS->hit_type == ALife::eHitTypeFireWound) &&
        ...
        if (Level().ObjectSpace.RayPick(position, dir, TRACE_DISTANCE, collide::rqtStatic, l_rq,
            ...
            if (l_rq.range < TRACE_DISTANCE) {
                ...
                SGameMtlPair* mtl_pair =
                    GMLib.GetMaterialPair(material().self_material_idx(), pTri->material);
                if (!mtl_pair)
                    continue;
                ...
                if (!mtl_pair->CollideSounds.empty()) {
                    ...
                    pos.mad(position, dir,
                            ((l_rq.range - 0.1f > 0) ? l_rq.range - 0.1f : l_rq.range));
