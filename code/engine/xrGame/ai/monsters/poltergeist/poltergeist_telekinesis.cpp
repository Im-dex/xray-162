...
void CPolterTele::load(LPCSTR section) {
    ...
    m_pmt_object_min_mass =
        READ_IF_EXISTS(pSettings, r_float, section, "Tele_Object_Min_Mass", 40.f);
    m_pmt_object_max_mass =
        READ_IF_EXISTS(pSettings, r_float, section, "Tele_Object_Max_Mass", 500.f);
    ...
    m_pmt_time_to_wait_in_objects =
        READ_IF_EXISTS(pSettings, r_u32, section, "Tele_Delay_Between_Objects_Time", 500);
    ...
    m_pmt_time_object_keep =
        READ_IF_EXISTS(pSettings, r_u32, section, "Tele_Time_Object_Keep", 10000);
    ...
    m_pmt_object_collision_damage =
        READ_IF_EXISTS(pSettings, r_float, section, "Tele_Collision_Damage", 0.5f);
    ::Sound->create(m_sound_tele_hold, pSettings->r_string(section, "sound_tele_hold"), st_Effect,
                    SOUND_TYPE_WORLD);
    ::Sound->create(m_sound_tele_throw, pSettings->r_string(section, "sound_tele_throw"), st_Effect,
                    SOUND_TYPE_WORLD);
...
void CPolterTele::update_schedule() {
    ...
    case eStartRaiseObjects:
        if (m_time + m_time_next < time()) {
            ...
            m_time = time();
            m_time_next = m_pmt_raise_time_to_wait_in_objects / 2 +
    ...
    case eFireObjects:
        if (m_time + m_time_next < time()) {
            ...
            m_time = time();
            m_time_next =
                m_pmt_time_to_wait_in_objects / 2 + Random.randI(m_pmt_time_to_wait_in_objects / 2);
        }
...
void CPolterTele::tele_find_objects(std::vector<CObject*>& objects, const Fvector& pos) {
    m_nearest.clear();
...
bool CPolterTele::tele_raise_objects() {
    ...
    std::vector<CObject*> tele_objects;
    ...
    //	CTelekinesis::activate		(obj, m_pmt_tele_raise_speed, m_pmt_tele_object_height,
    //m_pmt_tele_time_object_keep, rotate);
    //}
...
void CPolterTele::tele_fire_objects() {
    for (u32 i = 0; i < m_object->CTelekinesis::get_objects_total_count(); i++) {
        ...
        if ((tele_object.get_state() == TS_Raise) || (tele_object.get_state() == TS_Keep)) {
            ...
            VERIFY(hobj);
            hobj->set_collision_hit_callback(
                new SCollisionHitCallback(hobj, m_pmt_object_collision_damage));
