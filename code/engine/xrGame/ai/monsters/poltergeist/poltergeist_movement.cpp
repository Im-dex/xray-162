...
void CPoltergeisMovementManager::move_along_path(CPHMovementControl* movement_control,
    ...
    float dist =
        desirable_speed * time_delta; // пройденное расстояние в соостветствие с желаемой скоростью
    ...
    while (detail().curr_travel_point_index() < detail().path().size() - 2) {
        ...
        float cur_point_dist_to_next_point =
            detail().path()[detail().curr_travel_point_index()].position.distance_to(
                detail().path()[detail().curr_travel_point_index() + 1].position);
