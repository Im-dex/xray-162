...
void CPolterFlame::load(LPCSTR section) {
    ...
    sscanf(pSettings->r_string(ppi_section, "color_base"), "%f,%f,%f",
           &m_scan_effector_info.color_base.r, &m_scan_effector_info.color_base.g,
           &m_scan_effector_info.color_base.b);
    sscanf(pSettings->r_string(ppi_section, "color_gray"), "%f,%f,%f",
           &m_scan_effector_info.color_gray.r, &m_scan_effector_info.color_gray.g,
           &m_scan_effector_info.color_gray.b);
    sscanf(pSettings->r_string(ppi_section, "color_add"), "%f,%f,%f",
           &m_scan_effector_info.color_add.r, &m_scan_effector_info.color_add.g,
    ...
    m_scan_sound.create(pSettings->r_string(section, "flame_scan_sound"), st_Effect,
                        SOUND_TYPE_WORLD);
    //-----------------------------------------------------------------------------------------
...
void CPolterFlame::select_state(SFlameElement* elem, EFlameState state) {
    ...
    case eFire:
        ...
        elem->particles_object =
            m_object->PlayParticles(m_particles_fire, elem->position, elem->target_dir, FALSE);
        break;
    case eStop:
        // stop fire particles
...
void CPolterFlame::update_schedule() {
        ...
        case eFire:
            if (elem->time_started + m_time_fire_play < time())
                ...
                if (elem->time_last_hit + m_hit_delay < time()) {
                    ...
                    if (Level().ObjectSpace.RayPick(elem->position, elem->target_dir, m_length,
    ...
    m_flames.erase(std::remove_if(m_flames.begin(), m_flames.end(), remove_predicate()),
                   m_flames.end());

    bool const detected =
        m_object->get_current_detection_level() >= m_object->get_detection_success_level();
    ...
    if (m_object->g_Alive() && enemy && m_flames.size() < m_count &&
        ...
        if ((dist < m_pmt_aura_radius) &&
            m_object->control().path_builder().accessible(enemy->Position())) {
            // check timing
...
bool CPolterFlame::get_valid_flame_position(const CObject* target_object, Fvector& res_pos) {
    ...
    float angle = ai().level_graph().vertex_high_cover_angle(Obj->ai_location().level_vertex_id(),
                                                             PI_DIV_6, std::less<float>());
