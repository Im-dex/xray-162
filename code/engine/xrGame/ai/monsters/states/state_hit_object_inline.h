...
bool CStateMonsterHitObjectAbstract::check_start_conditions() {
    ...
    Level().ObjectSpace.GetNearest(m_nearest_objects, object->Position(), object->Radius() - 0.5f,
                                   object());

    std::vector<CObject*>::iterator B = m_nearest_objects.begin();
    std::vector<CObject*>::iterator E = m_nearest_objects.end();

    for (std::vector<CObject*>::iterator I = B; I != E; I++) {
