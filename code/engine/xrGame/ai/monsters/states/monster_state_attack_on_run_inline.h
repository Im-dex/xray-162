...
void ATTACK_ON_RUN_STATE::choose_next_atack_animation() {
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        m_animation_hit_time[i] =
            this->object->anim().get_animation_hit_time(animation_types[i], m_animation_index[i]);
    }
}
...
void ATTACK_ON_RUN_STATE::calculate_predicted_enemy_pos() {
    ...
    m_predicted_enemy_pos =
        enemy_pos + m_predicted_enemy_velocity * self2enemy_time * prediction_factor;
...
void ATTACK_ON_RUN_STATE::update_aim_side() {
    ...
    aim_side const new_attack_side =
        (self_dir.x * self_to_enemy.z - self_dir.z * self_to_enemy.x > 0) ? right : left;

    TTime const update_side_period =
        (TTime)(this->object->get_attack_on_move_update_side_period() * 1000);
...
void ATTACK_ON_RUN_STATE::update_movement_target() {
    ...
    if (m_reach_old_target) {
        ...
        Fvector const predicted2part =
            cr_fvector3(predicted2self.x * cos_alpha - predicted2self.z * sin_alpha, 0,
        ...
        float const sin_alpha =
            (attack_radius / self2predicted_mag) * (m_attack_side == right ? -1.f : 1.f);

        Fvector const self2atack_point =
            cr_fvector3(self2predicted.x * cos_alpha - self2predicted.z * sin_alpha, 0,
        ...
        Fvector dir2target =
            Fvector().crossproduct(self2predicted, cr_fvector3(0, 1, 0)).normalize();
#ifdef DEBUG_STATE
    ...
    u32 const target_vertex =
        ai().level_graph().check_position_in_direction(enemy_vertex, enemy_vertex_pos, m_target);
...
void ATTACK_ON_RUN_STATE::update_attack() {
    if (m_attacking) {
        ...
        for (ENEMIES_MAP::const_iterator it = memory.begin(); it != memory.end(); ++it) {
            ...
            // text_tree.add_line					("see_enemy_now",
            // see_enemy_now);
        ...
        if (can_attack) {
            ...
            MotionID motion;
            EMotionAnim const anim =
                m_attack_side == left ? eAnimAttackOnRunLeft : eAnimAttackOnRunRight;
            [[maybe_unused]]
