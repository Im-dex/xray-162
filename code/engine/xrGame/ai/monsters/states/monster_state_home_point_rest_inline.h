...
void CStateMonsterRestMoveToHomePointAbstract::execute() {
    object->path().set_target_point(ai().level_graph().vertex_position(m_target_node),
                                    m_target_node);
    object->anim().accel_activate(
        EAccelType(object->Home->is_aggressive() ? eAT_Aggressive : eAT_Calm));
