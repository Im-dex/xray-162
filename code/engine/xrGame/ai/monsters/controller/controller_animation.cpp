...
void CControllerAnimation::reinit() {
    ...
    m_man->animation().add_anim_event(m_torso[eTorsoPsyAttack], _pmt_psy_attack_time,
...
void CControllerAnimation::add_path_rotation(ELegsActionType action, float angle,
...
void CControllerAnimation::set_path_direction() {
    float cur_yaw = Fvector()
                        .sub(m_controller->custom_dir().get_head_look_point(), m_object->Position())
                        .getH();
