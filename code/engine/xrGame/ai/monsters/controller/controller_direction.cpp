...
void CControllerDirection::assign_bones() {
    ...
    if (!m_controller->PPhysicsShell()) { //нельзя ставить колбеки, если создан физ шел - у него
                                          //стоят свои колбеки!!!
...
void CControllerDirection::head_look_point(const Fvector& look_point) {
    ...
    float dy =
        xr::abs(angle_normalize_signed(dir_yaw - cur_yaw)); // дельта, на которую нужно поворачиваться
    ...
    float target_dy = xr::abs(bone_angle_head + bone_angle_torso);
    if (fis_zero(target_dy))
        ...
        bone_speed =
            _pmt_min_speed +
            _pmt_rotation_speed *
                (xr::abs((x_spine.cur_yaw + x_head.cur_yaw) - (bone_angle_head + bone_angle_torso)) /
    ...
    //// установить параметры вращения по pitch (более упрощеная схема, без расчета скорости
    ///вращения)
    // bone_angle_head		= _pmt_head_bone_limit	/ (_pmt_head_bone_limit + _pmt_torso_bone_limit) *
    // dir_pitch;  bone_angle_torso	= _pmt_torso_bone_limit / (_pmt_head_bone_limit +
    // _pmt_torso_bone_limit) * dir_pitch;

    // clamp				(bone_angle_head,	-_pmt_head_bone_limit,
    // _pmt_head_bone_limit);
    // clamp				(bone_angle_torso,	-_pmt_torso_bone_limit,
    // _pmt_torso_bone_limit);
    //
