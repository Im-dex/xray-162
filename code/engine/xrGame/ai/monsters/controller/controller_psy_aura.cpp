...
void CControllerAura::update_schedule() {
    ...
    if ((dist_to_actor > aura_radius + FAKE_MIN_ADD_DIST) &&
        (dist_to_actor < aura_radius + FAKE_MAX_ADD_DIST)) {
        ...
        if (m_time_fake_aura == 0) {
            ...
            if (active()) {
                ...
                // check to start
                if (m_time_fake_aura < time()) {
                    m_effector = xr_new<CPPEffectorControllerAura>(m_state, 5000, aura_sound.left,
                                                                   aura_sound.right);
...
void CControllerAura::load(LPCSTR section) {
    ...
    aura_sound.left.create(pSettings->r_string(section, "PsyAura_SoundLeftPath"), st_Effect,
                           sg_SourceType);
    aura_sound.right.create(pSettings->r_string(section, "PsyAura_SoundRightPath"), st_Effect,
                            sg_SourceType);
    ...
    m_time_fake_aura_duration =
        READ_IF_EXISTS(pSettings, r_u32, section, "PsyAura_Fake_Duration", 3000);
    ...
    m_fake_min_add_dist =
        READ_IF_EXISTS(pSettings, r_float, section, "PsyAura_Fake_MinAddDist", 20.f);
