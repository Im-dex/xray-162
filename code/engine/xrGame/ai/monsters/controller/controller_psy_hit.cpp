...
void CControllerPsyHit::play_anim() {
    SControlAnimationData* ctrl_anim =
...
void CControllerPsyHit::death_glide_start() {
    ...
    Actor()->Cameras().AddCamEffector(new CControllerPsyHitCamEffector(
...
void CControllerPsyHit::update_frame() {
    ...
    //		m_sound_state = ePull;
    //		monster->m_sound_tube_pull.play_at_pos(Actor(), Fvector().set(0.f, 0.f, 0.f),
    //sm_2D);
    //	}
    //}
}
...
void CControllerPsyHit::set_sound_state(ESoundState state) {
    ...
    if (state == ePrepare) {
        ...
        // monster->m_sound_tube_hit_left.play_at_pos(Actor(), Fvector().set(-1.f, 0.f, 1.f),
        // sm_2D);  monster->m_sound_tube_hit_right.play_at_pos(Actor(), Fvector().set(1.f, 0.f, 1.f),
        // sm_2D);
    } else if (state == eNone) {
        if (monster->m_sound_tube_start._feedback())
            monster->m_sound_tube_start.stop();
