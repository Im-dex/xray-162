...
vec random_vec() {
    ...
    return normalize(cr_fvector3(local::random_component(), local::random_component(),
...
vec wander::calc_acceleration() {
    ...
    const float cosa = std::cos(m_wander_angle);
    const float sina = std::sin(m_wander_angle);
...
vec containment::calc_acceleration() {
    ...
    for (params::Probes::iterator i = m_p_params->probes.begin(), e = m_p_params->probes.end();
        ...
        const vec probe = cr_fvector3(dotproduct(local_probe, right), dotproduct(local_probe, up),
...
vec grouping::calc_acceleration() {
    ...
    for (m_p_params->first_nearest(cur_nearest); !m_p_params->nomore_nearest();
        ...
        if (point2pos_mag < m_p_params->max_separate_range) {
            ...
            steer = steer +
                    pos2dest_norm * calc_dist_factor(m_p_params->separation_factor, point2pos_mag);
        }
    ...
    if (pos2center_mag > detail::near_zero) {
        steer = steer + normalize(pos2center) *
                            calc_dist_factor(m_p_params->cohesion_factor, pos2center_mag);
    }
...
void manager::remove_scheduled() {
    for (Behaviours::iterator i = m_schedule_remove.begin(), e = m_schedule_remove.end(); i != e;
         ++i) {
