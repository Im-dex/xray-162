...
public:
        IC bool							operator()
(const CSE_ALifeInventoryItem *tpALifeInventoryItem1, const CSE_ALifeInventoryItem
*tpALifeInventoryItem2)  const
        {
                return
                ...
                if (tpALifeInventoryItem1->m_dwCost == tpALifeInventoryItem2->m_dwCost)
                        if (tpALifeInventoryItem1->m_tPreviousParentID == m_tParentID)
                                if (tpALifeInventoryItem2->m_tPreviousParentID == m_tParentID)
                                        return
(tpALifeInventoryItem1->base()->ID < tpALifeInventoryItem2->base()->ID); else return
(true); else if (tpALifeInventoryItem2->m_tPreviousParentID == m_tParentID) return
(false); else return				(tpALifeInventoryItem1->base()->ID <
tpALifeInventoryItem2->base()->ID); else return
(tpALifeInventoryItem1->m_dwCost > tpALifeInventoryItem2->m_dwCost);
        }
};
/**/
...
{
        Msg
("%s[%d]",tpALifeHumanAbstract->name_replace(),tpALifeHumanAbstract->m_dwMoney); ITEM_P_IT
I = tpItemVector.begin(); ITEM_P_IT			E = tpItemVector.end(); for ( ; I != E; ++I)
...
{
        Msg
("%s[%d]",tpALifeHumanAbstract->name_replace(),tpALifeHumanAbstract->m_dwMoney); OBJECT_IT
I = tpALifeHumanAbstract->children.begin(); OBJECT_IT			E =
...
OBJECT_VECTOR &tpBlockedItems, ITEM_P_VECTOR &tpItems, int i, int &j)
{
        sort
...
smart_cast<CSE_ALifeInventoryItem*>(objects().object(*I));
                ...
                R_ASSERT			(tpALifeHumanAbstract->brain().m_dwTotalMoney >=
...
ITEM_P_VECTOR &tpItemVector, ITEM_P_VECTOR &tpOwnItems)
{
        ITEM_P_IT				I = tpItemVector.begin();
        ITEM_P_IT				E = tpItemVector.end();
        for ( ; I != E; ++I)
//		if ((std::find(tpOwnItems.begin(),tpOwnItems.end(),*I) != tpOwnItems.end()) &&
tpALifeHumanAbstract->bfCanGetItem(*I)) if
(std::binary_search(tpOwnItems.begin(),tpOwnItems.end(),*I)) tpALifeHumanAbstract->attach(*I,true);
}
...
*tpALifeTraderAbstract1, CSE_ALifeTraderAbstract *tpALifeTraderAbstract2, OBJECT_VECTOR
&tpObjectVector) #else void
CALifeCommunicationManager::vfAttachGatheredItems(CSE_ALifeTraderAbstract *tpALifeTraderAbstract1,
OBJECT_VECTOR &tpObjectVector) #endif
{
...
(l_tpALifeInventoryItem->m_tPreviousParentID != tpALifeTraderAbstract1->base()->ID) { R_ASSERT
(l_tpALifeInventoryItem->m_tPreviousParentID == tpALifeTraderAbstract2->base()->ID);
...
int iItemCount, ITEM_P_VECTOR &tpItemVector)
{
        tpItemVector.clear	();
...
#ifdef KEEP_SORTED
                                INT_IT I =
std::lower_bound(tpSums.begin(),tpSums.end(),iCurrentSum); if ((tpSums.end() == I) || (*I !=
iCurrentSum)) { tpSums.insert	(I,iCurrentSum); if (tpSums.size() >= SUM_COUNT_THRESHOLD) return;
                                }
#else
...
bool CALifeCommunicationManager::bfGetItemIndexes	(ITEM_P_VECTOR &tpTrader, int iSum1,
INT_VECTOR &tpIndexes, SSumStackCell *tpStack, int iStartI, int iStackPointer)
{
...
bool CALifeCommunicationManager::bfCheckForInventoryCapacity(CSE_ALifeHumanAbstract
*tpALifeHumanAbstract1, ITEM_P_VECTOR &tpTrader1, INT_VECTOR &tpIndexes1, CSE_ALifeHumanAbstract
...
bool CALifeCommunicationManager::bfCheckForInventoryCapacity(CSE_ALifeHumanAbstract
*tpALifeHumanAbstract1, ITEM_P_VECTOR &tpTrader1, int iSum1, int iMoney1, CSE_ALifeHumanAbstract
*tpALifeHumanAbstract2, ITEM_P_VECTOR &tpTrader2, int iSum2, int iMoney2, int iBalance)
{
        INT_VECTOR				l_tpIndexes1, l_tpIndexes2;
        int						l_iStartI1 = 0, l_iStackPointer1 = 0,
l_iStartI2 = 0, l_iStackPointer2 = 0; for (;;) { l_tpIndexes1.clear	();
                
                if (iSum1)
                        if
(!bfGetItemIndexes(tpTrader1,iSum1,l_tpIndexes1,m_tpStack1,l_iStartI1,l_iStackPointer1)) return
(false);
                ...
                for (;;) {
                        ...
                        if (iSum2 &&
!bfGetItemIndexes(tpTrader2,iSum2,l_tpIndexes2,m_tpStack2,l_iStartI2,l_iStackPointer2)) return
(false);
...
",tpALifeHumanAbstract1->name_replace());
                                ...
                                if (iSum1) {
                                        for (int i=0, n=l_tpIndexes1.size(); i<n ;++i)
                                                S1		+=
xr_sprintf(S1,"%3d",l_tpIndexes1[i]);
                                }
                        }
#endif
...
#endif
                                R_ASSERT	(int(tpALifeHumanAbstract1->m_dwMoney) >= iBalance +
iSum2 - iSum1); tpALifeHumanAbstract1->m_dwMoney -= iBalance + iSum2 - iSum1;
...
",tpALifeHumanAbstract2->name_replace());
                                ...
                                if (iSum2) {
                                        for (int i=0, n=l_tpIndexes2.size(); i<n ;++i)
                                                S1		+=
xr_sprintf(S1,"%3d",l_tpIndexes2[i]);
                                }
                        }
#endif
...
iBalance - iSum2); tpALifeHumanAbstract1->m_dwMoney += iSum1 - iBalance - iSum2;
...
*tpALifeHumanAbstract1, ITEM_P_VECTOR &tpTrader1, INT_VECTOR &tpSums1, int iMoney1,
CSE_ALifeHumanAbstract *tpALifeHumanAbstract2, ITEM_P_VECTOR &tpTrader2, INT_VECTOR &tpSums2, int
iMoney2, int iBalance)
{
...
bool CALifeCommunicationManager::bfCheckIfCanNullTradersBalance(CSE_ALifeHumanAbstract
...
",tpALifeHumanAbstract1->name_replace(),tpALifeHumanAbstract1->m_dwMoney); for (int i=0,
n=m_tpTrader1.size(); i<n; ++i) S1			+=
xr_sprintf(S1,"%6d",m_tpTrader1[i]->m_dwCost); Msg				("%s",S);
                        }
                        {
                                string4096		S;
                                char			*S1 = S;
                                S1				+= xr_sprintf(S1,"%s [%5d]:
",tpALifeHumanAbstract2->name_replace(),tpALifeHumanAbstract2->m_dwMoney); for (int i=0,
n=m_tpTrader2.size(); i<n; ++i) S1			+=
xr_sprintf(S1,"%6d",m_tpTrader2[i]->m_dwCost); Msg				("%s",S);
                        }
                ...
                if (psAI_Flags.test(aiALife)) {
                        {
                                ...
                                char			*S1 = S;
                                S1				+= xr_sprintf(S1,"%s :
",tpALifeHumanAbstract1->name_replace()); INT_IT			I = m_tpSums1.begin();
                        ...
                        }
                        {
                                ...
                                char			*S1 = S;
                                S1				+= xr_sprintf(S1,"%s :
",tpALifeHumanAbstract2->name_replace()); INT_IT			I = m_tpSums2.begin();
...
m_tpTrader1, m_tpSums1, tpALifeHumanAbstract1->m_dwMoney, tpALifeHumanAbstract2, m_tpTrader2,
m_tpSums2, tpALifeHumanAbstract2->m_dwMoney, _abs(iBalance))); else return
(bfCheckForTrade(tpALifeHumanAbstract2, m_tpTrader2, m_tpSums2, tpALifeHumanAbstract2->m_dwMoney,
tpALifeHumanAbstract1, m_tpTrader1, m_tpSums1, tpALifeHumanAbstract1->m_dwMoney, iBalance));
}

void CALifeCommunicationManager::vfAppendBlockedItems(CSE_ALifeHumanAbstract *tpALifeHumanAbstract,
OBJECT_VECTOR &tpObjectVector1, OBJECT_VECTOR &tpObjectVector2, int l_iItemCount1)
{
...
void CALifeCommunicationManager::vfPerformTrading(CSE_ALifeHumanAbstract *tpALifeHumanAbstract1,
...
#endif
        tpALifeHumanAbstract1->brain().m_dwTotalMoney = dwfComputeItemCost(m_tpItems1) +
tpALifeHumanAbstract1->m_dwMoney; tpALifeHumanAbstract2->brain().m_dwTotalMoney =
...
tpALifeHumanAbstract2->brain().m_dwTotalMoney)) break;
...
- l_iItemCount1);
                ...
                if (l_iItemCount1*l_iItemCount2) {
                        OBJECT_IT			I = tpALifeHumanAbstract1->children.end() -
l_iItemCount1, J; OBJECT_IT			E = tpALifeHumanAbstract1->children.end(); for ( ; I
!= E; ++I) { J				= std::find(tpALifeHumanAbstract2->children.end() -
l_iItemCount2,tpALifeHumanAbstract2->children.end(),*I); if (tpALifeHumanAbstract2->children.end()
!= J) { if
...
remove_if(m_temp_item_vector.begin(),m_temp_item_vector.end(),CRemoveAttachedItemsPredicate());
                        ...
                        if (!m_tpBlockedItems1.empty())
                                if (!m_tpBlockedItems2.empty()) {
                                        ...
                                        tpALifeHumanAbstract1->children.resize(tpALifeHumanAbstract1->children.size()
- l_iItemCount1); tpALifeHumanAbstract2->children.resize(tpALifeHumanAbstract2->children.size() -
l_iItemCount2); k = 3;
                                }
                                else {
                                        tpALifeHumanAbstract1->children.resize(tpALifeHumanAbstract1->children.size()
...
- l_iItemCount2); k = 2; vfAssignItemParents	(tpALifeHumanAbstract1,l_iItemCount1);
                                ITEM_P_IT			I =
remove_if(m_temp_item_vector.begin(),m_temp_item_vector.end(),CRemoveAttachedItemsPredicate());
                                m_temp_item_vector.erase(I,m_temp_item_vector.end());
                        }
...
//	VERIFY				(tpALifeHumanAbstract2->check_inventory_consistency());
        ...
        int					l_iItemCount1 =
tpALifeHumanAbstract1->children.size(); int					l_iItemCount2 =
tpALifeHumanAbstract2->children.size();
...
(tpALifeHumanAbstract1,m_tpItems1); vfRestoreItems	(tpALifeHumanAbstract2,m_tpItems2);
        }
...
void CALifeCommunicationManager::communicate_with_customer(CSE_ALifeHumanAbstract
...
E = l_tpALifeAbstractGroup->m_tpMembers.end(); for ( ; I != E; ++I) communicate_with_customer
(smart_cast<CSE_ALifeHumanAbstract*>(objects().object(*I)),tpALifeTrader); return;
        ...
        if (psAI_Flags.test(aiALife)) {
                Msg									("Selling
all the items to %s",tpALifeTrader->name_replace());
        }
#endif
...
E = tpALifeHumanAbstract->children.end(); for ( ; I != E; ++I) { CSE_ALifeInventoryItem
*l_tpALifeInventoryItem = smart_cast<CSE_ALifeInventoryItem*>(objects().object(*I));
                        CSE_ALifeItemPDA				*pda =
smart_cast<CSE_ALifeItemPDA*>(l_tpALifeInventoryItem); if (pda && (pda->m_original_owner ==
tpALifeHumanAbstract->ID)) { VERIFY						(!original_pda);
                                original_pda				= pda;
                        }
...
(tpALifeTrader->children,m_temp_item_vector);
        ...
        for (int i=0; i<8; ++i) {
                int l_iItemCount = 0;
                vfRunFunctionByIndex
(tpALifeHumanAbstract,m_tpBlockedItems1,m_temp_item_vector,i,l_iItemCount); if (l_iItemCount) {
                        vfAssignItemParents
(tpALifeHumanAbstract,l_iItemCount); ITEM_P_IT						I =
m_temp_item_vector.begin(); ITEM_P_IT						E =
m_temp_item_vector.end(); for ( ; I != E; ++I) { if (!(*I)->attached()) continue; OBJECT_IT
J =
...
tpALifeHumanAbstract->brain().m_dwTotalMoney; tpALifeHumanAbstract->m_dwMoney			=
tpALifeHumanAbstract->brain().m_dwTotalMoney; tpALifeHumanAbstract->brain().m_dwTotalMoney	=
u32(-1);

        R_ASSERT2
(int(tpALifeTrader->m_dwMoney) >= 0,"Trader must have enough money to pay for the artefacts!");
...
(tpALifeTrader,tpALifeHumanAbstract,m_tpBlockedItems2); #else vfAttachGatheredItems
...
(I != tpALifeTrader->children.end()); tpALifeTrader->detach
