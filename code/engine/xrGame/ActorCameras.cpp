...
void dbg_draw_viewport(const T& cam_info, float _viewport_near) {
    ...
    const Fvector near_plane_center =
        Fvector().mad(cam_info.Position(), cam_info.Direction(), _viewport_near);
    float h_w, h_h;
...
IC void get_cam_oob(Fvector& bc, Fvector& bd, Fmatrix33& mat, const Fmatrix& xform,
                    const SRotation& r_torso, float alpha, float radius, float c) {
...
IC void get_cam_oob(Fvector& bd, Fmatrix& mat, const Fmatrix& xform, const SRotation& r_torso,
        ...
        // if (tri_count)
        {
            ...
            if (test_point(pt, mat, ext, this)) {
                ...
                if (!fis_zero(r_torso.roll))
                    da *= r_torso.roll / xr::abs(r_torso.roll);

                float angle = 0.f;
                for (; xr::abs(angle) < xr::abs(alpha); angle += da) {
                    Fvector pt;
...
void CActor::cam_Update(float dt, float fFOV) {
    ...
    if (GamePersistent().GameType() != eGameIDSingle && ik_cam_shift &&
        ...
        if (xr::abs(y_shift - current_ik_cam_shift) > ik_cam_shift_tolerance) {

            cam_smooth_k = 1.f - ik_cam_shift_speed * dt / 0.01f;
        }

        if (xr::abs(y_shift) < ik_cam_shift_tolerance / 2.f)
    ...
    fCurAVelocity =
        vPrevCamDir.sub(cameras[eacFirstEye]->vDirection).magnitude() / Device.fTimeDelta;
