...
void CCoverManager::compute_static_cover() {
    ...
    xr_delete(m_covers);
    m_covers =
        new CPointQuadTree(ai().level_graph().header().box(),
    ...
    for (u32 i = 0; i < n; ++i) {
        ...
        if (vertex.high_cover(0) + vertex.high_cover(1) + vertex.high_cover(2) +
            vertex.high_cover(3)) {
    ...
    for (u32 i = 0; i < n; ++i)
        if (m_temp[i] && critical_cover(i))
            m_covers->insert(new CCoverPoint(
                ai().level_graph().vertex_position(ai().level_graph().vertex(i)), i));
...
void CCoverManager::remove_nearby_covers(smart_cover::cover const& cover,
    ...
    for (; I != E; ++I) {
        ...
        m_nearest.erase(
            std::remove_if(m_nearest.begin(), m_nearest.end(), smart_cover::predicate(object)),
            m_nearest.end());
...
CCoverManager::Cover const* CCoverManager::add_smart_cover(LPCSTR table_name,
                                                           smart_cover::object const& object,
                                                           bool const& is_combat_cover,
                                                           bool const& can_fire,
                                                           luabind::object const& loopholes) const {
    Cover* smart_cover = new Cover(object, m_smart_covers_storage->description(table_name),
...
CCoverManager::Cover* CCoverManager::smart_cover(shared_str const& cover_id) const {
    ...
    SmartCovers::iterator found = std::lower_bound(m_smart_covers.begin(), m_smart_covers.end(),
