...
bool CDetailPathManager::valid() const {
    ...
    if (m_state_patrol_path)
        b = b && (fis_zero(m_path[m_last_patrol_point].position.distance_to_xz_sqr(
...
bool CDetailPathManager::valid(const Fvector& position) const { return xr::valid(position); }
...
u32 CDetailPathManager::location_on_path(const CGameObject* object, float distance,
