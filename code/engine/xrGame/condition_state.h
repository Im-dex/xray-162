...
protected:
    std::vector<COperatorCondition> m_conditions;
    u32 m_hash;

public:
    IC CConditionState();
    virtual ~CConditionState();
    IC const std::vector<COperatorCondition>& conditions() const;
    ...
    IC void add_condition(typename std::vector<COperatorCondition>::const_iterator& J,
    ...
    IC const COperatorCondition*
    property(const typename COperatorCondition::_condition_type& condition) const;
};
