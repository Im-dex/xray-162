...
void CLevel::cl_Process_Event(u16 dest, u16 type, NET_Packet& P) {
    ...
    if (type != GE_DESTROY_REJECT) {
        if (type == GE_DESTROY) {
            ...
            //			{
            // = GameObject.cpp (210)
            //				Msg( "! ERROR (Level): GE_DESTROY arrived to object[%d][%s], that has
            //parent[%d][%s], frame[%d]", 					GO->ID(),
            //GO->cNameSect().c_str(),
...
#ifdef DEBUG
/*
                        for	(OBJECTS_LIST_it OIt = pObjects4CrPr.begin(); OIt !=
   pObjects4CrPr.end(); OIt++)
                        {
                                CGameObject* pObj = *OIt;
                                if (!pObj) continue;
                                pObj->PH_Ch_CrPr();
...
void CLevel::GetGameDateTime(u32& year, u32& month, u32& day, u32& hours, u32& mins, u32& secs,
                             u32& milisecs) {
