...
class FS_file_list {
    std::vector<LPSTR>* m_p;

public:
    FS_file_list(std::vector<LPSTR>* p) : m_p(p) {}
...
struct FS_item {
    ...
    LPCSTR ModifDigitOnly() {
        ...
        time_t t = modif;
        newtime = localtime(&t);
        xr_sprintf(buff, "%02d/%02d/%4d %02d:%02d", newtime->tm_mday, newtime->tm_mon + 1,
    ...
    module(L)
        [class_<FS_item>("FS_item")
         ...
         class_<CLocatorAPI>("FS")
             ...
             .enum_("FS_List")[value("FS_ListFiles", int(FS_ListFiles)),
             ...
             .def("exist",
                  (const CLocatorAPI::file* (CLocatorAPI::*)(LPCSTR, LPCSTR))(&CLocatorAPI::exist))
