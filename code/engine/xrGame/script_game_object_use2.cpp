...
void CScriptGameObject::bloodsucker_drag_jump(CScriptGameObject* e, LPCSTR e_str,
                                              const Fvector& position, float factor) {
...
CScriptSoundInfo CScriptGameObject::GetSoundInfo() {
    ...
    if (l_tpMonster) {
        if (l_tpMonster->SoundMemory.IsRememberSound()) {
            ...
            ret_val.set((pO && !pO->getDestroy()) ? pO->lua_game_object() : 0, bDangerous,
                        se.position, se.power, int(se.time));
        }
    } else {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError,
...
CScriptMonsterHitInfo CScriptGameObject::GetMonsterHitInfo() {
    ...
    if (l_tpMonster) {
        if (l_tpMonster->HitMemory.is_hit()) {
            CGameObject* pO =
                smart_cast<CGameObject*>(l_tpMonster->HitMemory.get_last_hit_object());
