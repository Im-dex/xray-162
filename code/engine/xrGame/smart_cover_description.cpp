...
void description::load_loopholes(shared_str const& table_id) {
    ...
    VERIFY2(result,
            make_string("bad or missing loopholes table in smart_cover [%s]", table_id.c_str()));
    ...
    for (; I != E; ++I) {
        ...
        VERIFY(std::find_if(m_loopholes.begin(), m_loopholes.end(), id_predicate(*loophole)) ==
               m_loopholes.end());
        m_loopholes.push_back(loophole);
    }

    VERIFY2(!m_loopholes.empty(),
            make_string("smart_cover [%s] doesn't have loopholes", m_table_id.c_str()));
    VERIFY2(std::find_if(m_loopholes.begin(), m_loopholes.end(), usable_predicate()) !=
                m_loopholes.end(),
...
void description::process_loopholes() {
    ...
    VERIFY2(std::find_if(m_loopholes.begin(), m_loopholes.end(), enterable_predicate()) !=
                m_loopholes.end(),
            make_string("smart_cover [%s] doesn't have enterable loopholes", m_table_id.c_str()));

    VERIFY2(std::find_if(m_loopholes.begin(), m_loopholes.end(), exitable_predicate()) !=
                m_loopholes.end(),
...
IC void delete_data(
    const CGraphAbstract<_data_type, _edge_weight_type, _vertex_id_type, _edge_data_type>& graph_) {
...
loophole const* description::loophole(shared_str const& loophole_id) const {
    ...
    Loopholes::const_iterator found =
        std::find_if(m_loopholes.begin(), m_loopholes.end(), id_predicate(loophole_id));
