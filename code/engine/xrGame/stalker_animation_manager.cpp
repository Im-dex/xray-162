...
CStalkerAnimationManager::CStalkerAnimationManager(CAI_Stalker* object)
    : m_object(object), m_global(object), m_head(object), m_torso(object), m_legs(object),
      m_script(object), m_start_new_script_animation(false) {}
...
void CStalkerAnimationManager::reload() {
    ...
    u16 bone_part =
        m_skeleton_animated->LL_GetMotionDef(m_data_storage->m_head_animations.A.front())
            ->bone_or_part;
...
void CStalkerAnimationManager::play_fx(float power_factor, int fx_index) {
    VERIFY(fx_index >= 0);
    VERIFY(fx_index < (int)m_data_storage->m_part_animations.A[object().movement().body_state()]
                          .m_global.A[0]
                          .A.size());
#ifdef DEBUG
        ...
        LPCSTR name = m_skeleton_animated
                          ->LL_MotionDefName_dbg(
                              m_data_storage->m_part_animations.A[object().movement().body_state()]
                                  ...
                                  .A[fx_index])
                          .first;
        Msg("%6d [%s][%s][%s][%f]", Device.dwTimeGlobal, *object().cName(), "FX", name,
            power_factor);
    }
#endif
