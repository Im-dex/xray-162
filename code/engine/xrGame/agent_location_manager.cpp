...
IC CAgentLocationManager::CDangerLocationPtr
CAgentLocationManager::location(const Fvector& position) {
...
bool CAgentLocationManager::suitable(CAI_Stalker* object, const CCoverPoint* location,
    ...
    for (; I != E; ++I) {
        ...
        if (!(*I)->cover()) {
            ...
            if ((*I)->object().Position().distance_to_sqr(location->position()) <= xr::sqr(5.f))
                return (false);
        ...
        if ((*I)->cover()->m_position.distance_to_sqr(location->position()) <= xr::sqr(5.f))
    ...
    if (use_enemy_info) {
        ...
        for (; I != E; ++I)
            if ((*I).m_enemy_position.distance_to_sqr(location->position()) <
...
void CAgentLocationManager::remove_old_danger_covers() {
    m_danger_locations.erase(std::remove_if(m_danger_locations.begin(), m_danger_locations.end(),
