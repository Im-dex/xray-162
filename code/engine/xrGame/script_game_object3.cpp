...
const CCoverPoint* CScriptGameObject::best_cover(const Fvector& position,
                                                 const Fvector& enemy_position, float radius,
...
const CCoverPoint* CScriptGameObject::safe_cover(const Fvector& position, float radius,
                                                 float min_distance) {
    CAI_Stalker* stalker = smart_cast<CAI_Stalker*>(&object());
    if (!stalker) {
        ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,
...
void CScriptGameObject::add_animation(LPCSTR animation, bool hand_usage,
    ...
    if (stalker->animation().global_selector()) {
        ai().script_engine().script_log(eLuaMessageTypeError,
                                        "Cannot add animation [%s]: global selector is set for "
                                        "object [%s], in_smart_cover returned [%s]!",
                                        animation, stalker->cName().c_str(),
...
void CScriptGameObject::add_animation(LPCSTR animation, bool hand_usage, Fvector position,
    ...
    stalker->animation().add_script_animation(animation, hand_usage, position, rotation,
                                              local_animation);
}
...
void CScriptGameObject::set_patrol_path(LPCSTR path_name,
    ...
    if (!stalker)
        ...
        stalker->movement().patrol().set_path(path_name, patrol_start_type, patrol_route_type,
                                              random);
}
...
void CScriptGameObject::set_dest_level_vertex_id(u32 level_vertex_id) {
    ...
    if (!stalker)
        ...
        if (!stalker->movement().restrictions().accessible(level_vertex_id)) {
            ai().script_engine().script_log(
                ScriptStorage::eLuaMessageTypeError,
                "! you are trying to setup destination for the stalker %s, which is not accessible "
                "by its restrictors in[%s] out[%s]",
                stalker->cName().c_str(),
    ...
    if (!stalker)
        ...
        if (fsimilar(desired_direction->magnitude(), 0.f))
            ...
            if (!fsimilar(desired_direction->magnitude(), 1.f))
                ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,
                                                "CAI_Stalker : [%s] set_desired_direction - you "
                                                "passed non-normalized direction!",
                                                stalker->cName().c_str());
        }
...
void CScriptGameObject::set_sight(CScriptGameObject* object_to_look, bool torso_look,
                                  bool fire_object, bool no_pitch) {
    CAI_Stalker* stalker = smart_cast<CAI_Stalker*>(&object());
    if (!stalker)
        ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,
                                        "CSightManager : cannot access class member set_sight!");
        ...
        stalker->sight().setup(
            CSightAction(&object_to_look->object(), torso_look, fire_object, no_pitch));
}
...
CScriptGameObject* CScriptGameObject::GetObjectByName(LPCSTR caObjectName) const {
    ...
    if (l_tpInventoryOwner) {
        CInventoryItem* l_tpInventoryItem =
            l_tpInventoryOwner->inventory().GetItemFromInventory(caObjectName);
...
bool CScriptGameObject::weapon_strapped() const {
    ...
    //	Msg					( "[%6d][%s] weapon_strapped = %s", Device.dwTimeGlobal,
    //stalker->cName().c_str(), result ? "true" : "false" );
    return (result);
}
...
bool CScriptGameObject::weapon_unstrapped() const {
    ...
    //	Msg					( "[%6d][%s] weapon_unstrapped = %s", Device.dwTimeGlobal,
    //stalker->cName().c_str(), result ? "true" : "false" );
    return (result);
}
...
void CScriptGameObject::sell_condition(CScriptIniFile* ini_file, LPCSTR section) {
    ...
    inventory_owner->trade_parameters().process(CTradeParameters::action_sell(0), *ini_file,
                                                section);
}
...
void CScriptGameObject::buy_condition(CScriptIniFile* ini_file, LPCSTR section) {
    ...
    inventory_owner->trade_parameters().process(CTradeParameters::action_buy(0), *ini_file,
                                                section);
}
...
void CScriptGameObject::show_condition(CScriptIniFile* ini_file, LPCSTR section) {
    ...
    inventory_owner->trade_parameters().process(CTradeParameters::action_show(0), *ini_file,
                                                section);
}
...
bool CScriptGameObject::is_weapon_going_to_be_strapped(CScriptGameObject const* object) const {
    if (!object) {
        ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,
                                        "CGameObject : cannot access class member "
