...
void moving_objects::fill_nearest_moving(moving_object* object) {
    ...
    m_nearest_moving.erase(std::remove_if(m_nearest_moving.begin(), m_nearest_moving.end(),
    ...
    m_nearest_moving.erase(std::set_difference(temp, temp + size, m_visited_emitters.begin(),
                                               m_visited_emitters.end(), m_nearest_moving.begin()),
                           m_nearest_moving.end());
}
...
//(m_nearest_moving.begin(),m_nearest_moving.end());
    ...
    std::inplace_merge(m_collision_emitters.begin(), m_collision_emitters.begin() + size,
                       m_collision_emitters.end());
}

bool moving_objects::exchange_all(moving_object* previous, moving_object* next,
...
bool moving_objects::fill_collisions(moving_object* object, const Fvector& object_position,
    ...
    for (; I != E; ++I) {
        ...
        if (priority) {
            if (!collided_dynamic(object, object_position, (*I),
                                  (*I)->predict_position(time_to_check), action))
                continue;
            ...
            if (!collided_dynamic((*I), (*I)->predict_position(time_to_check), object,
        ...
        m_collisions.push_back(std::make_pair(
            time_to_check, std::make_pair(action, std::make_pair(priority ? object : (*I),
                                                                 !priority ? object : (*I)))));
...

    {
        ...
        COLLISION_TIME* collisions =
            (COLLISION_TIME*)_alloca(collision_count * sizeof(COLLISION_TIME));
...
void moving_objects::generate_collisions(moving_object* object) {
    ...
    for (u32 i = 0; (i < step_count) && !m_nearest_moving.empty(); ++i, remove_already_waited()) {
        ...
        if (!fill_collisions(object, object->predict_position(i * step_to_check),
                             i * step_to_check))
            return;
    }
}
