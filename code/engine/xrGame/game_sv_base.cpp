...
/*ClientID				game_sv_GameState::get_it_2_id				(u32
it)
{
        xrClientData*	C	= (xrClientData*)m_server->client_Get		(it);
        if (0==C){
    ...
    if (l_tpIniFile->section_exist(type_name()))
        if (l_tpIniFile->r_string(type_name(), "script"))
            ai().script_engine().add_script_process(
                ...
                new CScriptProcess("game", l_tpIniFile->r_string(type_name(), "script")));
        else
            ai().script_engine().add_script_process(ScriptEngine::eScriptProcessorGame,
                                                    new CScriptProcess("game", ""));
...
void game_sv_GameState::ReadOptions(shared_str& options) {
    g_sv_base_dwRPointFreezeTime =
        get_option_i(*options, "rpfrz", g_sv_base_dwRPointFreezeTime / 1000) * 1000;
...
void game_sv_GameState::Update() {
    ...
    if (Level().game) {
        CScriptProcess* script_process =
            ai().script_engine().script_process(ScriptEngine::eScriptProcessorGame);
        if (script_process)
...
void game_sv_GameState::OnHit(u16 id_hitter, u16 id_hitted, NET_Packet& P) {
    ...
    //	CSE_ALifeCreatureActor*		a_hitter		= smart_cast <CSE_ALifeCreatureActor*>
    //(e_hitter);
...
void game_sv_GameState::OnEvent(NET_Packet& tNetPacket, u16 type, u32 time, ClientID sender) {
    ...
    case GAME_EVENT_ON_HIT: {
        ...
        if (!e_src) // && !IsGameTypeSingle() added by andy because of Phantom does not have server
                    // entity
        {
            if (IsGameTypeSingle())
                break;
    ...
    case GAME_EVENT_CREATE_PLAYER_STATE: {
        ...
        R_ASSERT2(
            CL,
            make_string("M_CREATE_PLAYER_STATE: client 0x%08x not found", sender.value()).c_str());
...
void game_sv_GameState::AddDelayedEvent(NET_Packet& tNetPacket, u16 type, u32 time,
    ...
    default: { m_event_queue->CreateSafe(tNetPacket, type, time, sender); } break;
    }
}
...
void game_sv_GameState::MapRotation_ListMaps() {
    ...
    for (u32 i = 0; i < m_pMapRotation_List.size(); i++) {
        ...
        if (i == 0)
            Msg("~   %d. %s (%s) (current)", i + 1, st.translate(R.map_name).c_str(),
                R.map_name.c_str());
        else
...
void game_sv_GameState::OnRender(){
    ...
    Fvector V0, V1;
    u32 TeamColors[TEAM_COUNT] = {D3DCOLOR_XRGB(255, 0, 0), D3DCOLOR_XRGB(0, 255, 0),
D3DCOLOR_XRGB(0, 0, 255), D3DCOLOR_XRGB(255, 255, 0)};
//	u32 TeamColorsDist[TEAM_COUNT] = {color_argb(128, 255, 0, 0), color_argb(128, 0, 255, 0),
color_argb(128, 0, 0, 255), color_argb(128, 255, 255, 0)};
...
(p_it); if (!PS) continue; if (PS->testFlag(GAME_PLAYER_FLAG_VERY_VERY_DEAD)) continue; CObject*
pPlayer = Level().Objects.net_Find(PS->GameID); if (!pPlayer) continue;
