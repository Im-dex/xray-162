...
EGameActions get_binded_action(int _dik) {
    for (int idx = 0; idx < bindings_count; ++idx) {
        ...
        bool b_is_group_matching =
            is_group_matching(binding->m_action->key_group, g_current_keygroup);
    ...
    virtual void Execute(LPCSTR args) {
        ...
        for (int idx = 0; idx < bindings_count; ++idx) {
            ...
            xr_sprintf(buff, "[%s] primary is[%s] secondary is[%s]",
                       ...
                       (pbinding->m_keyboard[0]) ? pbinding->m_keyboard[0]->key_local_name.c_str()
                                                 : "NULL",
                       (pbinding->m_keyboard[1]) ? pbinding->m_keyboard[1]->key_local_name.c_str()
                                                 : "NULL");
            Log(buff);
        }
        Log("- --- Bind list end   ---");
