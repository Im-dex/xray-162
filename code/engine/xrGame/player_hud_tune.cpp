...
void attachable_hud_item::tune(Ivector values) {
    ...
    if (hud_adj_mode == 3 || hud_adj_mode == 4) {
        ...
        if ((values.x) || (values.y) || (values.z)) {
            ...
            Msg("item_position				= %f,%f,%f", m_measures.m_item_attach[0].x,
                m_measures.m_item_attach[0].y, m_measures.m_item_attach[0].z);
            Msg("item_orientation			= %f,%f,%f", m_measures.m_item_attach[1].x,
                m_measures.m_item_attach[1].y, m_measures.m_item_attach[1].z);
            Log("-----------");
        }
    }
    ...
    if (hud_adj_mode == 5 || hud_adj_mode == 6 || hud_adj_mode == 7) {
        ...
        if ((values.x) || (values.y) || (values.z)) {
            ...
            Msg("fire_point				= %f,%f,%f",
                m_measures.m_fire_point_offset.x, m_measures.m_fire_point_offset.y,
    ...
    if (hud_adj_mode == 1 || hud_adj_mode == 2) {
        ...
        if ((values.x) || (values.y) || (values.z)) {
            if (idx == 0) {
                ...
                Msg("[%s]", m_attached_items[hud_adj_item_idx]->m_sect_name.c_str());
                Msg("aim_hud_offset_pos%s				= %f,%f,%f",
                    (is_16x9) ? "_16x9" : "", pos_.x, pos_.y, pos_.z);
                Msg("aim_hud_offset_rot%s				= %f,%f,%f",
                    (is_16x9) ? "_16x9" : "", rot_.x, rot_.y, rot_.z);
                Log("-----------");
            } else if (idx == 2) {
                Msg("[%s]", m_attached_items[hud_adj_item_idx]->m_sect_name.c_str());
                Msg("gl_hud_offset_pos%s				= %f,%f,%f",
                    (is_16x9) ? "_16x9" : "", pos_.x, pos_.y, pos_.z);
                Msg("gl_hud_offset_rot%s				= %f,%f,%f",
                    (is_16x9) ? "_16x9" : "", rot_.x, rot_.y, rot_.z);
                Log("-----------");
            }
        }
...
void hud_draw_adjust_mode() {
    ...
    if (pInput->iGetAsyncKeyState(DIK_LSHIFT) && hud_adj_mode)
        _text = "press SHIFT+NUM 0-return 1-hud_pos 2-hud_rot 3-itm_pos 4-itm_rot 5-fire_point "
                "6-fire_2_point 7-shell_point 8-pos_step 9-rot_step";
