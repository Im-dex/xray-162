...
inline BONE::bone(CGameObject* object, LPCSTR animation_id, bool animation_start,
                  Fvector const& target, LPCSTR (&bones)[bone_count])
...
void BONE::compute_bone(u32 const bone_id) {
    ...
    if (bone_id < bone_count - 1) {
        aim_at_position(m_bones[bone_id].c, m_bones[bone_count - 1].c, m_bones[bone_count - 1].k,
                        m_result[bone_id]);
        return;
    }
