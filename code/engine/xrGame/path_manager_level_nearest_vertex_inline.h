...
#define CNearestVertexPathManager                                                      \
    CPathManager<CLevelGraph, _DataStorage,                                            \
                 SNearestVertex<_dist_type, _index_type, _iteration_type>, _dist_type, \
...
IC bool CNearestVertexPathManager::is_goal_reached(const _index_type& node_index) {
    ...
    float current_distance =
        m_target_position.distance_to_xz_sqr(this->graph->vertex_position(this->best_node));
...
TEMPLATE_SPECIALIZATION
IC _dist_type CNearestVertexPathManager::evaluate(const _index_type& node_index1,
...
IC bool CNearestVertexPathManager::is_limit_reached(const _iteration_type iteration_count) const {
    ...
    return ((iteration_count >= this->max_iteration_count) ||
