...
manager::manager(CAI_Stalker* object)
    : m_object(object), m_current_cover(0), m_target_cover(0), m_current_loophole(0),
      m_target_loophole(0), m_current_transition(0), m_cover_id(""), m_default_behaviour(false) {
    VERIFY(object);
    invalidate_fire_position();
    m_transitions_processor = xr_new<manager::transition_processor>();
}

manager::~manager() { xr_delete(m_transitions_processor); }
...
void manager::loophole_path(cover const& cover, shared_str const& source_raw,
...
IC loophole const& manager::get_loophole_by_id(cover const& cover,
...
manager::transition_action const& manager::transition(cover const& cover,
...
void manager::build_enter_path() {
    ...
    for (; I != E; ++I) {
        ...
        smart_cover::loophole const& loophole =
            this->get_loophole_by_id(*m_target_cover, loophole_id);
...
void manager::actualize_path() {
    ...
    shared_str current_loophole_id =
        transform_vertex(m_current_loophole ? m_current_loophole->id() : "", true);
    shared_str target_loophole_id =
        transform_vertex(m_target_loophole ? m_target_loophole->id() : "", false);
    loophole_path(*m_current_cover, current_loophole_id, target_loophole_id, m_path);
