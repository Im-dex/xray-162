...
CSE_ALifeSchedulable *tpALifeSchedulable, GameGraph::_GRAPH_ID tGraphID) : manager(manager),
        ...
        IC	bool operator()	(CALifeGraphRegistry::OBJECT_REGISTRY::_iterator &I, u64
counter, bool) const
        {
        ...
        IC	void operator()	(CALifeGraphRegistry::OBJECT_REGISTRY::_iterator &I, u64
counter) const
        {
...
smart_cast<CSE_ALifeSchedulable*>((*I).second); if (!l_tpALifeSchedulable) return;
...
^ 1],l_iGroupIndex,l_bMutualDetection)) { case eMeetActionTypeAttack : { #ifdef DEBUG if
(psAI_Flags.test(aiALife)) { Msg("[LSS] %s started combat versus
...
^ 1]->base()->name_replace());
                                ...
                                for (int i=0; i<2*int(manager->m_dwMaxCombatIterationCount); ++i) {
                                        if (eCombatActionAttack ==
manager->choose_combat_action(l_iGroupIndex)) { #ifdef DEBUG if (psAI_Flags.test(aiALife)) {
...
eCombatResultRetreat2 : eCombatResultRetreat1; break;
                                                }
...
eCombatResult1Kill2 : eCombatResult2Kill1; break;
                                ...
                                if (psAI_Flags.test(aiALife)) {
                                        if (eCombatResultRetreat12 == l_tCombatResult)
                                                Msg("[LSS] both combat groups decided not to
continue combat");
                                }
#endif
                        ...
                        case eMeetActionTypeInteract : {
                                R_ASSERT2
(l_tpALifeHumanAbstract,"Non-human objects ñannot communicate with each other");
                                CSE_ALifeHumanAbstract	*l_tpALifeHumanAbstract2 =
smart_cast<CSE_ALifeHumanAbstract*>(l_tpALifeSchedulable); R_ASSERT2
(l_tpALifeHumanAbstract2,"Non-human objects ñannot communicate with each other"); #ifdef DEBUG if
(psAI_Flags.test(aiALife)) { Msg					("[LSS] %s interacted with
%s",manager->m_tpaCombatObjects[l_iGroupIndex]->base()->name_replace(),manager->m_tpaCombatObjects[l_iGroupIndex
^ 1]->base()->name_replace());
                                }
#endif
                        ...
                        case eMeetActionTypeIgnore : {
                                ...
                                if (psAI_Flags.test(aiALife)) {
                                        Msg					("[LSS] %s
refused from combat",manager->m_tpaCombatObjects[l_iGroupIndex]->base()->name_replace());
                                }
#endif
                                return;
                        }
...
(smart_zone); VERIFY					(l_tpALifeMonsterAbstract);
