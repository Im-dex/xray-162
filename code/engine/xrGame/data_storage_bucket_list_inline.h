...
#define TEMPLATE_SPECIALIZATION                                                   \
    template <typename _path_id_type, typename _bucket_id_type, u32 bucket_count, \
              bool clear_buckets>                                                 \
...
#define CBucketList                                                      \
    CDataStorageBucketList<_path_id_type, _bucket_id_type, bucket_count, \
                           clear_buckets>::CDataStorage<_data_storage, _vertex>
        ...
        if (!clear_buckets)
            for (++m_min_bucket_id; (m_min_bucket_id < bucket_count) &&
                                    (!m_buckets[m_min_bucket_id] ||
                                     (m_buckets[m_min_bucket_id]->m_path_id != this->current_path_id()) ||
                                     (m_buckets[m_min_bucket_id]->m_bucket_id != m_min_bucket_id));
                 ++m_min_bucket_id)
                ;
        else
            for (++m_min_bucket_id; (m_min_bucket_id < bucket_count) && !m_buckets[m_min_bucket_id];
                 ++m_min_bucket_id)
                ;
...
IC void CBucketList::verify_buckets() const {
    ...
    //			if (!j || (indexes[j->index()].m_path_id != current_path_id()) ||
    //(indexes[j->index()].vertex != j)) 				continue;
...
IC void CBucketList::add_to_bucket(CGraphVertex& vertex, u32 m_bucket_id) {
    ...
    if (!i || (!clear_buckets &&
               ((i->m_path_id != this->current_path_id()) || (i->m_bucket_id != m_bucket_id)))) {
