...
void CScriptGameObject::AddIconedTalkMessage(LPCSTR caption, LPCSTR text, LPCSTR texture_name,
                                             LPCSTR templ_name) {
...
void _give_news(LPCSTR caption, LPCSTR news, LPCSTR texture_name, int delay, int show_time,
                int type);
...
void CScriptGameObject::IterateInventoryBox(luabind::functor<void> functor,
...
void CScriptGameObject::SetGoodwill(int goodwill, CScriptGameObject* pWhoToSet) {
    ...
    RELATION_REGISTRY().SetGoodwill(pInventoryOwner->object_id(), pWhoToSet->object().ID(),
                                    goodwill);
}
...
void CScriptGameObject::ForceSetGoodwill(int goodwill, CScriptGameObject* pWhoToSet) {
    ...
    RELATION_REGISTRY().ForceSetGoodwill(pInventoryOwner->object_id(), pWhoToSet->object().ID(),
                                         goodwill);
}
...
void CScriptGameObject::ChangeGoodwill(int delta_goodwill, CScriptGameObject* pWhoToSet) {
    ...
    RELATION_REGISTRY().ChangeGoodwill(pInventoryOwner->object_id(), pWhoToSet->object().ID(),
                                       delta_goodwill);
}
...
void CScriptGameObject::enable_night_vision(bool value) {
    ...
    if (!torch) {
        ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,
                                        "CTorch : cannot access class member enable_night_vision!");
        return;
    }
...
void CScriptGameObject::RestoreWeapon() {
    ...
    ai().script_engine().script_log(eLuaMessageTypeMessage,
                                    "CScriptGameObject::RestoreWeapon called!!!");
...
#ifdef DEBUG
    ai().script_engine().script_log(eLuaMessageTypeMessage,
                                    "CScriptGameObject::HideWeapon called!!!");
    ai().script_engine().print_stack();
#endif //#ifdef DEBUG
...
void CScriptGameObject::register_door() {
    VERIFY2(!m_door, make_string("object %s has been registered as a door already",
