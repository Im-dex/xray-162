...
struct SPhraseDialogData : CSharedResource {
    ...
    //в окне выбора у актера диалоги будут сортироваться по этому значению от меньшего (снизу) к
    //большему (сверху)
    int m_iPriority;
};
...
public:
    CPhrase* AddPhrase(LPCSTR text, const shared_str& phrase_id, const shared_str& prev_phrase_id,
                       int goodwil_level);
    CPhrase* AddPhrase_script(LPCSTR text, LPCSTR phrase_id, LPCSTR prev_phrase_id,
                              int goodwil_level) {
