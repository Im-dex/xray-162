...
m_listWnd->SetAutoDelete(true); AttachChild					(m_listWnd);
        strconcat					(sizeof(_buff),_buff, _base,
":scroll_view"); CUIXmlInit::InitScrollView	(xml_doc, _buff, 0, m_listWnd);
        ...
        for(u32 i=ALife::eHitTypeBurn; i<= ALife::eHitTypeFireWound; ++i)
        {
                ...
                strconcat				(sizeof(_buff),_buff, _base, ":static_",
_imm_names[i]); CUIXmlInit::InitStatic	(xml_doc, _buff,	0, _s);
        }
...
void CUIOutfitInfo2::SetItem(u32 hitType, bool force_add)
{
        ...
        _val_outfit			= m_outfit ?
m_outfit->GetDefHitTypeProtection(ALife::EHitType(hitType)) : 1.0f;
        _val_outfit			= 1.0f - _val_outfit;


        _val_af				=
smart_cast<CActor*>(Level().CurrentControlEntity())->HitArtefactsOnBelt(1.0f,ALife::EHitType(hitType));
...
//	LPCSTR			_clr_outfit, _clr_af;
        ...
        _sz				+= sprintf_s	(_buff+_sz,sizeof(_buff)-_sz,"%s
%+3.0f%%", (_val_outfit>0.0f)?"%c[green]":"%c[red]", _val_outfit*100.0f);

        if( !fsimilar(_val_af, 0.0f) )
        {
                _sz		+= sprintf_s	(_buff+_sz,sizeof(_buff)-_sz,"%s %+3.0f%%",
(_val_af>0.0f)?"%c[green]":"%c[red]", _val_af*100.0f);
        }
        _s->SetText			(_buff);
