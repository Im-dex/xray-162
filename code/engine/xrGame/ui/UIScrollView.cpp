...
CUIScrollView::CUIScrollView(CUIFixedScrollBar* scroll_bar) {
    ...
    AddCallback(m_VScrollBar, SCROLLBAR_VSCROLL,
...
void CUIScrollView::InitScrollView() {
    ...
    if (tmp_scroll)
        tmp_scroll->InitScrollBar(Fvector2().set(GetWndSize().x, 0.0f), false,
                                  *m_scrollbar_profile);
    else {
        if (!!m_scrollbar_profile)
            ...
            m_VScrollBar->InitScrollBar(Fvector2().set(GetWndSize().x, 0.0f), GetWndSize().y,
                                        false);
    }
    Fvector2 sc_pos = { m_VScrollBar->GetWndPos().x - m_VScrollBar->GetWndSize().x,
...
void CUIScrollView::RecalcSize() {
    ...
    if (m_sort_function) {
        ...
        std::sort(m_pad->GetChildWndList().begin(), m_pad->GetChildWndList().end(),
                  m_sort_function);
    }
...
void CUIScrollView::SetSelected(CUIWindow* w) {
    ...
    for (WINDOW_LIST_it it = m_pad->GetChildWndList().begin(); m_pad->GetChildWndList().end() != it;
         ++it) {
