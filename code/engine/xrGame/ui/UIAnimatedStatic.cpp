...
CUIAnimatedStatic::CUIAnimatedStatic()
    : m_uFrameCount(0), m_uAnimationDuration(0), m_uTimeElapsed(0), m_uAnimCols(0xffffffff),
      m_bCyclic(true), m_bParamsChanged(true), m_uFrameWidth(0), m_uFrameHeight(0),
...
void CUISleepStatic::Update() {
    ...
    m_UIStaticItem.SetSize(
        Fvector2().set(iFloor((end_pixel - start_pixel) * UI().get_current_kx()), 128));
    ...
    if (end_pixel2 > 0) {
        ...
        m_UIStaticItem2.SetPos(m_UIStaticItem.GetPosX() + m_UIStaticItem.GetSize().x,
