...
extern u64 generate_time(u32 years, u32 months, u32 days, u32 hours, u32 minutes, u32 seconds,
                         u32 milliseconds = 0);
extern void split_time(u64 time, u32& years, u32& months, u32& days, u32& hours, u32& minutes,
...
void CUILogsWnd::Init() {
    ...
    AddCallback(m_filter_news, BUTTON_CLICKED,
                CUIWndCallback::void_function(this, &CUILogsWnd::UpdateChecks));
    AddCallback(m_filter_talk, BUTTON_CLICKED,
                CUIWndCallback::void_function(this, &CUILogsWnd::UpdateChecks));
    AddCallback(m_prev_period, BUTTON_CLICKED,
                CUIWndCallback::void_function(this, &CUILogsWnd::PrevPeriod));
    AddCallback(m_next_period, BUTTON_CLICKED,
...
void CUILogsWnd::ReLoadNews() {
    ...
    LPCSTR date_str =
        InventoryUtilities::GetDateAsString(m_selected_period, InventoryUtilities::edpDateToDay)
            .c_str();
    m_period->SetText(date_str);
    Fvector2 pos = m_period_caption->GetWndPos();
    pos.x =
        m_period->GetWndPos().x - m_period_caption->GetWidth() - m_prev_period->GetWidth() - 5.0f;
...
void CUILogsWnd::PerformWork() {
    if (!m_news_in_queue.empty()) {
        const auto count = std::min(size_t(30), m_news_in_queue.size());
