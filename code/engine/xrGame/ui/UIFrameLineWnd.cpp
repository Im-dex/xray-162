...
void CUIFrameLineWnd::InitFrameLineWnd(LPCSTR base_name, Fvector2 pos, Fvector2 size,
                                       bool horizontal) {
...
void CUIFrameLineWnd::InitTexture(LPCSTR texture, LPCSTR sh_name) {
    ...
    CUITextureMaster::InitTexture(strconcat(sizeof(buf), buf, texture, "_b"), sh_name, m_shader,
                                  m_tex_rect[flFirst]);
    CUITextureMaster::InitTexture(strconcat(sizeof(buf), buf, texture, "_e"), sh_name, m_shader,
                                  m_tex_rect[flSecond]);
    if (bHorizontal) {
...
void draw_rect(Fvector2 LTp, Fvector2 RBp, Fvector2 LTt, Fvector2 RBt, u32 clr,
               Fvector2 const& ts) {
    UI().AlignPixel(LTp.x);
    UI().AlignPixel(LTp.y);
    LTp.add(pt_offset);
...
bool CUIFrameLineWnd::inc_pos(Frect& rect, int counter, int i, Fvector2& LTp, Fvector2& RBp,
                              Fvector2& LTt, Fvector2& RBt) {
    ...
    } else // i==flBack
    {
        ...
        bool b_draw_reminder = (bHorizontal) ? (rect.lt.x + m_tex_rect[flBack].width() >
