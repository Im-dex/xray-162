...
void UITaskListWnd::init_from_xml(CUIXml& xml, LPCSTR path) {
    ...
    Register(m_bt_close);
    AddCallback(m_bt_close, BUTTON_DOWN,
...
bool UITaskListWndItem::init_task(CGameTask* task, UITaskListWnd* parent) {
    ...
    m_color_states[stt_activ] =
        CUIXmlInit::GetColor(xml, "second_task_wnd:task_item:activ", 0, (u32)(-1));
    m_color_states[stt_unread] =
        CUIXmlInit::GetColor(xml, "second_task_wnd:task_item:unread", 0, (u32)(-1));
    m_color_states[stt_read] =
        CUIXmlInit::GetColor(xml, "second_task_wnd:task_item:read", 0, (u32)(-1));
    update_view();
    return true;
}
...
void UITaskListWndItem::SendMessage(CUIWindow* pWnd, s16 msg, void* pData) {
    ...
    /*
            if ( pWnd == m_bt_view )
            {
                    ...
                    {
                            GetMessageTarget()->SendMessage( this, PDA_TASK_HIDE_MAP_SPOT,
    (void*)m_task );
    //			Msg( " HIDE task  id = %d", m_task->m_ID );
                    ...
                    if ( !m_bt_view->GetCheck() && msg == BUTTON_CLICKED )
                    {
                            GetMessageTarget()->SendMessage( this, PDA_TASK_SHOW_MAP_SPOT,
    (void*)m_task );
