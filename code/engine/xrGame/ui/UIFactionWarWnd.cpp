...
xml, "static_our_name", this ); m_st_our_frac_info		= UIHelper::CreateStatic( xml,
...
"progress_our_resource", this );

        m_enemy_icon			= UIHelper::CreateStatic( xml, "static_enemy_icon", this );
        m_enemy_icon_over		= UIHelper::CreateStatic( xml, "static_enemy_icon_over",
this ); m_enemy_name			= UIHelper::CreateStatic( xml, "static_enemy_name", this );
        ...
        m_st_enemy_resource		= UIHelper::CreateStatic( xml, "static_enemy_resource",
this );

        m_pb_enemy_state		= UIHelper::CreateProgressBar( xml, "progress_enemy_state",
this ); m_pb_enemy_mem_count	= UIHelper::CreateProgressBar( xml, "progress_enemy_mem_count",
this ); m_pb_enemy_resource		= UIHelper::CreateProgressBar( xml,
"progress_enemy_resource", this );
...
void CUIFactionWarWnd::UpdateInfo()
{
        ...
        if ( m_enemy_faction.get_faction_id2().size() == 0 || m_our_faction.member_count == 0 ||
xr_strlen( m_our_faction.get_name() )==0 )
        {
                ShowInfo( false );
                return;
        }
...
void CUIFactionWarWnd::UpdateWarStates( FactionState const& faction )
{
        ...
        for ( u8 i = 0; i < max_war_state; ++i )
        {
                if ( !m_war_state[i]->UpdateInfo( faction.get_war_state(i),
faction.get_war_state_hint(i) ) )
                {
                        break; // for i
                }
                ++cnt;
