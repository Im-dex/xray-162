    ...
    if (!m_pCurrentDialog) {
        ...
        if (m_pCurrentDialog->IsWeSpeaking(m_pOurDialogManager)) {
            ...
            if (!m_pCurrentDialog->PhraseList().empty() && m_pCurrentDialog->allIsDummy()) {
                CPhrase* phrase =
                    m_pCurrentDialog
                        ->PhraseList()[Random.randI(m_pCurrentDialog->PhraseList().size())];
...
void CUITalkWnd::AddIconedMessage(LPCSTR caption, LPCSTR text, LPCSTR texture_name,
                                  LPCSTR templ_name) {
