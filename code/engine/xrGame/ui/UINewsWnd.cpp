...
void CUINewsWnd::Init(LPCSTR xml_name, LPCSTR start_from)
{
        ...
        UIScrollWnd					=
xr_new<CUIScrollView>();UIScrollWnd->SetAutoDelete(true); AttachChild
(UIScrollWnd); xml_init.InitScrollView		(uiXml, pth, 0, UIScrollWnd);
}
...
void CUINewsWnd::LoadNews()
{
        ...
        if (Actor())
        {
                ...
                for (GAME_NEWS_VECTOR::reverse_iterator it = news_vector.rbegin(); it !=
news_vector.rend() && currentNews < NEWS_TO_SHOW ; ++it)
                {
                        AddNewsItem(*it);
                        ++currentNews;
                }
