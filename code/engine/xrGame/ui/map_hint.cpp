...
void CUIMapLocationHint::SetInfoTask(CGameTask* task) {
    ...
    S->AdjustHeightToText();
    // float new_w						= S->GetWndPos().x + S->GetWndSize().x +
    // 20.0f;

    S = m_info["t_time"];
    S->TextItemControl()->SetText(
        InventoryUtilities::GetTimeAndDateAsString(task->m_ReceiveTime).c_str());
    ...
    if (b_rem) {
        ...
        strconcat(sizeof(buff2), buff2, CStringTable().translate("ui_st_time_remains").c_str(), " ",
                  buff);
...

    {
        ...
        pos.y =
            std::max(pos.y, m_info["t_icon"]->GetWndPos().y + m_info["t_icon"]->GetWndSize().y + 7);
        m_info["t_hint_text"]->SetWndPos(pos);
    }
