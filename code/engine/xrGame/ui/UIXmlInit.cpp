...
Frect CUIXmlInit::GetFRect(CUIXml& xml_doc, LPCSTR path, int index) {
    R_ASSERT4(xml_doc.NavigateToNode(path, index), "XML node not found", path,
              xml_doc.m_xml_file_name);
    Frect rect;
    rect.set(0, 0, 0, 0);
...
bool CUIXmlInit::InitText(CUIXml& xml_doc, LPCSTR path, int index, CUILines* pLines) {
    ...
    pLines->SetTextComplexMode(xml_doc.ReadAttribInt(path, index, "complex_mode", 0) ? true
                                                                                     : false);
...
bool CUIXmlInit::Init3tButton(CUIXml& xml_doc, LPCSTR path, int index, CUI3tButton* pWnd) {
    ...
    pWnd->m_frameline_mode =
        (xml_doc.ReadAttribInt(path, index, "frame_mode", 0) == 1) ? true : false;
...
bool CUIXmlInit::InitDragDropListEx(CUIXml& xml_doc, LPCSTR path, int index,
...
bool CUIXmlInit::InitProgressShape(CUIXml& xml_doc, LPCSTR path, int index,
    ...
    //    InitStatic(xml_doc, strconcat(sizeof(_path),_path, path, ":front"), index,
    //    pWnd->m_pTexture);
...
void CUIXmlInit::InitAutoStaticGroup(CUIXml& xml_doc, LPCSTR path, int index,
...
void CUIXmlInit::InitAutoFrameLineGroup(CUIXml& xml_doc, LPCSTR path, int index,
                                        CUIWindow* pParentWnd) {
...
bool CUIXmlInit::InitAnimatedStatic(CUIXml& xml_doc, const char* path, int index,
...
bool CUIXmlInit::InitSleepStatic(CUIXml& xml_doc, const char* path, int index,
                                 CUISleepStatic* pWnd) {
    R_ASSERT4(xml_doc.NavigateToNode(path, index), "XML node not found", path,
              xml_doc.m_xml_file_name);
