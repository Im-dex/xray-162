...
void CUISequenceSimpleItem::Load(CUIXml* xml, int idx) {
    ...
    m_flags.set(etiGrabInput,
                (0 == _stricmp(str_grab_input, "on") || 0 == _stricmp(str_grab_input, "1")));
...
void CUISequenceSimpleItem::Update() {
    ...
    for (; _I != _E; ++_I) {
        SSubItem& s = *_I;
        bool bPlaying =
            (gt > (_start + s.m_start - EPS)) && (gt < (_start + s.m_start + s.m_length + EPS));
...
void CUISequenceSimpleItem::OnKeyboardPress(int dik) {
    ...
    for (u32 idx = 0; idx < m_actions.size(); ++idx) {
        ...
        if (b) {
            ...
            bool functor_exists =
                ai().script_engine().functor(itm.m_functor.c_str(), functor_to_call);
