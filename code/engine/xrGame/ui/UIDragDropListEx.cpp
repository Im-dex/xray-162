...
CUIDragDropListEx::CUIDragDropListEx() {
    ...
    AddCallbackStr("scroll_v", SCROLLBAR_VSCROLL,
...
void CUIDragDropListEx::OnScrollV(CUIWindow* w, void* pData) {
    m_container->SetWndPos(
        Fvector2().set(m_container->GetWndPos().x, float(-m_vScrollBar->GetScrollPos())));
}
...
void CUIDragDropListEx::OnItemRButtonClick(CUIWindow* w, void* pData) {
    //*	OnItemSelected						(w, pData); // instead call function
    //"SetCurrentItem(itm)";
    CUICellItem* itm = smart_cast<CUICellItem*>(w);
...
void CUICellContainer::PlaceItemAtPos(CUICellItem* itm, Ivector2& cell_pos) {
    ...
    if (!m_pParentDragDropList->GetVirtualCells())
        ...
        if (alignment_vec.x == 1)
            pos.x =
                (m_pParentDragDropList->GetWndSize().x - cs.x * (m_cellSpacing.x + m_cellSize.x)) /
                2;
        ...
        if (alignment_vec.y == 1)
            pos.y =
                (m_pParentDragDropList->GetWndSize().y - cs.y * (m_cellSpacing.y + m_cellSize.y)) /
                2;
...
Ivector2 CUICellContainer::TopVisibleCell() {
    return Ivector2().set(
        0, iFloor(m_pParentDragDropList->ScrollPos() / float(CellSize().y + m_cellSpacing.y)));
}
...
Ivector2 CUICellContainer::PickCell(const Fvector2& abs_pos) {
    ...
    ap.mul(-1);
    res.x = iFloor(ap.x /
                   (m_cellSize.x + m_cellSpacing.x * (m_cellsCapacity.x - 1) / m_cellsCapacity.x));
    res.y = iFloor(ap.y /
                   (m_cellSize.y + m_cellSpacing.y * (m_cellsCapacity.y - 1) / m_cellsCapacity.y));
    if (!ValidCell(res))
        res.set(-1, -1);
    return res;
}
...
void CUICellContainer::Draw() {
    ...
    tgt_cells.x2 = iFloor((float(clientArea.width()) + float(cell_sz.x) - EPS) / float(cell_sz.x)) +
                   tgt_cells.lt.x;
    tgt_cells.y2 =
        iFloor((float(clientArea.height()) + float(cell_sz.y) - EPS) / float(cell_sz.y)) +
        tgt_cells.lt.y;
