...
CUITreeViewItem* CUITreeViewItem::Find(int value) const {
    ...
    for (SubItems::const_iterator it = vSubItems.begin(); it != vSubItems.end(); ++it) {
        ...
        if ((*it)->GetSelectedItem()->GetTAG() == (u32)value)
            pResult = *it;
...
void CreateTreeBranch(shared_str nesting, shared_str leafName, CUIListBox* pListToAdd,
                      int leafProperty, CGameFont* pRootFont, u32 rootColor, CGameFont* pLeafFont,
