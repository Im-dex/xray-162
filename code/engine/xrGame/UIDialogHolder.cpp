...
void CDialogHolder::AddDialogToRender(CUIWindow* pDialog) {
    ...
    bool bAdd = (m_dialogsToRender_new.end() ==
                 std::find(m_dialogsToRender_new.begin(), m_dialogsToRender_new.end(), itm));
    if (!bAdd)
        return;

    bAdd = (m_dialogsToRender.end() ==
...
void CDialogHolder::OnFrame() {
    ...
    if (!m_dialogsToRender_new.empty()) {
        m_dialogsToRender.insert(m_dialogsToRender.end(), m_dialogsToRender_new.begin(),
...
bool CDialogHolder::IR_UIOnKeyboardPress(int dik) {
    ...
    if (dik == MOUSE_1 || dik == MOUSE_2 || dik == MOUSE_3) {
        ...
        EUIMessages action = (dik == MOUSE_1)
                                 ? WINDOW_LBUTTON_DOWN
                                 : (dik == MOUSE_2) ? WINDOW_RBUTTON_DOWN : WINDOW_CBUTTON_DOWN;
...
bool CDialogHolder::IR_UIOnKeyboardRelease(int dik) {
    ...
    if (dik == MOUSE_1 || dik == MOUSE_2 || dik == MOUSE_3) {
        ...
        EUIMessages action = (dik == MOUSE_1)
                                 ? WINDOW_LBUTTON_UP
                                 : (dik == MOUSE_2) ? WINDOW_RBUTTON_UP : WINDOW_CBUTTON_UP;
        if (TIR->OnMouseAction(cp.x, cp.y, action))
            return true;
    }
