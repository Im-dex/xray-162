...
void CAgentExplosiveManager::remove_links(CObject* object) {
    TO_BE_DESTROYED::iterator I =
        std::find(m_explosives_to_remove.begin(), m_explosives_to_remove.end(), object->ID());
...
void CAgentExplosiveManager::register_explosive(const CExplosive* explosive,
                                                const CGameObject* game_object) {
    {
        std::vector<CDangerExplosive>::iterator I =
            std::find(m_explosives.begin(), m_explosives.end(), explosive);
        if (I != m_explosives.end())
            return;
    }
    {
        TO_BE_DESTROYED::iterator I = std::find(m_explosives_to_remove.begin(),
...
bool CAgentExplosiveManager::process_explosive(CMemberOrder& member) {
    ...
    for (; I != E; ++I) {
        ...
        if (dist_sqr < min_dist_sqr) {
            if ((*I).m_reactor && ((*I).m_reactor->Position().distance_to_sqr(
...
void CAgentExplosiveManager::react_on_explosives() {
...

    {
        ...
        m_explosives.erase(
            std::remove_if(m_explosives.begin(), m_explosives.end(), CRemoveExplosivesPredicate()),
            m_explosives.end());
    }
}
