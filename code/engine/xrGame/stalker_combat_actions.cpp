...
void CStalkerActionGetItemToKill::initialize() {
    ...
    object().sight().setup(CSightAction(
        object().m_best_found_item_to_kill ? &object().m_best_found_item_to_kill->object() : 0,
        true));
...
CStalkerActionMakeItemKilling::CStalkerActionMakeItemKilling(CAI_Stalker* object,
                                                             LPCSTR action_name)
    : inherited(object, action_name) {}
...
CStalkerActionRetreatFromEnemy::CStalkerActionRetreatFromEnemy(CAI_Stalker* object,
                                                               LPCSTR action_name)
    : inherited(object, action_name) {}
...
void CStalkerActionRetreatFromEnemy::execute() {
    ...
    if (point) {
        ...
        if (object().memory().visual().visible_now(object().memory().enemy().selected())) {
            ...
            //(eObjectActionFire1,object().best_weapon(),min_queue_size, max_queue_size,
            //min_queue_interval, max_queue_interval);
            fire();
...
CStalkerActionGetReadyToKill::CStalkerActionGetReadyToKill(bool affect_properties,
...
void CStalkerActionGetReadyToKill::initialize() {
    ...
    //	m_movement_type										= Random.randI(2) ? eMovementTypeRun :
    //eMovementTypeWalk; 	m_movement_type
    //= eMovementTypeRun;
...
float current_cover(CAI_Stalker* object) {
    ...
    BOOL result = Level().ObjectSpace.RayPick(position, direction, 10.f, collide::rqtStatic,
                                              ray_query_result, NULL);
...
void CStalkerActionLookOut::execute() {
    ...
    const CCoverPoint* point = ai().cover_manager().best_cover(
        ...
        *object().m_ce_close); //,CStalkerMovementRestrictor(m_object,true,false));
    if (!point ||
        (point->position().similar(object().Position()) && object().movement().path_completed())) {
        object().m_ce_close->setup(position, 0.f, 170.f, 10.f);
        point = ai().cover_manager().best_cover(
            object().Position(), 30.f,
            *object().m_ce_close); //,CStalkerMovementRestrictor(m_object,true,false));
    }
    ...
    //(eWorldPropertyLookedOut,true); 		object().movement().set_nearest_accessible_position
    //();
    //	}
...
void CStalkerActionDetourEnemy::execute() {
    ...
    if (object().movement().path_completed()) {
        ...
        const CCoverPoint* point =
            ai().cover_manager().best_cover(object().Position(), 10.f, *object().m_ce_angle,
                                            CStalkerMovementRestrictor(m_object, true));
        if (!point) {
            object().m_ce_angle->setup(position, 10.f, object().ffGetRange(),
                                       mem_object.m_object_params.m_level_vertex_id);
...
CStalkerActionHideFromGrenade::CStalkerActionHideFromGrenade(CAI_Stalker* object,
                                                             LPCSTR action_name)
    : inherited(object, action_name) {}
...
void CStalkerActionCriticalHit::initialize() {
    ...
    if (object().memory().enemy().selected()) {
        ...
        select_queue_params(distance, min_queue_size, max_queue_size, min_queue_interval,
                            max_queue_interval);
        object().CObjectHandler::set_goal(eObjectActionIdle, object().best_weapon(), min_queue_size,
...
CStalkerCombatActionThrowGrenade::CStalkerCombatActionThrowGrenade(CAI_Stalker* object,
                                                                   LPCSTR action_name)
    : inherited(object, action_name) {}
...
void CStalkerCombatActionThrowGrenade::execute() {
    ...
    select_queue_params(distance, min_queue_size, max_queue_size, min_queue_interval,
                        max_queue_interval);
    object().CObjectHandler::set_goal(eObjectActionFire1, &grenade->object(), min_queue_size,
                                      max_queue_size, min_queue_interval, max_queue_interval);
}
...
CStalkerCombatActionSmartCover::CStalkerCombatActionSmartCover(CAI_Stalker* object,
                                                               LPCSTR action_name)
