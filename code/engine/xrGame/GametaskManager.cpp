...
CGameTask* CGameTaskManager::GiveGameTaskToActor(CGameTask* t, u32 timeToComplete,
                                                 bool bCheckExisting, u32 timer_ttl) {
...
void CGameTaskManager::UpdateTasks() {
...

    {
        ...
        Tasks tasks(_alloca(task_count * sizeof(SGameTaskKey)), task_count, GetGameTasks().begin(),
                    GetGameTasks().end());
...
const char* sTaskStates[] = { "eTaskStateFail", "TaskStateInProgress", "TaskStateCompleted",
                              "TaskStateDummy" };
...
void CGameTaskManager::DumpTasks() {
    ...
    for (; it != it_e; ++it) {
        ...
        LogMsg(" ID=[{0}] state=[{1}] prio=[{2}] ", gt->m_ID.c_str(), sTaskStates[gt->GetTaskState()],
            gt->m_priority);
    }
}