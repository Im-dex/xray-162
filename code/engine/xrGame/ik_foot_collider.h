...
public:
    ...
    ik_pick_query(ik_foot_geom::e_collide_point point, const Fvector& pos, const Fvector& dir,
                  float range)
    ...
    IC bool is_equal(const ik_pick_query& q) const {
        VERIFY(q.is_valid());
        // VERIFY( is_valid() );
        return is_valid() && q.point() == point() && fsimilar(q.range(), range()) &&
               q.pos().similar(pos()) && q.dir().similar(dir());
    }
