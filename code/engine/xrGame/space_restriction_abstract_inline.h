...
CSpaceRestrictionAbstract::accessible_neighbour_border(T& restriction, bool out_restriction) {
    ...
    VERIFY2(!m_accessible_neighbour_border.empty(),
            make_string("space restrictor %s has no accessible neighbours (border size[%d])",
                        *name(), border().size()));
    return (m_accessible_neighbour_border);
}

template <typename T>
IC bool CSpaceRestrictionAbstract::accessible_neighbours(T& restriction, u32 level_vertex_id,
...
template <typename T>
IC void CSpaceRestrictionAbstract::prepare_accessible_neighbour_border(T& restriction,
                                                                       bool out_restriction) {
