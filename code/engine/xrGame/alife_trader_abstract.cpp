...
void CSE_ALifeTraderAbstract::spawn_supplies() {
    ...
    CSE_Abstract* abstract = dynamic_object->alife().spawn_item(
        "device_pda", base()->o_Position, dynamic_object->m_tNodeID, dynamic_object->m_tGraphID,
        base()->ID);
...
void CSE_ALifeDynamicObject::attach(CSE_ALifeInventoryItem* tpALifeInventoryItem,
                                    bool bALifeRequest, bool bAddChildren) {
    if (!bALifeRequest)
        return;
    ...
    R_ASSERT2(std::find(children.begin(), children.end(), tpALifeInventoryItem->base()->ID) ==
                  children.end(),
              "Item is already inside the inventory");
    children.push_back(tpALifeInventoryItem->base()->ID);
}

void CSE_ALifeDynamicObject::detach(CSE_ALifeInventoryItem* tpALifeInventoryItem,
                                    ALife::OBJECT_IT* I, bool bALifeRequest, bool bRemoveChildren) {
    ...
    ALife::OBJECT_IT i =
        std::find(children.begin(), children.end(), tpALifeInventoryItem->base()->ID);
...
void add_online_impl(CSE_ALifeDynamicObject* object, const bool& update_registries) {
    ...
    for (; I != E; ++I) {
        ...
        Msg("[LSS][%d] Going online [%d][%s][%d] with parent [%d][%s] on '%s'", Device.dwFrame,
            Device.dwTimeGlobal, l_tpALifeInventoryItem->base()->name_replace(),
            l_tpALifeInventoryItem->base()->ID, object->ID, object->name_replace(), "*SERVER*");
#endif

        //		R_ASSERT3
        //(ai().level_graph().valid_vertex_id(l_tpALifeDynamicObject->m_tNodeID),"Invalid vertex for
        //object ",l_tpALifeInventoryItem->name_replace());
        l_tpALifeDynamicObject->o_Position = object->o_Position;
        l_tpALifeDynamicObject->m_tNodeID = object->m_tNodeID;
        object->alife().server().Process_spawn(tNetPacket, clientID, FALSE,
                                               l_tpALifeInventoryItem->base());
...
void add_offline_impl(CSE_ALifeDynamicObject* object,
    ...
    for (u32 i = 0, n = saved_children.size(); i < n; ++i) {
        ...
        Msg("[LSS][%d] Going offline [%d][%s][%d] with parent [%d][%s] on '%s'", Device.dwFrame,
            Device.dwTimeGlobal, inventory_item->base()->name_replace(), inventory_item->base()->ID,
