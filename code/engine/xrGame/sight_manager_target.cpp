...
bool CSightManager::aim_target(Fvector& my_position, Fvector& aim_target,
    ...
    m_object->XFORM().transform_tiny(
        my_position, Fvector().set(.2f, my_position.y - m_object->Position().y, 0.f));
#else
...
void CSightManager::SetDirectionLook() {
    ...
    //body_orientation = object().movement().body_orientation(); 	orientation.target
...
void CSightManager::SetLessCoverLook(const CLevelGraph::CVertex* tpNode, float fMaxHeadTurnAngle,
                                     bool bDifferenceLook) {
    float fAngleOfView, range, fMaxSquare = -1.f,
                               fBestAngle = object().movement().m_head.target.yaw;
    ...
    if (!bDifferenceLook || !bOk)
        for (float fIncrement = object().movement().m_body.target.yaw - fMaxHeadTurnAngle;
            ...
            float fSquare =
                ai().level_graph().compute_high_square(-fIncrement, fAngleOfView, tpNode);
