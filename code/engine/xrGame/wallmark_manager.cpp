...
void CWalmarkManager::AddWallmark(const Fvector& dir, const Fvector& start_pos, float range,
                                  float wallmark_size, IWallMarkArray& wallmarks_vector, int t) {
    ...
    if (pMaterial->Flags.is(SGameMtl::flBloodmark)) {
        ...
        if (pWallmarkShader)
        {
                ...
                ::Render->add_StaticWallmark(*pWallmarkShader, end_point, wallmark_size, pTri,
        pVerts);
        }
        */
    }
...
void CWalmarkManager::PlaceWallmark(const Fvector& dir, const Fvector& start_pos,
                                                                          float trace_dist, float
wallmark_size, SHADER_VECTOR& wallmarks_vector,CObject* ignore_obj)
{
...
void CWalmarkManager::PlaceWallmarks(const Fvector& start_pos) {
    m_pos = start_pos;
    //.	LPCSTR				sect				= pSettings->r_string(m_owner->cNameSect(),
    //"wallmark_section");
    Load("explosion_marks");

    //.	Device.seqParallel.push_back
    //(fastdelegate::FastDelegate0<>(this,&CWalmarkManager::StartWorkflow));

    StartWorkflow();
}

float Distance(const Fvector& rkPoint, const Fvector rkTri[3], float& pfSParam, float& pfTParam,
               Fvector& closest, Fvector& dir);
    ...
    */
    for (CDB::RESULT* Res = R_begin; Res != R_end; ++Res) {
        ...
        */
        ...
        if (fis_zero(pfSParam) || fis_zero(pfTParam) || fsimilar(pfSParam, 1.0f) ||
            fsimilar(pfTParam, 1.0f)) {
            ++_tri_not_plane;
            continue;
        }
