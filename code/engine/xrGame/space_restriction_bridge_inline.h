...
template <typename T>
IC u32 CSpaceRestrictionBridge::accessible_nearest(T& restriction, const Fvector& position,
                                                   Fvector& result, bool out_restriction) {
#pragma todo("Dima to Dima : _Warning : this place can be optimized in case of a slowdown")
    ...
    VERIFY2(ai().level_graph().valid_vertex_id(selected),
            make_string("vertex_id[%d], object[%s], position[%f][%f][%f]", selected, *name(),
                        VPUSH(position)));

    {
        ...
        for (; I != E; ++I) {
            ...
            float distance_sqr =
                ai().level_graph().vertex_position(current).distance_to_sqr(position);
            if (distance_sqr < min_dist_sqr) {
                min_dist_sqr = distance_sqr;
