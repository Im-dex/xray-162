...
CLevelFogOfWar* CFogOfWarMngr::GetFogOfWar(const shared_str& level_name) {
    if (GameID() != GAME_SINGLE)
        return NULL;
    FOG_STORAGE_IT it = std::find_if(GetFogStorage().begin(), GetFogStorage().end(),
                                     FindFogByLevelName(level_name));
...
void CLevelFogOfWar::Draw() {
    ...
    Frect vis_rect;
    vis_rect.set(clip_rect.lt.x - map_abs_pos.x, clip_rect.lt.y - map_abs_pos.y,
    ...
    u32 vOffset = 0;
    FVF::TL* start_pv =
        (FVF::TL*)RCache.Vertex.Lock(cells.width() * cells.height() * 6, hGeom.stride(), vOffset);
    FVF::TL* pv = start_pv;
        ...
        for (int y = 0; y < cells.height(); ++y) {
            ...
            for (u32 k = 0; k < 6; ++k, ++pv) {
                ...
                pv->set(iFloor(drawLT.x + p.x * (fw) + fw * x) - 0.5f,
                        iFloor(drawLT.y + p.y * (fh) + fh * y) - 0.5f, 0xFFFFFFFF, tp.x + uv.x,
                        tp.y + uv.y);
            }
        }
    }
