...
u32 CAI_Stalker::fill_items(CInventory& inventory, CGameObject* old_owner,
...
void CAI_Stalker::transfer_item(CInventoryItem* item, CGameObject* old_owner,
...
void CAI_Stalker::attach_available_ammo(CWeapon* weapon) {
    ...
    for (; I != E; ++I) {
        ...
        if (std::find(weapon->m_ammoTypes.begin(), weapon->m_ammoTypes.end(),
                      (*I).m_item->object().cNameSect()) == weapon->m_ammoTypes.end())
            continue;
...
bool CAI_Stalker::enough_ammo(const CWeapon* new_weapon) const {
    ...
    for (; I != E; ++I) {
        if (std::find(new_weapon->m_ammoTypes.begin(), new_weapon->m_ammoTypes.end(),
                      (*I)->object().cNameSect()) == new_weapon->m_ammoTypes.end())
            continue;
...
bool CAI_Stalker::conflicted(const CInventoryItem* item, const CWeapon* new_weapon,
...
void CAI_Stalker::remove_personal_only_ammo(const CInventoryItem* item) {
    ...
    for (; I != E; ++I) {
        ...
        for (; i != e; ++i) {
            ...
            if (std::find(temp->m_ammoTypes.begin(), temp->m_ammoTypes.end(), *I) ==
                temp->m_ammoTypes.end())
                continue;
