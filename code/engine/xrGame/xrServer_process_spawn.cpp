...
CSE_Abstract* xrServer::Process_spawn(NET_Packet& P, ClientID sender,
    ...
    if (!E) {
        ...
        if (
            ...
            !E->m_gameType.MatchType((u16)game->Type()) || !E->match_configuration() ||
            !game->OnPreCreate(E)) {
#ifndef MASTER_GOLD
    ...
    // log
    // Msg		("- SERVER: Spawning '%s'(%d,%d,%d) as #%d, on '%s'", E->s_name_replace, E->g_team(),
    // E->g_squad(), E->g_group(), E->ID, CL?CL->Name:"*SERVER*");
    return E;
}
