...
void target_selector::setup(animation_planner* object, CPropertyStorage* storage) {
    ...
    CActionPlanner::m_storage.set_property(eWorldPropertyLookedOut,
                                           m_random.randF() <= .7f ? true : false);
...
void target_selector::add_evaluators() {
    ...
    add_evaluator(eWorldPropertyLoopholeTooMuchTimeFiring,
                  new CPropertyEvaluatorMember<animation_planner>(
                      (CPropertyStorage*)0, eWorldPropertyLoopholeTooMuchTimeFiring, true, true,
    ...
    add_evaluator(eWorldPropertyLoopholeCanFire,
                  new evaluators::is_action_available_evaluator(&object(), "can fire", "fire"));
...
void target_selector::add_actions() {
    ...
    //	add_condition			(action, eWorldPropertyLoopholeCanFireAtEnemy,
    //true); 	add_condition			(action, eWorldPropertyLoopholeCanFire,
    //true); 	add_condition			(action, eWorldPropertyLookedOut,
    //true); 	add_condition			(action, eWorldPropertyLoopholeLastHitWasLongAgo,
    //true);
    ...
    action = new target_fire_no_lookout(&object(), "fire_no_lookout",
                                            eWorldPropertyLoopholeFireNoLookout, 0);
    add_condition(action, eWorldPropertyLoopholeCanFireAtEnemy, true);
