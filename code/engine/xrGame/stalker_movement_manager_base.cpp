...
stalker_movement_manager_base::stalker_movement_manager_base(CAI_Stalker* object)
    : inherited(object), m_last_query_object(0),
      ...
      m_last_query_result(false), m_last_query_distance(flt_max), m_force_update(false) {
    VERIFY(object);
    m_object = object;
    m_velocities = 0;
}
...
void stalker_movement_manager_base::reload(LPCSTR section) {
    ...
    m_velocities =
        &stalker_velocity_holder().collection(pSettings->r_string(section, "movement_speeds"));
    init_velocity_masks();
}
...
void stalker_movement_manager_base::init_velocity_masks() {
    ...
    add_velocity(eVelocityWalkFreePositive,
                 m_velocities->velocity(eMentalStateFree, eBodyStateStand, eMovementTypeWalk,
                                        eMovementDirectionForward),
                 PI * 100.f / 180.f, cf * PI * 100.f / 180.f);
    add_velocity(eVelocityRunFreePositive,
                 m_velocities->velocity(eMentalStateFree, eBodyStateStand, eMovementTypeRun,
                                        eMovementDirectionForward),
    ...
    add_velocity(eVelocityRunPanicStandPositive,
                 m_velocities->velocity(eMentalStatePanic, eBodyStateStand, eMovementTypeRun,
                                        eMovementDirectionForward),
                 PI_DIV_8 / 2, cf * PI_DIV_8 / 2);
...
IC void stalker_movement_manager_base::add_velocity(int mask, float linear, float compute_angular,
                                                    float angular) {
    detail().add_velocity(mask,
                          CDetailPathManager::STravelParams(linear, compute_angular, angular));
}
...
void stalker_movement_manager_base::set_desired_position(const Fvector* position) {
    VERIFY2(!position || accessible(*position) || show_restrictions(&restrictions()),
            *object().cName());
        ...
        if ((movement_params.m_path_type != MovementManager::ePathTypePatrolPath) &&
            ...
            if (!restrictions().accessible(level_path().dest_vertex_id())) {
                ...
                VERIFY2(restrictions().accessible(vertex_position) ||
                        ...
                        make_string("vertex_id[%d],position[%f][%f][%f],object[%s]", vertex_id,
                                    VPUSH(vertex_position), *object().cName()));
    ...
    case eVelocityFree: {
        ...
        if (m_object->brain().current_action_id() ==
            ...
            CStalkerCombatPlanner& planner =
                smart_cast<CStalkerCombatPlanner&>(m_object->brain().current_action());
...
void stalker_movement_manager_base::set_nearest_accessible_position(Fvector desired_position,
                                                                    u32 level_vertex_id) {
    if (!ai().level_graph().inside(level_vertex_id, desired_position))
        desired_position = ai().level_graph().vertex_position(level_vertex_id);
    else
        desired_position.y = ai().level_graph().vertex_plane_y(level_vertex_id, desired_position.x,
                                                               desired_position.z);

    if (!restrictions().accessible(desired_position)) {
        level_vertex_id =
            restrictions().accessible_nearest(Fvector().set(desired_position), desired_position);
    ...
    VERIFY2(restrictions().accessible(level_vertex_id) || show_restrictions(&restrictions()),
            *object().cName());
    set_level_dest_vertex(level_vertex_id);

    VERIFY2(restrictions().accessible(desired_position) || show_restrictions(&restrictions()),
            *object().cName());
...
float stalker_movement_manager_base::speed(const EMovementDirection& movement_direction) {
    ...
    return (
        m_velocities->velocity(mental_state(), body_state(), movement_type(), movement_direction));
}
...
bool stalker_movement_manager_base::is_object_on_the_way(const CGameObject* object,
...
IC float distance_to_line(const Fvector& p0, const Fvector& p1, const Fvector& p2) {
    ...
    float sin_alpha = std::sqrt(1.f - xr::sqr(cos_alpha));
    return (p0p2_magnitude * sin_alpha);
}

void stalker_movement_manager_base::update_object_on_the_way(const CGameObject* object,
                                                             const float& distance) {
    if (!actual())
        return;
...
void stalker_movement_manager_base::check_for_bad_path(stalker_movement_params& movement_params) {
    ...
    Fvector current_direction =
        Fvector().sub(path[point_index + 1].position, path[point_index].position);
