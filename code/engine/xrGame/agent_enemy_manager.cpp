...
void CAgentEnemyManager::fill_enemies() {
...

    {
        ...
        for (; I != E; ++I) {
            ...
            (*I)->object().memory().fill_enemies(
                CEnemyFiller(&m_enemies, object().member().mask(&(*I)->object())));
        }
    }
    ...
    if (!m_only_wounded_left && m_is_any_wounded) {
        enemies().erase(
            std::remove_if(enemies().begin(), enemies().end(), remove_wounded_predicate()),
            enemies().end());
    }
        ...
        for (; I != E; ++I) {
            ...
            for (; i != e; ++i) {
                if ((*I)->selected_enemy() == *i)
                    continue;
                float my_distance =
                    (*I)->object().Position().distance_to(m_enemies[*i].m_object->Position());
                if (my_distance < best) {
                    ...
                    for (; J; J &= J - 1) {
                        ...
                        std::vector<u32>::iterator ii = std::find(
                            (*j)->enemies().begin(), (*j)->enemies().end(), (*I)->selected_enemy());
                        ...
                        float my_to_his =
                            evaluate(&(*I)->object(), m_enemies[(*j)->selected_enemy()].m_object);
                        float his_to_my =
                            evaluate(&(*j)->object(), m_enemies[(*I)->selected_enemy()].m_object);
...
template <typename T>
IC void CAgentEnemyManager::setup_mask(std::vector<T>& objects, CMemberEnemy& enemy,
                                       const squad_mask_type& non_combat_members) {
    std::vector<T>::iterator I = std::find(objects.begin(), objects.end(), enemy.m_object->ID());
    if (I != objects.end()) {
...
void CAgentEnemyManager::assign_wounded() {
    ...
    WOUNDED_ENEMY* previous_wounded =
        (WOUNDED_ENEMY*)_alloca(previous_wounded_count * sizeof(WOUNDED_ENEMY));
    ...
    while (population(assigned) < combat_member_count) {
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            for (; I != E; ++I) {
                ...
                for (; J; J &= J - 1) {
                    ...
                    CAgentMemberManager::iterator i = object().member().member(K);
                    float distance_sqr =
                        (*i)->object().Position().distance_to_sqr((*I).m_object->Position());
            ...
            Msg("combat members(%d):", object().member().combat_members().size());
            {
                ...
                for (; I != E; ++I)
                    Msg("  [%s][0x%08x][0x%08x]", *(*I)->object().cName(),
                        object().member().mask(&(*I)->object()), (*I)->selected_enemy());
            }
        }
#endif
        ...
        //		Msg							("wounded enemy [%s] is assigned
        //to member [%s]",*enemy->m_object->cName(),*processor->cName());
    ...
    //	Msg								("[%6d] assigned =
    //%x",Device.dwTimeGlobal,assigned); 	ENEMIES::iterator				I =
    //m_enemies.begin(); 	ENEMIES::iterator				E = m_enemies.end();
    //	for ( ; I != E; ++I)
    //		Msg							("[%6d] [%s] =
...
void CAgentEnemyManager::remove_links(CObject* object) {
    m_wounded.erase(std::remove_if(m_wounded.begin(), m_wounded.end(), wounded_predicate(object)),
                    m_wounded.end());
}
...
void CAgentEnemyManager::wounded_processor(const CEntityAlive* object,
                                           const ALife::_OBJECT_ID& wounded_processor_id) {
    VERIFY(std::find_if(m_wounded.begin(), m_wounded.end(), find_wounded_predicate(object)) ==
           m_wounded.end());
