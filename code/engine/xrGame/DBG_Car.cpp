...
void CCar::DbgCreatePlots() {
    ...
    //////////////////////////////
    int y_pos = 0, y_w = 100;
    m_dbg_power_rpm.Init(CFunctionGraph::type_function(this, &CCar::Parabola), m_min_rpm, m_max_rpm,
                         0, y_pos, 500, y_w, 1000, D3DCOLOR_XRGB(0, 0, 255));
    ...
    m_dbg_torque_rpm.Init(CFunctionGraph::type_function(this, &CCar::TorqueRpmFun), m_min_rpm,
    ...
    xr_vector<Fvector>::iterator i = m_gear_ratious.begin() + 1, e = m_gear_ratious.end();
    for (; i < e; i++) {
        m_dbg_dynamic_plot->AddMarker(CStatGraph::stHor, (*i)[1] / rpm_pow_max_ratio,
                                      D3DCOLOR_XRGB(127, 0, 0));
        m_dbg_dynamic_plot->AddMarker(CStatGraph::stHor, (*i)[2] / rpm_pow_max_ratio,
...
void CCar::DbgUbdateCl() {
    ...
    if (m_pPhysicsShell && OwnerActor() &&
        ...
        if (ph_dbg_draw_mask.test(phDbgDrawCarDynamics)) {
            ...
            UI().Font().pFontStat->SetColor(D3DCOLOR_XRGB(255, !b_transmission_switching * 255,
                                                          !b_transmission_switching * 255));
            UI().Font().pFontStat->OutNext("Transmission num:      [%d]",
                                           m_current_transmission_num);
            UI().Font().pFontStat->SetColor(color_rgba(0xff, 0xff, 0xff, 0xff));
            UI().Font().pFontStat->OutNext("gear ratio:			  [%3.2f]",
                                           m_current_gear_ratio);
            UI().Font().pFontStat->OutNext("Power:      [%3.2f]",
                                           m_current_engine_power / (0.8f * 1000.f));
            UI().Font().pFontStat->OutNext("rpm:      [%3.2f]",
