...
void CBlackGraviArtefact::net_Relcase(CObject* O) {
    ...
    // for vector
    auto I = std::remove_if(m_GameObjectList.begin(), m_GameObjectList.end(),
...
void CBlackGraviArtefact::feel_touch_delete(CObject* O) {
    ...
    if (pGameObject && !pArtefact) {
        m_GameObjectList.erase(
            std::find(m_GameObjectList.begin(), m_GameObjectList.end(), pGameObject));
    }
}
...
void CBlackGraviArtefact::GraviStrike() {
    ...
    for (auto it = m_GameObjectList.begin(); m_GameObjectList.end() != it; ++it) {
        ...
        float impulse =
            100.f * m_fStrikeImpulse * (1.f - (distance / m_fRadius) * (distance / m_fRadius));

        if (impulse > .001f) {
            //?			BOOL		enabled = getEnabled();
            //?			setEnabled	(FALSE);
            impulse *=
                CExplosive::ExplosionEffect(rq_storage, NULL, pGameObject, Position(), m_fRadius);
            //?			setEnabled	(enabled);
        }
