...
void CPurchaseList::process(CInifile& ini_file, LPCSTR section, CInventoryOwner& owner) {
    ...
    auto E = S.Data.cend();
    for (; I != E; ++I) {
        VERIFY3((*I).second.size(), "PurchaseList : cannot handle lines in section without values",
                section);
...
void CPurchaseList::process(const CGameObject& owner, const shared_str& name, const u32& count,
    ...
    VERIFY3(!fis_zero(probability, EPS_S), "Invalid probability for section in the purchase list",
            *name);
    ...
    m_deficits.insert(
        std::make_pair(name, (float)count * probability / std::max((float)j, min_deficit_factor)));
}
