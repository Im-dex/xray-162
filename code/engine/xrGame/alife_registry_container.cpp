...
struct CRegistryHelperLoad {
    ...
    IC static void process(T1* self, T2& p1) {
        do_load<object_type_traits::is_base_and_derived<IPureLoadableObject<T2>, Head>::value>(self,
                                                                                               p1);
    }
};
...
struct CRegistryHelperSave {
    ...
    IC static void process(T1* self, T2& p1) {
        do_save<object_type_traits::is_base_and_derived<IPureSavableObject<T2>, Head>::value>(self,
                                                                                              p1);
    }
};

template <template <typename _1, typename _2, typename _3> class helper, typename T1, typename T2,
          typename TList>
...
void CALifeRegistryContainer::load(IReader& file_stream) {
    ...
    CRegistryHelperProcess<CRegistryHelperLoad, CALifeRegistryContainer, IReader&,
...
void CALifeRegistryContainer::save(IWriter& memory_stream) {
    ...
    CRegistryHelperProcess<CRegistryHelperSave, CALifeRegistryContainer, IWriter&,
