...
void CStalkerAnimationPair::play_global_animation(IKinematicsAnimated* skeleton_animated,
                                                  PlayCallback callback,
                                                  const bool& use_animation_movement_control)
#else

void CStalkerAnimationPair::play_global_animation(IKinematicsAnimated* skeleton_animated,
                                                  PlayCallback callback, const u32& bone_part,
                                                  const bool& use_animation_movement_control,
...
#endif
{
    ...
    for (u16 i = 0; i < MAX_PARTS; ++i) {
        ...
        if (!m_blend) {
            ...
            blend = skeleton_animated->LL_PlayCycle(i, animation(), mix_animations ? TRUE : FALSE,
                                                    callback, m_object);
...
void CStalkerAnimationPair::play(IKinematicsAnimated* skeleton_animated, PlayCallback callback,
                                 const bool& use_animation_movement_control,
                                 const bool& local_animation, bool continue_interrupted_animation,
                                 bool mix_animations)
#else
void CStalkerAnimationPair::play(IKinematicsAnimated* skeleton_animated, PlayCallback callback,
                                 const bool& use_animation_movement_control,
...
#endif
{
...
#ifndef USE_HEAD_BONE_PART_FAKE
        play_global_animation(skeleton_animated, callback, use_animation_movement_control,
                              local_animation, mix_animations);
#else
        play_global_animation(skeleton_animated, callback, bone_part,
                              use_animation_movement_control, local_animation, mix_animations);
#endif
    m_actual = true;
    ...
    if (psAI_Flags.is(aiAnimation)) {
        ...
        Msg("%6d [%s][%s][%s][%d][%c][%c][%c][%f][%f][%f]", Device.dwTimeGlobal, m_object_name,
            m_animation_type_name, name, motion->bone_or_part,
...
CStalkerAnimationPair::blend_id(IKinematicsAnimated* skeleton_animated,
...
void CStalkerAnimationPair::select_animation(const ANIM_VECTOR& array,
...
void CStalkerAnimationPair::on_animation_end() {
    ...
    Callbacks callbacks(_alloca(callback_count * sizeof(Callbacks::value_type)), callback_count,
