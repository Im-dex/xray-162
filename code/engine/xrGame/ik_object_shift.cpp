...
void object_shift::dbg_draw(const Fmatrix& current_pos, const extrapolation::points& predict,
    ...
    for (u16 i = 0; i < nb_points; ++i) {
        ...
        Fvector v0 =
            Fvector().add(p0, Fvector().mul(vadd, float(i))).add(Fvector().set(0, fshift0, 0));
        Fvector v1 =
            Fvector().add(p0, Fvector().mul(vadd, float(i + 1))).add(Fvector().set(0, fshift1, 0));
    ...
    DBG_DrawLine(Fvector().add(p0, Fvector().set(0, start_shift, 0)),
                 Fvector().add(p1, Fvector().set(0, end_shift, 0)), D3DCOLOR_XRGB(255, 0, 0));
}
#endif
...
bool square_equation(float a, float b, float c, float& x0,
...
// float clamp_taget_to_max_possible_shift_speed_return_shift_taget(float &taget, float current,
...
void object_shift::set_taget(float taget_, float time) {
    ...
    // float x = clamp_taget_to_max_possible_shift_speed_return_shift_taget( taget, current, speed,
    // time );
