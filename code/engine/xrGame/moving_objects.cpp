...
void moving_objects::on_level_load() {
    xr_delete(m_tree);
    m_tree = new TREE(ai().level_graph().header().box(),
                          ai().level_graph().header().cell_size() * .5f, 16 * 1024, 16 * 1024);
}
...
void moving_objects::unregister_object(moving_object* moving_object) {
    VERIFY2(m_objects.find(moving_object) != m_objects.end(),
            make_string("moving object %s is not yet registered or unregisters twice",
                        *moving_object->id()));
