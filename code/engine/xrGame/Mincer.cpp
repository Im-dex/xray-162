...
void CMincer::Load(LPCSTR section) {
    ...
    m_telekinetics.set_destroing_particles(
    ...
    m_tearing_sound.create(pSettings->r_string(section, "body_tearing_sound"), st_Effect,
                           sg_SourceType);
...
void CMincer::AffectThrow(SZoneObjectInfo* O, CPhysicsShellHolder* GO, const Fvector& throw_in_dir,
                          float dist) {
...
void CMincer::NotificateDestroy(CPHDestroyableNotificate* dn) {
    ...
    CreateHit(obj->ID(), ID(), throw_in_dir, power, 0, position_in_bone_space, impulse,
...
void CMincer::AffectPullAlife(CEntityAlive* EA, const Fvector& throw_in_dir, float dist) {
    ...
    if (!smart_cast<CActor*>(EA)) {
        ...
        CreateHit(EA->ID(), ID(), throw_in_dir, power, 0, pos_in_bone_space, 0.0f,
                  m_eHitTypeBlowout);
    }
