...
IC float CLevelGraph::distance(const Fvector& position, const Fvector& point0,
...
IC CLevelGraph::ELineIntersections
CLevelGraph::intersect(float x1, float y1,                     /* First line segment */
...
IC CLevelGraph::ELineIntersections
CLevelGraph::intersect_no_check(float x1, float y1,                     /* First line segment */
                                float x2, float y2, float x3, float y3, /* Second line segment */
                                float x4, float y4, float* x,
                                float* y /* Output value: point of intersection */
) const {
...
IC void CLevelGraph::intersect(SSegment& tSegment, const SContour& tContour0,
...
IC float CLevelGraph::nearest(Fvector& destination, const Fvector& position,
...
IC bool CLevelGraph::intersect(Fvector& dst, const Fvector& v1, const Fvector& v2,
                               const Fvector& v3, const Fvector& v4) const {
...
IC float CLevelGraph::square(float a1, float b1, float fAlpha) const {
    ...
    return (fAlpha * fAlpha * fAlpha * a * a / 6 + fAlpha * fAlpha * a * b / 2 +
            fAlpha * b * b / 2);
}
...
IC float CLevelGraph::compute_square(float fAngle, float fAngleOfView, float b1, float b0, float b3,
                                     float b2) const {
...
IC float CLevelGraph::compute_high_square(float fAngle, float fAngleOfView,
                                          const CLevelGraph::CVertex* vertex) const {
    return (compute_square(fAngle, fAngleOfView, NORMALIZE_NODE_COVER_HIGH(vertex, 0),
                           NORMALIZE_NODE_COVER_HIGH(vertex, 1),
                           NORMALIZE_NODE_COVER_HIGH(vertex, 2),
                           NORMALIZE_NODE_COVER_HIGH(vertex, 3)));
}

IC float CLevelGraph::compute_low_square(float fAngle, float fAngleOfView,
                                         const CLevelGraph::CVertex* vertex) const {
    return (compute_square(fAngle, fAngleOfView, NORMALIZE_NODE_COVER_LOW(vertex, 0),
                           NORMALIZE_NODE_COVER_LOW(vertex, 1), NORMALIZE_NODE_COVER_LOW(vertex, 2),
...
IC u32 CLevelGraph::check_position_in_direction(u32 start_vertex_id, const Fvector& start_position,
    ...
    return (check_position_in_direction(start_vertex_id,
                                        Fvector2().set(start_position.x, start_position.z),
...
float CLevelGraph::vertex_high_cover_angle(u32 vertex_id, float inc_angle,
...
template <class _predicate>
float CLevelGraph::vertex_low_cover_angle(u32 vertex_id, float inc_angle,
                                          _predicate compare_predicate) const {
    float best_angle = 0.f;
