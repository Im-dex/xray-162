...
Upgrade* Manager::add_upgrade(shared_str const& upgrade_id, Group& parent_group) {
    if (get_upgrade(upgrade_id)) {
        VERIFY2(0, make_string("Try add the existent upgrade (%s), in group <%s>. Such upgrade is "
                               "in group <%s> already!",
...
void Manager::load_all_inventory() {
    ...
    VERIFY2(pSettings->section_exist(items_section),
            make_string("Section [%s] does not exist !", items_section));
    VERIFY2(pSettings->line_count(items_section),
            make_string("Section [%s] is empty !", items_section));
...
void Manager::test_all_upgrades(CInventoryItem& item) {
    ...
    VERIFY2(root_p, make_string("Upgrades for item <%s> (id = %d) does not exist!",
...
Upgrade* Manager::upgrade_verify(shared_str const& item_section, shared_str const& upgrade_id) {
    ...
    VERIFY2(upgrade_p, make_string("Upgrade <%s> in item <%s> does not exist!", upgrade_id.c_str(),
                                   item_section.c_str()));

    VERIFY2(root_p->contain_upgrade(upgrade_id),
            make_string("Inventory item <%s> not contain upgrade <%s> !", item_section.c_str(),
                        upgrade_id.c_str()));
...
LPCSTR Manager::get_upgrade_by_index(CInventoryItem& item, Ivector2 const& index) {
    ...
    VERIFY2(upgrade,
            make_string("! Upgrade with index <%d,%d> in inventory item [%s] does not exist!",
