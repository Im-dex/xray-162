...
void CHUDTarget::CursorOnFrame() {
    ...
    if (Level().CurrentEntity()) {
        ...
        if (Level().ObjectSpace.RayQuery(RQR, RD, pick_trace_callback, &PP, NULL,
                                         Level().CurrentEntity()))
            ...
            if (IsGameTypeSingle()) {
                ...
                if (E && E->g_Alive() && E->cast_base_monster()) {
                    ...
                    if (our_inv_owner && others_inv_owner) {
                        ...
                        switch (
                            RELATION_REGISTRY().GetRelationType(others_inv_owner, our_inv_owner)) {
                        ...
                        if (fuzzyShowInfo > 0.5f) {
                            CStringTable strtbl;
                            F->SetColor(
                                subst_alpha(C, u8(iFloor(255.f * (fuzzyShowInfo - 0.5f) * 2.f))));
                            F->OutNext("%s", *strtbl.translate(others_inv_owner->Name()));
                            F->OutNext("%s",
                                       *strtbl.translate(
                                           others_inv_owner->CharacterInfo().Community().id()));
                        }
                    }
                ...
                if (E && (E->GetfHealth() > 0)) {
                    if (pCurEnt && GameID() == eGameIDSingle) {
                        ...
                        if (PP.RQ.range >= recon_mindist() && PP.RQ.range <= recon_maxdist()) {
                            float ddist = (PP.RQ.range - recon_mindist()) /
                                          (recon_maxdist() - recon_mindist());
                            float dspeed =
                                recon_minspeed() + (recon_maxspeed() - recon_minspeed()) * ddist;
