...
void CActorCondition::LoadCondition(LPCSTR entity_section) {
    ...
    m_max_wound_protection =
        READ_IF_EXISTS(pSettings, r_float, section, "max_wound_protection", 1.0f);
    m_max_fire_wound_protection =
        READ_IF_EXISTS(pSettings, r_float, section, "max_fire_wound_protection", 1.0f);
    ...
    if (IsGameTypeSingle()) {
        ...
        if (!ln.empty()) {
            ...
            if (!fsimilar(GetPsyHealth(), 1.0f, 0.05f)) {
                if (!ppe) {
                    AddEffector(m_object, effPsyHealth, pp_sect_name,
    ...
    for (u8 i = 0; i < eBoostMaxCount; i++) {
        ...
        if (it != m_booster_influences.end()) {
            it->second.fBoostTime -=
                m_fDeltaTime / (IsGameTypeSingle() ? Level().GetGameTimeFactor() : 1.0f);
        ...
        for (int i = 0; i < sizeof(hits) / sizeof(hits[0]); ++i) {
            ...
            if (damage > EPS) {
                SHit HDS = SHit(damage,
                                //. 0.0f,
                                Fvector().set(0, 1, 0), NULL, BI_NONE, Fvector().set(0, 0, 0), 0.0f,
                                type, 0.0f, false);
...
CActorDeathEffector::CActorDeathEffector(CActorCondition* parent, LPCSTR sect) // -((
    ...
    pe->m_on_b_remove_callback =
        SBaseEffector::CB_ON_B_REMOVE(this, &CActorDeathEffector::OnPPEffectorReleased);
    m_b_actual = true;
