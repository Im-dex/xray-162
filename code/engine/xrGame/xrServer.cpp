...
u32 xrServer::OnDelayedMessage(
    NET_Packet& P, ClientID sender) // Non-Zero means broadcasting with "flags" as returned
{
    u16 type;
    P.r_begin(type);
    ...
    // R_ASSERT2						(CL, make_string("packet type
    // [%d]",type).c_str());
    ...
    case M_REMOTE_CONTROL_AUTH: {
        ...
        if (0 == stricmp(user.c_str(), "logoff")) {
            ...
            if (res) {
                ...
                Msg("# User [%s] tried to login as remote administrator. Access denied.",
                    user.c_str());
        }
        NET_Packet P_answ;
...
void xrServer::SendTo_LL(ClientID, void* data, u32 size, u32, u32) {
...
bool xrServer::verify_entities() const {
    ...
    for (; I != E; ++I) {
        ...
        VERIFY3((*I).first == (*I).second->ID,
                "SERVER : ID mismatch - map key doesn't correspond to the real entity ID",
...
void xrServer::verify_entity(const CSE_Abstract* entity) const {
    ...
    if (entity->ID_Parent != 0xffff) {
        ...
        VERIFY2(J != entities.end(), make_string("SERVER : Cannot find parent in the map [%s][%s]",
        ...
        VERIFY3(std::find((*J).second->children.begin(), (*J).second->children.end(), entity->ID) !=
                ...
                "SERVER : Parent/Children relationship mismatch - Object has parent, but "
                "corresponding parent doesn't have children",
                (*J).second->name_replace());
    }
    ...
    for (; I != E; ++I) {
        ...
        VERIFY3(J != entities.end(), "SERVER : Cannot find children in the map",
                entity->name_replace());
        VERIFY3((*J).second, "SERVER : Null entity object in the map", entity->name_replace());
        VERIFY3((*J).first == (*J).second->ID,
                "SERVER : ID mismatch - map key doesn't correspond to the real entity ID",
        ...
        VERIFY3((*J).second->ID_Parent == entity->ID,
                "SERVER : Parent/Children relationship mismatch - Object has children, but "
                "children doesn't have parent",
                (*J).second->name_replace());
    }
}
...
std::string xrServer::level_name(const shared_str& server_options) const {
    ...
    struct MaxPingClientDisconnector {
        ...
        void operator()(IClient* client) {
            ...
            if (ps->ping > g_sv_dwMaxClientPing &&
                Client->m_ping_warn.m_dwLastMaxPingWarningTime + g_sv_time_for_ping_check <
                    Device.dwTimeGlobal) {
...
void xrServer::GetServerInfo(CServerInfo* si) {
    ...
    LPCSTR time = InventoryUtilities::GetTimeAsString(Device.dwTimeGlobal,
                                                      InventoryUtilities::etpTimeToSecondsAndDay)
                      .c_str();
