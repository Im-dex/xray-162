...
IC bool coincide_directions(const Fvector2& start_circle_center,
                            const Fvector2& start_tangent_point, float start_cross_product,
                            const Fvector2& dest_circle_center, const Fvector2& dest_tangent_point,
    ...
    if (fis_zero(start_cross_product)) {
        ...
        float cp1 =
            start_tangent_dest_tangent_direction.crossproduct(circle_tangent_point_direction);
...
bool CDetailPathManager::compute_tangent(const STrajectoryPoint& start,
                                         const SCirclePoint& start_circle,
                                         const STrajectoryPoint& dest,
                                         const SCirclePoint& dest_circle, SCirclePoint* tangents,
    ...
    if (start_cp * dest_cp >= 0.f) {
        ...
        if (start_circle.center.similar(dest_circle.center, EPS_S)) {
            ...
            float r_diff_abs = xr::abs(r_diff);
    ...
    if (coincide_directions(start_circle.center, tangents[0].point, start_cp, dest_circle.center,
                            tangents[1].point, dest_cp)) {
        ...
        assign_angle(tangents[1].angle, dest_yaw,
                     yaw2 + alpha < PI_MUL_2 ? yaw2 + alpha : yaw2 + alpha - PI_MUL_2, dest_cp >= 0,
    ...
    adjust_point(dest_circle.center, yaw2 - alpha, dest_circle.radius, tangents[1].point);
    assign_angle(tangents[0].angle, start_yaw,
                 yaw1 - alpha >= 0.f ? yaw1 - alpha : yaw1 - alpha + PI_MUL_2, start_cp >= 0,
                 direction_type);
    assign_angle(tangents[1].angle, dest_yaw,
                 yaw2 - alpha >= 0.f ? yaw2 - alpha : yaw2 - alpha + PI_MUL_2, dest_cp >= 0,
                 direction_type, false);
...
bool CDetailPathManager::build_circle_trajectory(const STrajectoryPoint& position,
    ...
    STravelPathPoint t;
    t.velocity = velocity;
    if (position.radius * xr::abs(position.angle) <= min_dist) {
        if (!path) {
            if (vertex_id)
    ...
    if (fis_zero(position.angular_velocity))
        ...
        int m = std::min(iFloor(xr::abs(angle) / position.angular_velocity * 10.f + 1.5f),
                         iFloor(position.radius * xr::abs(angle) / min_dist + 1.5f));
#ifdef DEBUG
        if (m >= 10000) {
    ...
    for (int i = 0; i <= n + k; ++i) {
        ...
        curr_vertex_id =
            ai().level_graph().check_position_in_direction(curr_vertex_id, curr_pos, t.position);
...
bool CDetailPathManager::build_line_trajectory(const STrajectoryPoint& start,
    ...
    return (path
                ? ai().level_graph().create_straight_path<false>(vertex_id, start.point, dest.point,
                                                                 *path, t, false, false)
...
bool CDetailPathManager::build_trajectory(STrajectoryPoint& start, STrajectoryPoint& dest,
    ...
    time = flt_max;
    SDist dist[4];
    float straight_velocity = xr::abs(velocity(velocity2).linear_velocity);
    {
        for (u32 i = 0; i < tangent_count; ++i) {
            dist[i].index = i;
            dist[i].time = xr::abs(tangents[i][0].angle) / start.angular_velocity +
                           xr::abs(tangents[i][1].angle) / dest.angular_velocity +
...
bool CDetailPathManager::compute_trajectory(STrajectoryPoint& start, STrajectoryPoint& dest,
    ...
    for (u32 i = 0; i < 2; ++i)
        for (u32 j = 0; j < 2; ++j)
            if (compute_tangent(start, start_circles[i], dest, dest_circles[j],
    ...
    return (build_trajectory(start, dest, tangent_points, tangent_count, path, time, velocity1,
                             velocity2, velocity3));
}
            ...
            if (compute_trajectory(start, dest, m_tpTravelLine ? &m_temp_path : 0, time, (*I).index,
                ...
                if (!m_try_min_time || (time < min_time)) {
                    ...
                    if (m_tpTravelLine) {
                        ...
                        m_tpTravelLine->insert(m_tpTravelLine->end(), m_temp_path.begin(),
                                               m_temp_path.end());
...
bool CDetailPathManager::init_build(const xr_vector<u32>& level_path, u32 intermediate_index,
    ...
    m_corrected_dest_position.y =
        ai().level_graph().vertex_plane_y(dest.vertex_id, dest.position.x, dest.position.y);
...
bool CDetailPathManager::fill_key_points(const xr_vector<u32>& level_path, u32 intermediate_index,
    ...
    for (int _i = 0, i = 0, n = (int)level_path.size() - 1, j = n, m = j; _i < n;) {
        if (!ai().level_graph().check_vertex_in_direction(start_point.vertex_id,
                                                          start_point.position, level_path[j])) {
            m = j;
            j = (i + j) / 2;
        } else {
            if ((j == n) &&
        ...
        if (i >= m - 1) {
            ...
            start_point.position =
                ai().level_graph().v2d(ai().level_graph().vertex_position(start_point.vertex_id));
            j = m = n;
        }
    }
    return (true);
...
CDetailPathManager::compute_better_key_point(const STravelPoint& point0, const STravelPoint& point1,
...
void CDetailPathManager::build_path_via_key_points(STrajectoryPoint& start, STrajectoryPoint& dest,
    ...
    for (B != E ? ++I : I; I != E; ++I) {
        ...
        bool succeed =
            compute_path(s, d, &m_path, m_start_params, last_point ? finish_params : m_start_params,
    ...
    if ((B == E) && !compute_path(s, dest, &m_path, m_start_params, finish_params,
                                  straight_line_index, straight_line_index_negative)) {
        m_path.clear();
        return;
    }
...
void CDetailPathManager::postprocess_key_points(const xr_vector<u32>& level_path,
    ...
    for (int i = 1, n = (int)m_key_points.size() - 1; i < n; ++i) {
        STravelPoint key_point0 = compute_better_key_point(m_key_points[i - 1], m_key_points[i],
                                                           m_key_points[i + 1], false);
        STravelPoint key_point1 = compute_better_key_point(m_key_points[i + 1], m_key_points[i],
...
void CDetailPathManager::add_patrol_point() {
    ...
    if ((m_path.size() > 1) && m_state_patrol_path && !fis_zero(extrapolate_length())) {
        ...
        ai().level_graph().create_straight_path<false>(
            m_path.back().vertex_id, ai().level_graph().v2d(m_path.back().position),
...
void CDetailPathManager::build_smooth_path(const xr_vector<u32>& level_path,
    ...
    if (m_restricted_object) {
        ...
        if (!((alvi && asp) || (!asp && !alvi))) {
            Msg("! vertex [%d], position [%f][%f][%f], alvi[%c], asp[%c]", start.vertex_id,
                VPUSH(start_pos), alvi ? '+' : '-', asp ? '+' : '-');
        }
    ...
    postprocess_key_points(level_path, intermediate_index, start, dest, finish_params,
                           straight_line_index, straight_line_index_negative);
