...
struct CRemoveMergedFreeInRestrictions {
    ...
    IC bool operator()(const CSpaceRestriction::CFreeInRestriction& free_in_restriction) const {
        return (std::find(m_restrictions->begin(), m_restrictions->end(),
                          free_in_restriction.m_restriction) != m_restrictions->end());
    }
};
...
bool CSpaceRestriction::accessible(const Fsphere& sphere) {
    ...
    return (ai().level_graph().valid_vertex_position(sphere.P) &&
            (m_out_space_restriction ? (m_out_space_restriction->inside(sphere) &&
                                        !m_out_space_restriction->on_border(sphere.P) &&
                                        !m_out_space_restriction->out_of_border(sphere.P))
                                     : true) &&
            (m_in_space_restriction ? (!m_in_space_restriction->inside(sphere) &&
...
bool CSpaceRestriction::accessible(u32 level_vertex_id, float radius) {
    ...
    return ((m_out_space_restriction
                 ? m_out_space_restriction->inside(level_vertex_id, false, radius)
                 : true) &&
            (m_in_space_restriction ? !m_in_space_restriction->inside(level_vertex_id, true, radius)
                                    : true));
}
...
bool CSpaceRestriction::affect(SpaceRestrictionHolder::CBaseRestrictionPtr bridge,
    ...
    // return							(start_position.distance_to(position) <= radius +
    // dependent_distance);
}

bool CSpaceRestriction::affect(SpaceRestrictionHolder::CBaseRestrictionPtr bridge,
                               u32 start_vertex_id, float radius) const {
    Fsphere sphere;
...
bool CSpaceRestriction::affect(SpaceRestrictionHolder::CBaseRestrictionPtr bridge,
                               const Fvector& start_position, const Fvector& dest_position) const {
...
bool CSpaceRestriction::affect(SpaceRestrictionHolder::CBaseRestrictionPtr bridge,
                               u32 start_vertex_id, u32 dest_vertex_id) const {
