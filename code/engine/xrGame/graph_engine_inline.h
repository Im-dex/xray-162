...
#endif
    typedef CPathManager<_Graph, CAlgorithm::CDataStorage, _Parameters, _dist_type, _index_type,
                         _iteration_type>
        CPathManagerGeneric;

    CPathManagerGeneric path_manager;

    path_manager.setup(&graph, &m_algorithm->data_storage(), node_path, start_node, dest_node,
                       parameters);
...
CGraphEngine::search(const CProblemSolver<T1, T2, T3, T4, T5, T6, T7, T8>& graph,
    ...
    typedef CPathManager<CSProblemSolver, CSolverAlgorithm::CDataStorage, _Parameters,
                         _solver_dist_type, _solver_index_type, GraphEngineSpace::_iteration_type>
        CSolverPathManager;

    CSolverPathManager path_manager;

    path_manager.setup(&graph, &m_solver_algorithm->data_storage(), node_path, start_node,
                       dest_node, parameters);
...
IC bool CGraphEngine::search(const _Graph& graph, const shared_str& start_node,
    ...
    typedef CPathManager<_Graph, CStringAlgorithm::CDataStorage, _Parameters, float, shared_str,
                         u32>
        CPathManagerGeneric;

    CPathManagerGeneric path_manager;

    path_manager.setup(&graph, &m_string_algorithm->data_storage(), node_path, start_node,
                       dest_node, parameters);
