...
CStalkerActionGetOutOfAnomaly::CStalkerActionGetOutOfAnomaly(CAI_Stalker* object,
                                                             LPCSTR action_name)
...
void CStalkerActionGetOutOfAnomaly::initialize() {
    ...
    if (object().memory().enemy().selected() && object().inventory().ActiveItem() &&
        object().best_weapon() &&
...
void CStalkerActionGetOutOfAnomaly::execute() {
    ...
    std::vector<CObject*>::const_iterator I = object().feel_touch.begin();
    std::vector<CObject*>::const_iterator E = object().feel_touch.end();
    for (; I != E; ++I) {
        ...
        if (zone && (zone->restrictor_type() != RestrictionSpace::eRestrictorTypeNone)) {
            ...
            if (std::find(restrictions.begin(), restrictions.end(), zone->ID()) !=
                restrictions.end())
                continue;
...
void CStalkerActionDetectAnomaly::execute() {
    ...
    object().CObjectHandler::set_goal(eObjectActionFire1,
                                      object().inventory().ItemFromSlot(BOLT_SLOT));
}
