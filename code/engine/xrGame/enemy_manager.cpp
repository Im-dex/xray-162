...
bool CEnemyManager::useful(const CEntityAlive* entity_alive) const {
    ...
    if (!ai().get_level_graph() ||
        !ai().level_graph().valid_vertex_id(entity_alive->ai_location().level_vertex_id()))
        return (false);
    ...
    return (m_useful_callback
                ? m_useful_callback(m_object->lua_game_object(), entity_alive->lua_game_object())
                : true);
}
...
void CEnemyManager::reload(LPCSTR section) {
    m_ignore_monster_threshold =
        READ_IF_EXISTS(pSettings, r_float, section, "ignore_monster_threshold", 1.f);
...
void CEnemyManager::restore_max_ignore_monster_distance() {
    m_max_ignore_distance =
        READ_IF_EXISTS(pSettings, r_float, *m_object->cNameSect(), "max_ignore_distance", 0.f);
}
...
void CEnemyManager::remove_wounded() {
    ...
    m_objects.erase(std::remove_if(m_objects.begin(), m_objects.end(), &no_wounded::predicate),
                    m_objects.end());
}
...
void CEnemyManager::update() {
    ...
    if (!m_ready_to_save) {
        //		Msg						("%6d %s DEcreased enemy counter for
        //player (%d ->
        //%d)",Device.dwTimeGlobal,*m_object->cName(),Level().autosave_manager().not_ready_count(),Level().autosave_manager().not_ready_count()-1);
        Level().autosave_manager().dec_not_ready();
    }
    ...
    if (!m_ready_to_save) {
        //		Msg						("%6d %s INcreased enemy counter for
        //player (%d ->
        //%d)",Device.dwTimeGlobal,*m_object->cName(),Level().autosave_manager().not_ready_count(),Level().autosave_manager().not_ready_count()+1);
