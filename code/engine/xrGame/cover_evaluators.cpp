...
bool CCoverEvaluatorBase::inertia(Fvector const& position, float radius) {
    ...
    //	m_actuality				= m_actuality && ((m_last_radius + EPS_L) >=
    //radius);
...
void CCoverEvaluatorCloseToEnemy::evaluate_cover(const CCoverPoint* cover_point, float weight) {
    ...
    // y						= angle_normalize(y);
    // float					cover_value =
    // ai().level_graph().cover_in_direction(y,cover_point->level_vertex_id());  if (cover_value >=
    // m_best_value) 	return;
...
void CCoverEvaluatorCloseToEnemy::evaluate_smart_cover(smart_cover::cover const* smart_cover,
                                                       float const& weight) {}
...
void CCoverEvaluatorFarFromEnemy::evaluate_smart_cover(smart_cover::cover const* smart_cover,
                                                       float const& weight) {}
...
void CCoverEvaluatorBest::evaluate_cover(const CCoverPoint* cover_point, float weight) {
    ...
    float high_cover_value =
        ai().level_graph().high_cover_in_direction(y, cover_point->level_vertex_id());
    float low_cover_value =
        ai().level_graph().low_cover_in_direction(y, cover_point->level_vertex_id());
...
void CCoverEvaluatorBest::evaluate_smart_cover(smart_cover::cover const* smart_cover,
                                               float const& weight) {
#if 0
	return;
#else
    ...
    float value;
    smart_cover::loophole* best_loophole =
        smart_cover->best_loophole(m_enemy_position, value, false,
...
void CCoverEvaluatorAngle::initialize(const Fvector& start_position, bool fake_call) {
    ...
    for (float alpha = 0.f, step = PI_MUL_2 / 360.f; alpha < PI_MUL_2; alpha += step) {
        float high_value =
            ai().level_graph().compute_high_square(alpha, PI_DIV_2, m_level_vertex_id);
...
void CCoverEvaluatorAngle::evaluate_smart_cover(smart_cover::cover const* smart_cover,
                                                float const& weight) {}
...
void CCoverEvaluatorSafe::evaluate_smart_cover(smart_cover::cover const* smart_cover,
                                               float const& weight) {}
...
void CCoverEvaluatorAmbush::setup(const Fvector& my_position, const Fvector& enemy_position,
...
void CCoverEvaluatorAmbush::evaluate_cover(const CCoverPoint* cover_point, float weight) {
    ...
    direction.getHP(y, p);
    y = angle_normalize(y);
    float high_cover_from_enemy =
        ai().level_graph().high_cover_in_direction(y, cover_point->level_vertex_id());
    float low_cover_from_enemy =
        ai().level_graph().low_cover_in_direction(y, cover_point->level_vertex_id());
    ...
    direction.getHP(y, p);
    y = angle_normalize(y);
    float high_cover_from_myself =
        ai().level_graph().high_cover_in_direction(y, cover_point->level_vertex_id());
    float low_cover_from_myself =
        ai().level_graph().low_cover_in_direction(y, cover_point->level_vertex_id());
...
void CCoverEvaluatorAmbush::evaluate_smart_cover(smart_cover::cover const* smart_cover,
                                                 float const& weight) {}
