...
void game_cl_GameState::TranslateGameMessage(u32 msg, NET_Packet& P) {
    ...
    case GAME_EVENT_PLAYER_CONNECTED: {
        ...
        xr_sprintf(Text, "%s%s %s%s", Color_Teams[0], PS->getName(), Color_Main,
                   *st.translate("mp_connected"));
        if (CurrentGameUI())
    ...
    case GAME_EVENT_PLAYER_DISCONNECTED: {
        ...
        xr_sprintf(Text, "%s%s %s%s", Color_Teams[0], PlayerName, Color_Main,
                   *st.translate("mp_disconnected"));
        if (CurrentGameUI())
            CurrentGameUI()->CommonMessageOut(Text);
        //---------------------------------------
    ...
    case GAME_EVENT_PLAYER_ENTERED_GAME: {
        ...
        xr_sprintf(Text, "%s%s %s%s", Color_Teams[0], PlayerName, Color_Main,
                   *st.translate("mp_entered_game"));
        if (CurrentGameUI())
            CurrentGameUI()->CommonMessageOut(Text);
    } break;
    default: { R_ASSERT2(0, "Unknown Game Message"); } break;
    };
}
