...
public:
    ...
    // }
    //! TO BE DOCUMENTED
    // IC	Point&		Min(const Point& p)								{ x = MIN(x, p.x); y = MIN(y, p.y); z = MIN(z, p.z);
    // return *this;	}
    //! TO BE DOCUMENTED
    // IC	Point&		Max(const Point& p)								{ x = MAX(x, p.x); y = MAX(y, p.y); z = MAX(z, p.z);
    // return *this;	}

    //! Computes square magnitude
    IC float SquareMagnitude() const { return x * x + y * y + z * z; }
    //! Computes magnitude
    IC float Magnitude() const { return std::sqrt(x * x + y * y + z * z); }
...
ICF bool planeBoxOverlap(const Point& normal, const float d, const Point& maxbox) {
    float norm_box_proj =
        xr::abs(maxbox.x * normal.x) + xr::abs(maxbox.y * normal.y) + xr::abs(maxbox.z * normal.z);
