...
int dcTriListCollider::dSortedTriBox(const dReal* triSideAx0, const dReal* triSideAx1,
                                     const dReal* triAx,
                                     // const dReal* v0,
                                     // const dReal* v1,
                                     // const dReal* v2,
...
/*
bool test_cross_side( dReal* outAx, dReal& outDepth, dReal *pos, dReal& outSignum, int &code, u8 c,
const dReal* R, const dReal* hside, const dReal* p,  const dReal* triSideAx, const dReal* vax,
const dReal* vox  )
{
                        ...
                        if(depth_ax*1.05f<outDepth)
                                                {
                                                        ...
                                                        dReal sgn1=sgn*dDOT14(axis,R+ix1)<0.f ? -1.f
: 1.f; dReal sgn2=sgn*dDOT14(axis,R+ix2)<0.f ? -1.f : 1.f; dVector3 crpos; for(int ii=0;ii<3;++ii)
                                                                crpos[ii]=p[ii]+R[ii*4+ix1]*hside[ix1]*sgn1+R[ii*4+ix2]*hside[ix2]*sgn2;
                                                
                                                        if( dcTriListCollider::CrossProjLine14( vax,
triSideAx, crpos,R+i, hside[i], pos ) )
                                                        {
                                                        outDepth=depth_ax;
                                                        outSignum=sgn;
                                                        outAx[0]=axis[0];
...
int dcTriListCollider::dTriBox(const dReal* v0, const dReal* v1, const dReal* v2, Triangle* T,
...
for(i = 0; i < 3; ++i) {                                                                           \
        ...
        int ix2 = (i + 2) % 3;                                                                     \
        sidePr =                                                                                   \
            dFabs(dDOT14(axis, R + ix1) * hside[ix1]) + dFabs(dDOT14(axis, R + ix2) * hside[ix2]); \
                                                                                                   \
        dist##ax = dDOT(v##ax, axis) - dDOT(p, axis);                                              \
        dist##ox = dDOT(v##ox, axis) - dDOT(p, axis);                                              \
        \
...
depth##ox = sidePr - dFabs(dist##ox);                                                              \
            ...
            if (depth##ax > 0.f) {                                                                 \
                if (depth##ax * 1.05f < outDepth) {                                                \
                    ...
                    for (int ii = 0; ii < 3; ++ii)                                                 \
                        crpos[ii] = p[ii] + R[ii * 4 + ix1] * hside[ix1] * sgn1 +                  \
    ...
    dVector3 crpos;
    //#define TEST(ax,ox,c)
    // test_cross_side( dReal* outAx, dReal& outDepth, dReal *pos, dReal& outSignum, u8 &code, u8 c,
    // const dReal* R, const dReal* hside, const dReal* p,  const dReal* triSideAx, const dReal* vax,
    // const dReal* vox  )  if( test_cross_side
...
#define TRI_CONTAIN_POINT(pos)                                                \
    {                                                                         \
        ...
        ds2 = dDOT(cross2, v2);                                               \
                                                                              \
        if (dDOT(cross0, pos) - ds0 > 0.f && dDOT(cross1, pos) - ds1 > 0.f && \
