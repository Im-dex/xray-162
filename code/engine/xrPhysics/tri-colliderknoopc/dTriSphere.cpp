...
inline dReal dcTriListCollider::FragmentonSphereTest(const dReal* center, const dReal radius,
                                                     const dReal* pt1, const dReal* pt2,
                                                     dReal* norm) {
...
IC bool dcTriListCollider::PointSphereTest(const dReal* center, const dReal radius, const dReal* pt,
...
int dcTriListCollider::dSortedTriSphere(const dReal* /**v1/**/, const dReal* /**v2/**/,
                                        const dReal* triAx, CDB::TRI* T, dReal dist, dxGeom* Sphere,
                                        dxGeom* Geometry, int Flags, dContactGeom* Contacts,
                                        int skip) {
...
int dcTriListCollider::dTriSphere(const dReal* v0, const dReal* v1, const dReal* v2, Triangle* T,
    ...
    if (TriContainPoint(v0, v1, v2, triAx, triSideAx0, triSideAx1, pos)) {
        ...
        // dVector3 ContactPos={pos[0]-triAx[0]* radius,pos[1]-triAx[1]* radius,pos[2]-triAx[2]*
        // radius};
    } else {
        // VERIFY( g_pGameLevel );
        ...
        if (FragmentonSphereTest(pos, radius, v0, v1, ContactNormal, Depth)) {
            ...
            if (gl_state.test(fl_engaged_v0) || gl_state.test(fl_engaged_v1) ||
