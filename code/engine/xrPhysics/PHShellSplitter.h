...
typedef std::vector<CPHShellSplitter>::reverse_iterator SPLITTER_RI;
    ...
    : public CPHUpdateObject // call all Fractures and Breakable Joints Updates
{
    ...
    virtual void PhTune(
        dReal step); // call fractures PhTune for element splitters
                     // m_pShell->m_elements[m_splitters[i]->m_element]->m_pFracturesHolder->PhTune()
    virtual void PhDataUpdate(
        dReal
            step); // call fractures PhDataUpdate for element splitters
