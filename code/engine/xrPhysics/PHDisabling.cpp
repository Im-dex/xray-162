...
void CBaseDisableData::Disabling() {
    ...
    if (dBodyIsEnabled(body)) {
        ReEnable();
        if (!m_disabled &&
            (ph_world->disable_count != m_count % worldDisablingParams.objects_params.L2frames)) {
...
void CPHDisablingTranslational::UpdateL1() {
    ...
    // ;  float			acceleration_param	=	m_mean_acceleration
    // .Update(* (Fvector*) velocity)		;  CheckState
