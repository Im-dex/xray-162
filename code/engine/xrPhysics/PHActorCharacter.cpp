...
void CPHActorCharacter::Jump(const Fvector& accel) {
    ...
    if (CanJump()) {
        ...
        if (m_elevator_state.ClimbingState()) {
            ...
            m_jump_accel.set(vel[0] * JUMP_INCREASE_VELOCITY_RATE + m_acceleration.x / amag * 0.2f,
                             jump_up_velocity,
...
void CPHActorCharacter::InitContact(dContact* c, bool& do_collide, u16 material_idx_1,
                                    u16 material_idx_2) {
    ...
    if (b_single_game) {
        ...
        if (b_restrictor && do_collide &&
            !(b1 ? static_cast<CPHCharacter*>(retrieveGeomUserData(c->geom.g2)->ph_object)
                       ->ActorMovable()
                 : static_cast<CPHCharacter*>(retrieveGeomUserData(c->geom.g1)->ph_object)
                       ->ActorMovable())) {
        ...
        if (D1 && D2) {
            ...
            if (A1 && A2 && A1->IsActor() && A2->IsActor()) {
                do_collide = do_collide && !b_restrictor &&
                             (A1->ObjectPPhysicsShell() == 0) == (A2->ObjectPPhysicsShell() == 0);
                c->surface.mu = 1.f;
            }
        }
...
void CPHActorCharacter::ChooseRestrictionType(ERestrictionType my_type, float my_depth,
                                              CPHCharacter* ch) {
    if (my_type != rtStalker ||
        (ch->RestrictionType() != rtStalker && ch->RestrictionType() != rtStalkerSmall))
        return;
    float checkR =
        m_restrictors[rtStalkerSmall]
