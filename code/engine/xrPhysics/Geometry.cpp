...
void GetBoxExtensions(dGeomID box, const dReal* axis, const dReal* pos, const dReal* rot,
...
void GetCylinderExtensions(dGeomID cyl, const dReal* axis, const dReal* pos, const dReal* rot,
                           float center_prg, dReal* lo_ext, dReal* hi_ext) {
...
void GetSphereExtensions(dGeomID sphere, const dReal* axis, const dReal* pos, float center_prg,
...
void TransformedGeometryExtensionLocalParams(dGeomID geom_transform, const dReal* axis,
                                             float center_prg, dReal* local_axis,
...
{
        dMULTIPLY0_331
((dReal*)(&form.c),dGeomGetRotation(m_geom_transform),dGeomGetPosition(geom())); form.c.add(*((const
Fvector*)dGeomGetPosition(m_geom_transform))); dMULTIPLY3_333
...
void CBoxGeom::get_Extensions(const Fvector& axis, float center_prg, float& lo_ext,
                              float& hi_ext) const {
...
void CBoxGeom::set_size(const Fvector& half_size) {
    ...
    VERIFY(geom());
    dGeomBoxSetLengths(geom(), m_box.m_halfsize.x * 2.f, m_box.m_halfsize.y * 2.f,
                       m_box.m_halfsize.z * 2.f);
}
...
void CSphereGeom::get_Extensions(const Fvector& axis, float center_prg, float& lo_ext,
                                 float& hi_ext) const {
    VERIFY(m_geom_transform);
    const dReal* rot = NULL;
    const dReal* pos = NULL;
...
void CSphereGeom::set_build_position(const Fvector& ref_point) {
    ...
    inherited::set_build_position(ref_point);
    dVector3 local_position = { m_sphere.P.x - ref_point.x, m_sphere.P.y - ref_point.y,
...
void CCylinderGeom::get_Extensions(const Fvector& axis, float center_prg, float& lo_ext,
                                   float& hi_ext) const {
    VERIFY(m_geom_transform);
    const dReal* rot = NULL;
    const dReal* pos = NULL;
