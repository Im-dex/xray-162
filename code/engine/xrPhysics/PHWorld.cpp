...
void __stdcall create_physics_world(bool mt, CObjectSpace* os, CObjectList* lo,
                                    CRenderDeviceBase* dv) // IPHWorldUpdateCallbck &commander,
{
...
CObjectSpace* __stdcall create_object_space() {
    // CFileReader* fr =
    // new CFileReader("D:/STALKER/resources/gamedata/levels/stohe_selo/level.cform");
...
CPHWorld::CPHWorld()
    : // IPHWorldUpdateCallbck		*_update_callback
      ...
      m_default_contact_shotmark(0), m_default_character_contact_shotmark(0),
...
void CPHWorld::Create(bool mt, CObjectSpace* os, CObjectList* lo, CRenderDeviceBase* dv) {
    ...
    // else								Device.seqFrame.Add
    // (this,REG_PRIORITY_LOW);
...
// Msg									("------------- physics: %d /
// %d",u32(Device.dwFrame),u32(m_steps_num));
...
#endif
    dJointGroupEmpty(
        ContactGroup); // this is to be called after PhDataUpdate!!!-the order is critical!!!
