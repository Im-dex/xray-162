...
//					O3,O4 - intersection between the line and sphere with center cp2
//radius r2  returns false if there is no intersection  computes distances O1-O3, O1-O4, O2-O3, O2-O4
// in "point" returns mean point between intersection points with smallest distance
/////////////////////////////////////////////////////////////////////////////////////////////////
inline bool circleIntersection(const dReal* n1, const dReal* cp1, dReal r1, const dReal* n2,
                               const dReal* cp2, dReal r2, dVector3 point) {
...
void lineClosestApproach(const dVector3 pa, const dVector3 ua, const dVector3 pb, const dVector3 ub,
...
extern "C" int dCylBox(const dVector3 p1, const dMatrix3 R1, const dReal radius, const dReal lz,
                       const dVector3 p2, const dMatrix3 R2, const dVector3 side2, dVector3 normal,
                       dReal* depth, int* code, int maxc, dContactGeom* contact, int skip) {
    dVector3 p, pp, normalC;
    const dReal* normalR = 0;
    dReal B1, B2, B3, R11, R12, R13, R21, R22, R23, R31, R32, R33, Q11, Q12, Q13, Q21, Q22, Q23,
        Q31, Q32, Q33, s, s2, l, sQ21, sQ22, sQ23;
    int i, invert_normal;
...
#define TEST(expr1, expr2, norm, cc)   \
    ...
    *code = 0;
    ...
    // if we did not need Q23 and Q22 the first way might be used to quiken the routine but then it
    // need to  check if Q21<=1.f, becouse it may slightly exeed 1.f.
...
#define TEST(expr1, expr2, n1, n2, n3, cc) \
    ...
    boxProj = dFabs(dDOT14(Ax, R2 + 0) * B1) + dFabs(dDOT14(Ax, R2 + 1) * B2) +
    ...
    boxProj = dFabs(dDOT14(Ax, R2 + 1) * B2) + dFabs(dDOT14(Ax, R2 + 0) * B1) +
              dFabs(dDOT14(Ax, R2 + 2) * B3);
    ...
    TEST(p[0] * Ax[0] + p[1] * Ax[1] + p[2] * Ax[2], (_sin * radius + _cos * hlz + boxProj), Ax[0],
         Ax[1], Ax[2], 5);
    ...
    dNormalize3(Ax);
    boxProj = dFabs(dDOT14(Ax, R2 + 1) * B2) + dFabs(dDOT14(Ax, R2 + 2) * B3) +
...
extern "C" int dCylCyl(const dVector3 p1, const dMatrix3 R1, const dReal radius1, const dReal lz1,
                       const dVector3 p2, const dMatrix3 R2, const dReal radius2, const dReal lz2,
                       dVector3 normal, dReal* depth, int* code, int maxc, dContactGeom* contact,
                       int skip) {
...
#define TEST(expr1, expr2, n1, n2, n3, cc) \
    ...
    TEST(p[0] * Ax[0] + p[1] * Ax[1] + p[2] * Ax[2], radius1 + _cos * hlz2 + _sin * radius2, Ax[0],
         Ax[1], Ax[2], 3);
    ...
    TEST(p[0] * Ax[0] + p[1] * Ax[1] + p[2] * Ax[2], radius2 + _cos * hlz1 + _sin * radius1, Ax[0],
         Ax[1], Ax[2], 4);
    ...
    //@ I still not sure about problems with sepparation but they have not been revealed during
    //testing.
    dVector3 point;
    {
        dVector3 ca, cb;
...
int dCollideCylB(dxGeom* o1, dxGeom* o2, int flags, dContactGeom* contact, int skip) {
    ...
    int num = dCylBox(dGeomGetPosition(o1), dGeomGetRotation(o1), cylRadius, cylLength,
                      dGeomGetPosition(o2), dGeomGetRotation(o2), boxSides, normal, &depth, &code,
...
int dCollideCylCyl(dxGeom* o1, dxGeom* o2, int flags, dContactGeom* contact, int skip) {
    ...
    int num = dCylCyl(dGeomGetPosition(o1), dGeomGetRotation(o1), cylRadius1, cylLength1,
                      dGeomGetPosition(o2), dGeomGetRotation(o2), cylRadius2, cylLength2, normal,
