    ...
    if (m_flags.test(fl_ray_motions)) {
        ...
        for (; E != I; I++) {
            ...
            if (debug_output().ph_dbg_draw_mask().test(phDbgDrawRayMotions)) {
                ...
                debug_output().DBG_DrawLine(*from,
...
void CPHObject::step(float time) // it is still not a true step for object because it collide the
                                 // object only not subsequent collision is doing
{
