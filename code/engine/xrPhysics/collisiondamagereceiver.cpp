...
void DamageReceiverCollisionCallback(bool& do_colide, bool bo1, dContact& c, SGameMtl* material_1,
                                     SGameMtl* material_2) {
    ...
    dxGeomUserData* ud_self =
        bo1 ? retrieveGeomUserData(c.geom.g1) : retrieveGeomUserData(c.geom.g2);
    dxGeomUserData* ud_damager =
    ...
    // CPHCollisionDamageReceiver	*dr	= static_cast<CPhysicsShellHolder*>( o_self
    ...
    pos.sub(*(Fvector*)c.geom.pos,
            *(Fvector*)dGeomGetPosition(bo1 ? c.geom.g1
                                            : c.geom.g2)); // it is not true pos in bone space

    dr->CollisionHit(source_id, ud_self->bone_id,
                     E_NL(b1, b2, c.geom.normal) * damager_material_factor / dfs, dir, pos);
}

void BreakableObjectCollisionCallback(bool& /**do_colide/**/, bool bo1, dContact& c,
