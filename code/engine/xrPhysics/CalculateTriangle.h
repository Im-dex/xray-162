...
ICF bool TriContainPoint(const dReal* v0, const dReal* v1, const dReal* v2, const dReal* triAx,
...
ICF bool TriContainPoint(Triangle* T, const float* pos, u16& c, const Fvector* V_array) {
    // TriContainPoint(const dReal* v0,const dReal* v1,const dReal* v2,const dReal* triAx,const
    // dReal* triSideAx0,const dReal* triSideAx1, const dReal* pos)  VERIFY( g_pGameLevel );  const
    // Fvector* V_array=inl_ph_world().ObjectSpace().GetStaticVerts();
    CDB::TRI* XTri = T->T;
    const float* VRT[3] = { (dReal*)&V_array[XTri->verts[0]], (dReal*)&V_array[XTri->verts[1]],
                            (dReal*)&V_array[XTri->verts[2]] };
...
IC float DistToFragmenton(const dReal* point, const dReal* pt1, const dReal* pt2, dReal* p,
                          dReal* to_point, u16& c) {
...
ICF float DistToTri(Triangle* T, const float* pos, float* dir, float* p, ETriDist& c,
                    const Fvector* V_array) {
