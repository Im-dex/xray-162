...
void TTestDepthCallback(bool& do_colide, bool bo1, dContact& c, SGameMtl* material_1,
    ...
    if (do_colide && !material_1->Flags.test(SGameMtl::flPassable) &&
        ...
        if (test_depth > Pars::depth_to_use_force) {
            ...
            if (b1)
                dBodyAddForce(b1, c.geom.normal[0] * force, c.geom.normal[1] * force,
                              c.geom.normal[2] * force);
            if (b2)
                dBodyAddForce(b2, -c.geom.normal[0] * force, -c.geom.normal[1] * force,
                              -c.geom.normal[2] * force);
...
/////////////////////////////////////////////////////////////////////////////////////
bool ActivateBoxDynamic(IPHMovementControl* mov_control, bool character_exist, DWORD id,
    ...
    //	void		CPHMovementControl::		SetOjectContactCallback (ObjectContactCallbackFun*
    //callback)
    //{
    //	if(m_character)
    ...
    // void		CPHMovementControl::		SetFootCallBack			(ObjectContactCallbackFun*
    // callback)
    //{
    //	VERIFY(m_character);
    ...
    for (int m = 0; num_steps > m; ++m) {
        ...
        for (int i = 0; num_it > i; ++i) {
            max_depth = 0.f;
            // Calculate(Fvector().set(0,0,0),Fvector().set(1,0,0),0,0,0,0);
            mov_control->actor_calculate(Fvector().set(0, 0, 0), Fvector().set(1, 0, 0), 0, 0, 0,
                                         0);
            // EnableCharacter();
            mov_control->character()->Enable();
            mov_control->character()->ApplyForce(
