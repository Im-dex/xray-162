...
void CRenderTarget::phase_scene_prepare() {
    ...
    if (RImplementation.o.advancedpp &&
        ...
        // D3DCLEAR_TARGET|D3DCLEAR_ZBUFFER|D3DCLEAR_STENCIL, 0x0, 1.0f, 0L) );
        ...
        if (!RImplementation.o.dx10_msaa)
            HW.pDevice->ClearDepthStencilView(HW.pBaseZB, D3D10_CLEAR_DEPTH | D3D10_CLEAR_STENCIL,
                                              1.0f, 0);
        else {
            HW.pDevice->ClearRenderTargetView(rt_Color->pRT, ColorRGBA);
            HW.pDevice->ClearRenderTargetView(rt_Accumulator->pRT, ColorRGBA);
            HW.pDevice->ClearDepthStencilView(rt_MSAADepth->pZRT,
        ...
        if (!RImplementation.o.dx10_msaa) {
            u_setrt(Device.dwWidth, Device.dwHeight, HW.pBaseRT, NULL, NULL, HW.pBaseZB);
            // CHK_DX	( HW.pDevice->Clear	( 0L, NULL, D3DCLEAR_ZBUFFER|D3DCLEAR_STENCIL, 0x0, 1.0f,
            // 0L) );
            HW.pDevice->ClearDepthStencilView(HW.pBaseZB, D3D10_CLEAR_DEPTH | D3D10_CLEAR_STENCIL,
                                              1.0f, 0);
        } else {
...
void CRenderTarget::phase_scene_begin() {
    ...
    if (!RImplementation.o.dx10_gbuffer_opt) {
        ...
        // else								u_setrt		(rt_Position,	rt_Color,
        // rt_Normal,		pZB);
    }
