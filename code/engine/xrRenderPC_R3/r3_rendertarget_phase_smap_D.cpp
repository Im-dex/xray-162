...
void CRenderTarget::phase_smap_direct(light* L, u32 sub_phase) {
    ...
    // else								u_setrt	(rt_smap_surf, NULL, NULL,
    // rt_smap_ZB);
    else
    ...
    if (sub_phase != SE_SUN_RAIN_SMAP)
        ...
        D3D_VIEWPORT VP = {
            L->X.D.minX, L->X.D.minY, (L->X.D.maxX - L->X.D.minX), (L->X.D.maxY - L->X.D.minY), 0, 1
        };
        // CHK_DX
    ...
    /*
    if (SE_SUN_NEAR==sub_phase)			RCache.set_CullMode			( CULL_CCW	);	//
    near else								{ if
    (RImplementation.o.HW_smap)	RCache.set_CullMode			( CULL_CW	);	// far,
    reversed
