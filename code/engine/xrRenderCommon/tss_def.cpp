    ...
    for (u32 it = 0; it < States.size(); it++) {
        ...
        case 2: {
            CHK_DX(HW.pDevice->SetSamplerState(
                ...
                ((D3DSAMPLERSTATETYPE)S.v2 == D3DSAMP_MAGFILTER && S.v3 == D3DTEXF_ANISOTROPIC)
                    ? D3DTEXF_LINEAR
                    : S.v3));
        } break;
        }
...
void SimulatorStates::UpdateDesc(D3D_RASTERIZER_DESC& desc) const {
    ...
    */
        ...
        // CHK_DX(HW.pDevice->SetTextureStageState	(S.v1,(D3DTEXTURESTAGESTATETYPE)S.v2,S.v3));
        //	TODO: DX10: Enable
        //	VERIFY(!"DirectX 10 doesn't support texture stage states. Implement shader
        //instead!"); 	break;
    }
}
        ...
        if (S.type == 0) {
            ...
            case D3DRS_STENCILZFAIL:
                desc.FrontFace.StencilDepthFailOp =
                    dx10StateUtils::ConvertStencilOp((D3DSTENCILOP)S.v2);
                break;
...
void SimulatorStates::UpdateDesc(D3D_SAMPLER_DESC descArray[D3D_COMMONSHADER_SAMPLER_SLOT_COUNT],
    ...
    for (u32 it = 0; it < States.size(); it++) {
        ...
        if (S.type == 2) {
            ...
            case D3DSAMP_MIPFILTER: /* D3DTEXTUREFILTER filter to use between mipmaps during
                                       minification */
                switch (S.v3) {
                case D3DTEXF_NONE:
                case D3DTEXF_POINT:
