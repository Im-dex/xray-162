...
void dxDebugRender::Render() {
    ...
    RCache.dbg_Draw(D3DPT_LINELIST, &*m_line_vertices.begin(), m_line_vertices.size(),
                    &*m_line_indices.begin(), m_line_indices.size() / 2);
...
void _add_lines(std::vector<FVF::L>& vertices, std::vector<u16>& indices, Fvector const* pvertices,
                u32 const& vertex_count, u16 const* pairs, u32 const& pair_count,
                u32 const& color) {
...
void dxDebugRender::add_lines(Fvector const* vertices, u32 const& vertex_count, u16 const* pairs,
