...
IC void MixInterlerp(CKey& Result, const CKey* R, const CBlend* const BA[MAX_BLENDED],
                     int b_count) {
...
//(TOTAL: %f) w: %f, w0: %f, w1: %f, ws:%f, BIS: %d",w0+w1,w,w0,w1,ws,BLEND_INST.Blend.size()); .
//}
#endif
...
//.							Debug.fatal		(DEBUG_INFO,"TO ALEXMX VERY
//IMPORTANT: (TOTAL: %f) w: %f, total: %f, count: %d, real count:
...
IC void q_scale_vs_basem(Fmatrix& m_res, const Fquaternion& q, const Fquaternion& base,
                         float scale_factor) {
    Fmatrix mb, imb;
    mb.rotation(base);
    imb.invert(mb);
...
IC void q_add_scaled_basem(Fquaternion& q, const Fquaternion& base, const Fquaternion& q0,
                           const Fquaternion& q1, float v1) {
    // VERIFY(0.f =< v && 1.f >= v );
    Fmatrix m0;
    m0.rotation(q0);
    Fmatrix m, ml1;
