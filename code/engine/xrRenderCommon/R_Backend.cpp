...
void CBackend::CreateQuadIB() {
    ...
    R_CHK(HW.pDevice->CreateIndexBuffer(dwIdxCount * 2, dwUsage, D3DFMT_INDEX16, D3DPOOL_DEFAULT,
                                        &QuadIB, NULL));
    HW.stats_manager.increment_stats_ib(QuadIB);
...
void CBackend::CreateConstantBuffers()
{
        ...
        dx10BufferUtils::CreateConstantBuffer(&m_pPixelConstants,
iVectorNumber*iVectorElements*sizeof(float));
        dx10BufferUtils::CreateConstantBuffer(&m_pVertexConstants,
iVectorNumber*iVectorElements*sizeof(float));
}
