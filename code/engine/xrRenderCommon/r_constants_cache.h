...
flush	()						{ lo=hi=0;
        ...
        ICF void				dirty	(u32 _lo, u32 _hi)		{ if
(_lo<lo) lo=_lo; if (_hi>hi) hi=_hi;	} ICF u32					r_lo	()
{ return lo;								} ICF u32
r_hi	()						{ return hi;
}
};
...
R_constant_load& L, const Fmatrix& A)
        {
                ...
                default:
#ifdef DEBUG
                        Debug.fatal		(DEBUG_INFO,"Invalid constant run-time-type for
'%s'",*C->name); #else NODEFAULT; #endif
                }
        }
...
R_constant_load& L, const Fvector4& A)
        ...
        void					seta	(R_constant* C, R_constant_load& L,
u32 e, const Fmatrix& A)
        {
                VERIFY		(RC_float == C->type);
                u32			base;
                Fvector4*	it;
...
{ if (C->destination&1)		{ a_pixel.set	(C,C->ps,A); a_pixel.b_dirty=TRUE;		} if
(C->destination&2)		{ a_vertex.set	(C,C->vs,A); a_vertex.b_dirty=TRUE;		}
        }
        ICF void				set		(R_constant* C, const Fvector4&
A)		{ if (C->destination&1)		{ a_pixel.set	(C,C->ps,A);
a_pixel.b_dirty=TRUE;		} if (C->destination&2)		{ a_vertex.set	(C,C->vs,A);
a_vertex.b_dirty=TRUE;		}
        }
        ICF void				set		(R_constant* C, float x, float y,
float z, float w)	{ Fvector4 data;		data.set(x,y,z,w); set
(C,data);
        }

        // fp, array versions
        ICF void				seta	(R_constant* C, u32 e, const Fmatrix& A)
{ if (C->destination&1)		{ a_pixel.seta	(C,C->ps,e,A); a_pixel.b_dirty=TRUE;	} if
(C->destination&2)		{ a_vertex.seta	(C,C->vs,e,A); a_vertex.b_dirty=TRUE;	}
        }
        ICF void				seta	(R_constant* C, u32 e, const Fvector4&
A)		{ if (C->destination&1)		{ a_pixel.seta	(C,C->ps,e,A);
a_pixel.b_dirty=TRUE;	} if (C->destination&2)		{ a_vertex.seta	(C,C->vs,e,A);
a_vertex.b_dirty=TRUE;	}
        }
        ICF void				seta	(R_constant* C, u32 e, float x, float y,
float z, float w)	{ Fvector4 data;		data.set(x,y,z,w); seta
(C,e,data);
        }
