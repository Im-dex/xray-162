...
void dxStatGraphRender::RenderBack(CStatGraph& owner) {
    ...
    pv_start = (FVF::TL0uv*)RCache.Vertex.Lock(2 + 2 * owner.grid.x + Num_H_LinesUp * 2 +
                                                   Num_H_LinesDwn * 2,
    ...
    for (int g_x = 1; g_x <= owner.grid.x; g_x++) {
        pv->set(int(owner.lt.x + g_x * owner.grid_step.x * elem_factor), owner.lt.y,
                owner.grid_color);
        pv++;
        pv->set(int(owner.lt.x + g_x * owner.grid_step.x * elem_factor), owner.rb.y,
                owner.grid_color);
        pv++;
    }
...
void dxStatGraphRender::RenderBars(CStatGraph& owner, FVF::TL0uv** ppv,
...
void dxStatGraphRender::RenderLines(CStatGraph& owner, FVF::TL0uv** ppv,
...
void dxStatGraphRender::RenderBarLines(CStatGraph& owner, FVF::TL0uv** ppv,
                                       CStatGraph::ElementsDeq* pelements) const {
    float elem_offs = float(owner.rb.x - owner.lt.x) / owner.max_item_count;
    float elem_factor = float(owner.rb.y - owner.lt.y) / float(owner.mx - owner.mn);
    float base_y = float(owner.rb.y) + (owner.mn * elem_factor);
...
void dxStatGraphRender::RenderMarkers(CStatGraph& owner, FVF::TL0uv** ppv,
