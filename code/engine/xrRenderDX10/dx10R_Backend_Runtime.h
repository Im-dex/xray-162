...
IC void CBackend::Render(D3DPRIMITIVETYPE T, u32 baseV, u32 startV, u32 countV, u32 startI,
                         u32 PC) {
    // VERIFY(vs);
...
IC void CBackend::set_Stencil(u32 _enable, u32 _func, u32 _ref, u32 _mask, u32 _writemask,
    ...
    // ( D3DRS_STENCILENABLE,		_enable				)); }  if (!stencil_enable)
    // return;  if (stencil_func		!= _func)		{ stencil_func=_func;
    // CHK_DX(HW.pDevice->SetRenderState	( D3DRS_STENCILFUNC,		_func				));
    // }  if (stencil_ref			!= _ref)		{ stencil_ref=_ref;
    ...
    // }  if (stencil_zfail		!= _zfail)		{ stencil_zfail=_zfail;
...
IC void CBackend::ApplyVertexLayout() {
    ...
    if (it == decl->vs_to_layout.end()) {
        ...
        it = decl->vs_to_layout
                 .insert(std::pair<ID3DBlob*, ID3DInputLayout*>(m_pInputSignature, pLayout))
                 .first;
    }
...
IC bool CBackend::CBuffersNeedUpdate(ref_cbuffer buf1[MaxCBuffers], ref_cbuffer buf2[MaxCBuffers],
                                     u32& uiMin, u32& uiMax) {
    bool bRes = false;
    int i = 0;
...
IC void CBackend::get_ConstantDirect(shared_str& n, u32 DataSize, void** pVData, void** pGData,
                                     void** pPData) {
