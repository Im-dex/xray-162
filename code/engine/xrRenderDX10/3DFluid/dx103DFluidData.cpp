...
void dx103DFluidData::CreateRTTextureAndViews(int rtIndex, D3D_TEXTURE3D_DESC TexDesc) {
    ...
    CHK_DX(HW.pDevice->CreateRenderTargetView(m_pRTTextures[rtIndex], &DescRT,
                                              &m_pRenderTargetViews[rtIndex]));
...
void dx103DFluidData::ParseProfile(const xr_string& Profile) {
    ...
    if (ini.line_exist("volume", "Type"))
        m_Settings.m_SimulationType =
            (SimulationType)ini.r_token("volume", "Type", simulation_type_token);
    ...
    for (u32 i = 0; i < iEmittersNum; ++i) {
        ...
        Emitter.m_eType = (dx103DFluidEmitters::EmitterType)ini.r_token(EmitterSectionName, "Type",
                                                                        emitter_type_token);
