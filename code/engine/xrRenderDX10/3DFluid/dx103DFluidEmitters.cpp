...
dx103DFluidEmitters::dx103DFluidEmitters(int gridWidth, int gridHeight, int gridDepth,
...
void dx103DFluidEmitters::ApplyDensity(const CEmitter& Emitter) {
    ...
    case ET_SimpleDraught:
        ...
        // );  float fFactor = 1.0f + 0.5f * _sin(t * (2.0f * float(PI)) / 30 );  FlowVelocity.mul(
        // fFactor );
        break;
    }

    // Color is the density of the smoke. We use a sinusoidal function of 't' to make it more
    // interesting.
    // static float t = 0.0f;
    // t += 0.05f;
    ...
    FLOAT density =
        1.5f * (((std::sin(t * 1.5f + 2.0f * float(PI) / 3.0f) * 0.5f + 0.5f)) * saturation +
...
void dx103DFluidEmitters::ApplyVelocity(const CEmitter& Emitter) {
    ...
    case ET_SimpleDraught:
        ...
        float fFactor =
            1.0f + Emitter.m_DraughtParams.m_fAmp *
                       std::sin((t + Emitter.m_DraughtParams.m_fPhase) * (2.0f * float(PI)) / fPeriod);
    ...
    Fvector4 center;
    center.set(Emitter.m_vPosition.x + lilrand(), Emitter.m_vPosition.y + lilrand(),
