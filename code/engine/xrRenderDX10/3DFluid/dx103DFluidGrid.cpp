...
void dx103DFluidGrid::CreateVertexBuffers() {
    ...
    // Vertex buffer for "m_vDim[2]" quads to draw all the slices of the 3D-texture as a flat
    // 3D-texture (used to draw all the individual slices at once to the screen buffer)
    index = 0;
    ...
    CHK_DX(dx10BufferUtils::CreateVertexBuffer(&m_pRenderQuadBuffer, renderQuad,
                                               vSize * m_iNumVerticesRenderQuad));
    ...
    CHK_DX(dx10BufferUtils::CreateVertexBuffer(&m_pSlicesBuffer, slices,
                                               vSize * m_iNumVerticesSlices));
    ...
    CHK_DX(dx10BufferUtils::CreateVertexBuffer(&m_pBoundarySlicesBuffer, boundarySlices,
    ...
    CHK_DX(dx10BufferUtils::CreateVertexBuffer(&m_pBoundaryLinesBuffer, boundaryLines,
...
void dx103DFluidGrid::InitScreenSlice(VS_INPUT_FLUIDSIM_STRUCT** vertices, int z, int& index) {
    ...
    // compute the offset (px, py) in the "flat 3D-texture" space for the slice with given 'z'
    // coordinate
    ...
    tempVertex2.Pos =
        D3DXVECTOR3((px + w) * 2.0f / Width - 1.0f, -((py)*2.0f / Height) + 1.0f, 0.0f);
    tempVertex2.Tex = D3DXVECTOR3(w, 0, float(z));

    tempVertex3.Pos =
        D3DXVECTOR3((px + w) * 2.0f / Width - 1.0f, -((py + h) * 2.0f / Height) + 1.0f, 0.0f);
    tempVertex3.Tex = D3DXVECTOR3(w, h, float(z));

    tempVertex4.Pos =
        D3DXVECTOR3((px)*2.0f / Width - 1.0f, -((py + h) * 2.0f / Height) + 1.0f, 0.0f);
