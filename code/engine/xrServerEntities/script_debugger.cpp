...
LRESULT CScriptDebugger::_SendMessage(u32 message, WPARAM wParam, LPARAM lParam) {
    //	if ( (m_pDebugger)&&(m_pDebugger->Active())&&(message >= _DMSG_FIRST_MSG && message <=
    //_DMSG_LAST_MSG) ) 		return m_pDebugger->DebugMessage(message, wParam, lParam);
...
LRESULT CScriptDebugger::DebugMessage(UINT nMsg, WPARAM wParam, LPARAM lParam) {
    ...
    case DMSG_ADD_STACKTRACE: {
        m_callStack->Add(((StackTrace*)wParam)->szDesc, ((StackTrace*)wParam)->szFile,
