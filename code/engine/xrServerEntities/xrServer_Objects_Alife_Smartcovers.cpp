...
BOOL is_combat_cover(shared_str const& table_id) {
    ...
    VERIFY2(result,
            make_string("bad or missing description in smart_cover [%s]", table_id.c_str()));
    ...
    if (value.type() == LUA_TNIL) {
        Msg("! is_combat_cover flag not found for smart_cover [%s], forcing to \"true\"",
            table_id.c_str());
        return (TRUE);
    }
...
void CSE_SmartCover::FillProps(LPCSTR pref, PropItemVec& items) {
    ...
    *fp_data.smart_covers.begin() = tmp.c_str();
    ...
    PHelper().CreateFloat(items, PrepareKey(pref, *s_name, "exit min enemy distance"),
                          &m_exit_min_enemy_distance, 0.f, 100.f);

    if (is_combat_cover(m_description)) {
        PHelper().CreateBOOL(items, PrepareKey(pref, *s_name, "is combat cover"),
                             &m_is_combat_cover);
...
void CSE_SmartCover::set_enterable(shared_str const& id) {
    ...
    std::vector<SSCDrawHelper>::iterator found =
        std::find_if(m_draw_data.begin(), m_draw_data.end(), id_predicate(id));
...
void draw_frustum(CDUInterface* du, float FOV, float _FAR, float A, Fvector& P, Fvector& D,
                  Fvector& U, u32 const& CL) {
...
void CSE_SmartCover::load_draw_data() {
    ...
    if (!result) {
        Msg("no or invalid smart cover description (bad or missing loopholes table in smart_cover "
            "[%s])",
            temp);
        return;
        //		VERIFY2					(result, make_string("bad or missing
        //loopholes table in smart_cover [%s]", temp));
    }
    ...
    for (; I != E; ++I) {
        ...
        /*		luabind::object	transitions;
                        ...
                        for ( ; I != E; ++I) {
                                ...
                                VERIFY2				(transition.type() ==
           LUA_TTABLE, "invalid loophole description passed");
                                shared_str			action_from =
           smart_cover::parse_vertex(transition, "action_from", true); if (action_from != "idle")
                                        continue;
                                shared_str			action_to =
           smart_cover::parse_vertex(transition, "action_to", false); if (action_to != "fire")
                                        continue;
...
void CSE_SmartCover::on_render(CDUInterface* du, ISE_AbstractLEOwner* owner, bool bSelected,
    ...
    for (; it != it_e; ++it) {
        ...
        // du->DrawCylinder(Fidentity, pos, Fvector().set(0.f, 1.f, 0.f), 1.f, .05f,
        // color_rgba(0,255,0,255), color_rgba(0,255,0,255), TRUE, FALSE);
