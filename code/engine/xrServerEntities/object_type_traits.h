...
#define declare_has(a)                                                         \
    ...
    struct has_##a {                                                           \
        ...
        static constexpr auto value =                                          \
            std::is_same<std::true_type, decltype(select<T>(nullptr))>::value; \
    };
...
struct is_base_and_derived {
    ...
    static constexpr auto value =
        std::is_class<T1>::value && std::is_class<T2>::value && std::is_base_of<T1, T2>::value;
};
...
struct is_base_and_derived_or_same_from_template {
    ...
    static constexpr auto value =
        is_same<std::true_type, decltype(select(static_cast<T2*>(nullptr)))>::value;
};
