...
static void initialize_lua_studio(lua_State* state, cs::lua_studio::world*& world,
...
static void finalize_lua_studio(lua_State* state, cs::lua_studio::world*& world,
                                lua_studio_engine*& engine) {
    world->remove(state);
...
#endif // #ifdef DEBUG
#endif // #ifndef USE_LUA_STUDIO
       //	lua_sethook							(lua(), lua_hook_call,	LUA_MASKLINE|LUA_MASKCALL|LUA_MASKRET,
       //0);
...
void CScriptEngine::process_file_if_exists(LPCSTR file_name, bool warn_if_not_exist) {
    ...
    if (m_reload_modules || (*file_name && !namespace_loaded(file_name))) {
        ...
        if (!warn_if_not_exist && !FS.exist(S)) {
            ...
            {
                print_stack();
                Msg("* trying to access variable %s, which doesn't exist, or to load script %s, "
                    "which doesn't exist too",
                    file_name, S1);
                m_stack_is_ready = true;
            }
#endif
...
void CScriptEngine::register_script_classes() {
    ...
    m_class_registrators =
        READ_IF_EXISTS(l_tpIniFile, r_string, "common", "class_registrators", "");
    xr_delete(l_tpIniFile);
