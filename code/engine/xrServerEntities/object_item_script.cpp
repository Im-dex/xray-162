...
ObjectFactory::SERVER_BASE_CLASS* CObjectItemScript::server_object(LPCSTR section) const {
    ...
    try {
        ...
        Msg("Exception [%s] raised while casting and adopting script server object from section "
            "[%s]",
            e.what(), section);
        return (0);
    } catch (...) {
...
CObjectItemScript::CObjectItemScript(luabind::object unknown_creator, const CLASS_ID& clsid,
                                     LPCSTR script_clsid)
    : inherited(clsid, script_clsid) {
