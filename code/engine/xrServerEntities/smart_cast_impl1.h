...
struct merge {
    template <typename T>
    struct iterator {
        typedef Loki::Typelist<typename T::Head, typename iterator<typename T::Tail>::result>
            result;
    };
            ...
            struct selector {
                ...
                struct _selector {
                    typedef Loki::Typelist<typename PrevHead::Head,
                                           Loki::Typelist<Target, Loki::NullType>>
                        result;
                };
                ...
                typedef typename _selector<
                    object_type_traits::is_same<Head, typename PrevHead::Head>::value>::result
                    result;
            };
...
struct conversion_sequence {
    ...
    struct list_iterator {
        ...
        struct helper {
            typedef typename conversion_sequence<
                Target, T, length, use_heritage /**,new_visited/**/>::result search_result;
        ...
        struct selector {
            ...
            typedef
                typename _selector<!std::is_same_v<Loki::NullType, helper_result>>::result result;
        };
        ...
        struct selector<false> {
            template <bool>
            struct _selector {
                typedef typename helper<typename Head::Head, max_length - 1, true>::result
                    helper_result;
                ...
                typedef typename _selector2<!std::is_same_v<Loki::NullType, helper_result>>::result
                    result;
            };
            ...
            typedef typename _selector<has_conversion<Source, typename Head::Head>::value>::result
                result;
        };

        typedef
            typename selector<can_use_heritage && object_type_traits::is_base_and_derived<
                                                      typename Head::Head, Source>::value>::result
                result;
    };
...
struct CSmartCaster {
    ...
    struct CHelper {
        IC static Target* smart_cast(Head* p) {
            return (
                CSmartCaster<Tail, Target>::smart_cast(SmartDynamicCast::smart_cast<NextHead>(p)));
        }
    };
...
struct CHelper2 {
    ...
    IC static T1* smart_cast(T2* p) {
        static_assert(!std::is_const_v<T2> || std::is_const_v<T1>,
                      "Cannot use smart cast to convert const to non const");
        ...
        VERIFY2(temp == test,
                make_string("SmartCast<%s*>(%s*) FAILED (result differs from the dynamic_cast) or "
                            "object is CORRUPTED (0x%08x -> 0x%08x)!",
    ...
    template <>
    static void* smart_cast<void>(T2* p) {
#ifdef SHOW_SMART_CAST_UNOPTIMIZED_CASES
#pragma todo("Dima to all : this smart_cast is not optimized!")
#endif
#ifdef DEBUG
...
T1 smart_cast(T2* p) {
    ...
    static_assert(std::is_void_v<std::remove_pointer_t<T1>> ||
                      std::is_polymorphic_v<std::remove_pointer_t<T1>>,
...
T1 smart_cast(T2& p) {
    ...
    static_assert(std::is_polymorphic_v<std::remove_reference_t<T1>>,
                  "Invalid target type_for_Dynamic_Cast");
    static_assert(std::is_polymorphic_v<T2>, "Invalid source type for dynamic_cast");
#endif
#ifdef SMART_CAST_STATS_ALL
...
#endif
    return (
        *SmartDynamicCast::CHelper2<T2>::smart_cast<object_type_traits::remove_reference<T1>::type>(
            &p));
}
