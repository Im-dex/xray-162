...
#ifndef DEBUG
static void put_function(lua_State* state, u8 const* buffer, u32 const buffer_size,
                         LPCSTR package_id) {
...
int CScriptStorage::vscript_log(ScriptStorage::ELuaMessageType tLuaMessageType, LPCSTR caFormat,
                                va_list marker) {
#ifndef NO_XRGAME_SCRIPT_ENGINE
#ifdef DEBUG
...
void CScriptStorage::print_stack() {
    ...
    for (int i = 0; lua_getstack(L, i, &l_tDebugInfo); ++i) {
        ...
        if (!l_tDebugInfo.name)
            script_log(ScriptStorage::eLuaMessageTypeError, "%2d : [%s] %s(%d) : %s", i,
...
int __cdecl CScriptStorage::script_log(ScriptStorage::ELuaMessageType tLuaMessageType,
                                       LPCSTR caFormat, ...) {
    va_list marker;
...
bool CScriptStorage::parse_namespace(LPCSTR caNamespaceName, LPSTR b, u32 const b_size, LPSTR c,
                                     u32 const c_size) {
    *b = 0;
    *c = 0;
    LPSTR S2;
    ...
    for (int i = 0;; ++i) {
        if (!xr_strlen(S)) {
            script_log(ScriptStorage::eLuaMessageTypeError, "the namespace name %s is incorrect!",
                       caNamespaceName);
            return (false);
        }
        LPSTR S1 = strchr(S, '.');
...
bool CScriptStorage::do_file(LPCSTR caScriptName, LPCSTR caNameSpaceName) {
    ...
    if (!load_buffer(lua(), static_cast<LPCSTR>(l_tpFileReader->pointer()),
    ...
    // because that's the first and the only call of the main chunk - there is no point to compile
    // it
