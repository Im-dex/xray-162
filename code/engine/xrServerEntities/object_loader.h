...
struct CLoader {
    ...
    struct CHelper1 {
        ...
        static void load_data(T& data, M& stream, const P& p) {
            static_assert(!std::is_polymorphic_v<T>,
                          "Cannot load polymorphic classes as binary data");
    ...
    struct CHelper {
        ...
        static void load_data<true>(T& data, M& stream, const P& p) {
            CLoader<M, P>::load_data(
                *(data = xr_new<object_type_traits::remove_pointer<T>::type>()), stream, p);
        }
    };
    ...
    struct CHelper3 {
        ...
        struct has_value_compare {
            ...
            static constexpr auto value =
                std::is_same_v<std::true_type, decltype(select<T>(nullptr))>;
        };
    ...
    static void load_data(std::pair<T1, T2>& data, M& stream, const P& p) {
        if (p(data, const_cast<typename object_type_traits::remove_const<T1>::type&>(data.first), true)) {
            ...
            VERIFY(!value);
            load_data(const_cast<typename object_type_traits::remove_const<T1>::type&>(data.first), stream,
                      p);
        }
    ...
    template <template <typename _1, typename _2, typename _3> class T1, typename T2, typename T3,
              typename T4>
