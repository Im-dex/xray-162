...
//Iassenev 	Description : Server base object
////////////////////////////////////////////////////////////////////////////
...
#pragma warning(disable : 4238)
        m_ini_file = new CInifile(&IReader((void*)(*(m_ini_string)), m_ini_string.size()),
...
BOOL CSE_Abstract::Spawn_Read(NET_Packet& tNetPacket) {
    ...
    if (m_wVersion > 70) {
        u16 client_data_size = (m_wVersion > 93)
                                   ? tNetPacket.r_u16()
                                   : tNetPacket.r_u8(); //не может быть больше 256 байт
        if (client_data_size > 0) {
            //			Msg					("SERVER:loading:load:%d
...
void CSE_Abstract::FillProps(LPCSTR pref, PropItemVec& items) {
    ...
    #ifdef XRGAME_EXPORTS
    #	ifdef DEBUG
            PHelper().CreateToken8		(items,	PrepareKey(pref,"Game Type"),
    &s_gameid,		game_types);
        PHelper().CreateU16			(items,	PrepareKey(pref, "Respawn Time
    (s)"),	&RespawnTime,	0,43200);
