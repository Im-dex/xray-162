...
char* engine::class_name(char* const buffer, unsigned int const size,
...
void engine::type_convert_class(char* const buffer, unsigned int const size, lua_State* state,
                                int index) {
...
static bool is_luabind_class(lua_State* state, int const index) {
    ...
    if (luabind::detail::class_registry::get_registry(state)->find_class(class_rep->type()) !=
        class_rep)
        return (false);
...
bool engine::type_convert_instance(char* buffer, unsigned int const size, lua_State* state,
                                   int index) {
...
void engine::type_convert_userdata(char* buffer, unsigned int const size, lua_State* state,
                                   int index) {
...
bool engine::type_to_string(char* const buffer, unsigned int const size, lua_State* const state,
                            int const index, bool& use_in_description) {
...
void engine::fill_class_info(cs::lua_studio::backend& backend, char* const buffer,
                             unsigned int const size, luabind::detail::object_rep* object,
                             luabind::detail::class_rep* class_rep, lua_State* state) {
    pstr stream = buffer;
    ...
    for (u32 i = 0; I != E; ++I) {
        ...
        if (use_in_description)
            stream +=
                xr_sprintf(stream, size - (stream - buffer), "%s[%s]=%s ", (*I).first, type, value);
        else
...
bool engine::value_convert_instance(cs::lua_studio::backend& backend, char* buffer,
                                    unsigned int size, luabind::detail::object_rep* object,
                                    lua_State* state) {
...
bool engine::value_convert_instance(cs::lua_studio::backend& backend, char* buffer,
    ...
    if (full_description && !value_convert_instance(backend, buffer, size, object, state))
        value_convert_class(backend, buffer, size, object->crep(), state, index, icon_type,
                            full_description);
    else
        sz_cpy(buffer, size, " ");
...
bool engine::value_to_string(cs::lua_studio::backend& backend, char* const buffer,
                             unsigned int const size, lua_State* const state, int const index,
    ...
    case engine::lua_type_user_data: {
        if (!luabind::detail::is_class_object(state, index)) {
            ...
            luabind::detail::class_rep* class_rep =
                static_cast<luabind::detail::class_rep*>(lua_touserdata(state, index));
            VERIFY(class_rep);
            value_convert_class(backend, buffer, size, class_rep, state, index, icon_type,
                                full_description);
            return (true);
        }

        if (value_convert_instance(backend, buffer, size, state, index, icon_type,
                                   full_description))
            return (true);
...
void engine::push_class_base(lua_State* const state, char const* const id) {
    luabind::detail::class_rep* class_rep =
        static_cast<luabind::detail::class_rep*>(lua_touserdata(state, -1));
    VERIFY(class_rep);
...
void engine::push_user_data(lua_State* const state, char const* const id,
...
bool engine::push_value(lua_State* const state, char const* const id,
                        cs::lua_studio::icon_type const icon_type) {
...
void engine::fill_class_data(cs::lua_studio::backend& backend,
    ...
    luabind::detail::object_rep* object =
        static_cast<luabind::detail::object_rep*>(lua_touserdata(state, -2));
...
void engine::expand_class(cs::lua_studio::backend& backend, cs::lua_studio::value_to_expand& value,
                          lua_State* const state) {
...
void engine::expand_class_instance(cs::lua_studio::backend& backend,
                                   cs::lua_studio::value_to_expand& value_to_expand,
                                   lua_State* const state) {
...
void engine::expand_user_data(cs::lua_studio::backend& backend,
                              cs::lua_studio::value_to_expand& value, lua_State* const state) {
    luabind::detail::object_rep* object = luabind::detail::is_class_object(state, -1);
    if (object) {
...
bool engine::expand_value(cs::lua_studio::backend& backend, cs::lua_studio::value_to_expand& value,
                          lua_State* const state) {
    switch (lua_type(state, -1)) {
    case engine::lua_type_nil:
    case engine::lua_type_table:
