...
public:
    PropItem(EPropType _type)
        : type(_type), prop_color(0), val_color(0), item(0), key(0), OnClickEvent(0),
          OnDrawTextEvent(0), OnItemFocused(0) {
        m_Flags.zero();
    }
    virtual ~PropItem() {
    ...
    IC std::string GetDrawText() {
        VERIFY(!values.empty());
        return m_Flags.is(flMixed) ? std::string("(mixed)")
...
public:
    ...
    typedef fastdelegate::FastDelegate3<CanvasValue*, void* /* TCanvas* */, const Irect&>
        TOnDrawCanvasEvent;
...
public:
    ...
    virtual std::string GetDrawText(TOnDrawTextEvent) {
        return std::string(_ListToSequence(value));
    }
    virtual void ResetValue() { ; }
...
public:
    typedef fastdelegate::FastDelegate3<ShortcutValue*, const xr_shortcut&, bool&>
        TOnValidateResult;
...
public:
    ChooseValue(shared_str* val, u32 cid, LPCSTR path, void* param, u32 sub_item_count,
                u32 choose_flags)
        : RTextValue(val), m_ChooseID(cid), m_StartPath(path), subitem(sub_item_count), m_Items(0),
          m_FillParam(param), OnChooseFillEvent(0), OnDrawThumbnailEvent(0),
...
public:
    CListValue(LPSTR val, u32 sz, std::string* _items, u32 cnt)
