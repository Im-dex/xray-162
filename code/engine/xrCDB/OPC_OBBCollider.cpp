 ...
 */
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
bool OBBCollider::Collide(OBBCache& cache, const OBB& box, OPCODE_Model* model,
    ...
    if (!model->HasLeafNodes()) {
        if (model->IsQuantized())
            return Collide(cache, box, (const AABBQuantizedNoLeafTree*)model->GetTree(), worldb,
                           worldm);
        else
 ...
 */
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
BOOL OBBCollider::InitQuery(OBBCache& cache, const OBB& box, const Matrix4x4* worldb,
    ...
    for (udword i = 0; i < 3; i++) {
        for (udword j = 0; j < 3; j++) {
            ...
            mAR.m[i][j] = 1e-6f + xr::abs(mRBoxToModel.m[i][j]);
        }
    }
    ...
    if (TemporalCoherenceEnabled()) {
        ...
        if (FirstContactEnabled()) {
            ...
            if (mTouchedPrimitives->GetNbEntries()) {
                ...
                // Perform overlap test between the cached triangle and the box (and set contact
                // status if needed)
...
inline_ BOOL OBBCollider::OBBContainsBox(const Point& bc, const Point& be) {
    ...
    mRModelToBox.m[2][1];	if(f>mB0.y || f<mB1.y) return FALSE;\ f = p.x * mRModelToBox.m[0][2]
    + p.y * mRModelToBox.m[1][2] + p.z * mRModelToBox.m[2][2];	if(f>mB0.z || f<mB1.z) return FALSE;
    ...
    */
    ...
    // - test AABB-in-AABB
    float NCx =
        bc.x * mRModelToBox.m[0][0] + bc.y * mRModelToBox.m[1][0] + bc.z * mRModelToBox.m[2][0];
    float NEx = xr::abs(mRModelToBox.m[0][0] * be.x) + xr::abs(mRModelToBox.m[1][0] * be.y) +
                xr::abs(mRModelToBox.m[2][0] * be.z);
    ...
    float NCy =
        bc.x * mRModelToBox.m[0][1] + bc.y * mRModelToBox.m[1][1] + bc.z * mRModelToBox.m[2][1];
    float NEy = xr::abs(mRModelToBox.m[0][1] * be.x) + xr::abs(mRModelToBox.m[1][1] * be.y) +
                xr::abs(mRModelToBox.m[2][1] * be.z);
    ...
    float NCz =
        bc.x * mRModelToBox.m[0][2] + bc.y * mRModelToBox.m[1][2] + bc.z * mRModelToBox.m[2][2];
    float NEz = xr::abs(mRModelToBox.m[0][2] * be.x) + xr::abs(mRModelToBox.m[1][2] * be.y) +
                xr::abs(mRModelToBox.m[2][2] * be.z);
