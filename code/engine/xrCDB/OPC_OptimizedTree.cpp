...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/**
 *	Builds an implicit tree from a standard one. An implicit tree is a complete tree (2*N-1
 *nodes) whose negative box pointers and primitive pointers have been made implicit, hence packing 3
 *pointers in one.
 *
 ...
 *	\fn			_BuildCollisionTree(AABBCollisionNode* linear, const udword boxid,
 *udword& curid, const AABBTreeNode* curnode)
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/**
 *	Builds a "no-leaf" tree from a standard one. This is a tree whose leaf nodes have been
 *removed.
 *
 ...
 *	\fn			_BuildNoLeafTree(AABBNoLeafNode* linear, const udword boxid, udword&
 *curid, const AABBTreeNode* curnode)
 *	\param		linear		[in] base address of destination nodes
 *	\param		boxid		[in] index of destination node
 *	\param		curid		[in] current running index
 *	\param		curnode		[in] current node from input tree
 */
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
static void _BuildNoLeafTree(AABBNoLeafNode* linear, const udword boxid, udword& curid,
                             const AABBTreeNode* curnode) {
...
#define FIND_MAX_VALUES                               \
    ...
    for (udword i = 0; i < mNbNodes; i++) {           \
        if (xr::abs(Nodes[i].mAABB.mCenter.x) > CMax.x)  \
            CMax.x = xr::abs(Nodes[i].mAABB.mCenter.x);  \
        if (xr::abs(Nodes[i].mAABB.mCenter.y) > CMax.y)  \
            CMax.y = xr::abs(Nodes[i].mAABB.mCenter.y);  \
        if (xr::abs(Nodes[i].mAABB.mCenter.z) > CMax.z)  \
            CMax.z = xr::abs(Nodes[i].mAABB.mCenter.z);  \
        if (xr::abs(Nodes[i].mAABB.mExtents.x) > EMax.x) \
            EMax.x = xr::abs(Nodes[i].mAABB.mExtents.x); \
        if (xr::abs(Nodes[i].mAABB.mExtents.y) > EMax.y) \
            EMax.y = xr::abs(Nodes[i].mAABB.mExtents.y); \
        if (xr::abs(Nodes[i].mAABB.mExtents.z) > EMax.z) \
            EMax.z = xr::abs(Nodes[i].mAABB.mExtents.z); \
    }
    ...
    if (gFixQuantized) {                                                                        \
        ...
        for (udword j = 0; j < 3; j++) { /* Dequantize the box center */                        \
            ...
            do { /* Dequantize the box extent */                                                \
                float qe =                                                                      \
                    float(((float*)mNodes[i].mAABB.mExtents)[j]) * ((float*)mExtentsCoeff)[j];  \
