...
inline_ BOOL RayCollider::SegmentAABBOverlap(const Point& center, const Point& extents) {
    ...
    if (xr::abs(Dx) > extents.x + mFDir.x)
        return FALSE;
    float Dy = mData2.y - center.y;
    if (xr::abs(Dy) > extents.y + mFDir.y)
        return FALSE;
    float Dz = mData2.z - center.z;
    if (xr::abs(Dz) > extents.z + mFDir.z)
        return FALSE;

    float f;
    f = mData.y * Dz - mData.z * Dy;
    if (xr::abs(f) > extents.y * mFDir.z + extents.z * mFDir.y)
        return FALSE;
    f = mData.z * Dx - mData.x * Dz;
    if (xr::abs(f) > extents.x * mFDir.z + extents.z * mFDir.x)
        return FALSE;
    f = mData.x * Dy - mData.y * Dx;
    if (xr::abs(f) > extents.x * mFDir.y + extents.y * mFDir.x)
        return FALSE;
...
inline_ BOOL RayCollider::RayAABBOverlap(const Point& center, const Point& extents) {
    ...
    //	float Dx = mOrigin.x - center.x;	if(xr::abs(Dx) > extents.x && Dx*mDir.x>=0.0f)	return
    //FALSE; 	float Dy = mOrigin.y - center.y;	if(xr::abs(Dy) > extents.y && Dy*mDir.y>=0.0f)
    //return FALSE; 	float Dz = mOrigin.z - center.z;	if(xr::abs(Dz) > extents.z &&
