...
// RayTest - Occluded/No
//--------------------------------------------------------------------------------
BOOL CObjectSpace::RayTest(const Fvector& start, const Fvector& dir, float range,
...
// RayPick
//--------------------------------------------------------------------------------
BOOL CObjectSpace::RayPick(const Fvector& start, const Fvector& dir, float range, rq_target tgt,
                           rq_result& R, CObject* ignore_object) {
    std::lock_guard<decltype(Lock)> lock(Lock);
    BOOL _res = _RayPick(start, dir, range, tgt, R, ignore_object);
    r_spatial.clear();
    return _res;
}
BOOL CObjectSpace::_RayPick(const Fvector& start, const Fvector& dir, float range, rq_target tgt,
                            rq_result& R, CObject* ignore_object) {
    r_temp.r_clear();
    R.O = 0;
    R.range = range;
...
// RayQuery
//--------------------------------------------------------------------------------
BOOL CObjectSpace::RayQuery(collide::rq_results& dest, const collide::ray_defs& R,
...
BOOL CObjectSpace::_RayQuery2(collide::rq_results& r_dest, const collide::ray_defs& R,
                              collide::rq_callback* CB, LPVOID user_data,
...
BOOL CObjectSpace::_RayQuery3(collide::rq_results& r_dest, const collide::ray_defs& R,
                              collide::rq_callback* CB, LPVOID user_data,
                              collide::test_callback* tb, CObject* ignore_object) {
    // initialize query
    r_dest.r_clear();
...
BOOL CObjectSpace::_RayQuery(collide::rq_results& r_dest, const collide::ray_defs& R,
                             collide::rq_callback* CB, LPVOID user_data, collide::test_callback* tb,
                             CObject* ignore_object) {
#ifdef DEBUG
                ...
                if (xrc.r_count()) {
                    if (s_res.set_if_less(xrc.r_begin())) {
                        ...
                        if (!(fis_zero(s_res.range, EPS) || s_res.range >= 0.f))
                            Debug.fatal(DEBUG_INFO, "Invalid RayQuery static range: %f (%f). /#1/",
        ...
        if ((R.tgt & d_mask) && sd_test.is_any(d_mask) && (next_test & d_mask)) {
...
#ifdef DEBUG
                    if (!((0 == r_temp.r_count()) ||
                          (r_temp.r_count() && (fis_zero(r_temp.r_begin()->range, EPS) ||
            ...
            if (r_temp.r_count()) {
                ...
                if (!(fis_zero(d_res.range, EPS) || d_res.range >= 0.f))
                    Debug.fatal(DEBUG_INFO, "Invalid RayQuery dynamic range: %f (%f). /#3/",
...
BOOL CObjectSpace::RayQuery(collide::rq_results& r_dest, ICollisionForm* target,
