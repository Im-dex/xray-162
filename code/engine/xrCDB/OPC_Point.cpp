 ...
 *	3D point.
 *
 *	The name is "Point" instead of "Vector" since a vector3 is N-dimensional, whereas a point is
 *an implicit "vector3 of dimension 3". So the choice was between "Point" and "Vector3", the first
 *one looked better (IMHO).
 *
 *	Some people, then, use a typedef to handle both points & vectors using the same class:
 *typedef Point Vector3; This is bad since it opens the door to a lot of confusion while reading the
 *code. I know it may sounds weird but check this out:
 *
 *	\code
 ...
 *	\endcode
 *
 *	Subtle things like this are not caught at compile-time, and when you find one in the code,
 *you never know whether it's a mistake from the author or something you don't get.
 *
 *	One way to handle it at compile-time would be to use different classes for Point & Vector3,
 *only overloading operator "-" for vectors. But then, you get a lot of redundant code in thoses
 *classes, and basically it's really a lot of useless work.
 *
 *	Another way would be to use homogeneous points: w=1 for points, w=0 for vectors. That's why
 *the HPoint class exists. Now, to store your model's vertices and in most cases, you really want to
 *use Points to save ram.
 *
 *	\class		Point
 *	\author		Pierre Terdiman
 *	\version	1.0
...
Point& Point::Unfold(Plane& p, Point& a, Point& b) {
    ...
    /*	Point v = *this;
            ...
            float DpZ = FastFabs(Axis.z);
                            if(FastFabs((Axis|Point(0.0f, 0.0f, 1.0f)) - 1.0f) < 0.0001f)
       M.Identity(); else	if(FastFabs((Axis|Point(0.0f, 0.0f, 1.0f)) + 1.0f) < 0.0001f)	{
       M.Identity(); Angle = -Angle; } else
       M.MapToZ(Axis);
            ...
            // 4) Rotate back (since M is a rotation matrix we don't invert or transpose it, just
       swap mul conventions) p0t = p0r * M; p1t = p1r * M; p2t = p2r * M;
...
Point& Point::Mult2(const Matrix3x3& mat1, const Point& a1, const Matrix3x3& mat2,
                    const Point& a2) {
    x = a1.x * mat1.m[0][0] + a1.y * mat1.m[0][1] + a1.z * mat1.m[0][2] + a2.x * mat2.m[0][0] +
        a2.y * mat2.m[0][1] + a2.z * mat2.m[0][2];
    y = a1.x * mat1.m[1][0] + a1.y * mat1.m[1][1] + a1.z * mat1.m[1][2] + a2.x * mat2.m[1][0] +
        a2.y * mat2.m[1][1] + a2.z * mat2.m[1][2];
    z = a1.x * mat1.m[2][0] + a1.y * mat1.m[2][1] + a1.z * mat1.m[2][2] + a2.x * mat2.m[2][0] +
