 ...
 *	tests, you're requested to provide the triangle-vertices corresponding to a given face
 *index.
 *
 *	Ex:
 *
 *	\code
 *		static void ColCallback(udword triangleindex, VertexPointers& triangle, udword
 *user_data)
 *		{
 ...
 *	store the geometry & topology in the collision system as well (as in RAPID) but we have
 ...
 *		TC.GetNbBVBVTests()		= number of BV-BV overlap tests performed during last
 *query TC.GetNbPrimPrimTests()	= number of Triangle-Triangle overlap tests performed during
 *last query TC.GetNbBVPrimTests()	= number of Triangle-BV overlap tests performed during last
 *query \endcode
 *
 *	\class		OPCODE_Model
 *	\author		Pierre Terdiman
...
bool OPCODE_Model::Build(const OPCODECREATE& create) {
    ...
    if (!(create.Rules & SPLIT_COMPLETE))
        return SetIceError; //("OPCODE WARNING: supports complete trees only! Use
                            //SPLIT_COMPLETE.\n");

    // Check topology. If the model contains degenerate faces, collision report can be wrong in some
    // cases. e.g. it happens with the standard MAX teapot. So clean your meshes first... If you
    // don't have a mesh cleaner you can try this: www.codercorner.com/Consolidation.zip
