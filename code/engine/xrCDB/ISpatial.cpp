...
void ISpatial_DB::insert(ISpatial* S) {
    ...
    BOOL bValid = xr::valid(S->spatial.sphere.R) && xr::valid(S->spatial.sphere.P);
    if (!bValid) {
        ...
        if (O)
            ...
            if (P)
                Debug.fatal(DEBUG_INFO,
                            "Invalid PS spatial position{%3.2f,%3.2f,%3.2f} or radius{%3.2f}",
                            VPUSH(S->spatial.sphere.P), S->spatial.sphere.R);
            else
                Debug.fatal(DEBUG_INFO,
                            "Invalid OTHER spatial position{%3.2f,%3.2f,%3.2f} or radius{%3.2f}",
                            VPUSH(S->spatial.sphere.P), S->spatial.sphere.R);
        }
    }
#endif
