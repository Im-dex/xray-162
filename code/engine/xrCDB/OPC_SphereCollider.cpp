 ...
 *	This class performs a collision test between a sphere and an AABB tree. You can use this to
 *do a standard player vs world collision, in a Nettle/Telemachos way. It doesn't suffer from all
 *reported bugs in those two classic codes - the "_new_" one by Paul Nettle is a debuggued version
 *I think. Collision response can be driven by reported collision data - it works extremely well
 *for me. In sake of efficiency, all meshes (that is, all AABB trees) should of course also be kept
 *in an extra hierarchical structure (octree, whatever).
 *
 ...
 */
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
bool SphereCollider::Collide(SphereCache& cache, const Sphere& sphere, OPCODE_Model* model,
    ...
    if (!model->HasLeafNodes()) {
        if (model->IsQuantized())
            return Collide(cache, sphere, (const AABBQuantizedNoLeafTree*)model->GetTree(), worlds,
                           worldm);
        else
            return Collide(cache, sphere, (const AABBNoLeafTree*)model->GetTree(), worlds, worldm);
    } else {
        if (model->IsQuantized())
            return Collide(cache, sphere, (const AABBQuantizedTree*)model->GetTree(), worlds,
                           worldm);
        else
            return Collide(cache, sphere, (const AABBCollisionTree*)model->GetTree(), worlds,
                           worldm);
    }
}
...
BOOL SphereCollider::InitQuery(SphereCache& cache, const Sphere& sphere, const Matrix4x4* worlds,
    ...
    if (TemporalCoherenceEnabled()) {
        ...
        if (FirstContactEnabled()) {
            // We're only interested in the first contact found => test the unique previously
            // touched face
            if (mTouchedPrimitives->GetNbEntries()) {
                ...
                // Perform overlap test between the cached triangle and the sphere (and set contact
                // status if needed)
 ...
 */
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
bool SphereCollider::Collide(SphereCache& cache, const Sphere& sphere,
                             const AABBCollisionTree* tree, const Matrix4x4* worlds,
                             const Matrix4x4* worldm) {
    // Checkings
    if (!tree)
        return false;
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
bool SphereCollider::Collide(SphereCache& cache, const Sphere& sphere,
                             const AABBQuantizedTree* tree, const Matrix4x4* worlds,
                             const Matrix4x4* worldm) {
    // Checkings
    if (!tree)
        return false;
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
bool SphereCollider::Collide(SphereCache& cache, const Sphere& sphere,
                             const AABBQuantizedNoLeafTree* tree, const Matrix4x4* worlds,
                             const Matrix4x4* worldm) {
    // Checkings
    if (!tree)
        return false;
