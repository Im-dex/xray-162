...
bool PlanesCollider::Collide(PlanesCache& cache, const Plane* planes, udword nb_planes,
                             OPCODE_Model* model, const Matrix4x4* worldm) {
    // Checkings
    if (!planes || !model)
        return false;
    ...
    if (!model->HasLeafNodes()) {
        if (model->IsQuantized())
            ...
            return Collide(cache, planes, nb_planes, (const AABBNoLeafTree*)model->GetTree(),
                           worldm);
    } else {
        if (model->IsQuantized())
            return Collide(cache, planes, nb_planes, (const AABBQuantizedTree*)model->GetTree(),
                           worldm);
        else
            return Collide(cache, planes, nb_planes, (const AABBCollisionTree*)model->GetTree(),
                           worldm);
    }
}
 ...
 */
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
BOOL PlanesCollider::InitQuery(PlanesCache& cache, const Plane* planes, udword nb_planes,
    ...
    if (worldm) {
        ...
        std::memcpy(mPlanes, planes, nb_planes * sizeof(Plane));
    ...
    if (TemporalCoherenceEnabled()) {
        ...
        if (FirstContactEnabled()) {
            ...
            if (mTouchedPrimitives->GetNbEntries()) {
                ...
                // Perform overlap test between the cached triangle and the planes (and set contact
                // status if needed)
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
bool PlanesCollider::Collide(PlanesCache& cache, const Plane* planes, udword nb_planes,
                             const AABBCollisionTree* tree, const Matrix4x4* worldm) {
    // Checkings
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
bool PlanesCollider::Collide(PlanesCache& cache, const Plane* planes, udword nb_planes,
                             const AABBNoLeafTree* tree, const Matrix4x4* worldm) {
    // Checkings
    if (!tree || !planes || !nb_planes)
        return false;
#ifdef OPC_USE_CALLBACKS
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
bool PlanesCollider::Collide(PlanesCache& cache, const Plane* planes, udword nb_planes,
                             const AABBQuantizedTree* tree, const Matrix4x4* worldm) {
    // Checkings
    if (!tree || !planes || !nb_planes)
        return false;
#ifdef OPC_USE_CALLBACKS
...
void PlanesCollider::_Collide(const AABBCollisionNode* node, udword clipmask) {
    // Test the box against the planes. If the box is completely culled, so are its children, hence
    // we exit.
    udword OutClipMask;
