 ...
 *	This is a vanilla AABB tree, without any particular optimization. It contains anonymous
 *references to user-provided primitives, which can theoretically be anything - triangles, boxes,
 *etc. Each primitive is surrounded by an AABB, regardless of the primitive's nature. When the
 *primitive is a triangle, the resulting tree can be converted into an optimized tree. If the
 *primitive is a box, the resulting tree can be used for culling - VFC or occlusion -, assuming you
 *cull on a mesh-by-mesh basis (modern way).
 *
 *	\class		AABBTree
 *	\author		Pierre Terdiman
 *	\version	1.2
...
udword AABBTreeNode::Split(udword axis, AABBTreeBuilder* builder) {
    ...
    for (udword i = 0; i < mNbPrimitives; i++) {
        ...
        // Test against the splitting value. The primitive value is tested against the enclosing-box
        // center. [We only need an approximate partition of the enclosing box here.]
