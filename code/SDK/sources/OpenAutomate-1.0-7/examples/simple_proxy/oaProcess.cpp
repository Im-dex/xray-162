...
struct Process_InfoType
{
  STARTUPINFOA ChildStartupInfo;
  PROCESS_INFORMATION ProcInfo;
}; 

static string ConstructCmd(const vector<string> &cmd, string& cur_path)
{
  string Ret;
  ...
  const char* p = Ret.data();
  const char* q = strrchr(p, '\\');
  if(q)
    cur_path.append(p, q-p+1);
...
bool oaProcess::RunAsync(void)
{
  assert(!pIsRunning);

  Process_InfoType *Info = (Process_InfoType *)pProcInfo;

  BOOL Ret;
  string Cmd, Path;
  Cmd = ConstructCmd(pCmd, Path);
  ...
  Ret = CreateProcessA(NULL,
                       ...
                       0, 
                       NULL,
                       NULL,//Path.empty() ? NULL : Path.data(),
...
int oaProcess::Id(void)
{
  Process_InfoType *Info = (Process_InfoType *)pProcInfo;
  return((int)Info->ProcInfo.dwProcessId);
}

bool oaProcess::Wait(int &exit_code)
{
  if(!pIsRunning)
    return(false);
  ...
  if(Ret == WAIT_FAILED)
    return(false);
  Result = GetExitCodeProcess(Info->ProcInfo.hProcess, &ExitCode);
...
void oaProcess::Init(void)
{
  pProcInfo = malloc(sizeof(Process_InfoType));
  memset(pProcInfo, 0, sizeof(Process_InfoType));

  pIsRunning = false;
}

oaProcess::oaProcess(int argc, char *argv[])
{
  Init();
...
oaProcess::oaProcess(const std::vector<std::string> &cmd)
{
  Init();
