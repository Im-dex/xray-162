...
class oaProxyCmdBase;

class oaProxy
{
public:
  oaProxy();
  oaProxy(int port, const char* trace_file);
  ~oaProxy();
  ...
  void AddCmd(oaProxyCmdBase *cmd);

  void IssueCommand(oaCommand *command);
  void IssueExitAndRestart(void);

  std::map<oaCommandType,bool> oaCmdReset;

protected:

  oaBool RunProxySever(std::vector<std::string> app_cmd);
  std::string GetOAPluginPath(void);
  std::string ThisExePath(void);

  void ClearCmdList(){pProxyCmds.clear();}

private:

  oaRPCTransport pTransport;
  oaRPCServer *pServer;

  oaProcess *pClientProcess;
  ...
  std::vector<oaProxyCmdBase*> pProxyCmds;
  std::vector<oaProxyCmdBase*>::iterator pCurrentProxyCmd;

  bool RunClientApp(std::vector<std::string> app_cmd);
  void oaProxy::RunCmds(void);
    
  static oaProxy *pCurrent;

  /*
   * OA callbacks for proxy server.
   */
  static oaCommandType pGetNextCommand(void* proxy, oaCommand *command);
  static oaNamedOption* pGetNextOption(void* proxy);
  
  static void pAddOption(void* proxy, const oaNamedOption *option);
  static void pAddOptionValue(void* proxy, 
                             const oaChar *name,
                             oaOptionDataType value_type,
                             const oaValue *value);
  static void pAddBenchmark(void* proxy, const oaChar *benchmark_name);
  static oaBool pSendSignal(void* proxy, oaSignalType signal, void *param);
  static void pStartBenchmark(void* proxy);
  static void pDisplayFrame(void* proxy, oaFloat t);
  static void pEndBenchmark(void* proxy);
  static void pAddResultValue(void* proxy, 
                             const oaChar *name, 
                             oaOptionDataType value_type,
                             const oaValue *value);
  static void pAddFrameValue(void* proxy, 
                            const oaChar *name, 
                            oaOptionDataType value_type,
                            const oaValue *value);
};
