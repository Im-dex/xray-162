...
oaProxy *oaProxy::pCurrent = NULL;
oaRPCFunctionTable oaProxy::pFuncTable;

oaProxy::oaProxy()
{
  memset(&pFuncTable, 0, sizeof(pFuncTable));
  pFuncTable.GetNextCommand = pGetNextCommand;
  pFuncTable.GetNextOption = pGetNextOption;
  pFuncTable.AddOption = &pAddOption;
  pFuncTable.AddOptionValue = pAddOptionValue;
  pFuncTable.AddBenchmark = pAddBenchmark;
  pFuncTable.SendSignal = pSendSignal;
  pFuncTable.StartBenchmark = pStartBenchmark;
  pFuncTable.EndBenchmark = pEndBenchmark;
  pFuncTable.DisplayFrame = pDisplayFrame;
  pFuncTable.AddResultValue = pAddResultValue;
  pFuncTable.AddFrameValue = pAddFrameValue;
...
oaProxy::oaProxy(int port, const char* trace_file)
{
  oaProxy();

  pPort = port;
  pTraceFilename = trace_file;
  pClientProcess = NULL;
}

oaProxy::~oaProxy()
{
  std::vector<oaProxyCmdBase*>::iterator Iter = pProxyCmds.begin();
  for(; Iter != pProxyCmds.end(); Iter++)
    delete *Iter;

}

void oaProxy::AddCmd(oaProxyCmdBase *cmd)
{
  assert(cmd != NULL);
  
  cmd->SetProxy(this);

  pProxyCmds.push_back(cmd);
}

void oaProxy::OAMainLoop(const char *opt)
{
  oaCommand Command;
  ...
  {
   fprintf(stderr, "OA did not initialize properly.");
   return;
  }
...
oaBool oaProxy::RunProxySever(std::vector<std::string> app_cmd)
{
  app_cmd.push_back(std::string("-openautomate"));
  char Str[1024];
  sprintf(Str, "%s;localhost:%d", GetOAPluginPath().data(), pPort);
  app_cmd.push_back(std::string(Str));

  oaRPCTransport Transport;
  oaRPCTransportErrorType TsRet;
  oaRPCServerErrorType    SvrRet;

  TsRet = oaRPCInitSocketServerTransport(&Transport, pPort);
  if(TsRet != OARPC_TRANSPORT_ERROR_OK)
  {
    fprintf(stderr, "Couldn't start server on tcp port %d", pPort); 
    return OA_TRUE;
  }

  pServer = oaRPCCreateServer(&pFuncTable, 
    &Transport, 
    NULL);

  RunClientApp(app_cmd);

  SvrRet = oaRPCRunServer(pServer, OARPC_INFINITE);

  oaRPCDestroyServer(pServer);

  if(pClientProcess)
  {
    int ExitCode = 69;

    if(!pClientProcess->Wait(ExitCode))
      fprintf(stderr, "Wait for OA application failed.");

    if(ExitCode != 0)
      fprintf(stderr, "OA application exited with non-zero exit code %d",
      ExitCode);

    delete pClientProcess;
    pClientProcess = NULL;
  }
...
void oaProxy::IssueCommand(oaCommand *command)
{
  assert(command != NULL);
...
static oaBool PostExit(void *data)
{
  assert(data != NULL);

  oaProcess *ClientApp = (oaProcess *)data;

  int ExitCode = 0;
  ClientApp->Wait(ExitCode);

  if (ExitCode != 0)
    fprintf(stderr,"Client exited with non-zero exit code %d.",ExitCode);

  if (!ClientApp->Run())
    fprintf(stderr,"Restarting client application failed.");

  return(OA_TRUE);
}

void oaProxy::IssueExitAndRestart(void)
{
#ifdef _DEBUG
  fprintf(stderr,"IssueExutAndRestart issued\n");
#endif
  ...
  Ret = oaRPCServerSendExitCommand(pServer, &PostExit, pClientProcess);
  if(OARPC_SERVER_ERROR_OK != Ret)
    fprintf(stderr,"Could not restart client application.");
}

void oaProxy::RunCmds(void)
{
   pCurrentProxyCmd = pProxyCmds.begin();
  for(int i=1; pCurrentProxyCmd != pProxyCmds.end(); pCurrentProxyCmd++, i++)
  {
    oaProxyCmdBase *Cmd = *pCurrentProxyCmd;
    Cmd->Run(); 
  }
}

bool oaProxy::RunClientApp(std::vector<std::string> app_cmd)
{
  int ExitCode;

  pCurrent = this;

  if(pClientProcess != NULL)
  {
    if(!pClientProcess->Wait(ExitCode))
      fprintf(stderr, "Wait for OA application to exit failed.");
  ...
  pClientProcess = new oaProcess(app_cmd);
  if(!pClientProcess->RunAsync())
    fprintf(stderr, "Couldn't launch OA application.");

  return(true);
}

std::string oaProxy::ThisExePath(void)
{
  char Path[4096];
...
std::string oaProxy::GetOAPluginPath(void)
{
  std::string FullPath = ThisExePath() + std::string("\\oaremote_plugin.dll");
...
/*
* OA callbacks for proxy server.
*/

oaCommandType oaProxy::pGetNextCommand(void* proxy, oaCommand *command)
{
...
oaNamedOption *oaProxy::pGetNextOption(void* proxy)
{

  return((*pCurrent->pCurrentProxyCmd)->GetNextOption());

  return NULL;
}

void oaProxy::pAddOption(void* proxy, const oaNamedOption *option)
{
  oaAddOption(option);
  return;
}

void oaProxy::pAddOptionValue(void* proxy, 
                              const oaChar *name,
                              oaOptionDataType value_type,
                              const oaValue *value)
{
  oaAddOptionValue(name, value_type, value);
  return;
}

void oaProxy::pAddBenchmark(void* proxy, const oaChar *benchmark_name)
{
  oaAddBenchmark(benchmark_name);
  return;
}

oaBool oaProxy::pSendSignal(void* proxy, oaSignalType signal, void *param)
{
  oaSendSignal(signal, param);
  return OA_TRUE;
}

void oaProxy::pStartBenchmark(void* proxy)
{
  oaStartBenchmark();
  return;
}

void oaProxy::pDisplayFrame(void* proxy, oaFloat t)
{
  oaDisplayFrame(t);
  return;
}

void oaProxy::pEndBenchmark(void* proxy)
{
  oaEndBenchmark();
  return;
}

void oaProxy::pAddResultValue(void* proxy, 
                              const oaChar *name, 
                              oaOptionDataType value_type,
                              const oaValue *value)
{
  oaAddResultValue(name, value_type,  value);
  return;
}

void oaProxy::pAddFrameValue(void* proxy, 
                             const oaChar *name, 
                             oaOptionDataType value_type,
                             const oaValue *value)
{
  oaAddFrameValue(name, value_type, value);
  return;
}