...
#ifndef _oaRPC_Internal_h
#define _oaRPC_Internal_h
...
#define OARPC_ERROR(str) \
  { oaRPCLogMsg(__FILE__, __LINE__, (str)); exit(1);} 
#define OARPC_WARNING(str) \
  oaRPCLogMsg(__FILE__, __LINE__, (str));
#endif
...
/*******************************************************************************
*** Public functions prototypes for both server side and client side
******************************************************************************/
//oaBool Recv(oaRPCTransport *transport, oaRPCBuf *buf, int time_out);

/*******************************************************************************
*** Globals
******************************************************************************/
extern const char Magic[];
extern const int MagicLen;
extern const unsigned long RPCVersionMajor;
extern const unsigned long RPCVersionMinor;
...
#  define OARPC_BUF_SIZE(buf) ((buf)->Size)
  ...
  ******************************************************************************/
  ...
  void oaRPCSerializeString(oaRPCBuf *buf, 
                            const oaChar *str,
                            oaBool reverse_bytes_flag);
  void oaRPCSerializeInt(oaRPCBuf *buf, 
                         oaInt val, 
                         oaBool reverse_bytes_flag);
  void oaRPCSerializeFloat(oaRPCBuf *buf, 
                           oaFloat val, 
                           oaBool reverse_bytes_flag);
  void oaRPCSerializeBool(oaRPCBuf *buf, 
                          oaBool val, 
                          oaBool reverse_bytes_flag);
  ...
  const oaChar *oaRPCDeserializeString(const oaRPCBuf *buf, 
                                       oaRPCSize *offset,
                                       oaBool reverse_bytes_flag);
  oaInt oaRPCDeserializeInt(const oaRPCBuf *buf, 
                            oaRPCSize *offset, 
                            oaBool reverse_bytes_flag);
  oaFloat oaRPCDeserializeFloat(const oaRPCBuf *buf, 
                                oaRPCSize *offset,
                                oaBool reverse_bytes_flag);
  oaBool oaRPCDeserializeBool(const oaRPCBuf *buf, 
                              oaRPCSize *offset,
                              oaBool reverse_bytes_flag);
...
#define OARPC_SWAP(a, b) Tmp = (a); (a) = (b); (b) = Tmp;

#define OARPC_REVERSE_BYTES(cond, val) \
    if(cond) \
    { \
        ...
        { \
            case 1: \
                break; \
            case 2: \
                OARPC_SWAP(Buf[0], Buf[1]) \
                break; \
            case 4: \
                OARPC_SWAP(Buf[0], Buf[3]) \
                OARPC_SWAP(Buf[1], Buf[2]) \
                break; \
            case 8: \
                OARPC_SWAP(Buf[0], Buf[7]) \
                OARPC_SWAP(Buf[1], Buf[6]) \
                OARPC_SWAP(Buf[2], Buf[5]) \
                OARPC_SWAP(Buf[3], Buf[4]) \
                break; \
        } \
    } 

#define DESERIALIZE_VALUE_HEADER(buf, type, size, offset, reverse_bytes_flag) \
{ \
