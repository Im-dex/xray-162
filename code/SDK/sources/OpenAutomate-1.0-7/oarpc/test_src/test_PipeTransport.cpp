...
static const char *PipeName = "test_pipe";
...
static void TestLoopback(oaRPCTransport *transport, 
                         const char *str, 
                         int n)
{
  oaRPCBuf *Buf = oaRPCAllocBuf();
  oaBool NewConnection;

  oaRPCPushBuf(Buf, str, n);
  TEST(transport->Send(transport->UserData, Buf, OARPC_INFINITE) == OARPC_TRANSPORT_ERROR_OK); 

  oaRPCClearBuf(Buf); 
  TEST(transport->Recv(transport->UserData, Buf, &NewConnection, OARPC_INFINITE) == OARPC_TRANSPORT_ERROR_OK); 

  TEST(memcmp(OARPC_GET_BUF(Buf, 0), str, n) == 0); 
...
static void StartClient(void)
{
  ...
  Ret = oaRPCInitPipeClientTransport(&Transport, PipeName);
...
#   define TEST_LOOPBACK(str) \
      TestLoopback(&Transport, str, sizeof(str));

    TEST_LOOPBACK("test number one")
    TEST_LOOPBACK("test \0with embedded NULL\0")
    TEST_LOOPBACK("\0")
    TEST_LOOPBACK("")
    TEST_LOOPBACK("last test")

    oaRPCPushBuf(Buf, "end", 4);
    TEST(Transport.Send(Transport.UserData, Buf, OARPC_INFINITE) == OARPC_TRANSPORT_ERROR_OK); 
...
static oaBool StartServer(void)
{
  ...
  Ret = oaRPCInitPipeServerTransport(&Transport, PipeName);
  TEST(Ret == OARPC_TRANSPORT_ERROR_OK);
...
static void ServerLoop(void)
{
  ...
  while(OARPC_TRANSPORT_ERROR_OK == Transport.Recv(Transport.UserData, Buf, &NewConnection, OARPC_INFINITE))
  {
    if(RecvCount++ == 0)
      TEST(NewConnection == OA_TRUE)
    else
      TEST(NewConnection == OA_FALSE)
    
    if(!strncmp(OARPC_GET_BUF(Buf, 0), "end", 3))
      break;
...
//    __asm int 3;
  ...
  if(StartServer() == OA_TRUE)
  {
    ...
    ServerLoop();
    TEST(test_WaitProcess(ClientProcess, &ClientExitCode) != 0);
    TEST(ClientExitCode == 0);
  }
