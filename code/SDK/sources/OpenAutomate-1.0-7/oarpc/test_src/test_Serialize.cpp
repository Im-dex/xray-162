...
TEST_INIT_PLANNED(22)


  // Test with both endians
  for(int i=0; i < 2; ++i)
  {
      oaBool ReverseBytes = (i == 0) ? OA_FALSE : OA_TRUE;

      oaRPCBuf *Buf = oaRPCAllocBuf();
      TEST(Buf != NULL);
      TEST(OARPC_BUF_SIZE(Buf) == 0);


      char TestStr[] = "My test string";

      oaRPCSerializeString(Buf, TestStr, ReverseBytes);
      oaRPCSerializeInt(Buf, 69, ReverseBytes);
      oaRPCSerializeFloat(Buf, 3.1419, ReverseBytes);
      oaRPCSerializeBool(Buf, OA_TRUE, ReverseBytes);
      oaRPCSerializeString(Buf, "", ReverseBytes);
      oaRPCSerializeBool(Buf, OA_FALSE, ReverseBytes);
      oaValue Val;
      Val.Int = 96;
      oaRPCSerializeValue(Buf, OA_TYPE_INT, Val, ReverseBytes);


      oaRPCSize Offset = 0;
      const oaChar *Str = oaRPCDeserializeString(Buf, &Offset, ReverseBytes);
      TEST(Str != NULL);
      TEST(!strcmp(Str, TestStr));

      TEST(oaRPCDeserializeInt(Buf, &Offset, ReverseBytes) == 69);
      TEST(oaRPCDeserializeFloat(Buf, &Offset, ReverseBytes) == 3.1419);
      TEST(oaRPCDeserializeBool(Buf, &Offset, ReverseBytes) == OA_TRUE);
      TEST(*oaRPCDeserializeString(Buf, &Offset, ReverseBytes) == '\0');
      TEST(oaRPCDeserializeBool(Buf, &Offset, ReverseBytes) == OA_FALSE);
      oaOptionDataType Type;
      TEST(oaRPCDeserializeValue(Buf, &Type, &Offset, ReverseBytes).Int == 96);
