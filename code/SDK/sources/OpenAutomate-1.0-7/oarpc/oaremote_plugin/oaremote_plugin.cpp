...
static ConnectType  Type        = TYPE_SOCKET;
static char         Host[1024]  = "localhost";
static int          Port        = 6969;

void Usage(bool verbose = false)
{
  cerr << "Usage: <PATH>/oaremote_plugin.dll;  [-h|-help] [-namedpipe] [hostname[:port]]" << endl;
 ...
 #define INDENT "         "
  ...
  if(verbose)
  {
    cerr << 
      ...
      INDENT "-namedpipe      : Use named-pipe transport.\n" 
      INDENT "hostname[:port] : Host tand optional port to connect to.\n"
...
void ParseHostAddr(char *host_addr)
{
  if(Type == TYPE_PIPE)
  {
  ...
  {
    char *Tmp  = strtok(host_addr, ":");
    strncpy(Host, Tmp, sizeof(Host));
    Tmp = strtok(NULL, "");
    if(Tmp)
    {
      Port = atoi(Tmp);
...
void ParseArgs(int argc, char *argv[])
{
  ...
  for(i=0;  i < argc; ++i)
  {
    ...
    else if(!strcmp(argv[i], "-namedpipe"))
    {
      Type = TYPE_PIPE;
    }
    else
...
{
  OA_PLUGIN_INIT_VERSION_STRUCT(version, app_version)

  char *Opt = new char[strlen(init_str) + 1];
  if(!Opt)
    return NULL;
  strcpy(Opt, init_str);
  ...
  oaRPCTransportErrorType TransportError;
  if(Type == TYPE_PIPE)
  {
    TransportError = oaRPCInitPipeClientTransport(
      &Transport, 
      Host);
  }
  else // if( Type == TYPE_SOCKET )
  {
    TransportError = oaRPCInitSocketClientTransport(
      &Transport, 
      Host, 
      Port);
  }

  if(TransportError != OARPC_TRANSPORT_ERROR_OK)
  {
  ...
  const oaiFunctionTable *TmpTable = oaRPCInitClient(&Transport, version);
