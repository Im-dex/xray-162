...
#define DUMP_REQUEST(req_buf, filename, reverse_bytes_flag)   \
  if(filename) oaRPCDumpRequest(req_buf, filename, reverse_bytes_flag);

#define DUMP_RESPONSE(req_buf, filename, reverse_bytes_flag)  \
  if(filename) oaRPCDumpResponse(req_buf, filename, reverse_bytes_flag);

#define SERIALIZE_RESPONSE_HEADER(error_code, error_str) \
  assert(send_buf); \
  oaRPCSerializeInt(send_buf, error_code, reverse_bytes_flag); \
  oaRPCSerializeString(send_buf, error_str, reverse_bytes_flag);
...
#define SHIFT_STRING_PARAM(buf, dst) \
  ...
  (dst) = oaRPCDeserializeString((buf), &Offset, reverse_bytes_flag); 

#define SHIFT_INT_PARAM(buf, dst) \
  CHECK_BUF_SAFE(buf,oaInt); \
  (dst) = oaRPCDeserializeInt((buf), &Offset, reverse_bytes_flag);

#define SHIFT_BOOL_PARAM(buf, dst) \
  CHECK_BUF_SAFE(buf, oaRPCUInt8) \
  (dst) = oaRPCDeserializeBool((buf), &Offset, reverse_bytes_flag); 

#define SHIFT_FLOAT_PARAM(buf, dst) \
  CHECK_BUF_SAFE(buf, oaFloat) \
  (dst) = oaRPCDeserializeFloat((buf), &Offset, reverse_bytes_flag); 

#define SHIFT_VALUE_PARAM(buf, type, dst) \
  { \
    oaOptionDataType DESERIALIZE_VALUE_Type; \
    CHECK_BUF_SAFE(buf, oaInt) \
    (dst) = oaRPCDeserializeValue((buf), &DESERIALIZE_VALUE_Type, \
                                  &Offset, reverse_bytes_flag);  \
...
*** Prototypes
******************************************************************************/
static void oaRPCDumpRequest(const oaRPCBuf *request, 
                             const char* file_name,
                             oaBool reverse_bytes_flag);
static void oaRPCDumpResponse(const oaRPCBuf *response, 
                              const char* file_name,
                              oaBool reverse_bytes_flag);
...
oaRPCServerErrorType oaRPCRunServer(oaRPCServer *server, int IO_time_out)
{
  ...
  while(1)
  {
    ...
    DUMP_REQUEST(RecvBuf, LogFilename, server->ReverseBytesFlag);
    if((RecvBuf)->Size - Offset < 5 + sizeof(oaInt))
    {
      SvrErrCode = OARPC_SERVER_ERROR_BADREQUEST;
      break;
    }
    FuncId = oaRPCDeserializeInt(RecvBuf, &Offset, server->ReverseBytesFlag);
    ...
    SvrErrCode = oaRPCDispatch(
      ...
      FuncTable, 
      &ExitLoop,
      server->ReverseBytesFlag);

    if(SvrErrCode == OARPC_SERVER_ERROR_OK)
    {
      DUMP_RESPONSE(SendBuf, LogFilename, server->ReverseBytesFlag);
...
oaRPCServerErrorType oaRPCServerNestCommand(oaRPCServer *server, const oaCommand *command)
{
  ...
  SerializeGetNextCommandResponse(command, SendBuf, server->ReverseBytesFlag);
...
{
  assert(server);
  if(server->LogFilename)
    free(server->LogFilename);
  free(server);
...
static void oaRPCPrintParams(const oaRPCBuf *buf, 
                             oaRPCBuf *ret, 
                             oaInt *offset, 
                             oaBool reverse_bytes_flag)
{
  ...
  while(BufSize - *offset >= 5)
  {
    ...
    DESERIALIZE_VALUE_HEADER(buf, Type, Size, &TmpOffset, reverse_bytes_flag);
    ...
    case OA_TYPE_STRING :
      sprintf(Msg, "Param %d (string[%d]): \"%s\"\n", 
        ParamCount, Size, 
        oaRPCDeserializeString(buf, offset, reverse_bytes_flag));
      break;

    case OA_TYPE_INT :
      sprintf(Msg, "Param %d (int[%d]): %d\n", 
        ParamCount, Size, 
        oaRPCDeserializeInt(buf, offset, reverse_bytes_flag));
      break;

    case OA_TYPE_FLOAT :
      sprintf(Msg, "Param %d (float[%d]): %f\n", 
        ParamCount, Size, 
        oaRPCDeserializeFloat(buf, offset, reverse_bytes_flag));
      break;

    case OA_TYPE_BOOL :
      sprintf(Msg, "Param %d (bool[%d]): %s\n", 
        ParamCount, Size, 
        (oaRPCDeserializeBool(buf, offset, reverse_bytes_flag) == OA_TRUE) ? 
            "TRUE" : "FALSE" );
      break;
...
void oaRPCDumpRequest(const oaRPCBuf *request, 
                      const char *file_name, 
                      oaBool reverse_bytes_flag)
{
  oaBool Ret; 
  ...
  if(request->Size - Offset < 5 + sizeof(oaInt))
    return;
  FuncId = oaRPCDeserializeInt(request, &Offset, reverse_bytes_flag);
  ...
  oaRPCPrintParams(request, Tmp, &Offset, reverse_bytes_flag);
  if(ReqSize != Offset)
  {
...
void oaRPCDumpResponse(const oaRPCBuf *response, 
                       const char* file_name,
                       oaBool reverse_bytes_flag)
{
  oaBool Ret; 
  ...
  if(response->Size - Offset < 5 + sizeof(oaInt))
    return;
  ErrorCode = oaRPCDeserializeInt(response, &Offset, reverse_bytes_flag);
  sprintf(Msg, "Error code: %d\n", ErrorCode);
  PUSH_BUF_STR(Tmp, Msg);

  if(response->Size - Offset < 5)
    return;
  ErrorStr = oaRPCDeserializeString(response, &Offset, reverse_bytes_flag);
  sprintf(Msg, "Error string: \"%s\"\n", ErrorStr);
  PUSH_BUF_STR(Tmp, Msg);

  oaRPCPrintParams(response, Tmp, &Offset, reverse_bytes_flag);
                        ...
                        oaBool reverse_bytes_flag)
{
  ...
  SerializeGetNextCommandResponse(&Command, send_buf, reverse_bytes_flag);
                       ...
                       oaBool reverse_bytes_flag)
{
  ...
  if(Option == NULL)
  {
    oaRPCSerializeInt(send_buf, 0, reverse_bytes_flag);
  }
  else
  {
    oaRPCSerializeInt(send_buf, Option->DataType, reverse_bytes_flag);
    oaRPCSerializeString(send_buf, Option->Name, reverse_bytes_flag);
    oaRPCSerializeValue(send_buf, Option->DataType, Option->Value, reverse_bytes_flag);
  }
                    ...
                    oaBool reverse_bytes_flag)
{
  ...
  if(Ret == OARPC_SERVER_ERROR_OK && func_table->SendSignal)
  {
    result = func_table->SendSignal(func_table->UserData, 
      (oaSignalType)Signal, 
      Param);
    oaRPCSerializeBool(send_buf, result, reverse_bytes_flag);
  }
                                       ...
                                       oaBool reverse_bytes_flag)
{
  ...
  SERIALIZE_RESPONSE_HEADER(OARPC_ERROR_NONE, "");
  oaRPCSerializeInt(send_buf, command->Type, reverse_bytes_flag);

  if(command->Type == OA_CMD_RUN_BENCHMARK)
    oaRPCSerializeString(send_buf, command->BenchmarkName, reverse_bytes_flag);
  else  
    oaRPCSerializeString(send_buf, NULL, reverse_bytes_flag);
                            ...
                            oaRPCBuf *response)
{
  ...
  reverse_bytes_flag = server->ReverseBytesFlag = (Endian != SystemEndian);
  ...
  do 
  {
    ...
    {
      oaRPCSerializeInt(response, OARPC_ERROR_NEGOTIATION_FAILED, reverse_bytes_flag);
      oaRPCSerializeString(response, 
        "Negotiation failed. RPC major versions don't match.", reverse_bytes_flag);
    ...
    oaRPCSerializeInt(response, OARPC_ERROR_NONE, reverse_bytes_flag);
    oaRPCSerializeString(response, "", reverse_bytes_flag);
    oaRPCSerializeInt(response, OA_VERSION_MAJOR, reverse_bytes_flag);
    oaRPCSerializeInt(response, OA_VERSION_MINOR, reverse_bytes_flag);
    oaRPCSerializeInt(response, OA_VERSION_CUSTOM, reverse_bytes_flag);
    oaRPCSerializeInt(response, OA_VERSION_BUILD, reverse_bytes_flag);
  } while(0);
