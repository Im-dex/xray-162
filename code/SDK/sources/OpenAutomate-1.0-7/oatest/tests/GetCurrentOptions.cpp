...
static void PrintOptionValue(oacContext::OptionValue *Option)
{

    switch(Option->Type)
    {
    case OA_TYPE_ENUM:
      OAC_MSG("Current "<< oacOAUtils::TypeToStr(Option->Type) << 
        " option '" << Option->Name << "' -> " << Option->Value.Enum );
      break;
    case OA_TYPE_STRING:
	  if ( Option->Value.String == NULL)
	  {
	    OAC_MSG("Added "<< oacOAUtils::TypeToStr(Option->Type) << 
		  " option '" << Option->Name << "' -> <NULL Pointer>"  );
	  }
	  else
	  {
	    OAC_MSG("Added "<< oacOAUtils::TypeToStr(Option->Type) << " option '" 
          << Option->Name << "' -> "<< Option->Value.String  );
	  }
      break;
    case OA_TYPE_INT:
      OAC_MSG("Current "<< oacOAUtils::TypeToStr(Option->Type) <<
        " option '" << Option->Name <<"' -> "<< Option->Value.Int );
      break;
    case OA_TYPE_FLOAT:
      OAC_MSG("Current "<< oacOAUtils::TypeToStr(Option->Type) << 
        " option '" << Option->Name << "' -> " << Option->Value.Float );
      break;
    case OA_TYPE_BOOL:
      OAC_MSG("Current "<< oacOAUtils::TypeToStr(Option->Type) <<
        " option '" << Option->Name << "' -> " << Option->Value.Bool );
      break;
    }
}


oaChar* OptValToStr(oacContext::OptionValue *option)
{
  switch(option->Type)
  {
  case OA_TYPE_INT:
    sprintf(ValueStr, "%d", option->Value.Int);
    break;

  case OA_TYPE_BOOL:
    sprintf(ValueStr, "%d", (int)option->Value.Bool);
    break;

  case OA_TYPE_FLOAT:
    sprintf(ValueStr, "%f", option->Value.Float);
    break;
  ...
  case OA_TYPE_STRING:
    if(option->Value.String == NULL)
      ...
      sprintf(ValueStr, "%s", option->Value.String);
    break;

  case OA_TYPE_ENUM:
    sprintf(ValueStr, "%s", option->Value.Enum);
    break;
  }
  return ValueStr;
}
...
bool CheckEnumValue(oacContext::OptionValue *option,
                   const oacContext::OptionVec *stored_option)

{
  vector<oaNamedOption>::const_iterator Iter = stored_option->begin();
  for(; Iter != stored_option->end(); Iter++)
  {
    if(!strcmp(Iter->Value.Enum, option->Value.Enum))
      return true;
  }
  return false;
}

bool CheckIntValue(oacContext::OptionValue *option,
                   const oacContext::OptionVec *stored_option)
{
  oaInt Min = (*stored_option->begin()).MinValue.Int;
  oaInt Max = (*stored_option->begin()).MaxValue.Int;
  oaInt StepNum = (*stored_option->begin()).NumSteps;
  oaInt Value = option->Value.Int;

  // If -1 everything is ok
  if(StepNum == -1)
    return true;

  // Value only has to be between Min & Max.
  // If StepNum equal to 0
  if(StepNum == 0)
  {
  ...
  oaInt Step = (Max - Min) / StepNum;

  // Make sure the Step makes sense for an int.
  if(Step == 0)
  {
    return false;
  }

  // Check both negative and positve versions of Step
  if (Step < 0)
  {
    for(oaInt i = Min; i >= Max; i = i + Step)
    {
      if(i == Value)
        return true;
    }
...
bool CheckFloatValue(oacContext::OptionValue *option,
                     const oacContext::OptionVec *stored_option)
{
  oaFloat Min = (*stored_option->begin()).MinValue.Float;
  oaFloat Max = (*stored_option->begin()).MaxValue.Float;
  int StepNum = (*stored_option->begin()).NumSteps;
  oaFloat Value = option->Value.Float;
  ...
  // Due to float precision can't check steps

  return true;
}

bool CheckOptionValue(oacContext::OptionValue *option,
                      const oacContext::OptionVec *stored_option)
{

  if (stored_option == NULL ) 
    return false;

  switch(option->Type)
  {
  case OA_TYPE_ENUM:
    if(CheckEnumValue(option, stored_option))
      return true;
    break;
  case OA_TYPE_STRING:
    //Can't test a string value.
    return true;
    break;
  case OA_TYPE_INT:
    if(CheckIntValue(option, stored_option))
      return true;
    break;
  case OA_TYPE_FLOAT:
    if(CheckFloatValue(option, stored_option))
      return true;
    break;
  case OA_TYPE_BOOL:
    if(option->Value.Bool == OA_TRUE || option->Value.Bool == OA_FALSE)
      return true;
    break;
  }
...
OAC_SIMPLE_TEST_HEAD(GetCurrentOptions, 
  ...
  for(; Iter != Options.end(); Iter++)
  {
    ...
    OAC_TEST_MSG(CheckOptionValue((*Iter),Option),
    "The current value '"<<OptValToStr(*Iter) <<"' for option '"
    << Name << "' doesn't match any corresponding values received from the"
    " OA_CMD_GET_ALL_OPTIONS command.")
  }
